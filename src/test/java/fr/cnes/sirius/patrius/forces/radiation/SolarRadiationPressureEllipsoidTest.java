...
* VERSION:4.3:FA:FA-2096:15/05/2019:[PATRIUS] Attribut coefficient multiplicatif non mis a jour dans SolarRadiationPressureEllipsoid
...
import fr.cnes.sirius.patrius.tools.ephemeris.UserEphemeris;
import fr.cnes.sirius.patrius.utils.Constants;
import fr.cnes.sirius.patrius.utils.exception.PatriusException;

/**
 * Test class for {@link SolarRadiationPressureEllipsoid}
 * 
 ...
 * 
 */
public class SolarRadiationPressureEllipsoidTest {
    ...
    /** ae */
    private final double ae = 6.3781360000000000E+06;
    /** f */
    private final double f = 3.3528040724231161E-03;
    /** ITRF2008 */
    private Frame itrf;
    /** CIRF2000 */
    private Frame cirf;
    ...
    /** srp */
    private SolarRadiationPressureEllipsoid srp;
    /** sun */
    ...
    public static void setUpBeforeClass() {
        Report.printClassHeader(
            SolarRadiationPressureEllipsoidTest.class.getSimpleName(),
     ...
     */
    private Assembly
            createAssemblySphereAndFacets(final double mass,
        ...
        builder.addPart(array1, mainPart, new Transform(this.date, new Rotation(
            Vector3D.PLUS_I, 0.25)));
        builder.addPart(array2, mainPart, new Transform(this.date, new Rotation(
            Vector3D.PLUS_K, 0.18)));
     ...
     * @throws PatriusException
     */
    private Assembly createAssemblySphereAndFacets(final Parameter massMain,
                                                   final Parameter mass1, final Parameter mass2,
                                                   final Parameter kaMain, final Parameter ksMain,
                                                   final Parameter kdMain, final Parameter ka1, final Parameter ks1,
                                                   final Parameter kd1, final Parameter ka2, final Parameter ks2,
        ...
        builder.addProperty(new RadiativeProperty(kaMain, ksMain, kdMain),
            mainPart);
    ...
    public void testOtherOccultingBody() throws PatriusException {
        ...
        final GeometricBodyShape moonModel = new ExtendedOneAxisEllipsoid(1500000, 0, moon.getBodyOrientedFrame(), "Moon");
        final SolarRadiationPressureEllipsoid srp = new SolarRadiationPressureEllipsoid(sun, moonModel, new SphericalSpacecraft(50.0, 0.5, 1, 0., 0.,
                "Default"));
        ...
        final Vector3D pos1 = sun.getPVCoordinates(AbsoluteDate.J2000_EPOCH, moon.getInertiallyOrientedFrame()).getPosition().scalarMultiply(-0.001);
        final SpacecraftState state1 = new SpacecraftState(new CartesianOrbit(new PVCoordinates(pos1, Vector3D.ZERO), moon.getInertiallyOrientedFrame(), AbsoluteDate.J2000_EPOCH, Constants.WGS84_EARTH_MU), massModel);
        ...
        final Vector3D pos2 = moon.getInertiallyOrientedFrame().getTransformTo(FramesFactory.getGCRF(), AbsoluteDate.J2000_EPOCH).transformPosition(pos1);
    ...
    public void testMultipleOccultingBodies() throws PatriusException {
        ...
        final GeometricBodyShape body1Model = new ExtendedOneAxisEllipsoid(6378000, 0, FramesFactory.getGCRF(), "Body1");
        final SolarRadiationPressureEllipsoid srp = new SolarRadiationPressureEllipsoid(sun, body1Model, new SphericalSpacecraft(50.0, 0.5, 1, 0., 0.,
                "Default"));
        srp.addOccultingBody(new ExtendedOneAxisEllipsoid(1500000, 0, body2.getInertiallyOrientedFrame(), "Body2"));
        final MassProvider massModel = new SimpleMassModel(1500., "Default");

        // Case 1: state behind first body: acc = 0
        final Vector3D pos1 = sun.getPVCoordinates(AbsoluteDate.J2000_EPOCH, body1.getInertiallyOrientedFrame()).getPosition().scalarMultiply(-0.001);
        final SpacecraftState state1 = new SpacecraftState(new CartesianOrbit(new PVCoordinates(pos1, Vector3D.ZERO), body1.getInertiallyOrientedFrame(), AbsoluteDate.J2000_EPOCH, Constants.WGS84_EARTH_MU), massModel);
        final Vector3D actualAcc = srp.computeAcceleration(state1);
        Assert.assertEquals(0, actualAcc.getNorm(), 0.);
        
        // Case 2: state behind second body: acc = 0
        final Vector3D pos2 = sun.getPVCoordinates(AbsoluteDate.J2000_EPOCH, body2.getInertiallyOrientedFrame()).getPosition().scalarMultiply(-0.001);
        final SpacecraftState state2 = new SpacecraftState(new CartesianOrbit(new PVCoordinates(pos2, Vector3D.ZERO), body2.getInertiallyOrientedFrame(), AbsoluteDate.J2000_EPOCH, Constants.WGS84_EARTH_MU), massModel);
        final Vector3D actualAcc2 = srp.computeAcceleration(state2);
        Assert.assertEquals(0, actualAcc2.getNorm(), 0.);
        
        // Case 3: state not behind any body: acc != 0
        final Vector3D pos3 = Vector3D.PLUS_I.scalarMultiply(1E8);
        final SpacecraftState state3 = new SpacecraftState(new CartesianOrbit(new PVCoordinates(pos3, Vector3D.ZERO), body1.getInertiallyOrientedFrame(), AbsoluteDate.J2000_EPOCH, Constants.WGS84_EARTH_MU), massModel);
        ...
        final SolarRadiationPressureEllipsoid srp2 = new SolarRadiationPressureEllipsoid(sun, body1Model, new SphericalSpacecraft(50.0, 0.5, 1, 0., 0.,
                "Default"));
        srp2.addOccultingBody(new ExtendedOneAxisEllipsoid(6378000, 0, body3.getInertiallyOrientedFrame(), "Body2"));

        final Vector3D pos4 = sun.getPVCoordinates(AbsoluteDate.J2000_EPOCH, body1.getInertiallyOrientedFrame()).getPosition().scalarMultiply(-0.001);
        final SpacecraftState state4 = new SpacecraftState(new CartesianOrbit(new PVCoordinates(pos4, Vector3D.ZERO), body1.getInertiallyOrientedFrame(), AbsoluteDate.J2000_EPOCH, Constants.WGS84_EARTH_MU), massModel);
    ...
    public void testHeliocentric() throws PatriusException {
        ...
        final Orbit initialOrbit = new KeplerianOrbit(150E9, 0, 0, 0, 0, 0, PositionAngle.TRUE, sun.getInertiallyOrientedFrame(), initialDate, Constants.IERS92_SUN_GRAVITATIONAL_PARAMETER);
        propagator.setInitialState(new SpacecraftState(initialOrbit, massModel));
        propagator.setOrbitFrame(sun.getInertiallyOrientedFrame());
    ...
    public void testLightningAlignment() throws PatriusException {
        ...
        final Vector3D position = sun.getPVCoordinates(AbsoluteDate.J2000_EPOCH, FramesFactory.getGCRF()).getPosition().scalarMultiply(-1);
        final Vector3D satSunVector = sun.getPVCoordinates(AbsoluteDate.J2000_EPOCH, FramesFactory.getGCRF()).getPosition()
                .subtract(position);
        final SolarRadiationPressureEllipsoid prs = new SolarRadiationPressureEllipsoid(sun, this.earth, null);
        // Check lightning ratio is equal to 0
        Assert.assertEquals(0, prs.getLightningRatio(satSunVector, this.earth, position, FramesFactory.getGCRF(), AbsoluteDate.J2000_EPOCH), 0.);

        // Check lightning ratio is equal to 1 because eclipses are not computed
        prs.setEclipsesComputation(false);
        Assert.assertEquals(1., prs.getLightningRatio(satSunVector, this.earth, position, FramesFactory.getGCRF(), AbsoluteDate.J2000_EPOCH), 0.);
        Assert.assertFalse(prs.isEclipseComputation());
    }
     ...
     * @testedMethod {@link SolarRadiationPressureCircular#getParameters()}
     * 
    ...
    public void testParamListSRP() throws PatriusException {
        ...
        final SolarRadiationPressureCircular prs = new SolarRadiationPressureCircular(
            CelestialBodyFactory.getSun(), this.earth.getConjugateRadius(),
            model);

        double k = 5;
        for (final Parameter p : prs.getParameters()) {
            p.setValue(k);
            Assert.assertTrue(Precision.equals(k, p.getValue(), 0));
            k++;
        }
        Assert.assertEquals(11, prs.getParameters().size());
    }
     ...
     * @testedMethod {@link SolarRadiationPressureEllipsoid#getParametersNames()}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#getParameter(String)}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#setParameter()}
     * 
     * @description Test for the parameters
     * 
     * @input a parameter
     ...
     */
    @Test
    public void testParamList() throws PatriusException, IOException,
                               ParseException {
        ...
        final DirectRadiativeModel model = new DirectRadiativeModel(assembly);
        Assert.assertEquals(10, model.getParameters().size());
        final SolarRadiationPressureEllipsoid prs = new SolarRadiationPressureEllipsoid(
            CelestialBodyFactory.getSun(), this.earth, model);
     ...
     * 
     * @testedMethod {@link SolarRadiationPressureEllipsoid#computeAcceleration(PVCoordinates, Frame, AbsoluteDate, double)}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#computeAcceleration(SpacecraftState)}
     * 
    ...
    public void testSRP() throws PatriusException {
        ...
        Report.printMethodHeader("testSRP", "Acceleration computation",
            "Unknwon", this.eps, ComparisonType.RELATIVE);
        ...
        // SRP
        this.srp = new SolarRadiationPressureEllipsoid(this.dRef, this.pRef, this.sun, this.earth, sc);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 8.930000000000291, this.tai);
        this.pos = new Vector3D(-3051443.873181594, 0.08004112908428662,
            -6299896.119758646);
        this.vel = new Vector3D(6791.335823416453, 4.623958780615747E-5,
            -3289.4796714774893);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-0.0, -0.0, -0.0);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);

        Report.printToReport("Acceleration (shadow)", expected, result);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 13.930000000000291, this.tai);
        this.pos = new Vector3D(-3017443.03254199, 0.08027122004181056,
            -6316251.924802766);
        this.vel = new Vector3D(6808.967505417617, 4.579652856753137E-5,
            -3252.8265069236845);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-0.0, -0.0, -0.0);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
        ...
        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 18.93000000000029, this.tai);
        this.pos = new Vector3D(-2983354.527822928, 0.08049909171346586,
            -6332424.227323494);
        this.vel = new Vector3D(6826.401370304048, 4.535187542297378E-5,
            -3216.0788398278287);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-0.0, -0.0, -0.0);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
        ...
        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 23.93000000000029, this.tai);
        this.pos = new Vector3D(-2949179.349378623, 0.08072473615900333,
            -6348412.5574760055);
        this.vel = new Vector3D(6843.636911578195, 4.4905640058221286E-5,
            -3179.237737799262);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-4.051681754196597E-10,
            -1.9874333261941255E-13, -1.0093398114753024E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        ...
        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 28.93000000000029, this.tai);
        this.pos = new Vector3D(-2914918.49008138, 0.08094814549656688,
            -6364216.450760323);
        this.vel = new Vector3D(6860.67362850433, 4.445783422729028E-5,
            -3142.304271161838);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-2.893565907654407E-9, -1.4220166119104913E-12,
            -7.222957840073693E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, 1E-8);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 33.93000000000029, this.tai);
        this.pos = new Vector3D(-2880572.9452927453, 0.08116931190443193,
            -6379835.448034807);
        this.vel = new Vector3D(6877.511026124235, 4.4008470313641964E-5,
            -3105.279512922826);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-3.6103324586755545E-9,
            -1.7775876307772434E-12, -9.030350220816393E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 38.93000000000029, this.tai);
        this.pos = new Vector3D(-2846143.712834581, 0.0813882276246434,
            -6395269.095529501);
        this.vel = new Vector3D(6894.148615271639, 4.355756136776451E-5,
            -3068.16453874174);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-3.610334007328543E-9, -1.7809110225098502E-12,
            -9.048499352435866E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 43.93000000000029, this.tai);
        this.pos = new Vector3D(-2811631.792960075, 0.08160488496501007,
            -6410516.944859311);
        this.vel = new Vector3D(6910.5859125853185, 4.310512055443129E-5,
            -3030.960426899085);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-3.6103355599893424E-9,
            -1.7842343947634942E-12, -9.066603386144998E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 48.93000000000029, this.tai);
        this.pos = new Vector3D(-2777038.1883246796, 0.0818192762987035,
            -6425578.553037031);
        this.vel = new Vector3D(6926.822440522512, 4.265116084505521E-5,
            -2993.6682582650337);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-3.610337116607478E-9, -1.787557800982937E-12,
            -9.084662446218315E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 53.93000000000029, this.tai);
        this.pos = new Vector3D(-2742363.903956986, 0.08203139406334341,
            -6440453.482486216);
        this.vel = new Vector3D(6942.857727372894, 4.219569506360693E-5,
            -2956.2891162680216);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-3.610338677135245E-9, -1.7908812144162532E-12,
            -9.102676310547437E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 58.93000000000029, this.tai);
        this.pos = new Vector3D(-2707609.9472295274, 0.0822412307604966,
            -6455141.301053894);
        this.vel = new Vector3D(6958.691307272457, 4.1738735976655487E-5,
            -2918.824086863274);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-3.6103402415248136E-9,
            -1.7942046083110903E-12, -9.120644758335267E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);

        this.date = new AbsoluteDate(2000, 3, 20, 8, 11, 3.930000000000291, this.tai);
        this.pos = new Vector3D(-2672777.327829512, 0.08244877895554736,
            -6469641.582023118);
        this.vel = new Vector3D(6974.322720217183, 4.128029635339913E-5,
            -2881.274258501254);
        pv = new PVCoordinates(this.pos, this.vel);
        expected = new Vector3D(-3.610341809725377E-9, -1.7975280361112809E-12,
            -9.138567917791417E-13);
        scs = new SpacecraftState(new CartesianOrbit(pv, this.cirf, this.date, this.mu),
            massProvider);
        result = this.srp.computeAcceleration(scs);
        this.assertEqualsVector3D(expected, result, this.eps);
     ...
     * @testedFeature {@link features#SRP_WITH_ELLIPSOID}
     * 
     * @testedMethod {@link SolarRadiationPressureEllipsoid#getEventsDetectors()}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#getParameters()}
     * 
     * @description test acceleration computation
     * 
     * @input PVCoordinates, frame, date and mass
    ...
    public void testEvents() throws PatriusException, IOException {
        ...
        final AssemblySphericalSpacecraft sc = new AssemblySphericalSpacecraft(
            FastMath.PI, 2.2, 1., 0., 0.);

        this.srp = new SolarRadiationPressureEllipsoid(this.dRef, this.pRef, this.sun, this.earth, sc);

        Assert.assertEquals(5, sc.getJacobianParameters().size());
        Assert.assertEquals(6, this.srp.getParameters().size());

        final EventDetector[] events = this.srp.getEventsDetectors();
     ...
     * @testedFeature {@link features#RADIATIVE_MODEL}
     * 
     * @testedMethod {@link SolarRadiationPressureEllipsoid#addDAccDParam(SpacecraftState, String, double[])}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#computeAcceleration(SpacecraftState)}
     * 
    ...
    public void testPDerivatives() throws PatriusException {
        ...
        this.date = new AbsoluteDate(2000, 3, 20, 8, 10, 43.93000000000029, this.tai);
        this.pos = new Vector3D(-1.5698127651877177E+06, -4.0571199222644530E+06,
            5.1323187886121357E+06);
        this.vel = new Vector3D(2.6780951868939924E+03, -5.2461908201159367E+03,
        ...
        final Attitude attitude = new LofOffset(orbit.getFrame(), LOFType.LVLH)
            .getAttitude(orbit, orbit.getDate(), orbit.getFrame());
        final SpacecraftState scs = new SpacecraftState(orbit, attitude,
            massProvider);
        ...
        final Assembly assembly = this.createAssemblySphereAndFacets(massMain,
            mass1, mass2, kaMain, ksMain, kdMain, ka1, ks1, kd1, ka2, ks2,
            kd2);
        ...
        Assembly assembly1 = this.createAssemblySphereAndFacets(mass,
            0.6 - diff / 2., 0.1, 0.3);
        assembly1.initMainPartFrame(scs);
        DirectRadiativeModel sc1 = new DirectRadiativeModel(assembly1);

        // Assembly 2
        Assembly assembly2 = this.createAssemblySphereAndFacets(mass,
            0.6 + diff / 2., 0.1, 0.3);
        ...
        this.srp = new SolarRadiationPressureEllipsoid(refFlux, this.sun, this.earth, sc);
        SolarRadiationPressureEllipsoid srp1 = new SolarRadiationPressureEllipsoid(
            this.dRef, this.pRef, this.sun, this.earth, sc1);
        SolarRadiationPressureEllipsoid srp2 = new SolarRadiationPressureEllipsoid(
            this.dRef, this.pRef, this.sun, this.earth, sc2);

        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), kaMain, dAccdParam);
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), ka1, dAccdParam);
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), ka2, dAccdParam);

        // finite differences:
        Vector3D acc1 = srp1.computeAcceleration(new SpacecraftState(orbit,
            attitude, new MassModel(assembly1)));
        Vector3D acc2 = srp2.computeAcceleration(new SpacecraftState(orbit,
            attitude, new MassModel(assembly2)));
        final double[] damFabs = { (acc2.getX() - acc1.getX()) / diff,
            (acc2.getY() - acc1.getY()) / diff,
        ...
        // Assembly 1
        assembly1 = this.createAssemblySphereAndFacets(mass, 0.6, 0.1 - diff / 2.,
            0.3);
        sc1 = new DirectRadiativeModel(assembly1);
        // Assembly 2
        assembly2 = this.createAssemblySphereAndFacets(mass, 0.6, 0.1 + diff / 2.,
            0.3);
        sc2 = new DirectRadiativeModel(assembly2);

        // SRP
        this.srp = new SolarRadiationPressureEllipsoid(this.dRef, this.pRef, this.sun, this.earth, sc);
        srp1 = new SolarRadiationPressureEllipsoid(this.dRef, this.pRef, this.sun, this.earth, sc1);
        srp2 = new SolarRadiationPressureEllipsoid(this.dRef, this.pRef, this.sun, this.earth, sc2);

        dAccdParam = new double[3];
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), ksMain, dAccdParam);
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), ks1, dAccdParam);
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), ks2, dAccdParam);

        assembly1.initMainPartFrame(new SpacecraftState(orbit, attitude,
            new MassModel(assembly1)));
        assembly2.initMainPartFrame(new SpacecraftState(orbit, attitude,
            new MassModel(assembly2)));
        // finite differences:
        acc1 = srp1.computeAcceleration(new SpacecraftState(orbit, attitude,
            new MassModel(assembly1)));
        acc2 = srp2.computeAcceleration(new SpacecraftState(orbit, attitude,
            new MassModel(assembly2)));
        ...
        // DIFFUSE COEFFICIENT
        // Assembly 1
        assembly1 = this.createAssemblySphereAndFacets(mass, 0.6, 0.1,
            0.3 - diff / 2.);
        sc1 = new DirectRadiativeModel(assembly1);
        // Assembly 2
        assembly2 = this.createAssemblySphereAndFacets(mass, 0.6, 0.1,
            0.3 + diff / 2.);
        sc2 = new DirectRadiativeModel(assembly2);
        ...
        dAccdParam = new double[3];
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), kdMain, dAccdParam);
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), kd1, dAccdParam);
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), kd2, dAccdParam);
        ...
        this.srp.computeAcceleration(new SpacecraftState(orbit, attitude,
            new MassModel(assembly)));
        this.srp.addDAccDParam(new SpacecraftState(orbit, attitude, new MassModel(
            assembly)), k0, dAccdParam);
        ...
        this.srp.addDAccDState(scs, dAccdPos, dAccdVel);
        for (int i = 0; i < dAccdPos.length; i++) {
            for (int j = 0; j < dAccdPos.length; j++) {
                Assert.assertEquals(0., dAccdPos[i][j],
                    Precision.DOUBLE_COMPARISON_EPSILON);
                Assert.assertEquals(0., dAccdVel[i][j],
                    Precision.DOUBLE_COMPARISON_EPSILON);
            }
        }
    }
    ...
    public final void testUnsupportedParam() throws PatriusException {
        ...
        final Assembly assembly = this.createAssemblySphereAndFacets(1000, 0.8, 0.1,
            0.1);
        ...
        try {
            this.srp.addDAccDParam(scs, refFlux, dAccdParam);
            Assert.fail();
     ...
     * @testType UT
     * 
     * @testedMethod {@link SolarRadiationPressureEllipsoid#PatriusSolarRadiationPressure(PVCoordinatesProvider, fr.cnes.sirius.patrius.bodies.GeometricBodyShape, RadiationSensitive, boolean)}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#PatriusSolarRadiationPressure(Parameter, PVCoordinatesProvider, fr.cnes.sirius.patrius.bodies.GeometricBodyShape, RadiationSensitive, boolean)}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#PatriusSolarRadiationPressure(double, double, PVCoordinatesProvider, fr.cnes.sirius.patrius.bodies.GeometricBodyShape, RadiationSensitive, boolean)}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#computeGradientPosition()}
     * @testedMethod {@link SolarRadiationPressureEllipsoid#computeGradientVelocity()}
     * 
    ...
    public void testNullPD() throws PatriusException {
        ...
        // Instance
        final SphericalSpacecraft spacecraft = new SphericalSpacecraft(10.0,
            1.7, 0, 0, 0, "");
        final EllipsoidBodyShape body = new ExtendedOneAxisEllipsoid(6378000,
            0.001, FramesFactory.getITRF(), "Earth");
        final SolarRadiationPressureEllipsoid prs = new SolarRadiationPressureEllipsoid(
            new Parameter("toto", 1.), this.sun, body, spacecraft, false);
        final SolarRadiationPressureEllipsoid prs2 = new SolarRadiationPressureEllipsoid(
            this.sun, body, spacecraft, false);
        final SolarRadiationPressureEllipsoid prs3 = new SolarRadiationPressureEllipsoid(
            1, 1, this.sun, body, spacecraft, false);
     ...
     * @testedFeature {@link features#SRP_WITH_ELLIPSOID}
     * 
     * @testedMethod {@link SolarRadiationPressureEllipsoid#getSolarFlux()}
     * 
     * @description Test computation for solar flux
     * 
     * @input SolarRadiationPressureEllipsoid
     * 
     * @output solar flux
     * 
     ...
     */
    @Test
    public final void testSolarFlux() throws PatriusException {

        // Initialization
        final Orbit orbit = new KeplerianOrbit(7000000, 0, 0, 0, 0, 0,
            PositionAngle.MEAN, FramesFactory.getGCRF(),
            AbsoluteDate.J2000_EPOCH, Constants.EGM96_EARTH_MU);
        final SpacecraftState state = new SpacecraftState(orbit);
        final SphericalSpacecraft spacecraft = new SphericalSpacecraft(1, 2,
            0.1, 0.2, 0.3, "Spacecraft");
        final CelestialBody sun = new MeeusSun();
        final SolarRadiationPressureEllipsoid srp = new SolarRadiationPressureEllipsoid(
            sun, this.earth, spacecraft);

        // Computation
        final Vector3D actual = srp.getSolarFlux(state);
        final Vector3D expectedRelPos = state
            .getPVCoordinates()
            .getPosition()
            .subtract(
                sun.getPVCoordinates(orbit.getDate(), orbit.getFrame())
                    .getPosition());
        final double expectedDistance = expectedRelPos.getNorm();
        final Vector3D expectedDirection = expectedRelPos.normalize();
        final double expectedCst = Constants.CONST_SOL_N_M2
            * Constants.SEIDELMANN_UA * Constants.SEIDELMANN_UA
            / (expectedDistance * expectedDistance);
        final Vector3D expected = expectedDirection.scalarMultiply(expectedCst);

        // Check
        Assert.assertEquals(0.,
            expected.subtract(actual).getNorm() / expected.getNorm(), 1E-15);
    }

    /**
     * @throws PatriusException
     *         SolarRadiationPressureEllipsoid model creation
     * @testType UT
     * 
     * 
    ...
    public void testGetters() throws PatriusException {
        Utils.setDataRoot("regular-dataPBASE");
        final EllipsoidBodyShape earthBody = new ExtendedOneAxisEllipsoid(
            Constants.CNES_STELA_AE, Constants.GRIM5C1_EARTH_FLATTENING,
            FramesFactory.getCIRF(), "earth");
        SolarActivityDataFactory
            .addSolarActivityDataReader(new ACSOLFormatReader("ACSOL.act"));
        final RadiationSensitive spacecraft = new PatriusSphericalSpacecraft(20, 0.3,
            0.4, 0.1, 0.1, "mainpart");
        final SolarRadiationPressureEllipsoid solarRadiationPressureEllipsoid = new SolarRadiationPressureEllipsoid(
            CelestialBodyFactory.getSun(), earthBody, spacecraft);
        Assert.assertTrue(earthBody.equals(solarRadiationPressureEllipsoid
            .getOccultingBodies().get(0)));
        Assert.assertTrue(CelestialBodyFactory.getSun().equals(
            solarRadiationPressureEllipsoid.getSunBody()));
        ...
        builder.addProperty(new RadiativeCrossSectionProperty(new Sphere(
            new Vector3D(1, 2, 3), 15)), array2);
        ...
        final SolarRadiationPressureEllipsoid solarRadiationPressureEllipsoid2 = new SolarRadiationPressureEllipsoid(
            200, 10, CelestialBodyFactory.getSun(), earthBody,
            builder.returnAssembly(), 3);
        Assert.assertEquals(3,
            solarRadiationPressureEllipsoid2.getMultiplicativeFactor(), 0);
        final AssemblyBuilder builder2 = new AssemblyBuilder();

        builder2.addMainPart(mainPart);

        builder2.addPart(array1, mainPart, new Transform(
            AbsoluteDate.J2000_EPOCH, new Rotation(Vector3D.PLUS_I, 0.25)));
        builder2.addPart(array2, mainPart, new Transform(
            AbsoluteDate.J2000_EPOCH, new Rotation(Vector3D.PLUS_K, 0.18)));
        builder2.addProperty(new MassProperty(10), array1);
        builder2.addProperty(new RadiativeProperty(0.4, 0.3, 0.5), array2);
        builder2.addProperty(new RadiativeCrossSectionProperty(new Sphere(
            new Vector3D(1, 2, 3), 15)), array2);
        // adding of a simple mass property
        builder2.addProperty(shapeProp, mainPart);
        final Assembly assembly = builder2.returnAssembly();
        final SolarRadiationPressureEllipsoid srp2 =
            new SolarRadiationPressureEllipsoid(this.dRef, this.pRef, CelestialBodyFactory.getSun(), earthBody,
                assembly, 12.);
        final SolarRadiationPressureEllipsoid solarRadiationPressureEllipsoid3 = new SolarRadiationPressureEllipsoid(
            srp2, assembly);

        Assert.assertEquals(12,
            solarRadiationPressureEllipsoid3.getMultiplicativeFactor(), 0);
    }
    ...
    public void testEnrichParameterDescriptors() throws PatriusException {
        ...
        final CelestialBody sun = CelestialBodyFactory.getSun();
        final CelestialBody moon = CelestialBodyFactory.getMoon();
        final GeometricBodyShape moonModel = new ExtendedOneAxisEllipsoid(1500000, 0,
                moon.getBodyOrientedFrame(), "Moon");
        final SolarRadiationPressureEllipsoid forceModel = new SolarRadiationPressureEllipsoid(sun,
                moonModel, new SphericalSpacecraft(50.0, 0.5, 1, 0., 0., "Default"));
    ...
    public void setup() throws PatriusException, IOException {
        ...
        // bodies
        final String location = "SRPwithFlattening" + File.separator
            + "ephem_soleil_CIRF_PRS_pas_5sec";
        final UserEphemeris sunEphemeris = new UserEphemeris(location,
            DateComponents.FIFTIES_EPOCH, this.tai, this.cirf);
        final IEphemerisBody sunBody = new SunEphemeris();

        this.sun = new UserCelestialBody(sunBody, sunEphemeris);

        // Earth with GRIM5C1 flattening
        this.earth = new ExtendedOneAxisEllipsoid(this.ae, this.f, this.itrf, this.name);
    }

    private class SunEphemeris implements IEphemerisBody {
        ...
        public IAUPole getIAUPole() {
            return new IAUPole(){
                ...
                /** Serial number UID. */
                private static final long serialVersionUID = 5819439436262850147L;

                @Override
                public Vector3D getPole(final AbsoluteDate date) {
                    final double x = 286.13;
                    final double x2 = 63.87;
                    return new Vector3D(MathLib.toRadians(x),
                        MathLib.toRadians(x2));
                }
                ...
                public double getPrimeMeridianAngle(final AbsoluteDate date) {
                    ...
                    final double e = 84.176;
                    final double e2 = 14.1844000;
                    return MathLib.toRadians(e + e2 * d);
                }
                ...
                public double getPrimeMeridianAngleDerivative(final AbsoluteDate date) {
                    ...
                    final double w0 = getPrimeMeridianAngle(date);
                    final double w1 = getPrimeMeridianAngle(date.shiftedBy(dt));
                    return (w1 - w0) / dt;
                }
            };
        };
        ...
        @Override
        public double getGM() {
            final double d = 1.3271244001798696E20;
            return d;
        }
     ...
     *        threshold
     */
    public void assertEqualsVector3D(final Vector3D expected,
                                     final Vector3D actual, final double thr) {
        if (expected.getNorm() < Precision.EPSILON) {
            Assert.assertTrue(actual.subtract(expected).getNorm() < Precision.EPSILON);
        } else {
            Assert.assertEquals(0,
                (expected.getX() - actual.getX()) / expected.getX(), thr);
            Assert.assertEquals(0,
                (expected.getY() - actual.getY()) / expected.getY(), thr);
            Assert.assertEquals(0,
                (expected.getZ() - actual.getZ()) / expected.getZ(), thr);
        }
    }

    /**
     * Assert two arrays are equal, relatively
     * 
     * @param expected
     *        expected array
     * @param actual
     ...
     *        threshold
     */
    public void assertArrayEquals(final double[] expected,
                                  final double[] actual, final double thr) {
        Assert.assertEquals(expected.length, actual.length);
        for (int i = 0; i < expected.length; i++) {
            Assert.assertEquals(0, (actual[i] - expected[i]) / actual[i], thr);
        }
    }
}
