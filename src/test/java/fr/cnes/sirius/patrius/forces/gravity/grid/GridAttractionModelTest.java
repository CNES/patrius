...
/**
 * Unit tests for {@link GridAttractionModel} class.
 * 
 * @author Emmanuel Bignon
 * 
 * @since 4.7
 */
public class GridAttractionModelTest {
     ...
     */
    @Test
    public final void accelerationCartesianOnKnotsTest() throws PatriusException {
        ...
        final GridAttractionModel force = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
     ...
     */
    @Test
    public final void accelerationCartesianWithinKnotsTest() throws PatriusException {
        // Build attraction model
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_cube.txt";
        final CartesianGridAttractionLoader loader = new CartesianGridAttractionLoader(filename);
        final GridAttractionModel force = new GridAttractionModel(loader, new TricubicSplineInterpolator(),
                null, FramesFactory.getGCRF());
     ...
     */
    @Test
    public final void accelerationSphericalOnKnotsTest() throws PatriusException {
        // Build attraction model
     ...
     */
    @Test
    public final void accelerationShericalCoordsNormalizedLongitudeTest() throws PatriusException {
        // Build attraction model
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_sphere.txt";
        final SphericalGridAttractionLoader loader = new SphericalGridAttractionLoader(filename);
        final GridAttractionModel force = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
        ...
        // Check
        final Vector3D actual1 = force.computeAcceleration(state1);
        final Vector3D actual2 = force.computeAcceleration(state2);
     ...
     */
    @Test
    public final void accelerationSphericalWithinKnotsTest() throws PatriusException {
        // Build attraction model
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_sphere.txt";
        final SphericalGridAttractionLoader loader = new SphericalGridAttractionLoader(filename);
        final GridAttractionModel force = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
     ...
     */
    @Test
    public final void potentialCartesianOnKnotsTest() throws PatriusException {
        // Build attraction model
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_cube.txt";
        final CartesianGridAttractionLoader loader = new CartesianGridAttractionLoader(filename);
        final GridAttractionModel force = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
        ...
        // Check
        final double actual = force.computePotential(state);
     ...
     */
    @Test
    public final void potentialSphericalOnKnotsTest() throws PatriusException {
        // Build attraction model
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_sphere.txt";
        final SphericalGridAttractionLoader loader = new SphericalGridAttractionLoader(filename);
        final GridAttractionModel force = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
     ...
     */
    @Test
    public final void accelerationOutsideGridTest() throws PatriusException {
        // Back-up model
        final double[][] cnorm = { { 1.0 }, { 0.0 }, { -4.84165270522E-4 } };
        final double[][] snorm = { { 0.0 }, { 0.0 }, { 0.0 } };
        final ForceModel backupModel = new BalminoAttractionModel(FramesFactory.getGCRF(), 20000,
        ...
        // Check
        final Vector3D actual = force.computeAcceleration(state);
        final Vector3D expected = backupModel.computeAcceleration(state);
     ...
     */
    @Test
    public final void potentialOutsideGridTest() throws PatriusException {
        // Build attraction model
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_cube.txt";
        final CartesianGridAttractionLoader loader = new CartesianGridAttractionLoader(filename);
        final GridAttractionModel force = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
        ...
        // Check
        final double actual = force.computePotential(state2);
        final double expected = force.computePotential(state1);
     ...
     * @testType UT
     * 
     * @description check basic functional features of {@link GridAttractionModel} are as expected:
     *              <ul>
     ...
     */
    @Test
    public final void functionalTest() throws PatriusException {
        // Build attraction model
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_cube.txt";
        final CartesianGridAttractionLoader loader = new CartesianGridAttractionLoader(filename);
        final GridAttractionModel force = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
                new BalminoAttractionModel(FramesFactory.getTIRF(), Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                        Constants.WGS84_EARTH_MU, new double[2][2], new double[2][2]), FramesFactory.getGCRF());
        force.setMultiplicativeFactor(5.);

        // Factor k check
        final GridAttractionModel force2 = new GridAttractionModel(loader, new TriLinearIntervalsInterpolator(),
                new BalminoAttractionModel(FramesFactory.getTIRF(), Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                        Constants.WGS84_EARTH_MU, new double[2][2], new double[2][2]), FramesFactory.getGCRF());
        final SpacecraftState state = new SpacecraftState(new CartesianOrbit(new PVCoordinates(
                new Vector3D(-0.1000000000E+02, -0.5000000000E+01, -0.1000000000E+02).scalarMultiply(1000), 
                new Vector3D(0, 0, 0)), FramesFactory.getGCRF(), AbsoluteDate.J2000_EPOCH, force.getMu()));
        Assert.assertEquals(force.computeAcceleration(state), force2.computeAcceleration(state).scalarMultiply(5.));
        Assert.assertEquals(force.computePotential(state), force2.computePotential(state) * 5.);

        // Functional Checks
        Assert.assertEquals(0, force.getEventsDetectors().length);
        ...
        force.addDAccDState(null, dAccdPos, dAccdVel);
        ...
        try {
            force.addDAccDParam(null, new Parameter("", 0), new double[3]);
            Assert.fail();
        } catch (final PatriusException e) {
            Assert.assertTrue(true);
        }
        Assert.assertEquals(+0.712699979843467446E-03 * 1E9, force.getMu(), 0.);
        force.checkData(null, null);
        Assert.assertEquals(5., force.getMultiplicativeFactor());
    }
     ...
     */
    @Test
    public final void integrationTest() throws PatriusException, IOException, ParseException {
        ...
        final String filename = "src" + File.separator + "test" + File.separator + "resources" + File.separator + "grid" + File.separator + "GRA_grille_cube.txt";
        final CartesianGridAttractionLoader loader = new CartesianGridAttractionLoader(filename);
        final GridAttractionModel force1 = new GridAttractionModel(loader, new TricubicSplineInterpolator(),
                null, FramesFactory.getGCRF());
        ...
        propagator1.addForceModel(force1);
        propagator1.disableNewtonianAttraction();
     ...
     */
    @Test
    public final void performanceTest() throws PatriusException, IOException, ParseException {
        ...
        final BalminoAttractionModel force2 = new BalminoAttractionModel(FramesFactory.getGCRF(), 10000, force1.getMu(), c, s);

        // Build propagator
        final NumericalPropagator propagator2 = new NumericalPropagator(new ClassicalRungeKuttaIntegrator(30.));
        propagator2.setInitialState(state);
        propagator2.setOrbitType(OrbitType.CARTESIAN);
        propagator2.addForceModel(force2);
