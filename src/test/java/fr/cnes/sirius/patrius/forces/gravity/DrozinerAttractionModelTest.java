...
public class DrozinerAttractionModelTest {
    ...
    public void testMultiplicativeFactor() throws PatriusException, IOException, ParseException {
        ...
        final DrozinerAttractionModel actualModel = new DrozinerAttractionModel(FramesFactory.getGCRF(), new Parameter(
            "", ae),
    ...
    public void testAddDAccDStateAndDParam() throws PatriusException, IOException, ParseException {
        ...
        Utils.setDataRoot("regular-data");
        final DrozinerAttractionModel grav = new DrozinerAttractionModel(FramesFactory.getITRF(), provider.getAe(),
            provider.getMu(), C, S);

        final double[][] dAccdPos = new double[3][3];
        final double[][] dAccdVel = new double[3][3];
        try {
            grav.addDAccDState(null, dAccdPos, dAccdVel);
            Assert.fail();
        } catch (final PatriusException e) {
            // expected !
