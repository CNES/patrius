...
public class ForceModelsDataTest {
    ...
    public void testForceModelsData() throws PatriusException, IOException, ParseException {
        ...
        final DirectBodyAttraction earthPotentialAttractionModel =
            new DirectBodyAttraction(new BalminoGravityModel(FramesFactory.getITRF(),
        ...
        final SolarRadiationPressure solarRadiationPressure = new SolarRadiationPressure(CelestialBodyFactory.getSun(),
            new OneAxisEllipsoid(Constants.CNES_STELA_AE, Constants.GRIM5C1_EARTH_FLATTENING, FramesFactory.getGCRF(),
                "earth"), new DirectRadiativeModel(assembly));
        ...
        final ThirdBodyAttraction jupiterThirdBodyAttraction =
            new ThirdBodyAttraction(CelestialBodyFactory.getJupiter().getGravityModel());
        // Ocean tides
        ...
        final List<ForceModel> forceModelsListOriginal = new ArrayList<>();
        forceModelsListOriginal.add(earthPotentialAttractionModel);
        forceModelsListOriginal.add(solarRadiationPressure);
        ...
        final ForceModelsData data = new ForceModelsData(earthPotentialAttractionModel, solarActivityDataProvider,
            solarRadiationPressure, rediffusedRadiationPressure, moonThirdBodyAttraction,
        ...
        Assert.assertEquals(solarRadiationPressure, data.getSolarRadiationPressure());
        ...
        data2.setSolarRadiationPressure(solarRadiationPressure);
        ...
        Assert.assertEquals(solarRadiationPressure, data2.getSolarRadiationPressure());
        ...
        final SpacecraftState spacecraftOriginal = propagate(forceModelsListOriginal, assembly);
        final SpacecraftState spacecraftModel = propagate(forceModelsList, assembly);
        Assert.assertEquals(
            0,
        ...
        final SpacecraftState spacecraftModel2 = propagate(data.getForceModelsList(), assembly2);
        Assert.assertEquals(
            0,
            spacecraftOriginal.getPVCoordinates().getPosition()
     ...
     */
    private static SpacecraftState propagate(final List<ForceModel> forceList, final Assembly assembly)
        throws PatriusException {
        // Initial orbit
        ...
        final NumericalPropagator prop = new NumericalPropagator(integrator, initialState.getFrame(),
