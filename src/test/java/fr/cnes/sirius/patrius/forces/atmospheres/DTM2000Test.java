...
public class DTM2000Test {
    ...
    public void testWithOriginalTestsCases() throws PatriusException, ParseException {
        ...
        data =
            this.computeData(earth, 185, 800 * 1000, 0, MathLib.toRadians(40), 16 * FastMath.PI / 12, 150, 150, 0, 0);
        ...
        data =
            this.computeData(earth, 275, 800 * 1000, 0, MathLib.toRadians(40), 16 * FastMath.PI / 12, 150, 150, 0, 0);
        Assert.assertEquals(0, (roTestCase - data.getDensity()) / roTestCase, 1e-14);
        Assert.assertEquals(0, (tzTestCase - data.getLocalTemperature()) / tzTestCase, 1e-13);
        Assert.assertEquals(0, (tinfTestCase - data.getExosphericTemperature()) / tinfTestCase, 1e-13);
        ...
        data =
            this.computeData(earth, 355, 800 * 1000, 0, MathLib.toRadians(40), 16 * FastMath.PI / 12, 150, 150, 0, 0);
        ...
        data = this.computeData(earth, 85, 800 * 1000, 0, MathLib.toRadians(40), 16 * FastMath.PI / 12, 150, 150, 0, 0);
        ...
        this.computeData(earth, 15, 500 * 1000, 0, MathLib.toRadians(-70), 16 * FastMath.PI / 12, 70, 70, 0, 0);

        // IDEM., alt=800.
        // ro= 1.9556768571305D-18
        // tz= 793.65549797919
        // tinf= 793.65549802348
        data = this.computeData(earth, 15, 800 * 1000, 0, MathLib.toRadians(-70), 16 * FastMath.PI / 12, 70, 70, 0, 0);
    ...
    public final void testGetSpeedOfSound() throws PatriusException {
        ...
        final double ref = 391.1109242375014;
    ...
    public void testAtmosphereData() throws PatriusException {
        ...
        // Non-regression
        Assert.assertEquals(8.743030418768101E-17, density, 0.);
        Assert.assertEquals(1176.5772892517557, localTemperature, 0.);
        Assert.assertEquals(1176.5772892565271, exosphericTemperature, 0.);
        Assert.assertEquals(4.5195134429592984E-17, densityHe, 0.);
        Assert.assertEquals(1.003251773895721E-22, densityO, 0.);
        Assert.assertEquals(2.5130906021254873E-31, densityN2, 0.);
        Assert.assertEquals(2.460372355474353E-35, densityO2, 0.);
        Assert.assertEquals(0., densityAr, 0.);
        Assert.assertEquals(4.223506943291039E-17, densityH, 0.);
        Assert.assertEquals(0., densityN, 0.);
        Assert.assertEquals(0., densityAnomalousOxygen, 0.);
        Assert.assertEquals(2.550799913441807, meanAtomicMass, 0.);
    }
