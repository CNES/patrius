...
public class VariableMassTest {
    ...
    public void testMassModel() throws PatriusException {
        ...
        builder.addPart(this.part2, "Main2", Transform.IDENTITY);

        // inertia simple prop
        final TankProperty mp = new TankProperty(1500);
        builder.addProperty(mp, "Main2");
        ...
        Assert.assertEquals(0,
            ((result.getMass("Main2") + (result.getMass("part2")) - massProvider.getTotalMass()) / (result
                .getMass("Main2") + (result.getMass("part2")))), Precision.DOUBLE_COMPARISON_EPSILON);
