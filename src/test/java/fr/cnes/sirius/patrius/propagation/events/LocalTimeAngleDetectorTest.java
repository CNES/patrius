...
public class LocalTimeAngleDetectorTest {
    ...
    public static CircularOrbit orbitRetro;

    /** A Circular prograde orbit used for the tests. */
    public static CircularOrbit orbitPro;

    /** A state used for the tests. */
    public static SpacecraftState state;

    /** Initial propagator date. */
    public static AbsoluteDate iniDate;
     ...
     *
     * @throws PatriusException
     *         If an error occurs in the detector creation
     *
     * @referenceVersion 4.11
     *
     * @nonRegressionVersion 4.11
    ...
    public void testLocalTimeAngleDetectorCelestialBodyFrame() throws PatriusException {
        ...
        final CelestialBodyFrame frame = new CelestialBodyFrame(FramesFactory.getGCRF(), Transform.IDENTITY, "Frame",
            true, earth);
        final LocalTimeAngleDetector detectorCelestialBody =
     ...
     * @testType UT
     *
     * @testedFeature Getters from CelestialBodyFactory
     *
     * @testedMethod {@link LocalTimeAngleDetector#getSun(), LocalTimeAngleDetector#getFrame() }
     *
     * @description simple getter test
     *
     * @input constructor parameters: the sun CelestialBody and its CelestialBodyFrame
     *
     * @output a {@link LocalTimeAngleDetector}
     *
     * @testPassCriteria the getters work successfully
     *
    ...
    public void testGetters() throws PatriusException {
        ...
        // Construct object
        final CelestialBody sun = CelestialBodyFactory.getSun();
        final CelestialBodyFrame frame = CelestialBodyFactory.getSun().getInertialFrame();

        final LocalTimeAngleDetector detector = new LocalTimeAngleDetector(-FastMath.PI,
            LocalTimeAngleDetector.DEFAULT_MAXCHECK, LocalTimeAngleDetector.DEFAULT_THRESHOLD,
            frame, Action.STOP, false, sun);

        // Test getters
        // Check that the getter for the sun returns the input value
        Assert.assertEquals(sun, detector.getSun());

        // Check that the getter for the frame returns the input value
        Assert.assertEquals(frame, detector.getFrame());
    }
