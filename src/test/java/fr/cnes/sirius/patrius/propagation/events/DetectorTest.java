...
public class DetectorTest {
    ...
    public void testDetectionPropagationDelay() throws PatriusException {
        ...
        Report.printMethodHeader("testDetectionPropagationDelay", "Delay computation", "Math", 2E-8, ComparisonType.ABSOLUTE);
        ...
        final Frame bodyFrame = new Frame(station.getTopoFrame(), new Transform(AbsoluteDate.J2000_EPOCH, Vector3D.PLUS_K.scalarMultiply(311E3)), "BodyFrame1");
        ...
        final Frame bodyFrame2 = new Frame(station.getTopoFrame(), new Transform(AbsoluteDate.J2000_EPOCH, Vector3D.PLUS_K.scalarMultiply(311E3)), "BodyFrame1");
        ...
        // Beta angle detector (accuracy probably limited because beta angle remains almost constant due to simplified configuration)
        ...
        // Sensor inhibition detector (reference result is similar to station result (see below) but inhibition target is 311km above station
    ...
    private static Assembly buildAssembly(final String name) throws PatriusException {
        ...
        final BodyShape earth = new OneAxisEllipsoid(Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                0, FramesFactory.getTIRF(), "");
        final TopocentricFrame topo = new TopocentricFrame(earth, new GeodeticPoint(0, FastMath.PI / 2., 0), "");
        final Frame bodyFrame = new Frame(topo, new Transform(AbsoluteDate.J2000_EPOCH, Vector3D.PLUS_K.scalarMultiply(311E3)), "BodyFrame1");
    ...
    private static CelestialBody buildDelayedSun(final AbsoluteDate eventDate) throws PatriusException {
        ...
        // Build "Delayed Sun" sun2. Delayed Sun is such that a ray leaving sun1 at light speed should arrive on the spacecraft at
        // same date than a instantaneous ray of sun2
        final AbsoluteDate sunDateAtEvent = VacuumSignalPropagationModel.getSignalEmissionDate(CelestialBodyFactory.getSun(), initialOrbit,
                eventDate, 1E-12, PropagationDelayType.LIGHT_SPEED, FramesFactory.getICRF());
        return new MeeusSun() {
            /** Serializable UID. */
     ...
     * @throws PatriusException thrown if failed
     */
    private static void checkPropagationWithStation(final AbstractDetector detector, final double expectedDelay) throws PatriusException {
