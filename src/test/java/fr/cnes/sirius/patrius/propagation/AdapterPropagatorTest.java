...
public class AdapterPropagatorTest {
    ...
    @Test
    public void testLowEarthOrbit() throws PatriusException {
        ...
        final AdapterPropagator.DifferentialEffect effect = new SmallManeuverAnalyticalModel(
                adapterPropagator.propagate(t0), dV.negate(), isp, THRUSTER);
    ...
    public void testEccentricOrbit() throws PatriusException {
        final Orbit heo = new KeplerianOrbit(90000000.0, 0.92, MathLib.toRadians(98.0), MathLib.toRadians(12.3456),
                MathLib.toRadians(123.456), MathLib.toRadians(1.23456), PositionAngle.MEAN, FramesFactory.getEME2000(),
                new AbsoluteDate(new DateComponents(2004, 01, 01), new TimeComponents(23, 30, 00.000),
                        TimeScalesFactory.getUTC()), Constants.EIGEN5C_EARTH_MU);
        final double mass = 5600.0;
    ...
    public void testNonKeplerian() throws PatriusException, ParseException, IOException {
        ...
        final BoundedPropagator withoutManeuver = getEphemeris(leo, mass, 10,
                new LofOffset(leo.getFrame(),
                LOFType.VNC), t0, Vector3D.ZERO, f, isp, true, true, gravityField, 8, 8);
        final BoundedPropagator withManeuver = getEphemeris(leo, mass, 10,
                new LofOffset(leo.getFrame(),
                LOFType.VNC), t0, dV, f, isp, true, true, gravityField, 8, 8);

        // we set up a model that reverts the maneuvers
        final AdapterPropagator adapterPropagator = new AdapterPropagator(withManeuver);
        final SpacecraftState state0 = adapterPropagator.propagate(t0);
        final AdapterPropagator.DifferentialEffect directEffect = new SmallManeuverAnalyticalModel(state0, dV.negate(),
                isp, THRUSTER);
        final AdapterPropagator.DifferentialEffect derivedEffect = new J2DifferentialEffect(state0, directEffect,
                false, gravityField);
    ...
    private static BoundedPropagator getEphemeris(final Orbit orbit,
            final double mass,
            final int nbOrbits,
            ...
            final double f,
            final double isp,
            final boolean sunAttraction,
            final boolean moonAttraction,
        ...
        if (dV.getNorm() > 1.0e-6) {
            ...
            final double vExhaust = Constants.G0_STANDARD_GRAVITY * isp;
            final double dt = -(mass * vExhaust / f) * MathLib.expm1(-dV.getNorm() / vExhaust);
            final ContinuousThrustManeuver maneuver = new ContinuousThrustManeuver(t0.shiftedBy(-0.5 * dt), dt,
                    new PropulsiveProperty(f, isp), dV.normalize(), massModel, tank);
        ...
        if (sunAttraction) {
            ...
            propagator.addForceModel(new ThirdBodyAttraction(sunGravityModel));
        }
        ...
        if (moonAttraction) {
            ...
            propagator.addForceModel(new ThirdBodyAttraction(moonGravityModel));
        }

        if (gravityField != null) {
            final CunninghamGravityModel earthGravityModel = new CunninghamGravityModel(FramesFactory.getGTOD(false),
                gravityField.getAe(), gravityField.getMu(), gravityField.getC(degree, order, false), gravityField.getS(
                    degree, order, false));
            earthGravityModel.setCentralTermContribution(false);
            propagator.addForceModel(new DirectBodyAttraction(earthGravityModel));
        }
     ...
     *         if an error occurs
     * @description Evaluate the adapter propagator serialization / deserialization process.
     *
     * @testPassCriteria The adapter propagator can be serialized and deserialized.
     */
    @Test
    public void testSerialization() throws PatriusException {
        ...
        // Serialize and deserialize the adapter propagator
        final AdapterPropagator deserializedAdapterPropagator = TestUtils
                .serializeAndRecover(adapterPropagator);

        for (AbsoluteDate t = t0.shiftedBy(0.5 * dt); t.compareTo(withoutManeuver.getMaxDate()) < 0; t = t
                .shiftedBy(60.0)) {
            final PVCoordinates pvWithout = withoutManeuver.getPVCoordinates(t, leo.getFrame());
            final PVCoordinates pvReverted = deserializedAdapterPropagator.getPVCoordinates(t,
                    leo.getFrame());
            final double revertError = new PVCoordinates(pvWithout, pvReverted).getPosition()
                    .getNorm();
            Assert.assertEquals(0, revertError, 0.45);
        }
    }
