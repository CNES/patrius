...
public class TabulatedEphemerisTest {
    ...
    private static void checkEphemerides(final Propagator eph1, final Propagator eph2,
     ...
     *         if an error occurs
     * @description Evaluate the tabulated ephemeris serialization / deserialization process.
     *
     * @testPassCriteria The tabulated ephemeris can be serialized and deserialized.
     */
    @Test
    public void testSerialization() throws PatriusException {
        ...
        final MassProvider massModel = new SimpleMassModel(mass, "DEFAULD_MASS");
        final int nbIntervals = 720;
        final EcksteinHechlerPropagator eck = new EcksteinHechlerPropagator(transPar, this.ae, mu,
                FramesFactory.getCIRF(), this.c20, this.c30, this.c40, this.c50, this.c60,
                massModel, ParametersType.OSCULATING);
        eck.setAttitudeProvider(new ConstantAttitudeLaw(frame, Rotation.IDENTITY));
        final List<SpacecraftState> tab = new ArrayList<>(nbIntervals + 1);
        for (int j = 0; j <= nbIntervals; j++) {
            final AbsoluteDate current = initDate.shiftedBy((j * deltaT) / nbIntervals);
            tab.add(eck.propagate(current));
        ...
        final Ephemeris ephem = new Ephemeris(tab, 2);
        final Ephemeris deserializedEphem = TestUtils.serializeAndRecover(ephem);

        for (int j = 0; j <= nbIntervals; j++) {
            final AbsoluteDate current = initDate.shiftedBy((j * deltaT) / nbIntervals);
            Assert.assertEquals(ephem.getPVCoordinates(current, frame),
                    deserializedEphem.getPVCoordinates(current, frame));
        }
    }
