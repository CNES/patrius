...
public class IntegratedEphemerisTest {
    ...
    public void testNormalKeplerIntegration() throws PatriusException {
        ...
        final SpacecraftState initialState = new SpacecraftState(this.initialOrbit, mass);
        this.numericalPropagator = new NumericalPropagator(this.integrator, initialState.getFrame());
        this.numericalPropagator.setEphemerisMode();
        this.numericalPropagator.setInitialState(initialState);
        this.numericalPropagator.addForceModel(new DirectBodyAttraction(new NewtonianGravityModel(
            this.numericalPropagator.getMu())));
        this.numericalPropagator.setMassProviderEquation(mass);
        final PVCoordinates finalPVNum = this.numericalPropagator.propagate(finalDate)
                .getPVCoordinates();
        Assert.assertTrue(this.numericalPropagator.getCalls() < 3200);
        final IntegratedEphemeris ephemeris = (IntegratedEphemeris) this.numericalPropagator
        ...
        this.numericalPropagator = new NumericalPropagator(this.integrator, state.getFrame());
        ...
        Assert.assertEquals(0, finalPVNum.getPosition().subtract(actualPV.getPosition()).getNorm(),
                Precision.EPSILON);
        Assert.assertEquals(0, finalPVNum.getVelocity().subtract(actualPV.getVelocity()).getNorm(),
                Precision.EPSILON);
        Assert.assertEquals(0, finalPVNum.getAcceleration().subtract(actualPV.getAcceleration())
    ...
    public void testPartialDerivativesIssue16() throws PatriusException {
        ...
        this.numericalPropagator = new NumericalPropagator(this.integrator, FramesFactory.getEME2000(),
            OrbitType.CARTESIAN,
            PositionAngle.TRUE);
        this.numericalPropagator.setEphemerisMode();
        final PartialDerivativesEquations derivatives = new PartialDerivativesEquations(eqName,
                this.numericalPropagator);
        // The initial state should contain the partial derivatives:
        final SpacecraftState initialState = derivatives.setInitialJacobians(new SpacecraftState(
                this.initialOrbit, this.initialAttitude));
        ...
        ephemeris.setMasterMode(new PatriusStepHandler() {
            ...
            public void
                ...
                try {
                    ...
                    final double[] p = interpolator.getInterpolatedState().getAdditionalState(
                            eqName);
                    Assert.assertEquals(mapper.getAdditionalStateDimension(), p.length);
                    mapper.getStateJacobian(state, this.dYdY0.getDataRef());
                    mapper.getParametersJacobian(state, null); // no parameters, this is a no-op and
                                                               // should work
                    final RealMatrix deltaId = this.dYdY0.subtract(MatrixUtils
     ...
     *         if an error occurs
     * @description Evaluate the integrated ephemeris serialization / deserialization process.
     *
     * @testPassCriteria The integrated ephemeris can be serialized and deserialized.
     */
    @Test
    public void testSerialization() throws PatriusException {
        ...
        final AbsoluteDate finalDate = initialDate.shiftedBy(10);
        ...
        final IntegratedEphemeris ephemeris = (IntegratedEphemeris) this.numericalPropagator
                .getGeneratedEphemeris();
        final IntegratedEphemeris deserializedEphemeris = TestUtils.serializeAndRecover(ephemeris);

        for (int i = 0; i <= 10; i++) {
            final AbsoluteDate currentDate = initialDate.shiftedBy(i);
    ...
    public void setUp() throws PatriusException {
        ...
        this.integrator = new DormandPrince853Integrator(0.001, 500,
                absTolerance, relTolerance);
        this.integrator.setInitialStepSize(100);
        this.numericalPropagator = new NumericalPropagator(this.integrator);
