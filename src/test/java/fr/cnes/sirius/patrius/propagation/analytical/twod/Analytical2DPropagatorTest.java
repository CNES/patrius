...
public class Analytical2DPropagatorTest {
    ...
    public void setup() throws URISyntaxException, IOException, PatriusException {
        ...
        try {
            new Analytical2DPropagator(new AttitudeLawLeg(new LofOffset(FramesFactory.getGCRF(), LOFType.TNW),
                this.ref.shiftedBy(10), this.ref.shiftedBy(100)), model, this.ref);
            Assert.fail();
        ...
        this.prop = new Analytical2DPropagator(model, this.ref);
        this.prop = new Analytical2DPropagator(new LofOffset(FramesFactory.getGCRF(), LOFType.TNW), model, this.ref);
    }
     ...
     */
    @Test
    public void testDifferentOrders() throws URISyntaxException, IOException, PatriusException {
     ...
     * @description checks that the conversion mean - osc - mean returns the initial orbit. This method uses different orders
    ...
    public void testMeanOsculatingConversion() throws PatriusException {
        ...
        this.prop = new Analytical2DPropagator(model, this.ref, new int[] { 10, 11, 12, 13, 14, 15 });

        // Initialization with osculating values
        final AbsoluteDate date = AbsoluteDate.FIFTIES_EPOCH_TAI.shiftedBy(17532 * 86400.);
        final CircularOrbit orbit = (CircularOrbit) prop.propagateOrbit(date);
        
        // Osculating to mean conversion
        final CircularOrbit mean = (CircularOrbit) prop.osc2mean(orbit);
        
        // Mean to osculating conversion
        final CircularOrbit osc = (CircularOrbit) prop.mean2osc(mean);
        
        // Mean parameters
        final CircularOrbit mean2 = (CircularOrbit) prop.propagateMeanOrbit(date);
    ...
    public void testMeanOsculatingConversionFramesTypes() throws PatriusException {
        ...
        // Osculating to mean conversion
        final Orbit mean = prop.osc2mean(initialOrbit);
        
        // Mean to osculating conversion
        final KeplerianOrbit osc = (KeplerianOrbit) prop.mean2osc(mean);
...
class MyImpulseManeuver extends AbstractDetector implements EventDetector {
...
class MyElevationDetector extends ElevationDetector {
    ...
    TreeMap<AbsoluteDate, PVCoordinates> data = new TreeMap<AbsoluteDate, PVCoordinates>();
