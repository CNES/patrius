...
public class Analytical2DPropagatorTest {
    ...
    public void setup() throws URISyntaxException, IOException, PatriusException {
        ...
        try {
            new Analytical2DPropagator(new AttitudeLawLeg(new LofOffset(FramesFactory.getGCRF(), LOFType.TNW),
 this.ref.shiftedBy(10), this.ref.shiftedBy(100)), this.model,
                    this.ref);
            Assert.fail();
        ...
        this.prop = new Analytical2DPropagator(new LofOffset(FramesFactory.getGCRF(), LOFType.TNW),
                this.model, this.ref);
    }
     ...
     * @output an orbit
     * 
     * @testPassCriteria the resulting orbit is the same as the expected one, to the threshold 2e-14 on a relative
     *                   scale.
     * 
    ...
    public void testPropagate() throws URISyntaxException, IOException, PatriusException {
        ...
        final Analytical2DOrbitModel model =
            new Analytical2DOrbitModel(this.aModel, this.exModel, this.eyModel, this.iModel, this.rModel, this.lModel,
                this.DEFAULT_MASS, Constants.EGM96_EARTH_MU);
     ...
     * 
     * @testPassCriteria developement orders have been taken into account, not maximum orders (orbit is as expected)
     * 
    ...
    public void testDifferentOrders() throws PatriusException {
        ...
        final Analytical2DParameterModel a = new Analytical2DParameterModel(new DatePolynomialFunction(date, coefa),
            trig);
        final Analytical2DParameterModel ex = new Analytical2DParameterModel(new DatePolynomialFunction(date, coef0),
            trig);
        final Analytical2DParameterModel ey = new Analytical2DParameterModel(new DatePolynomialFunction(date, coef0),
            trig);
        final Analytical2DParameterModel i = new Analytical2DParameterModel(new DatePolynomialFunction(date, coef0),
            trig);
        final Analytical2DParameterModel lna = new Analytical2DParameterModel(new DatePolynomialFunction(date, coef0),
            trig);
        final Analytical2DParameterModel alpha = new Analytical2DParameterModel(
            new DatePolynomialFunction(date, coef0), trig);
        ...
        // Check that elements are same as initial => order taken into accounts where only first orders
     ...
     * @description checks that the conversion mean - osc - mean returns the initial orbit. This method uses different
     *              orders
    ...
    public void testMeanOsculatingConversion() throws PatriusException {
        ...
        this.prop = new Analytical2DPropagator(this.model, this.ref, new int[] { 10, 11, 12, 13,
                14, 15 });
        ...
        Assert.assertEquals(0.,
                relDiff(orbit.getRightAscensionOfAscendingNode(),
                        osc.getRightAscensionOfAscendingNode()), eps);
        Assert.assertEquals(0.,
                relDiff(orbit.getAlphaM() % (2. * FastMath.PI), osc.getAlphaM()
                        % (2. * FastMath.PI)), eps);

        // Check the mean parameters from the propagateMeanOrbit method are equal to the mean parameters from the osc -
        // mean conversion
        ...
        Assert.assertEquals(0.,
                relDiff(mean2.getRightAscensionOfAscendingNode(),
                        mean.getRightAscensionOfAscendingNode()), eps);
        Assert.assertEquals(0.,
                relDiff(mean2.getAlphaM() % (2. * FastMath.PI), mean.getAlphaM()
                        % (2. * FastMath.PI)), eps);
    }
     ...
     * @testedMethod {@link Analytical2DPropagator#mean2osc(Orbit)}
     * @testedMethod {@link Analytical2DPropagator#osc2mean(Orbit)}
     * @description Checks that a mean - osculating (as well as the other way around) conversion returns an orbit in
    ...
    public void testMeanOsculatingConversionFramesTypes() throws PatriusException {
        ...
        Assert.assertEquals(0.,
                relDiff(osc.getPerigeeArgument(), initialOrbit.getPerigeeArgument()), eps);
        Assert.assertEquals(0.,
                relDiff(osc.getRightAscensionOfAscendingNode(),
                        initialOrbit.getRightAscensionOfAscendingNode()), eps);
        Assert
            .assertEquals(
                0.,
                relDiff(osc.getMeanAnomaly() % (2. * FastMath.PI), initialOrbit.getMeanAnomaly()
                        % (2. * FastMath.PI)), eps);
    }
     ...
     *              Test that osc2mean(orbit) method throws a PatriusException when called if convergence threshold is
     *              too low.
     *              Then the threshold is set to a larger value and the algorithm should converge
     * @testPassCriteria Exception is thrown if threshold is too low, convergence is reached is threshold is set to a
     *                   higher value
     */
    @Test
    ...
    public void testSerialization() throws PatriusException {
        ...
        final Analytical2DOrbitModel model = new Analytical2DOrbitModel(this.aModel, this.exModel,
                this.eyModel, this.iModel, this.rModel, this.lModel, massModel,
                Constants.EGM96_EARTH_MU);

        final Analytical2DPropagator propagator = new Analytical2DPropagator(model, this.ref,
                model.getMaxOrders());
        final Analytical2DPropagator deserializedPropagator = TestUtils
...
class MyImpulseManeuver extends AbstractDetector {
    ...
    public SpacecraftState resetState(final SpacecraftState oldState) throws PatriusException {
        ...
        final CartesianOrbit newOrbitCartesian =
...
class MyElevationDetector extends ElevationDetector {
    ...
    public MyElevationDetector(final double elevation,
