...
public class TLEPropagatorTest {
    ...
    public void testSlaveMode() throws PatriusException {
        ...
        final TLEPropagator propagator = TLEPropagator.selectExtrapolator(this.tle, null, this.mass);
    ...
    /**
     * Test if body center belongs to the direction pointed by the satellite. The threshold for the first
     * Assert.assertEquals has been modified (from 2.0e-7 to 4.0e-7) to avoid an error. The threshold was modified again
     * during the D-1006 to 6e-7.
     * 
     * A dedicated validation test with external references exists in patrius-tools package
     * fr.cnes.sirius.patrius.tools.tle.validation. The aforementioned test includes a non regression mechanism and
     * ensures the modifications introduced during the D-1006 have no effect on the test cases implemented therein.
     */
    @Test
    ...
    private static class DistanceChecker implements PatriusFixedStepHandler {
        ...
        public void handleStep(final SpacecraftState currentState, final boolean isLast) throws PropagationException {
            try {
                ...
                final Transform transform = currentState.getFrame().getTransformTo(this.itrf, currentState.getDate());
                final Vector3D zSatITRF = transform.transformVector(zSat);

                // Transform satellite position/velocity from inertial frame to ITRF
                final PVCoordinates pvSatITRF = transform.transformPVCoordinates(currentState.getPVCoordinates());

                // Line containing satellite point and following pointing direction
                final Line pointingLine = new Line(pvSatITRF.getPosition(), pvSatITRF.getPosition().add(
                    Constants.WGS84_EARTH_EQUATORIAL_RADIUS, zSatITRF));
    ...
    public void propagationWithMassChange() throws PatriusException {
        ...
        propagator = TLEPropagator.selectExtrapolator(this.tle, new ConstantAttitudeLaw(FramesFactory.getEME2000(),
            Rotation.IDENTITY), this.mass);
        // Get the initial state:
