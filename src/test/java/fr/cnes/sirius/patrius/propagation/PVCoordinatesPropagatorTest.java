...
public class PVCoordinatesPropagatorTest {
    ...
    public static final void setUpBeforeClass() throws PatriusException {
        ...
        Report.printClassHeader(PVCoordinatesPropagatorTest.class.getSimpleName(), "PV coordinates propagator");
     ...
     * 
     * @description Test the instantiation without exception of a PVCoordinateProvider from an EphemerisPvHermite
     *              based on a table of spacecraft states computed with a numerical propagation in setup
     * 
     * @input EphemerisPvHermite ephPvHermite = new EphemerisPvHermite(tabSpacecraftStates, null, null);
     * 
     ...
     */
    @Test
    public final void testPVCoordinatePropagator() throws PatriusException {
        ...
        final PVCoordinatesPropagator pvCoordPropag = new PVCoordinatesPropagator(ephPvHermite,
            tabSpacecraftStates[0].getDate(), tabSpacecraftStates[0].getMu(), tabSpacecraftStates[0].getFrame());
     ...
     * 
     * @description Test the instantiation without exception of a PVCoordinateProvider from an EphemerisPvHermite
     *              based on a table of spacecraft states computed with a numerical propagation in setup and associated
     *              to a table of
     *              additional states named "TANK" with values { 21 bar, 18 deg } with constant evolution of 1 (bar,
     *              deg) per second.
     *              For SimpleAdditionalStateProvider, test that a propagation to a date between two step return the
     *              expected result.
     * 
     * @input EphemerisPvHermite ephPvHermite = new EphemerisPvHermite(tabSpacecraftStates, null, null);
     * @input SimpleAdditionalStateProvider simpleAddStateProv build from an extraction of the spacecraft states table
     *        computed in setup
     * @input PVCoordinatePropagator pvCoordPropag a propagator based on ephPvHermite and simpleAddStateProv
     * 
     ...
     */
    @Test
    public final void testPVCoordinatePropagatorWithAdditionalStateProvider() throws PatriusException {
        ...
        final SimpleAdditionalStateProvider simpleAddStateProv =
            new SimpleAdditionalStateProvider("TANK", tabDate, tabAdditionalStates, null);
        ...
        final List<AdditionalStateProvider> listAddStatesProv = new ArrayList<AdditionalStateProvider>();
        ...
        final PVCoordinatesPropagator pvCoordPropag =
            new PVCoordinatesPropagator(ephPvHermite,
        ...
        final SpacecraftState sstate = pvCoordPropag.propagate(interval.getLowerData().shiftedBy(10.5 * STEP));
        ...
        for (final String key : addStatesResult.keySet()) {
            ...
            Assert.assertEquals(0., (addStatesValues[0] - (21.0 + 10.5 * STEP)) / addStatesValues[0], EPS);
            Assert.assertEquals(0., (addStatesValues[1] - (18.0 + 10.5 * STEP)) / addStatesValues[1], EPS);
        }
    }
     ...
     * @testedMethod {@link SimpleAdditionalStateProvider#getAdditionalState(fr.cnes.sirius.patrius.time.AbsoluteDate)}
     * 
     * @description Test the propagation with a simple additional state provider until a date out of bounds
     *              of the additional state ephemeris used to build the provider.
     * 
     * @input KeplerianPropagator a keplerian propagator to be able to propagate further than the setup ephemeris
     * @input SimpleAdditionalStateProvider simpleAddStateProv build from an extraction of the spacecraft states table
     *        computed in setup
     * 
     * @output SpacecraftState sstate : the propagation state at a known date further than the last input additional
     *         states
     * 
     * @testPassCriteria sstate contains <"TANK", {21.0+duration, 18.0+duration}> additional state
     * 
     ...
     */
    @Test
    public final void testAdditionalStateProviderOutOfBound() throws PatriusException {
        ...
        final SimpleAdditionalStateProvider simpleAddStateProv =
            new SimpleAdditionalStateProvider("TANK", tabDate, tabAdditionalStates, searchIndex);
        // check if it has been correctly instantiate
        Assert.assertNotNull(simpleAddStateProv);

        // Create a keplerian propagator
        final KeplerianPropagator kepProp = new KeplerianPropagator(
            new KeplerianOrbit(7000E3, 0.001, MathLib.toRadians(70), 0., 0., 0.,
                PositionAngle.MEAN, FramesFactory.getEME2000(), interval.getLowerData(), mu));
        ...
        // The additional state shall be "TANK" {21+duration, 18+duration}
        final SpacecraftState sstate = kepProp
            .propagate(tabSpacecraftStates[length - 1].getDate().shiftedBy(10 * STEP));
        final Map<String, double[]> addStatesResult = sstate.getAdditionalStates();
        for (final String key : addStatesResult.keySet()) {
            Assert.assertTrue(key.equalsIgnoreCase("TANK"));
            final double[] addStatesValues = addStatesResult.get(key);
            ...
            Assert.assertEquals(0.,
                (addStatesValues[0] -
                    (tabSpacecraftStates[length - 1].getAdditionalState("TANK")[0] + (10 * STEP)))
                    / addStatesValues[0],
                EPS);
            Assert.assertEquals(0.,
                (addStatesValues[1] -
                    (tabSpacecraftStates[length - 1].getAdditionalState("TANK")[1] + (10 * STEP)))
                    / addStatesValues[1],
                EPS);
        }
    }
     ...
     *              The PV Coordinates from the spacecraft state return by propagation should be equals
     *              to PV Coordinates from interpolation.
     * 
     * @input EphemerisPvHermite ephPvHermite instantiate from spacecraft states table computed in setup
     ...
     *                   relative to the norm is equal to zero plus or minus double comparison precision
     * 
     * @throws PatriusException
     *         should not happens
     * 
    ...
    public void testPropagateOrbit() throws PatriusException {
        ...
        Report.printMethodHeader("testPropagateOrbit", "Propagation", "Math", EPS, ComparisonType.RELATIVE);

        final EphemerisPvHermite ephPvHermite = new EphemerisPvHermite(tabSpacecraftStates, null, null);
        final PVCoordinatesPropagator pvCoordPropagator =
            new PVCoordinatesPropagator(ephPvHermite, tabSpacecraftStates[0].getDate(),
        ...
        final PVCoordinates pvCoordProp = pvCoordPropagator.propagate(targetDate).getPVCoordinates();
        Assert.assertEquals(
            pvCoordInter.getPosition().distance(pvCoordProp.getPosition()) / pvCoordInter.getPosition().getNorm(),
            0.0, EPS);
        Assert.assertEquals(
            pvCoordInter.getVelocity().distance(pvCoordProp.getVelocity()) / pvCoordInter.getVelocity().getNorm(),
            0.0, EPS);
     ...
     * 
     * @input EphemerisPvLagrange ephPvLagrange = new EphemerisPvLagrange(tabSpacecraftStates, 8, null);
    ...
    public final void testPVCoordinateConstructorException() throws PatriusException {
     ...
     */
    @Test(expected = PatriusException.class)
    public final void testPVCoordinatePropagatorException() throws PatriusException {
        ...
        // build a propagator
        final PVCoordinatesPropagator pvCoordPropag =
            new PVCoordinatesPropagator(ephPvLagrange, tabSpacecraftStates[4].getDate(),
                tabSpacecraftStates[4].getMu(), tabSpacecraftStates[4].getFrame());
        Assert.assertNotNull(pvCoordPropag);
     ...
     * @testedFeature {@link features#PROPAGATION}
     * 
     * @testedMethod {@link PVCoordinatesPropagator#resetInitialState(fr.cnes.sirius.patrius.propagation.SpacecraftState)}
     * 
     * @description Test the reset initial state exception
     * 
     * @testPassCriteria raise an propagation exception
     * 
     ...
     */
//    @Test
//    public final void testResetInitialStateException() throws PatriusException {
//
...
//
//        try {
//            pvCoordPropag.resetInitialState(tabSpacecraftStates[0]);
//            Assert.fail();
...
//    }
     ...
     * 
     * @description TODO explanation of the test case
     * 
     * @input TODO one per input of the function for this test case
     *        with format : <inputType> <inputName> = <inputValue> : <inputDescription>
     * 
     * @output TODO the type of the output
     * 
     * @testPassCriteria TODO the expected output of the method for this test case
     * 
     * @see the references used for the validation - TODO
     * 
     * @comments TODO any more comments
     * 
     * @referenceVersion TODO the number of the last reference version (eg 1.2)
     * 
     * @nonRegressionVersion TODO the number of the last non-regression version (eg 2.3)
     */
    // @Test
    // public final void perfTest() throws OrekitException {
    //
    // // EphemerisPvHermite instantiation from spacecraft states table computed in setup
    // final EphemerisPvHermite ephPvHermite =
    // new EphemerisPvHermite(tabSpacecraftStates, null, null);
    //
    // //HermiteEphemeris instantiation from spacecraft states table computed in setup
    // final HermiteEphemeris HermiteEphem =
    // new HermiteEphemeris(tabSpacecraftStates);
    //
    // // Number of iteration to have test duration around 5 or 10 seconds.
    // final int nbIteration = 10000000;
    // AbsoluteDate target;
    // final double step = interval.getDuration() / nbIteration;
    //
    // // Performance of the EphPvHermite (provider new implementation)
    // PVCoordinates pvEphPvHermite;
    // long msBef = System.currentTimeMillis();
    // for (int i = 0; i < nbIteration; i++) {
    // target = tabSpacecraftStates[0].getDate().shiftedBy(step * i / 1000.);
    // pvEphPvHermite =
    // ephPvHermite.getPVCoordinates(target, null);
    // }
    // long msAft = System.currentTimeMillis();
    // System.out.println("Perf. hermite provider (new) : " + (msAft - msBef));
    //
    //
    // final PVCoordinatePropagator pvCoordPropagator =
    // new PVCoordinatePropagator(ephPvHermite, tabSpacecraftStates[0].getDate(),
    // tabSpacecraftStates[0].getMu(), tabSpacecraftStates[0].getFrame());
    // msBef = System.currentTimeMillis();
    // for (int i = 0; i < nbIteration; i++) {
    // target = tabSpacecraftStates[0].getDate().shiftedBy(step * i / 1000.);
    // final PVCoordinates pvCoordProp = pvCoordPropagator.propagate(target).getPVCoordinates();
    // }
    // msAft = System.currentTimeMillis();
    // System.out.println("Perf. propagator with a hermite provider (new) : " + (msAft - msBef));
    //

    // // Performance of the PVHermiteEphem (old implementation)
    // PVCoordinates pvHermiteEphem;
    // msBef = System.currentTimeMillis();
    // for (int i = 0; i < nbIteration; i++) {
    // target = tabSpacecraftStates[0].getDate().shiftedBy(step);
    // pvEphPvHermite =
    // ephPvHermite.getPVCoordinates(target, null);
    // pvHermiteEphem = HermiteEphem.propagate(target).getPVCoordinates();
    // }
    // msAft = System.currentTimeMillis();
    // System.out.println("Perf. hermite propag (old) : " + (msAft - msBef));
