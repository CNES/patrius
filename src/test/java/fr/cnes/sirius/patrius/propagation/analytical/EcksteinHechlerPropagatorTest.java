 ...
 * Unless required by applicable law or agreed to in writing, software
 git diif* distributed under the License is distributed on an "AS IS" BASIS,
...
public class EcksteinHechlerPropagatorTest {
    ...
    public static void setUpBeforeClass() {
        Report.printClassHeader(EcksteinHechlerPropagatorTest.class.getSimpleName(),
    ...
    public void testMultipleMuValues() throws PatriusException {
        ...
        final EcksteinHechlerPropagator propagator1 = new EcksteinHechlerPropagator(initialOrbit,
            this.ae, mu1, initialOrbit.getFrame(), this.c20, this.c30, this.c40, this.c50,
            this.c60, ParametersType.OSCULATING);

        final EcksteinHechlerPropagator propagator2 = new EcksteinHechlerPropagator(initialOrbit,
            this.ae, mu2, initialOrbit.getFrame(), this.c20, this.c30, this.c40, this.c50,
            this.c60, ParametersType.OSCULATING);

        final SpacecraftState finalOrbit1 = propagator1.propagate(AbsoluteDate.J2000_EPOCH
            .shiftedBy(10.0));
        final SpacecraftState finalOrbit2 = propagator2.propagate(AbsoluteDate.J2000_EPOCH
            .shiftedBy(10.0));

        Assert.assertTrue(MathLib.abs(finalOrbit1.getA() - finalOrbit2.getA()) > 0);
        Assert.assertTrue(MathLib.abs(finalOrbit1.getEquinoctialEx()
                - finalOrbit2.getEquinoctialEx()) > 0);
        Assert.assertTrue(MathLib.abs(finalOrbit1.getEquinoctialEy()
                - finalOrbit2.getEquinoctialEy()) > 0);
    ...
    public void sameDateCartesian() throws PatriusException {
        ...
        // -----------------------
        final EcksteinHechlerPropagator extrapolator = new EcksteinHechlerPropagator(initialOrbit,
            this.ae, this.mu, initialOrbit.getFrame(), this.c20, this.c30, this.c40, this.c50,
            this.c60, ParametersType.OSCULATING);
        ...
        Assert.assertEquals(0.0, Vector3D.distance(initialOrbit.getPVCoordinates().getPosition(),
            finalOrbit.getPVCoordinates().getPosition()), 2E-8);

        // velocity match perfectly
        Assert.assertEquals(0.0, Vector3D.distance(initialOrbit.getPVCoordinates().getVelocity(),
            finalOrbit.getPVCoordinates().getVelocity()), 2E-11);
    }
    ...
    public void sameDateKeplerian() throws PatriusException {
        ...
        final Orbit initialOrbit = new KeplerianOrbit(7209668.0, 0.5e-4, 1.7, 2.1, 2.9, 6.2,
            PositionAngle.TRUE, FramesFactory.getEME2000(), initDate, this.mu);
        ...
        // velocity match perfectly
        Assert.assertEquals(0.0, Vector3D.distance(initialOrbit.getPVCoordinates().getVelocity(),
            finalOrbit.getPVCoordinates().getVelocity()), 2.0e-11);
    ...
    public void almostSphericalBody() throws PatriusException {
        ...
        // -------------------------
        final EcksteinHechlerPropagator extrapolatorAna = new EcksteinHechlerPropagator(
            initialOrbit, this.ae, this.mu, initialOrbit.getFrame(), zc20, zc30, zc40, zc50,
            zc60, ParametersType.OSCULATING);
        ...
        Assert.assertEquals(finalOrbitAna.getA(), finalOrbitKep.getA(), 10 * Utils.epsilonTest
                * finalOrbitKep.getA());
        ...
        Assert.assertEquals(MathUtils.normalizeAngle(finalOrbitAna.getHx(), finalOrbitKep.getHx()),
            finalOrbitKep.getHx(), Utils.epsilonAngle * MathLib.abs(finalOrbitKep.getI()));
        Assert.assertEquals(MathUtils.normalizeAngle(finalOrbitAna.getHy(), finalOrbitKep.getHy()),
            finalOrbitKep.getHy(), Utils.epsilonAngle * MathLib.abs(finalOrbitKep.getI()));
        Assert.assertEquals(MathUtils.normalizeAngle(finalOrbitAna.getLv(), finalOrbitKep.getLv()),
            finalOrbitKep.getLv(), Utils.epsilonAngle * MathLib.abs(finalOrbitKep.getLv()));
        Assert.assertEquals(MathUtils.normalizeAngle(finalOrbitAna.getLE(), finalOrbitKep.getLE()),
            finalOrbitKep.getLE(), Utils.epsilonAngle * MathLib.abs(finalOrbitKep.getLE()));
        Assert.assertEquals(MathUtils.normalizeAngle(finalOrbitAna.getLM(), finalOrbitKep.getLM()),
            finalOrbitKep.getLM(), Utils.epsilonAngle * MathLib.abs(finalOrbitKep.getLM()));

    }

    /**
     * This test was added to ascertain the correctness of the public mean computation method.
     * In order to ensure this method is disconnected from the internal propagation state, a random
     * orbit
     * is given to the propagation at instantiation time. Then, an orbit taken from another test is
     * used
     * and the mean parameters obtained are compared to the mean parameters contained in the
     * internal state
    ...
    public void propagatedMeanComputation() throws PatriusException {
        ...
        final Orbit initialOrbit1 = new KeplerianOrbit(6371e3 + 400e3, .0001, .5, 0, 0, 0,
            PositionAngle.TRUE, FramesFactory.getGCRF(), initDate1, this.mu);

        // Extrapolator definition
        // -----------------------
        final MassProvider massModel = new SimpleMassModel(1000., "DEFAULT_MASS");
        final EcksteinHechlerPropagator extrapolator = new EcksteinHechlerPropagator(initialOrbit1,
            new ConstantAttitudeLaw(FramesFactory.getEME2000(), Rotation.IDENTITY), this.ae,
            this.mu, initialOrbit1.getFrame(), this.c20, this.c30, this.c40, this.c50,
            this.c60, massModel, ParametersType.OSCULATING);
    ...
    public void propagatedCartesian() throws PatriusException {
        ...
        final EcksteinHechlerPropagator extrapolator = new EcksteinHechlerPropagator(initialOrbit,
            new ConstantAttitudeLaw(FramesFactory.getEME2000(), Rotation.IDENTITY), this.ae,
            this.mu, initialOrbit.getFrame(), this.c20, this.c30, this.c40, this.c50, this.c60,
        ...
        Assert.assertEquals(LM, finalOrbit.getLM(),
            Utils.epsilonAngle * MathLib.abs(finalOrbit.getLM()));

        // test of tan ((LE - Lv)/2) :
        Assert.assertEquals(
            MathLib.tan((finalOrbit.getLE() - finalOrbit.getLv()) / 2.),
            tangLEmLv(finalOrbit.getLv(), finalOrbit.getEquinoctialEx(),
                finalOrbit.getEquinoctialEy()), Utils.epsilonAngle);
        ...
        Assert.assertEquals(deltaM, deltaE - delta,
            Utils.epsilonAngle * MathLib.abs(deltaE - delta));
        ...
        final Vector3D U = new Vector3D((1. + hx2 - hy2) / h2p1, (2. * hx * hy) / h2p1, (-2. * hy)
                / h2p1);

        final Vector3D V = new Vector3D((2. * hx * hy) / h2p1, (1. - hx2 + hy2) / h2p1, (2. * hx)
                / h2p1);
    ...
    public void hyperbolic() throws PropagationException {
        final KeplerianOrbit hyperbolic = new KeplerianOrbit(-1.0e10, 2, 0, 0, 0, 0,
            PositionAngle.TRUE, FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH,
            3.986004415e14);
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(hyperbolic,
            this.ae, this.mu, hyperbolic.getFrame(), this.c20, this.c30, this.c40, this.c50,
            this.c60, ParametersType.OSCULATING);
    ...
    public void criticalInclination() throws PatriusException {
        final AbsoluteDate initDate = AbsoluteDate.J2000_EPOCH;
        final Orbit initialOrbit = new CircularOrbit(new PVCoordinates(new Vector3D(
            -3862363.8474653554, -3521533.9758022362, 4647637.852558916), new Vector3D(
            65.36170817232278, -6056.563439401233, -4511.1247889782757)),
            FramesFactory.getEME2000(), initDate, this.mu);
    ...
    public void equatorialOrbitOK() throws PropagationException {
        final KeplerianOrbit equa = new KeplerianOrbit(1.0e10, 0, 1.0e-9, 0, 0, 0,
            PositionAngle.TRUE, FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH,
            3.986004415e14);
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(equa, this.ae,
            this.mu, equa.getFrame(), this.c20, this.c30, this.c40, this.c50, this.c60,
            ParametersType.OSCULATING);
        final SpacecraftState finalOrbit = propagator.propagate(AbsoluteDate.J2000_EPOCH
            .shiftedBy(10.0));
        final CircularOrbit ofinalOrbit = new CircularOrbit(finalOrbit.getOrbit());

        Assert.assertEquals(10.0, ofinalOrbit.getDate().durationFrom(AbsoluteDate.J2000_EPOCH),
            Utils.epsilonTest);
        ...
        Assert.assertEquals(ofinalOrbit.getRightAscensionOfAscendingNode() % (2. * FastMath.PI), 0,
            Utils.epsilonAngle);
    ...
    public void equatorialOrbitKO1() throws PropagationException {
        final KeplerianOrbit equa = new KeplerianOrbit(1.0e10, 0, 0, 0, 0, 0, PositionAngle.TRUE,
    ...
    public void equatorialOrbitKO2() throws PropagationException {
        final KeplerianOrbit equa = new KeplerianOrbit(1.0e10, 0, 1.0e-10, 0, 0, 0,
            PositionAngle.TRUE, FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH,
            3.986004415e14);
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(equa, this.ae,
            this.mu, equa.getFrame(), this.c20, this.c30, this.c40, this.c50, this.c60,
    ...
    public void ascendingNode() throws PatriusException {
        final KeplerianOrbit orbit = new KeplerianOrbit(7.8e6, 0.032, 0.4, 0.1, 0.2, 0.3,
            PositionAngle.TRUE, FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH, this.mu);
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(orbit, this.ae,
            this.mu, orbit.getFrame(), this.c20, this.c30, this.c40, this.c50, this.c60,
            ParametersType.OSCULATING);
        ...
        propagator.setMasterMode(step, new PatriusFixedStepHandler(){
            ...
            public void handleStep(final SpacecraftState currentState, final boolean isLast) {
                if (this.previous != null) {
                    Assert.assertEquals(step, currentState.getDate().durationFrom(this.previous),
                        1.0e-10);
                }
    ...
    public void setting() throws PatriusException {
        ...
        final TopocentricFrame topo = new TopocentricFrame(new EllipsoidPoint(earthShape,
            earthShape.getLLHCoordinatesSystem(), 0.389, -2.962, 0, ""), null);
    ...
    public void testPropagation() throws PatriusException {
        ...
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(orbit,
            6378136.3, 3.98600442e+14, FramesFactory.getCIRF(), -0.001082626613,
            0.000002532393, 0.000001619137, 0.000000227742, -0.000000538219,
            ParametersType.MEAN);
        ...
        Assert.assertEquals(0., relDiff(6998642.12795516569, actual.getA()), eps);
        Assert.assertEquals(0., relDiff(-0.00039236065810061, actual.getCircularEx()), eps);
        Assert.assertEquals(0., relDiff(0.00099424303447362, actual.getCircularEy()), eps);
        Assert.assertEquals(0., relDiff(0.99993775242592675, actual.getI()), eps);
        Assert.assertEquals(0.,
            relDiff(0.03183650896649683, actual.getRightAscensionOfAscendingNode()), eps);
        Assert.assertEquals(0.,
            relDiff(93.3603771547069101 % (2. * FastMath.PI), actual.getAlphaM()), 2E-13);
     ...
     * @testedMethod {@link EcksteinHechlerPropagator#propagateOrbit(AbsoluteDate)}
     *
     * @description test the Eckstein-Hechler propagation (nominal case) with and without attitude
     *              law
     *
     * @input Keplerian orbit
     *
    ...
    public void testPropagationAttitude() throws PatriusException {
        ...
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(orbit,
            ...
            0.000002532393, 0.000001619137, 0.000000227742, -0.000000538219,
            ParametersType.MEAN);
        final AttitudeProvider attitudeLaw = new ConstantAttitudeLaw(FramesFactory.getGCRF(),
            Rotation.IDENTITY);
        final EcksteinHechlerPropagator propagator2 = new EcksteinHechlerPropagator(orbit,
            attitudeLaw, 6378136.3, 3.98600442e+14, FramesFactory.getCIRF(), -0.001082626613,
            0.000002532393, 0.000001619137, 0.000000227742, -0.000000538219,
            ParametersType.MEAN);
        ...
        Assert.assertEquals(0., relDiff(actual2.getA(), actual.getA()), 0);
        Assert.assertEquals(0., relDiff(actual2.getCircularEx(), actual.getCircularEx()), 0);
        Assert.assertEquals(0., relDiff(actual2.getCircularEy(), actual.getCircularEy()), 0);
        Assert.assertEquals(0., relDiff(actual2.getI(), actual.getI()), 0);
        Assert.assertEquals(
            0.,
            relDiff(actual2.getRightAscensionOfAscendingNode(),
                actual.getRightAscensionOfAscendingNode()), 0);
        Assert.assertEquals(0., relDiff(actual2.getAlphaM(), actual.getAlphaM()), 0);
     ...
     *
     * @testPassCriteria orbit is as expected (reference : use of validated osculating propagation
     *                   and osc <=> mean
    ...
    public void testPropagationMean() throws PatriusException {
        ...
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(orbit, 6378000,
            3.9860047e14, orbit.getFrame(), -1.08263e-3, 2.54e-6, 1.62e-6, 2.3e-7, -5.5e-7,
            ParametersType.OSCULATING);

        final double eps = 3E-15;

        // Propagation
        final CircularOrbit actual = (CircularOrbit) propagator.propagateMeanOrbit(date
            .shiftedBy(0.));
        ...
        Assert.assertEquals(0., relDiff(expected.getA(), actual.getA()), eps);
        Assert.assertEquals(0., relDiff(expected.getCircularEx(), actual.getCircularEx()), eps);
        Assert.assertEquals(0., relDiff(expected.getCircularEy(), actual.getCircularEy()), eps);
        Assert.assertEquals(0., relDiff(expected.getI(), actual.getI()), eps);
        Assert.assertEquals(
            0.,
            relDiff(expected.getRightAscensionOfAscendingNode(),
                actual.getRightAscensionOfAscendingNode()), eps);
        Assert.assertEquals(0., relDiff(expected.getAlphaM(), actual.getAlphaM()), eps);
    }
    ...
    public void testMeanOscConversion() throws PatriusException {
        ...
        Report.printMethodHeader("testMeanOscConversion", "Mean <=> Osculating conversion",
            "Celestlab 3.1.0", eps, ComparisonType.RELATIVE);
        ...
        Assert.assertEquals(0., relDiff(7006166.97380257677, osc.getA()), eps);
        Assert.assertEquals(0., relDiff(0.00061325022465691, osc.getCircularEx()), eps);
        Assert.assertEquals(0., relDiff(0.00124881004056462, osc.getCircularEy()), eps);
        Assert.assertEquals(0., relDiff(1.000282787687367, osc.getI()), eps);
        Assert.assertEquals(0.,
            relDiff(0.10014288651414052, osc.getRightAscensionOfAscendingNode()), eps);
        Assert.assertEquals(0., relDiff(0.20019742975551447, osc.getAlphaM()), eps);
        ...
        // Check results (Celestlab reference)
        Assert.assertEquals(0., relDiff(7000000, mean.getA()), eps);
        Assert.assertEquals(0., relDiff(0., mean.getCircularEx()), eps);
        Assert.assertEquals(0., relDiff(0.001, mean.getCircularEy()), eps);
        Assert.assertEquals(0., relDiff(1., mean.getI()), eps);
        Assert.assertEquals(0., relDiff(0.1, mean.getRightAscensionOfAscendingNode()), eps);
        Assert.assertEquals(0., relDiff(0.2, mean.getAlphaM()), eps);
    }
     ...
     * @output Exception
     *
     * @testPassCriteria Exception thrown if convergence threshold is 1E-13, no exception if
    ...
    public void testConvergenceThreshold() {
        ...
        // Initialization
        final AbsoluteDate date = new AbsoluteDate(2018, 6, 23, 0, 00, 0.0,
            TimeScalesFactory.getTAI());
        final Orbit orbitCIRF = new CircularOrbit(7183363.89522035, -8.941808805400402E-5,
            8.383863708607163E-4, 1.7212891584949481, -312.18039057983583, 4435.468035494116,
            PositionAngle.TRUE, FramesFactory.getCIRF(), date, 3.986004415E14);

        // Propagation with default convergence threshold (1E-13) - Exception is expected
        try {
            new EcksteinHechlerPropagator(orbitCIRF, 6378136.46, 3.986004415E14,
                orbitCIRF.getFrame(), -0.0010826264444855536, 2.532558865422469E-6,
                1.61998777956621E-6, 2.2779633055219165E-7, -5.406540965936291E-7,
                ParametersType.OSCULATING);
            Assert.fail();
        ...
        // Propagation with new convergence threshold (1E-12) - Propagation is expected to end
        // properly
        try {
            new EcksteinHechlerPropagator(orbitCIRF, 6378136.46, 3.986004415E14,
                orbitCIRF.getFrame(), -0.0010826264444855536, 2.532558865422469E-6,
     ...
     * @testedMethod {@link EcksteinHechlerPropagator#propagateOrbit(AbsoluteDate)}
     *
     * @description test the Eckstein-Hechler propagation with orbit frame (GCRF) different from
     *              body frame (CIRF)
     *
     * @input Keplerian orbit
     *
     ...
     *
     * @testPassCriteria orbit is close (< 22m) to orbit obtained with numerical integration
     *                   including J2 to J6 only
     *
     * @referenceVersion 3.2
     *
     * @nonRegressionVersion 3.2
    ...
    public void testPropagationBodyFrame() throws PatriusException, IOException, ParseException {
        ...
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(orbit, ae, mu,
            bodyFrame, c[2][0], c[3][0], c[4][0], c[5][0], 0, ParametersType.OSCULATING);

        // Numerical propagator
        final NumericalPropagator numerical = new NumericalPropagator(
            new DormandPrince853Integrator(0.01, 100, 1E-7, 1E-12));
        numerical.addForceModel(new DirectBodyAttraction(new CunninghamGravityModel(bodyFrame, ae, mu, c, s)));
        ...
        final Orbit actual = propagator.propagate(date.shiftedBy(orbit.getKeplerianPeriod()))
            .getOrbit();
        final Orbit expected = new CircularOrbit(numerical.propagate(
        ...
        Assert.assertEquals(
            propagator.propagateMeanOrbit(date.shiftedBy(orbit.getKeplerianPeriod()))
                .getFrame(), FramesFactory.getGCRF());
    }
     ...
     */
    @Test
    public void testPropagationBodyFrameITRF() throws IOException, ParseException {

        try {
            final PotentialCoefficientsProvider provider = GravityFieldFactory
                .getPotentialProvider();
            final double[][] c = provider.getC(5, 0, false);
            final Orbit orbit = new KeplerianOrbit(7000000, 0.001, 0.001, FastMath.PI / 2., 0.1,
                0.2, PositionAngle.MEAN, FramesFactory.getGCRF(),
                AbsoluteDate.FIFTIES_EPOCH_TAI, provider.getMu());
            new EcksteinHechlerPropagator(orbit, provider.getAe(), provider.getMu(),
                FramesFactory.getITRF(), c[2][0], c[3][0], c[4][0], c[5][0], 0,
                ParametersType.OSCULATING);
            Assert.fail();
     ...
     * @testedMethod {@link EcksteinHechlerPropagator#propagateOrbit(AbsoluteDate)}
     *
     * @description test the Eckstein-Hechler propagation with orbit frame (MOD) different from
     ...
     */
    @Test
    public void testPropagationPropagationFrame() throws PatriusException {
        ...
        final EcksteinHechlerPropagator propagator = new EcksteinHechlerPropagator(orbit,
            6378136.3, 3.98600442e+14, FramesFactory.getCIRF(), -0.001082626613,
            0.000002532393, 0.000001619137, 0.000000227742, -5.41e-7, ParametersType.OSCULATING);
        ...
        final EcksteinHechlerPropagator propagator2 = new EcksteinHechlerPropagator(orbit2,
            6378136.3, 3.98600442e+14, FramesFactory.getCIRF(), -0.001082626613,
            0.000002532393, 0.000001619137, 0.000000227742, -5.41e-7, ParametersType.OSCULATING);
        ...
        Assert.assertEquals(
            0.,
            relDiff(actual2.getPVCoordinates().getPosition().getX(), actual.getPVCoordinates()
                .getPosition().getX()), eps2);
        Assert.assertEquals(
            0.,
            relDiff(actual2.getPVCoordinates().getPosition().getY(), actual.getPVCoordinates()
                .getPosition().getY()), eps2);
        Assert.assertEquals(
            0.,
            relDiff(actual2.getPVCoordinates().getPosition().getZ(), actual.getPVCoordinates()
                .getPosition().getZ()), eps2);
        Assert.assertEquals(
            0.,
            relDiff(actual2.getPVCoordinates().getVelocity().getX(), actual.getPVCoordinates()
                .getVelocity().getX()), eps2);
        Assert.assertEquals(
            0.,
            relDiff(actual2.getPVCoordinates().getVelocity().getY(), actual.getPVCoordinates()
                .getVelocity().getY()), eps2);
        Assert.assertEquals(
            0.,
            relDiff(actual2.getPVCoordinates().getVelocity().getZ(), actual.getPVCoordinates()
                .getVelocity().getZ()), eps2);
        ...
        Assert.assertEquals(
            0.,
            relDiff(orbit_rs1.getPVCoordinates().getPosition().getX(), orbit_rs2
                .getPVCoordinates().getPosition().getX()), eps3);
        Assert.assertEquals(
            0.,
            relDiff(orbit_rs1.getPVCoordinates().getPosition().getY(), orbit_rs2
                .getPVCoordinates().getPosition().getY()), eps3);
        Assert.assertEquals(
            0.,
            relDiff(orbit_rs1.getPVCoordinates().getPosition().getZ(), orbit_rs2
                .getPVCoordinates().getPosition().getZ()), eps3);
        Assert.assertEquals(
            0.,
            relDiff(orbit_rs1.getPVCoordinates().getVelocity().getX(), orbit_rs2
                .getPVCoordinates().getVelocity().getX()), eps3);
        Assert.assertEquals(
            0.,
            relDiff(orbit_rs1.getPVCoordinates().getVelocity().getY(), orbit_rs2
                .getPVCoordinates().getVelocity().getY()), eps3);
        Assert.assertEquals(
            0.,
            relDiff(orbit_rs1.getPVCoordinates().getVelocity().getZ(), orbit_rs2
                .getPVCoordinates().getVelocity().getZ()), eps3);
    }
    ...
    private static double tangLEmLv(final double Lv, final double ex, final double ey) {
        ...
        return (ey * MathLib.cos(Lv) - ex * MathLib.sin(Lv))
                / (1 + ex * MathLib.cos(Lv) + ey * MathLib.sin(Lv) + MathLib.sqrt(1 - ex * ex - ey
                        * ey));
    }
     ...
     * @throws PatriusException
     *         if an error occurs
     * @description Evaluate the Eckstein-Hechler propagator serialization / deserialization
     *              process.
     *
     * @testPassCriteria The Eckstein-Hechler propagator can be serialized and deserialized.
     */
    @Test
    public void testSerialization() throws PatriusException {
        ...
        // Loop over each parameters types
        for (final ParametersType type : ParametersType.values()) {
            final EcksteinHechlerPropagator extrapolator = new EcksteinHechlerPropagator(
                initialOrbit, new ConstantAttitudeLaw(FramesFactory.getEME2000(),
                    Rotation.IDENTITY), this.ae, this.mu, initialOrbit.getFrame(),
                this.c20, this.c30, this.c40, this.c50, this.c60, massModel, type);
            final EcksteinHechlerPropagator deserializedExtrapolator = TestUtils
                .serializeAndRecover(extrapolator);

            for (int i = 0; i < 10; i++) {
                final AbsoluteDate currentDate = initDate.shiftedBy(i);
                Assert.assertEquals(extrapolator.getPVCoordinates(currentDate, frame),
                    deserializedExtrapolator.getPVCoordinates(currentDate, frame));
            }
        }
    }
