...
public class MultiPartialDerivativesEquationsTest {
     ...
     *
     * @description Test multi-sat numerical orbit and partial derivatives propagation. It basically checks that propagated
     *              orbit and partial derivatives are exactly the same in 5 mono-sat propagation and 1 single multi-sat propagation
     *              state transition matrix and partial derivatives wrt drag k0 are computed
     * 
     * @input MultiNumericalPropagator and its single propagators counter-parts, forcel models (Earth potential, drag, Sun, Moon)
     *
     ...
     * @testedMethod all MultiPartialDerivativesEquations methods
     *
     * @description Test multi-sat numerical orbit and partial derivatives propagation using Cowell integrator. It basically checks that propagated
     *              orbit and partial derivatives are the same in 5 mono-sat propagation and 1 single multi-sat propagation
     *              state transition matrix and partial derivatives wrt drag k0 are computed
     ...
     * @output final states and partial derivatives
     *
     * @testPassCriteria final states and partial derivatives are almost the same (cannot be the same since time step depends on state vector size)
     ...
     */
    @Test
    public void testPartialDerivativesCowell() throws PatriusException, IOException, ParseException, ClassNotFoundException {
        // STM initialized with identity and partial derivatives initialized with empty matrix
    ...
    public void testCoverage(){
        ...
        List<ParameterConfiguration> parameters = multiEquations.getSelectedParameters();
     ...
     * @param isDefault true if partial derivatives are initialized with default value (STM = identity matrix and PD = empty matrix)
     */
    public void checkPartialDerivatives(final boolean isDefault,
            final FirstOrderIntegrator integrator,
            final double tol,
            final double tolPD) throws PatriusException, IOException, ParseException, ClassNotFoundException {
        ...
        for (int i = 0; i < nPropagators; i++) {
            ...
            multiEquations[i] = new MultiPartialDerivativesEquations("equations" + i, multiNumericalPropagator, "state" + i);
        ...
        for (int i = 0; i < nPropagators; i++) {
            ...
            for (int j = 0; j < equations[i].getAvailableParameters().size(); j++) {
                Assert.assertEquals(equations[i].getAvailableParameters().get(i).getName(), multiEquations[i].getAvailableParameters().get(i).getName());
                Assert.assertEquals(equations[i].getAvailableParameters().get(i).getValue(), multiEquations[i].getAvailableParameters().get(i).getValue());
            }
        }
        
        // Exceptions
        // Un-initialized jacobian
        final MultiPartialDerivativesEquations multiEquationsException = new MultiPartialDerivativesEquations("", multiNumericalPropagator, "state0");
        try {
        ...
        try {
            multiEquationsException.selectParameters(new ArrayList<Parameter>(){{add(new Parameter("", 0));}});
        ...
        // Inconsistent matrix sizes
        try {
            multiEquationsException.selectParameters(new ArrayList<Parameter>(){{add(k0[0]);}});
     ...
     */
    private void checkExternalization(final BoundedPropagator ephemeris, final int id) throws IOException, ClassNotFoundException, PatriusException {
        ...
        while (currentDate.compareTo(ephemeris.getMaxDate()) <= 0) {
            ...
            Assert.assertEquals(refState.getMass("Tank" + id), actState.getMass("Tank" + id), 0.);
            for (int i = 0; i < 6; i++) {
                for (int j = 0; j < 7; j++) {
                    check(refState.getAdditionalState("equations" + id)[i + j * 6], actState.getAdditionalState("equations" + id)[i + j * 6], 0);
                }
            }
