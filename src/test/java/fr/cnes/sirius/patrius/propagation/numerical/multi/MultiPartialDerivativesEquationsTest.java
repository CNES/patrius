...
public class MultiPartialDerivativesEquationsTest {
    ...
    public void testPartialDerivativesCowell()
        ...
        // STM initialized with identity and partial derivatives initialized with empty matrix
        checkPartialDerivatives(true, new CowellIntegrator(9, 1E-10, 1E-10), 0.007, 2E-6);
    }
    ...
    public void checkPartialDerivatives(final boolean isDefault,
        ...
        for (int i = 0; i < nPropagators; i++) {
            if (integrator instanceof CowellIntegrator) {
                propagators[i] = new NumericalPropagator(integrator, FramesFactory.getGCRF(), OrbitType.CARTESIAN,
                    PositionAngle.TRUE);
                propagators[i].setEphemerisMode();
            } else {
                propagators[i] = new NumericalPropagator(integrator, FramesFactory.getGCRF(), OrbitType.KEPLERIAN,
                    PositionAngle.TRUE);
            }

        }

        final MultiNumericalPropagator multiNumericalPropagator;
        HashMap<String, Frame> propFrameMap = new HashMap<String,Frame>();
        for (int i = 0; i < nPropagators; i++) {
            propFrameMap.put("state" + i, FramesFactory.getGCRF());
        }
        if (integrator instanceof CowellIntegrator) {
            multiNumericalPropagator = new MultiNumericalPropagator(integrator, propFrameMap,
        ...
        for (int i = 0; i < nPropagators; i++) {
            ...
            multiNumericalPropagator.removeInitialState("state" + i);
        ...
        for (int i = 0; i < nPropagators; i++) {
            ...
            propagators[i].addForceModel(newtonianAttraction);
            ...
            multiNumericalPropagator.addForceModel(newtonianAttraction, "state" + i);
        }
