...
public class NumericalPropagatorTest {
     ...
     * @testType UT
     * 
     * @description check that solar activity data availability is properly checked at the beginning
     *              of the propagation.
     * 
     * @testPassCriteria solar activity data availability is properly checked (with an exception if
     *                   data is not fully
     *                   available on propagation timespan)
     * 
    ...
    public void testDataValidity() throws PatriusException {
        ...
        final OneAxisEllipsoid earthShape = new OneAxisEllipsoid(6378136.460, 1.0 / 298.257222101,
        ...
        checkPropagationWithDataControl(new SimpleExponentialAtmosphere(earthShape, 0.0004,
                42000.0, 7500.0), false);
        ...
        checkPropagationWithDataControl(
                new MSISE2000(new ContinuousMSISE2000SolarData(getProvider(minFlux, maxFlux,
                        minApKp, maxApKp)), earthShape, new MeeusSun()), false);
        ...
        checkPropagationWithDataControl(
                new MSISE2000(new ContinuousMSISE2000SolarData(getProvider(minFlux, maxFlux,
                        minApKp, maxApKp)), earthShape, new MeeusSun()), true);
        ...
        checkPropagationWithDataControl(
                new DTM2000(new DTMSolarData(getProvider(minFlux, maxFlux, minApKp, maxApKp)),
                        new MeeusSun(), earthShape), false);
     ...
     */
    private static SolarActivityDataProvider getProvider(final AbsoluteDate minFlux,
            final AbsoluteDate maxFlux, final AbsoluteDate minApKp, final AbsoluteDate maxApKp) {
        return new SolarActivityDataProvider() {

            /** Serializable UID. */
            private static final long serialVersionUID = -5550054730939920728L;
            ...
            public SortedMap<AbsoluteDate, Double> getInstantFluxValues(final AbsoluteDate date1,
                ...
                final SortedMap<AbsoluteDate, Double> res = new TreeMap<>();
                res.put(date1, 140.);
                res.put(date2, 140.);
                return res;
            }
    ...
    /**
     * Check an exception is properly thrown or not when checking data at the beginning of the
     * propagation.
     * 
     ...
     */
    private static void checkPropagationWithDataControl(final Atmosphere atmosphere,
            final boolean expectedException) throws PatriusException {
        // Initialization
        final AbsoluteDate initialDate = AbsoluteDate.J2000_EPOCH;
        final Orbit initialOrbit = new KeplerianOrbit(7000000, 0.001, 0, 0, 0, 0,
     ...
     * @testType UT
     * 
     * @description check that the end of a propagation is performed properly with only one isLast
     *              call and without
     *              retropropagation.
     * 
     * @testPassCriteria end date is expected end date, there is no retropolation and there is only
     *                   one isLast
     * 
    ...
    public void testAccurateFinalDatePropagation() throws PatriusException {
        ...
        final Orbit initialOrbit = new KeplerianOrbit(7000000, 0.001, MathLib.toRadians(5),
                MathLib.toRadians(3), MathLib.toRadians(2), MathLib.toRadians(1),
                PositionAngle.TRUE, FramesFactory.getGCRF(), initialDate, Constants.WGS84_EARTH_MU);
        final NumericalPropagator propagator = new NumericalPropagator(
                new ClassicalRungeKuttaIntegrator(30.));
        ...
        propagator.setMasterMode(new PatriusStepHandler() {
            ...
            /** Serializable UID. */
            private static final long serialVersionUID = 4230456511217210795L;
        ...
        Assert.assertEquals(state.getPVCoordinates().getPosition().getNorm(), state2
                .getPVCoordinates().getPosition().getNorm(), Utils.epsilonTest);
    }
    ...
    public void testRemoveDetectorMultiplePropagation() throws PatriusException {
        ...
        // Add node detector
        final NodeDetector detector = new NodeDetector(FramesFactory.getGCRF(),
                NodeDetector.ASCENDING, 1.e2, 1.e-4, Action.CONTINUE, true) {
     ...
     * @testType UT
     * 
     * @description check the "Action.STOP" behavior within a detector, especially if the detector
     *              is well
     *              removed when the event occurred and it's expected with its attribute.
     *
     * @testPassCriteria the detector is removed when the event occurred and the remove attribute is
     *                   set to "true", and
    ...
    public void testRemoveStopDetector() throws PatriusException {
        ...
        final Orbit initialOrbit = new KeplerianOrbit(7000000, 0.001, MathLib.toRadians(5),
                MathLib.toRadians(3), MathLib.toRadians(2), MathLib.toRadians(1),
                PositionAngle.TRUE, FramesFactory.getGCRF(), initDate, Constants.WGS84_EARTH_MU);
        final NumericalPropagator propagator1 = new NumericalPropagator(
                new ClassicalRungeKuttaIntegrator(30.));
        propagator1.setInitialState(new SpacecraftState(initialOrbit));
        propagator1.addForceModel(new DirectBodyAttraction(new NewtonianGravityModel(propagator1.getMu())));
        final NumericalPropagator propagator2 = new NumericalPropagator(
                new ClassicalRungeKuttaIntegrator(30.));
     ...
     * @testType UT
     * 
     * @description check that propagation in some numerically degraded case ends exactly at
     *              required date.
     * 
    ...
    public void testInterpolationRK6() throws PatriusException {
        ...
        final NumericalPropagator propagator2 = new NumericalPropagator(integrator, initialState.getFrame(),
        ...
        final SpacecraftState initialState3 = new SpacecraftState(orbit);
        final NumericalPropagator propagator3 = new NumericalPropagator(integrator2, initialState3.getFrame(),
            OrbitType.CARTESIAN, PositionAngle.TRUE);
        ...
        for (int i = 0; i < 5000; i++) {
            ...
            final double diffPos = state2.getPVCoordinates().getPosition()
                    .distance(state3.getPVCoordinates().getPosition());
            final double diffVel = state2.getPVCoordinates().getVelocity()
                    .distance(state3.getPVCoordinates().getVelocity());
     ...
     * 
     * @description
     *              Test first guess of adaptive stepsize integrators: first guess may be too far in
     *              the
     *              future (some data such as attitude data may therefore be missing). Solution is
     *              to clamp first guess
     *              value
     *              within integrator min and max steps. In this test case, first guess is 21 days
     *              further than last
     *              available
     *              attitude data.
     * 
     ...
     */
    @Test
    public void testFirstGuess() throws IllegalArgumentException, PatriusException {
        ...
        final Orbit initialOrbit = new KeplerianOrbit(24400e3, 0.72, MathLib.toRadians(5),
                MathLib.toRadians(180), MathLib.toRadians(2), MathLib.toRadians(180),
                PositionAngle.TRUE, FramesFactory.getGCRF(), initDate, Constants.WGS84_EARTH_MU);
        ...
        final NumericalPropagator propagator = new NumTestPropagator(initialOrbit, spacecraft,
                massModel, attLeg);
     ...
     *              Check that ephemeris is properly generated (originally ephemeris generation via
     *              step handler
    ...
    public void testRetropolationHandler() throws PatriusException {
        ...
        final ApsisRadiusParameters par = new ApsisRadiusParameters(per, apo, inc, pa, raan, anm,
        ...
        final AttitudeLaw law1 = new ConstantAttitudeLaw(GCRF, new Rotation(RotationOrder.ZYX, 0.,
                0., 0.));

        // Events that will switch from a law to another
        final double maxCheck = 10.;
        final double threshold = 1.e-3;
        final EventDetector event0 = new DateDetector(date.shiftedBy(0.), maxCheck, threshold,
                Action.RESET_STATE);
        ...
        final ArrayList<SpacecraftState> listOfStates = new ArrayList<>();
        final PatriusFixedStepHandler myStepHandler = new PatriusFixedStepHandler() {

            /** Serializable UID. */
            private static final long serialVersionUID = 2144760253431952067L;
     ...
     * 
     * @description
     *              Performs a propagation with maneuvers and analytical propagator in ephemeris
     *              mode. Some events are
     *              detected.
     ...
     */
    @Test
    public void testAnalyticalEphemeris() throws IllegalArgumentException, PatriusException {
        ...
        final Orbit initialOrbit = new KeplerianOrbit(42164.e3, 0.001, 0, 0, 0, 0,
                PositionAngle.TRUE, FramesFactory.getGCRF(), initDate, Constants.WGS84_EARTH_MU);
        ...
        final EventDetector visiCentreTerre = new CircularFieldOfViewDetector(
                CelestialBodyFactory.getEarth(), Vector3D.PLUS_J, MathLib.toRadians(3), 60.) {

            /** Serializable UID. */
            private static final long serialVersionUID = -3067496842731882960L;
        ...
        final ImpulseManeuver impulse = new ImpulseManeuver(new DateDetector(tman, 60, 1e-6),
                new Vector3D(-1000, 0, 0), FramesFactory.getGCRF(), 300, massModel, "Reservoir1");
     ...
     */
    @Test
    public void testLeakFlowFix() throws IllegalArgumentException, PatriusException {
        ...
        class MyPatriusStepHandler implements PatriusStepHandler {
            ...
            /** Serializable UID. */
            private static final long serialVersionUID = 2123325728727437292L;
        ...
        final SpacecraftState initialState = new SpacecraftState(initialOrbit, massProviders);
     ...
     * 
     * @description
     *              Check that a propagation with or without partial derivatives returns exactly the
     *              same PV
     *              coordinates.
     *              This means adaptive stepsize integrators do not take into account partial
     *              derivatives (abs tol =
     *              +inf, rel tol = 0)
     *              when estimating error. Test is performed with DormandPrince853Integrator,
    ...
    public void testPropagationErrorEstimation() throws PatriusException {
        ...
        testErrorEstimation(new DormandPrince853Integrator(0.01, 500., vecAbsoluteTolerance, vecRelativeTolerance));
        testErrorEstimation(new DormandPrince54Integrator(0.01, 500., vecAbsoluteTolerance, vecRelativeTolerance));
        testErrorEstimation(new HighamHall54Integrator(0.01, 500., vecAbsoluteTolerance, vecRelativeTolerance));
    }
     ...
     *        integrator
     */
    private static void testErrorEstimation(final FirstOrderIntegrator integrator)
            throws PatriusException {
        // Initial orbit
        ...
        // Propagation
        final NumericalPropagator p = new NumericalPropagator(integrator, initialState.getFrame(), OrbitType.CARTESIAN,
            PositionAngle.TRUE);
    ...
    public static Assembly getSphericalVehicle(final double mass, final double radius,
            final double DragCoef, final double ka, final double ks, final double kd)
            throws PatriusException {
     ...
     * Attitude guidance.
     */
    private class Guidage implements AttitudeLeg {

        /** Serializable UID. */
        private static final long serialVersionUID = 6711849817567136455L;
        ...
        public Guidage(final Frame inertialFrame, final AbsoluteDate refDate, final double duration) {
            final Attitude att1 = new Attitude(refDate, inertialFrame, Rotation.IDENTITY,
                    Vector3D.ZERO);
            final Attitude att2 = new Attitude(refDate.shiftedBy(duration), inertialFrame,
                    Rotation.IDENTITY, Vector3D.ZERO);
            final ArrayList<Attitude> quatList = new ArrayList<>();
            quatList.add(att1);
            quatList.add(att2);
            try {
    ...
    private class NumTestPropagator extends NumericalPropagator {
        ...
        /** Serializable UID. */
        private static final long serialVersionUID = 1596137791799394534L;

        /**
         * Numerical propagator.
         */
        public NumTestPropagator(final Orbit initialOrbit, final Assembly spacecraft,
                final MassProvider massProvider, final AttitudeProvider attProv)
                throws PatriusException {
            super(new DormandPrince853Integrator(1e-6, 500, new double[] { 1e-5, 1e-5, 1e-5, 1e-8,
                    1e-8, 1e-8 }, new double[] { 1e-12, 1e-12, 1e-12, 1e-12, 1e-12, 1e-12 }));

            // Mass
            this.setMassProviderEquation(massProvider);
            this.setAdditionalStateTolerance("MASS_BODY", new double[] { 1e-06 },
                    new double[] { 1e-09 });
            this.setAdditionalStateTolerance("MASS_Reservoir1", new double[] { 1e-06 },
                    new double[] { 1e-09 });
            this.setAdditionalStateTolerance("MASS_Reservoir2", new double[] { 1e-06 },
                    new double[] { 1e-09 });
            ...
            this.addForceModel(new SolarRadiationPressure(sun, earth.getEquatorialRadius(),
                radiativeModel));
