...
public class EclipseDetectorTest {
    ...
    public void testConstructors() throws PatriusException {
        ...
        final GeometricBodyShape body = new GeometricBodyShape(){
            ...
            @Override
            public double
                getLocalRadius(final Vector3D posObserver, final Frame frame, final AbsoluteDate date,
            ...
            @Override
            public GeometricBodyShape resize(MarginType marginType, double marginValue)
                throws PatriusException {
                return null;
            }
        ...
        isTestOk = false;
        try {
            new EclipseDetector(CelestialBodyFactory.getSun(), this.sunRadius, body, 1e-12, 100, 1E-6, Action.CONTINUE, false, 3);
        ...
        // Evaluate the EclipseDetector(IDirection, GeometricBodyShape, double, double, double, Action, Action, boolean,
        ...
        isTestOk = false;
        try {
            detector6.setPropagationDelayType(PropagationDelayType.LIGHT_SPEED, new Frame(FramesFactory.getGCRF(), Transform.IDENTITY, "frame", false));
        } catch (final IllegalArgumentException e) {
            isTestOk = true;
        }
        Assert.assertTrue(isTestOk);
