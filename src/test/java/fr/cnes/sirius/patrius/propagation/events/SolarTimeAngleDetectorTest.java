...
public class SolarTimeAngleDetectorTest {
    ...
    public void testStateFrameNotPseudoInertial() throws PatriusException {
        ...
        final SolarTimeAngleDetector detector =
            new SolarTimeAngleDetector(MathUtils.DEG_TO_RAD * 25, AbstractDetector.DEFAULT_MAXCHECK,
                AbstractDetector.DEFAULT_THRESHOLD,
                FramesFactory.getTIRF());
        assertEquals(-0.7675548008246268, detector.g(state), Precision.DOUBLE_COMPARISON_EPSILON);
     ...
     */
    @Test
    public void testStateFrameNotPseudoInertialNoFrame() throws PatriusException {
        // Creation of the orbit
        final double mu = 3.9860047e14;
        final Vector3D position = new Vector3D(-6142438.668, 3492467.560, -25767.25680);
        final Vector3D velocity = new Vector3D(505.8479685, 942.7809215, 7435.922231);
        ...
        // Verification of the correct value for a detector without Frame configuration
        final SpacecraftState state = new SpacecraftState(orbit);
        assertFalse(state.getFrame().isPseudoInertial());
        assertEquals(-0.7675556902899832, detector.g(state), Precision.DOUBLE_COMPARISON_EPSILON);
        assertNotEquals(detector.g(state), detector.g(stateInertial));
