...
public class CowellIntegratorTest {
    ...
    public void testPropagationPRS() throws PatriusException {
        ...
        // List of force models for full model
        final List<ForceModel> forceModels = new ArrayList<>();
        forceModels.add(new SolarRadiationPressure(new MeeusSun(), new OneAxisEllipsoid(
    ...
    private BoundedPropagator propagate(final String name,
        ...
        final NumericalPropagator propagator1 = new NumericalPropagator(new DormandPrince853Integrator(0.01, 500,
        ...
        if (isTest) {
            ...
            Assert.assertEquals(0., this.maxPosError, tolPos);
        }
        if (isMC) {
            // Monte-Carlo
    ...
    private void monteCarlo(final String name,
        ...
        System.out
            .println("Name                  | Order | Tolerance | Max error (m) | Duration (Cowell) | Duration (DOPRI) | Calls (Cowell) | Calls (DOPRI)");
        ...
        for (int order = 4; order < 12; order++) {
            for (int i = 6; i < 16; i++) {
                ...
                propagate(String.format(Locale.US, "%s - Keplerian |     %d |  %1.2e", name, order, tol),
                    initialOrbit, finalDate, new ArrayList<ForceModel>(), new ArrayList<EventDetector>(), assembly,
                    massModel, order, tol, false, true, false, true, false, Double.POSITIVE_INFINITY);
            }
        }
        // Full model
        // for (int order = 4; order < 12; order++) {
        ...
        // initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, order, tol, false,
        // true, false, true, false, Double.POSITIVE_INFINITY);
        // }
        // }
    }
