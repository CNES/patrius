...
public class CowellIntegratorTest {
     ...
     */
    @Test
    public void testPropagationKeplerian() throws PatriusException, IOException, ParseException {
        // Initial orbit
        final AbsoluteDate initialDate = AbsoluteDate.J2000_EPOCH;
        final Orbit initialOrbit = new KeplerianOrbit(7000000, 0, FastMath.PI / 2., 0, 0, 0, PositionAngle.TRUE, FramesFactory.getGCRF(), initialDate, Constants.WGS84_EARTH_MU);
        ...
        propagate("LEO - Keplerian", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 17, 1E-12, true, false, true, false, false, tolPos);
    }
    ...
    public void testPropagationLEO() throws PatriusException, IOException, ParseException {
        ...
        forceModels.add(new ThirdBodyAttraction(new MeeusMoon()));
        forceModels.add(new ThirdBodyAttraction(new MeeusSun()));
        
        // Propagation and ephemeris check
        final double tolPos = 0.0005;
        propagate("LEO", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, true, true, false, false, tolPos);
    }
    ...
    public void testPropagationGTO() throws PatriusException, IOException, ParseException {
        // Initial orbit
        final AbsoluteDate initialDate = AbsoluteDate.J2000_EPOCH;
        final Orbit initialOrbit = new KeplerianOrbit(24000E3, 0.7, 0, 0, 0, 0, PositionAngle.TRUE, FramesFactory.getGCRF(), initialDate, Constants.WGS84_EARTH_MU);
        final AbsoluteDate finalDate = initialOrbit.getDate().shiftedBy(86400.);
        final Assembly assembly = buildAssembly();
        final MassProvider massModel = new MassModel(assembly);
        ...
        propagate("GTO", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-12, false, false, true, false, false, tolPos);
    }
    ...
    public void testPropagationHEO() throws PatriusException, IOException, ParseException {
        // Initial orbit
        final AbsoluteDate initialDate = AbsoluteDate.J2000_EPOCH;
        final Orbit initialOrbit = new ApsisOrbit(6700E3, 40000E3, MathLib.toRadians(15.), 0, 0, 0, PositionAngle.TRUE, FramesFactory.getGCRF(), initialDate, Constants.WGS84_EARTH_MU);
        final AbsoluteDate finalDate = initialOrbit.getDate().shiftedBy(86400.);
        final Assembly assembly = buildAssembly();
        final MassProvider massModel = new MassModel(assembly);
        ...
        // Propagation and ephemeris check
        final double tolPos = 0.01;
        propagate("HEO", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-11, false, false, true, false, false, tolPos);
    }
    ...
    public void testRetroPropagation() throws PatriusException, IOException, ParseException {
        ...
        propagate("Retropropagation", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, false, true, false, false, tolPos);
    }
    ...
    public void testMasterMode() throws PatriusException, IOException, ParseException {
        ...
        propagate("Master mode", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, false, true, false, false, tolPos);
    }
    ...
    public void testFixedMasterMode() throws PatriusException, IOException, ParseException {
        ...
        // Propagation and ephemeris check
        final double tolPos = 0.001;
        propagate("Master mode (fixed)", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, true, false, true, false, false, tolPos);
    }
    ...
    public void testEphemerisMode() throws PatriusException, IOException, ParseException {
        ...
        // Propagation and ephemeris check
        final double tolPos = 0.001;
        propagate("Ephemeris mode", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, true, true, false, false, tolPos);
    }
    ...
    public void testEventDetection() throws PatriusException, IOException, ParseException {
        ...
        final List<AbsoluteDate> refEventDates = new ArrayList<AbsoluteDate>();
        final List<AbsoluteDate> actEventDates = new ArrayList<AbsoluteDate>();
        ...
        // Propagation
        propagate("Event detection", initialOrbit, finalDate, forceModels, detectors, assembly, massModel, 9, 1E-10, false, false, true, false, false, Double.POSITIVE_INFINITY);
    ...
    public void testImpulseManeuver() throws PatriusException, IOException, ParseException {
        ...
        final List<EventDetector> detectors = new ArrayList<EventDetector>();
        detectors.add(new ImpulseManeuver(new DateDetector(initialDate.shiftedBy(10000)), new Vector3D(100, 0, 0), 200, "Tank"));
 
        // Propagation
        final double tolPos = 0.015;
        propagate("Impulse maneuver", initialOrbit, finalDate, forceModels, detectors, assembly, massModel, 9, 1E-10, true, false, true, false, false, tolPos);
    }
    ...
    public void testContinuousManeuver() throws PatriusException {
        ...
        // List of force models (gravity only)
        final List<ForceModel> forceModels = new ArrayList<ForceModel>();
        final PropulsiveProperty engine = (PropulsiveProperty) assembly.getMainPart().getProperty(PropertyType.PROPULSIVE);
        final TankProperty tank = (TankProperty) assembly.getPart("Tank").getProperty(PropertyType.TANK);
        forceModels.add(new ContinuousThrustManeuver(initialDate.shiftedBy(10000), 1000, engine, Vector3D.PLUS_I, massModel, tank, FramesFactory.getGCRF()));
        ...
        propagate("Continuous maneuver", initialOrbit, finalDate, forceModels, detectors, assembly, massModel, 9, 1E-11, true, false, true, false, false, tolPos);
    }
    ...
    public void testPropagationPRS() throws PatriusException {
        ...
        // List of force models for full model
        final List<ForceModel> forceModels = new ArrayList<ForceModel>();
        forceModels.add(new SolarRadiationPressureEllipsoid(new MeeusSun(),
                new ExtendedOneAxisEllipsoid(Constants.WGS84_EARTH_EQUATORIAL_RADIUS, Constants.WGS84_EARTH_FLATTENING,
                        FramesFactory.getITRF(), ""), new DirectRadiativeModel(buildAssembly())));
        
        // Propagation and ephemeris check
        final double tolPos = 0.05;
        propagate("PRS", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-11, false, false, true, false, false, tolPos);
    }

    /**
     * @testType VT
     * 
     * @description test Cowell propagation on small interval: 0.1s (no exact interval) and 1E-9s (lower than Cowell first initial timestep). Order = 9, tol = 1E-10.
     * 
    ...
    public void testSmallTimeInterval() throws PatriusException {
        ...
        final BoundedPropagator ephemeris1 = propagate("Small interval (0.1s)", initialOrbit, initialOrbit.getDate().shiftedBy(duration1), forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, true, true, false, false, 0.);
        // Check final date
        ...
        final BoundedPropagator ephemeris2 = propagate("Small interval (1E-9s)", initialOrbit, initialOrbit.getDate().shiftedBy(duration2), forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, true, true, false, false, 0.);
        // Check final date
     ...
     * @testType VT
     * 
     * @description test Cowell propagation on several successive integration with same integrator. Order = 9, tol = 1E-10.
     * 
     * @testPassCriteria ephemeris (every time step) is same as reference ephemeris (reference: DOPRI 853 with
     *                   small time step, absolute position threshold: 0.001m)
     * 
    ...
    public void testMultipleIntegration() throws PatriusException, IOException, ParseException {
        // Initial orbit
        final AbsoluteDate initialDate = AbsoluteDate.J2000_EPOCH;
        Orbit initialOrbit = new KeplerianOrbit(7000000, 0, FastMath.PI / 2., 0, 0, 0, PositionAngle.TRUE, FramesFactory.getGCRF(), initialDate, Constants.WGS84_EARTH_MU);
        ...
        for (int i = 0; i < 24; i++) {
            ...
            final BoundedPropagator ephemeris = propagate("Multiple integration", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, true, display, false, false, tolPos);
     ...
     */
    @Test
    public void testFunctional() throws PatriusException, IOException, ParseException {
        ...
        try {
            ...
            propagator2.setOrbitType(OrbitType.KEPLERIAN);
     ...
     * @testType VT
     * 
     * @description test Cowell externalization: a propagation is performed and ephemeris is stored in memory. This ephemeris is then read and is compared to original ephemeris.
     *              PV, mass and partial derivatives are compared
     * 
     * @testPassCriteria read ephemeris (every time step) is same as reference ephemeris (reference: original ephemeris before writing, threshold: 0.)
     * 
     * @referenceVersion 4.6
     * 
     * @nonRegressionVersion 4.6
    ...
    public void testExternalization() throws IOException, ClassNotFoundException, DimensionMismatchException,
        ...
        // Propagation
        final BoundedPropagator ephemeris = propagate(String.format(Locale.US, "LEO (SSO) - Full      |     %d |   %1.2e", 9, 1E-10), initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-12, false, true, false, false, true, Double.POSITIVE_INFINITY);
        ...
        while (currentDate.compareTo(finalDate) <= 0) {
            ...
            for (int i = 0; i < 6; i++) {
                for (int j = 0; j < 8; j++) {
                    check(refState.getAdditionalState("Partial")[i + j * 6], actState.getAdditionalState("Partial")[i + j * 6], 0);
                }
            }
     ...
     * @testType VT
     * 
     * @description test Cowell simple integration on non-orbit equations: here the harmonic system y" = -y is integrated over 100s.
     * 
     ...
     */
    @Test
    public void testSimpleIntegration() throws PatriusException, IOException, ParseException {
        ...
        final SecondOrderStateMapper mapper = new SecondOrderStateMapper() {
            @Override
            public double[] extractYDot(double[] fullState) {
                return new double[] { fullState[1] };
            }
            
            @Override
            public double[] extractY(double[] fullState) {
                return new double[] { fullState[0] };
            }
            
            @Override
            public double[] buildFullState(double[] y,
                    double[] yDot) {
     ...
     * @testType VT
     * 
     * @description test Cowell partial derivatives propagation (full force model) including state transition matrix and sensitivity matrix. Order = 9, tol = 1E-10.
     * 
     * @testPassCriteria final partial derivatives matrix is same as reference partial derivatives matrix (reference: DOPRI 853 with
     *                   small time step, relative threshold on partial derivatives: 1E-5, absolute position threshold: 0.001m)
     * 
     * @referenceVersion 4.6
     * 
     * @nonRegressionVersion 4.6
    ...
    public void testPartialDerivatives() throws PatriusException, IOException, ParseException {
        ...
        // Propagation and ephemeris check
        final double tolPos = 0.001;
        propagate("Partial derivatives", initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, 9, 1E-10, false, false, true, false, true, tolPos);
    }
    ...
    private BoundedPropagator propagate(final String name,
        ...
        final NumericalPropagator propagator1 = new NumericalPropagator(new DormandPrince853Integrator(0.01, 500, tolerances[0], tolerances[1]));
        ...
        if (isPartialDerivatives) {
            ...
            list.add(propagator1.getNewtonianAttractionForceModel().getParameters().get(0));
        ...
        // Cowell propagation
        final NumericalPropagator propagator2 = new NumericalPropagator(new CowellIntegrator(order, tol, tol));
        ...
        if (isPartialDerivatives) {
            ...
            List<Parameter> list = new ArrayList<>();
            list.add(forceModels.get(0).getParameters().get(0));
            list.add(propagator2.getNewtonianAttractionForceModel().getParameters().get(0));
        ...
        if (isEphemerisMode) {
            ...
            if (isFixedMasterMode) {
                ...
                propagator2.setMasterMode(new PatriusStepHandler() {
                    ...
                    @Override
                    public void handleStep(PatriusStepInterpolator interpolator, boolean isLast) throws PropagationException {
                        try {
        ...
        if (isTest) {
            // Write to report
            Report.printToReport(String.format(Locale.US, "%-22s (%d, %2.0e)", name, order, tol), 0, maxPosError);
            // Check max position error
            Assert.assertEquals(0., maxPosError, tolPos);
        }
        if (isMC) {
            // Monte-Carlo
            System.out.println(String.format(Locale.US, "%s |      %2.2e |          %2.2e |         %2.2e |          %d |         %d", name,
                    maxPosError, tAct, tRef, propagator2.getCalls(), propagator1.getCalls()));
        }
    ...
    private void checkSpacecraftState(final BoundedPropagator reference, final SpacecraftState currentState,
        ...
        try {
            ...
            maxPosError = MathLib.max(maxPosError, ref.subtract(act).getNorm());
            ...
            if (DISPLAY_RES) {
                System.out.println(currentState.getDate().durationFrom(AbsoluteDate.J2000_EPOCH) + " "
                        + ref.subtract(act).getNorm() + " " + (currentState.getA() - 7000000) + " " + currentState.getMass("Tank")
     ...
     */
    private void check(final double value1, final double value2, final double tol) {
        if (value1 == 0) {
     ...
     * @testType VT
     * 
     * @description Monte-Carlo on LEO propagation on integrator tolerance / order.
     * 
     * @testPassCriteria this test is only used for displaying error = f(tolerance, order)
     * 
     * @referenceVersion 4.6
     * 
     * @nonRegressionVersion 4.6
    ...
    @Test
    @Ignore
    public void testMonteCarlo() throws PatriusException, IOException, ParseException {
        final AbsoluteDate initialDate = AbsoluteDate.J2000_EPOCH;
        // LEO SSO
        monteCarlo("LEO (SSO)", new KeplerianOrbit(7000000, 0, FastMath.PI / 2., 0, 0, 0, PositionAngle.TRUE, FramesFactory.getGCRF(), initialDate, Constants.WGS84_EARTH_MU));
    }

    private void monteCarlo(final String name,
            final Orbit initialOrbit) throws PatriusException, IOException, ParseException {
        // 10 days propagation
        final AbsoluteDate finalDate = initialOrbit.getDate().shiftedBy(86400. * 10.);
        ...
        System.out.println("Name                  | Order | Tolerance | Max error (m) | Duration (Cowell) | Duration (DOPRI) | Calls (Cowell) | Calls (DOPRI)");
        // Keplerian model only
        for (int order = 4; order < 12; order++) {
            for (int i = 6; i < 16; i++) {
                final double tol = MathLib.pow(10, -i);
                propagate(String.format(Locale.US, "%s - Keplerian |     %d |  %1.2e", name, order, tol),
                        initialOrbit, finalDate, new ArrayList<ForceModel>(), new ArrayList<EventDetector>(), assembly, massModel, order, tol, false, true, false, true, false, Double.POSITIVE_INFINITY);
            }
        }
        // Full model
//        for (int order = 4; order < 12; order++) {
//            for (int i = 6; i < 16; i++) {
//                final double tol = MathLib.pow(10, -i);
//                propagate(String.format(Locale.US, "%s - Full      |     %d |  %1.2e", name, order, tol),
//                        initialOrbit, finalDate, forceModels, new ArrayList<EventDetector>(), assembly, massModel, order, tol, false, true, false, true, false, Double.POSITIVE_INFINITY);
//            }
//        }
    }
     ...
     * @return force model
     */
    private ForceModel balminoAttraction() throws IOException, ParseException, PatriusException {
        Utils.setDataRoot("regular-dataPBASE");
        FramesFactory.setConfiguration(FramesConfigurationFactory.getSimpleConfiguration(true));
        ...
        return new BalminoAttractionModel(FramesFactory.getITRF(), Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                Constants.WGS84_EARTH_MU, c, s, 0, 0);
    }
     ...
     * @return force model
     */
    private ForceModel balminoAttractionDP() throws IOException, ParseException, PatriusException {
        Utils.setDataRoot("regular-dataPBASE");
        FramesFactory.setConfiguration(FramesConfigurationFactory.getSimpleConfiguration(true));
        GravityFieldFactory.addPotentialCoefficientsReader(new GRGSFormatReader("GRGS_EIGEN_GL04S.txt", true));
        final PotentialCoefficientsProvider data = GravityFieldFactory.getPotentialProvider();
        ...
        final double[][] s = data.getS(8, 8, true);
        return new BalminoAttractionModel(FramesFactory.getITRF(), Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                Constants.WGS84_EARTH_MU, c, s, 8, 8);
    }
     ...
     * @return force model
     */
    private ForceModel dragMSIS2000(final Assembly spacecraft) throws PatriusException {
     ...
     * @return force model
     */
    private ForceModel dragMSIS2000PD(final Assembly spacecraft) throws PatriusException {
        final CelestialBody sun = new MeeusSun();
        final OneAxisEllipsoid earth = new OneAxisEllipsoid(Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                Constants.WGS84_EARTH_FLATTENING, FramesFactory.getITRF());
        final MSISE2000InputParameters data = new ContinuousMSISE2000SolarData(new ConstantSolarActivity(140, 15));
    ...
    private class Equations implements SecondOrderDifferentialEquations, FirstOrderDifferentialEquations {
        ...
        @Override
        public void computeSecondDerivatives(double t,
                double[] y,
                double[] yDot,
                double[] yDDot) {
            // y" = -y
