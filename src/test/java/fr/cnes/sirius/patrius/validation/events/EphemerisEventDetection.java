...
public class EphemerisEventDetection {
    ...
    public void testEphemerisEvent() throws PatriusException {
        ...
        for (final int step : steps) {
            ...
            // [(int) (interval.getDuration() / step + 1)];
    ...
    private void compareWithRef(final int step, final List<SpacecraftState> states,
        ...
        final EphemerisPvLagrange eph;
        final PVCoordinatesPropagator pvp;
        final BoundedPropagator ephemeris;
        switch (type) {
            case EPHEMERIS:
                ephemeris = new Ephemeris(states, 2);
                break;
            case LAGRANGE_EPHEMERIS_8:
                eph = new EphemerisPvLagrange(states.toArray(new SpacecraftState[states.size()]),
                        8, null);
                pvp = new PVCoordinatesPropagator(eph, eph.getMinDate(), states.get(0).getMu(),
                        states.get(0).getFrame());
                ephemeris = new BoundedPropagatorWrapper(pvp, eph);
                break;
            case LAGRANGE_EPHEMERIS_10:
                eph = new EphemerisPvLagrange(states.toArray(new SpacecraftState[states.size()]),
                        10, null);
                pvp = new PVCoordinatesPropagator(eph, eph.getMinDate(), states.get(0).getMu(),
                        states.get(0).getFrame());
            ...
            case LAGRANGE_EPHEMERIS_12:
                ...
                break;
            default:
                throw new EnumConstantNotPresentException(BoundedPropagatorType.class, type.name());
        }
    ...
    public void setUp() throws PatriusException {
        ...
        final JPLCelestialBodyLoader loaderSun = new JPLCelestialBodyLoader(UNXP2000_405,
            EphemerisType.SUN);

        final JPLCelestialBodyLoader loaderEMB = new JPLCelestialBodyLoader(UNXP2000_405,
            EphemerisType.EARTH_MOON);
        final JPLCelestialBodyLoader loaderSSB = new JPLCelestialBodyLoader(UNXP2000_405,
        ...
        final JPLCelestialBodyLoader loaderEarth = new JPLCelestialBodyLoader(UNXP2000_405,
            EphemerisType.EARTH);
    ...
    private class BoundedPropagatorWrapper implements BoundedPropagator {
        ...
        /** Serializable UID. */
        private static final long serialVersionUID = -8250156734171326345L;
        ...
        @Override
        public void setOrbitFrame(final Frame frame) {
