...
public class SaastamoinenModelTest {
     ...
     * 
     * @testedMethod {@link SaastamoinenModel#SaastamoinenModel(double, double, double, double)}
     * @testedMethod {@link SaastamoinenModel#computeSignalDelay(double)}
     * @testedMethod {@link SaastamoinenModel#calculatePathDelay(double)}
     * 
     */
    @Test
    public void testDeprecated() {
        ...
        final MeteorologicalConditionsProvider meteoConditionsProvider = new ConstantMeteorologicalConditionsProvider(
            standardMeteoConditions);

        final double temperature = standardMeteoConditions.getTemperature();
        final double pressure = standardMeteoConditions.getPressure();
        final double humidity = standardMeteoConditions.getHumidity();

        final double elevation = MathLib.toRadians(10.0);

        // Order of conditions in old constructors = pressure, temperature, humidity
        final SaastamoinenModel model = new SaastamoinenModel(temperature, pressure, humidity, altitude);
        final double signalDelay = model.computeSignalDelay(elevation);
        final double pathDelay = model.calculatePathDelay(elevation);

        final SaastamoinenModel newModel = new SaastamoinenModel(meteoConditionsProvider, altitude);
        final double newSignalDelay = newModel.computeSignalDelay(this.defaultDate, elevation);
        final double newPathDelay = newModel.calculatePathDelay(elevation);

        Assert.assertEquals(newSignalDelay, signalDelay, Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(newPathDelay, pathDelay, Precision.DOUBLE_COMPARISON_EPSILON);
