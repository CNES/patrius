...
public class MeteorologicalConditionsTest {
    ...
    public void testDeprecated() {
        ...
        final double refAltitude = 0.0;
        final double altitude = 100.0;

        final double[] standardValues = MeteorologicalConditions.computeStandardValues(DEFAULT_PRESSURE,
            DEFAULT_TEMPERATURE, DEFAULT_HUMIDITY, refAltitude, altitude);

        final MeteorologicalConditions referenceMeteoConditions = new MeteorologicalConditions(DEFAULT_PRESSURE,
            DEFAULT_TEMPERATURE, DEFAULT_HUMIDITY);
        final MeteorologicalConditions newStandardValues = MeteorologicalConditions.computeStandardValues(
            referenceMeteoConditions, refAltitude, altitude);

        Assert.assertEquals(standardValues[0], newStandardValues.getPressure(),
            Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(standardValues[1], newStandardValues.getTemperature(),
            Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(standardValues[2], newStandardValues.getHumidity(),
            Precision.DOUBLE_COMPARISON_EPSILON);
    }
}
