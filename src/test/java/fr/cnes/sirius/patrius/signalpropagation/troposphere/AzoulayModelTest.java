...
public class AzoulayModelTest {
     ...
     * 
     * @testedMethod {@link AzoulayModel#AzoulayModel(double, double, double, double)}
     * @testedMethod {@link AzoulayModel#AzoulayModel(double, double, double, double, boolean)}
     * @testedMethod {@link AzoulayModel#getCorrectionsFromApparentElevation(double)}
     * @testedMethod {@link AzoulayModel#getCorrectionsFromGeometricElevation(double)}
     * @testedMethod {@link AzoulayModel#computeElevationCorrection(double)}
     * @testedMethod {@link AzoulayModel#computeSignalDelay(double)}
     * 
     */
    ...
    public void testDeprecated() {
        ...
        // Deprecated method arguments in order : pressure temperature, humidity
        final AzoulayModel model = new AzoulayModel(pressure, temperature, humidity, altitude);

        final double elevationCorrection = model.computeElevationCorrection(elevation);
        final double newElevatioCorrection = this.correction.computeElevationCorrection(this.defaultDate, elevation);
        Assert.assertEquals(newElevatioCorrection, elevationCorrection, Precision.DOUBLE_COMPARISON_EPSILON);

        final double signalDelay = model.computeSignalDelay(elevation);
        final double newSignalDelay = this.correction.computeSignalDelay(this.defaultDate, elevation);
        Assert.assertEquals(newSignalDelay, signalDelay, Precision.DOUBLE_COMPARISON_EPSILON);

        final AzoulayModel modelFalse = new AzoulayModel(pressure, temperature, humidity, altitude, false);

        final double elevationCorrectionFalse = modelFalse.computeElevationCorrection(elevation);
        final double newElevationCorrectionFalse = this.correctionFalse.computeElevationCorrection(this.defaultDate,
            elevation);
        Assert.assertEquals(newElevationCorrectionFalse, elevationCorrectionFalse, Precision.DOUBLE_COMPARISON_EPSILON);

        final double signalDelayFalse = modelFalse.computeSignalDelay(elevation);
        final double newSignalDelayFalse = this.correctionFalse.computeSignalDelay(this.defaultDate,
            elevation);
        Assert.assertEquals(newSignalDelayFalse, signalDelayFalse, Precision.DOUBLE_COMPARISON_EPSILON);
    }
}
