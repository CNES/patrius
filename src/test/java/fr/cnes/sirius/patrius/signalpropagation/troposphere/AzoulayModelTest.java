...
/**
 * Unit tests for the Azoulay model
 * 
...
public class AzoulayModelTest {
    ...
    /** correction */
    final AzoulayModel correction = new AzoulayModel(this.pressure, this.temperature, this.moisture, this.altitude);
    final AzoulayModel correctionFalse = new AzoulayModel(this.pressure, this.temperature, this.moisture,
        this.altitude, false);
    ...
    public void testCorrectionsFromGeometricElevation() {
        ...
        // wrong creation
        try {
            new AzoulayModel(this.pressure, -274.0 + 273.16, this.moisture, this.altitude);
            Assert.fail();
        ...
        for (int i = 0; i < this.fromGeometricFDS.length; i++) {
            ...
            correctionRes[0] = this.correctionFalse.computeElevationCorrection(geometricElevation);
            correctionRes[1] = this.correctionFalse.computeSignalDelay(geometricElevation) * Constants.SPEED_OF_LIGHT;

            Assert.assertEquals(0., (this.fromGeometricFDS[i][1] - correctionRes[0]) / this.fromGeometricFDS[i][1], this.comparisonEpsilon);
            Assert.assertEquals(0., (this.fromGeometricFDS[i][2] - correctionRes[1]) / this.fromGeometricFDS[i][2], this.comparisonEpsilon);
    ...
    public void testCorrectionsFromApparentElevation() {
        ...
        for (final double[] element : this.fromApparentFDS) {
            ...
            final double[] correctionRes = new double[2];
            correctionRes[0] = this.correction.computeElevationCorrection(apparentElevation);
            correctionRes[1] = this.correction.computeSignalDelay(apparentElevation) * Constants.SPEED_OF_LIGHT;
    ...
    public void testSerialization() {
        ...
        Assert.assertEquals(model.computeSignalDelay(MathUtils.HALF_PI), 
            deserializedModel.computeSignalDelay(MathUtils.HALF_PI), 0.);
        Assert.assertEquals(model.computeSignalDelay(MathUtils.HALF_PI - 0.1), 
            deserializedModel.computeSignalDelay(MathUtils.HALF_PI - 0.1), 0.);
        Assert.assertEquals(model.computeSignalDelay(MathUtils.HALF_PI - 0.2), 
            deserializedModel.computeSignalDelay(MathUtils.HALF_PI - 0.2), 0.);
    }

    /**
     * @testType UT
     * @testedMethod {@link AzoulayModel#AzoulayModel(double)}
     * @description constructor with negative temperature : he won't like this.
     * @testPassCriteria createIllegalArgumentException
     * @since 2.3
     */
    @Test(expected = IllegalArgumentException.class)
    public void constructorErrorTest() {

        final double negativeTemp = -10.0;
        new AzoulayModel(this.pressure, negativeTemp, this.moisture, this.altitude);
    }
}
