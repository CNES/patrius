...
public class AzoulayModelFactoryTest {
    ...
    public void testAzoulayModelFactory() throws PatriusException {
        ...
        AbstractMeteoBasedCorrectionFactory<AzoulayModel> tropoFactory = new AzoulayModelFactory();
        ...
        final MeteorologicalConditionsProvider meteoConditionsProvider = new ConstantMeteorologicalConditionsProvider(
            meteoConditions);
        TroposphericCorrection refTropoCorr = new AzoulayModel(meteoConditionsProvider, ellipsoidPoint
        ...
        // Call the method AbstractMeteoBasedCorrectionFactory#getCorrectionModel(...) to initialize the
        // tropospheric correction
        AzoulayModel tropoCorrection = tropoFactory.getCorrectionModel(meteoConditionsProvider, ellipsoidPoint);
        Assert.assertNotNull(tropoCorrection);

        // Compute and evaluate the signal delays
        Assert.assertEquals(expectedTropoDelay, tropoCorrection.computeSignalDelay(date, elevation), 0.);

        // Test the "isGeometricElevation" boolean set to false
        tropoFactory = new AzoulayModelFactory(false);
        tropoCorrection = tropoFactory.getCorrectionModel(meteoConditionsProvider, ellipsoidPoint);
        ...
        Assert.assertEquals(refTropoCorr.computeSignalDelay(date, elevation),
            tropoCorrection.computeSignalDelay(date, elevation), 0.);
    }
}
