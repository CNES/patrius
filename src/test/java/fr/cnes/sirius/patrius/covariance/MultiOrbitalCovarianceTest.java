 ...
 * VERSION:4.11:DM:DM-3317:22/05/2023:[PATRIUS] Possibilite d'utiliser des parametres additionels non rattaches a une orbite dans MultiOrbitalCovariance
...
public class MultiOrbitalCovarianceTest {
    ...
    private static final String INVALID_COVARIANCE_SIZE_FORMAT =
        "Invalid orbital coordinate descriptor: row number %d is mapped to %s (wrong %s: %s != %s)";
    ...
    public static void setUpAfterClass() {
        // Set patrius models to default value
    ...
    public void testConstructorFromCovariance() {
        ...
        // Test a case with non orbital additional parameters
        nbAdditionalParameters = new int[] { 3, 2 };
    ...
    public void testConstructorInvalidCovarianceDimension() {
        ...
        final String expectedMessage =
            "Invalid covariance matrix for 2 orbits: the matrix should be at least 12x12, not 11x11";
    ...
    public void testTransform6x6ToLOF() throws PatriusException {
        ...
        for (final LOFType lofType : evaluatedLOF) {
            ...
            for (final Orbit orbit : ORBITS) {
                ...
                final String expectedFrameName =
                    lofType.name() + "_" + orbit.getFrame().getFirstPseudoInertialAncestor().getName();
     ...
     * orbital frame.
     * <p>
     * This test specifies the initial frame as a non-pseudo inertial frame (ITRF). All the orbits are defined in a
     * pseudo inertial frame (GCRF). The LOF should be built in GCRF, but the reference orbit shouldn't be converted.
     * </p>
     * <p>
     * The transformed multi orbital covariance is also evaluated against the associated transformed orbital
     * covariances.
     * </p>
     *
     * <p>
     ...
     * orbital frame.
     * <p>
     * This test specifies the initial frame as a non-pseudo inertial frame (ITRF). All the orbits are also defined in a
     * non-pseudo inertial frame (ITRF). The LOF should be built in GCRF and the reference orbit should be converted in
     * this frame before being stored in the LOF.
     * </p>
     * <p>
     * The transformed multi orbital covariance is also evaluated against the associated transformed orbital
     * covariances.
    ...
    public void testTransform6x6ToLOFInertialFrame2nd() throws PatriusException {
        ...
        for (final LOFType lofType : evaluatedLOF) {
            ...
            for (final Orbit orbit : initialOrbits) {
                ...
                final Frame destFrame = new LocalOrbitalFrame(FramesFactory.getCIRF(), lofType, orbitInertial,
                    expectedFrameName);
     ...
     * orbital frame.
     * <p>
     * This test specifies the initial frame as a pseudo inertial frame (GCRF). All the orbits are defined in a
     * non-pseudo inertial frame (ITRF). The LOF should be built in GCRF and the reference orbit should be converted in
     * this frame before being stored in the LOF.
     * </p>
     * <p>
    ...
    public void testTransform6x6ToFrozenLOF() throws PatriusException {
        ...
        for (final LOFType lofType : evaluatedLOF) {
            ...
            for (int i = 0; i < ORBITS.size(); i++) {
                ...
                // Expected frame name
                final String expectedFrameName =
                    "Frozen_" + lofType.name() + "_" + orbit.getFrame().getFirstPseudoInertialAncestor().getName();
    ...
    public void testShiftedBy6x6() throws PatriusException {
        ...
        final List<Orbit> shiftedOrbits = new ArrayList<>();
        for (final Orbit orbit : ORBITS) {
    ...
    public void testGetOrbitalCovariance() {
        ...
        // Test with non orbital additional parameters
        final int[] nbOrbitalAdditionalParameters = new int[] { 3, 1 };

        final MultiOrbitalCovariance multiOrbitalCovarianceWithNonOrbAddParam = new MultiOrbitalCovariance(
            covariance, ORBITS, nbOrbitalAdditionalParameters, frame, orbitType, positionAngle);

        final OrbitalCovariance orbitalCovariance1WithoutNonOrb = multiOrbitalCovarianceWithNonOrbAddParam
            .getOrbitalCovariance(0, false);
        final OrbitalCovariance orbitalCovariance1WithNonOrb = multiOrbitalCovarianceWithNonOrbAddParam
            .getOrbitalCovariance(0, true);
        final OrbitalCovariance orbitalCovariance2WithoutNonOrb = multiOrbitalCovarianceWithNonOrbAddParam
            .getOrbitalCovariance(1, false);
        final OrbitalCovariance orbitalCovariance2WithNonOrb = multiOrbitalCovarianceWithNonOrbAddParam
            .getOrbitalCovariance(1, true);

        // Build the expected covariance from the initial data
        final SymmetricPositiveMatrix expectedCovarianceMatrix1WithNonOrbAddParam =
            COVARIANCE_MATRIX_18X18.getSubMatrix(new int[] { 0, 1, 2, 3, 4, 5, 6, 7, 8, 16, 17 });
        final SymmetricPositiveMatrix expectedCovarianceMatrix2WithoutNonOrbAddParam = COVARIANCE_MATRIX_18X18
            .getSubMatrix(9, 15);

        expectedParameterDescriptors1.add(DEFAULT_CARTESIAN_PARAMETER_DESCRIPTORS_18X18.get(16));
        expectedParameterDescriptors1.add(DEFAULT_CARTESIAN_PARAMETER_DESCRIPTORS_18X18.get(17));
        final OrbitalCovariance expectedOrbitalCovariance1WithNonOrbAddParam = new OrbitalCovariance(new Covariance(
            expectedCovarianceMatrix1WithNonOrbAddParam, expectedParameterDescriptors1), ORBITS.get(0));

        expectedParameterDescriptors2.remove(DEFAULT_CARTESIAN_PARAMETER_DESCRIPTORS_18X18.get(17));
        expectedParameterDescriptors2.remove(DEFAULT_CARTESIAN_PARAMETER_DESCRIPTORS_18X18.get(16));
        final OrbitalCovariance expectedOrbitalCovariance2WithoutNonOrbAddParam = new OrbitalCovariance(new Covariance(
            expectedCovarianceMatrix2WithoutNonOrbAddParam, expectedParameterDescriptors2), ORBITS.get(1));

        // expectedOrbitalCovariance1.toString(MatrixUtils.VISUAL_FORMAT)
        checkEquality(expectedOrbitalCovariance1, orbitalCovariance1WithoutNonOrb, ABSTOL, RELTOL);
        checkEquality(expectedOrbitalCovariance2WithoutNonOrbAddParam, orbitalCovariance2WithoutNonOrb, ABSTOL, RELTOL);

        checkEquality(expectedOrbitalCovariance1WithNonOrbAddParam, orbitalCovariance1WithNonOrb, ABSTOL, RELTOL);
        checkEquality(expectedOrbitalCovariance2, orbitalCovariance2WithNonOrb, ABSTOL, RELTOL);
    }
    ...
    public void testAdditionalParametersArrayException() {
        ...
        try {
            ...
            Assert.assertEquals("Invalid additional parameters count array: its length of "
                    + "does not match the number of orbits (3 != 2)",
                e.getMessage());
        }
        try {
        ...
        try {
            ...
            Assert.assertEquals("Negative additional parameters count: "
                    + "-1 was supplied for orbit number {1}",
                e.getMessage());
        }
        try {
    ...
    public void testToString() throws PatriusException {
        ...
        builder.append("                      ");
        builder
            .append("{{259.86, 136.45, 110.17, 113.8, 168.13, 146.05, 117.44, 177.67, 122.62, 85.204, 38.256, 97.704}, "
        ...
        builder
            .append(
                "[[      259.86,      136.45,      110.17,      113.80,      168.13,      146.05,      117.44,      177.67,      122.62,      85.204,      38.256,      97.704]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
                "[      136.45,      245.65,      162.38,      168.30,      213.88,      142.47,      151.86,      144.55,      120.50,      156.87,      54.902,      99.309]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
                "[      110.17,      162.38,      201.05,      157.60,      175.45,      151.72,      92.051,      72.361,      168.31,      127.19,      20.749,      146.50]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
                "[      113.80,      168.30,      157.60,      244.84,      206.50,      138.34,      162.25,      126.17,      228.24,      115.13,      91.438,      156.41]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
                "[      168.13,      213.88,      175.45,      206.50,      352.42,      163.31,      171.79,      154.16,      223.26,      117.69,      73.916,      182.56]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
        ...
        builder
            .append(
                "[      117.44,      151.86,      92.051,      162.25,      171.79,      67.276,      208.05,      93.774,      123.41,      107.10,      101.11,      74.419]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
                "[      177.67,      144.55,      72.361,      126.17,      154.16,      195.39,      93.774,      250.08,      129.29,      79.810,      24.577,      93.665]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
                "[      122.62,      120.50,      168.31,      228.24,      223.26,      186.87,      123.41,      129.29,      292.39,      89.817,      70.426,      230.67]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
                "[      85.204,      156.87,      127.19,      115.13,      117.69,      86.425,      107.10,      79.810,      89.817,      190.27,      43.504,      76.690]");
        builder.append(System.lineSeparator());
        builder.append("                       ");
        builder
            .append(
        ...
        builder
            .append(
                "[      97.704,      99.309,      146.50,      156.41,      182.56,      153.29,      74.419,      93.665,      230.67,      76.690,      70.746,      311.96]]]");
        Assert.assertEquals(builder.toString(), result);
    ...
    private static void checkMultiOrbCovVSOrbCov(final MultiOrbitalCovariance result,
                                                 final OrbitalCovariance covariance1,
                                                 final OrbitalCovariance covariance2,
                                                 final int[] indices1, final int[] indices2, final double absTol,
                                                 final double relTol) {
