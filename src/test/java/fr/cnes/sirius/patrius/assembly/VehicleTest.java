...
public class VehicleTest {
     ...
     */
    @Test
    public void testSerialization() throws IOException, ClassNotFoundException, PatriusException {
        ...
        for (int i = 0; i < panel1.size(); i++) {
            ...
            Assert.assertEquals(panel1.get(i).getNormal().distance(panel2.get(i).getNormal()), 0., 0.);
        }
    }
     ...
     */
    @Test
    public void testVehicleProperties() throws PatriusException {
     ...
     */
    @Test
    public void testCreationAssembly() throws PatriusException {
        ...
        Assert.assertTrue(part5.hasProperty(expectedMainProp[3]) && part5.hasProperty(expectedMainProp[4])
                && part5.hasProperty(expectedPartsProp[2]) && part5.hasProperty(expectedPartsProp[3]));
        Assert.assertTrue(part6.hasProperty(expectedMainProp[3]) && part6.hasProperty(expectedMainProp[4])
                && part6.hasProperty(expectedPartsProp[2]) && part6.hasProperty(expectedPartsProp[3]));
     ...
     */
    @Test
    public void testCreationAssemblyWithFactors() throws PatriusException {
        ...
        final SolarRadiationPressure srpForce = new SolarRadiationPressure(new MeeusSun(), 6378000,
            new DirectRadiativeModel(assembly));
        final SolarRadiationPressure srpForceMass = new SolarRadiationPressure(new MeeusSun(), 6378000,
            new DirectRadiativeModel(assemblyMass));
        final SolarRadiationPressure srpForceSRP = new SolarRadiationPressure(new MeeusSun(), 6378000,
     ...
     *              properties. Build aero model (DragForce) and radiative model (srp + rediffused
     *              srp). Add this models to a numerical propagator and propagate. This a non
    ...
    public void testPropagation() throws PatriusException {
        ...
        final RightCircularCylinder cylinder = new RightCircularCylinder(Vector3D.ZERO, Vector3D.PLUS_K, radius, heigh);
        ...
        // Build the radiative force model : srp and rediffused srp
        final RadiationSensitive radiativeModel = new DirectRadiativeModel(assembly);
        final SolarRadiationPressure srp = new SolarRadiationPressure(sun, earth, radiativeModel, false);
        ...
        final FirstOrderIntegrator integrator = new DormandPrince853Integrator(1E-6, 1000, absTol, relTol);
        ...
        final AbsoluteDate date = new AbsoluteDate("2012-01-01T00:00:00.000", TimeScalesFactory.getTAI());
        ...
        propagator.addForceModel(rediffusedSRP);
        ...
        // km
        final Orbit initialOrb = new KeplerianOrbit(6700000, 0.001, MathLib.toRadians(20.), 0., 0., 0.,
        ...
        Assert.assertEquals(-6132894.431769333, finalOrbit.getPVCoordinates().getPosition().getX(), 0.);
        Assert.assertEquals(2967003.856999945, finalOrbit.getPVCoordinates().getPosition().getY(), 0.);
        Assert.assertEquals(1079892.967961955, finalOrbit.getPVCoordinates().getPosition().getZ(), 0.);
        Assert.assertEquals(-3552.8220610004814, finalOrbit.getPVCoordinates().getVelocity().getX(), 0.);
        Assert.assertEquals(-6267.477704918801, finalOrbit.getPVCoordinates().getVelocity().getY(), 0.);
        Assert.assertEquals(-2281.1629523867737, finalOrbit.getPVCoordinates().getVelocity().getZ(), 0.);
     ...
     */
    @Test
    public void testDragLiftPropagation() throws PatriusException {
     ...
     */
    @Test
    public void testVehicleException() throws PatriusException {
     ...
     * @testedFeature {@link features#VEHICULE_PROPAGATION}
     * 
     * @description Check that solar panels are properly taken into account. This test is extracted from PSIMU
     *              tutorials.
     * 
     ...
     */
    @Test
    public void testSolarPanels() throws PatriusException {
     ...
     */
    public SpacecraftState propagate(final boolean addSolarPanels) throws PatriusException {
