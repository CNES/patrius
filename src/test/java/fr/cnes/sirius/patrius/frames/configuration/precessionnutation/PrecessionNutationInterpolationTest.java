...
public class PrecessionNutationInterpolationTest {
	...
	public void testOrigin() {
		...
		final PrecessionNutationModel pnN = new PrecessionNutationInterpolation(
		...
		final PrecessionNutationModel pnN3 = new PrecessionNutationInterpolation(
	...
	@Test
	public void testCIPInterpolationsError() {
		...
		try {
			pnN.getCIPCoordinates(date).getCIPMotion();
	...
	public void testCIPInterpolationsCoverage() {
		...
		// This cover test checks the different computation branches in
		// getCIPCoordinates(date)
	...
	public void testExceptions() {
		...
		PrecessionNutationInterpolation pnN = new PrecessionNutationInterpolation(
				new IERS20032010PrecessionNutation(PrecessionNutationConvention.IERS2010));
		...
		// Try to initialize an ephemeris that exceeds the maximum allowed size (should
		// fail)
		pnN = new PrecessionNutationInterpolation(
				new IERS20032010PrecessionNutation(PrecessionNutationConvention.IERS2010), 60, 2, 5, 3);
		final AbsoluteDate firstUsableDate = AbsoluteDate.J2000_EPOCH;
    ...
    /**
     * when large shifts are used in AbsoluteDate.shiftedBy(), some dilution of numerical precision
     * can produce a slightly negative offset, which is normally not allowed (the offset must be between 0 and 1).
     * This negative offset can pose a problem in generateMeshDate() since it historically only used
     * the epoch of the date, discarding the offset entirely.
     * 
     * This test case ensures that the class PrecessionNutationInterpolation is robust to negative
     * offsets in dates, like the rest of PATRIUS.
     * 
     * The expected result of this test is that no exception is thrown and the method returns
     * nominally.
     */
	@Test
	public void testNegativeOffsetInDate() {
	    final PrecessionNutationModel model2010 = new IERS20032010PrecessionNutation(
                PrecessionNutationConvention.IERS2010);
        final PrecessionNutationInterpolation pn2010 = new PrecessionNutationInterpolation(model2010);
        
        // Manufacture a date with negative offset
        final AbsoluteDate firstUsableDate = new AbsoluteDate((int) Constants.JULIAN_DAY, -0.1);
        final AbsoluteDate lastUsableDate = firstUsableDate.shiftedBy(2 * Constants.JULIAN_DAY);
        
        // Run a function that calls generateMeshDate() internally
        pn2010.initializeCIPEphemeris(firstUsableDate, lastUsableDate);
