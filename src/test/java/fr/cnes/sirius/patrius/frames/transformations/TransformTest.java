...
public class TransformTest {
    ...
    public void testIdentityTranslation() {
        this.checkNoTransform(new Transform(AbsoluteDate.J2000_EPOCH, new Vector3D(0, 0, 0)),
            new Random(0xfd118eac6b5ec136l));
    }

    @Test
    public void testIdentityRotation() {
        this.checkNoTransform(new Transform(AbsoluteDate.J2000_EPOCH, new Rotation(false, 1, 0, 0, 0)), new Random(
            0xfd118eac6b5ec136l));
    }
    ...
    public void testAcceleration() {
        ...
        for (double dt = 0; dt < 1; dt += 0.01) {
            ...
            final PVCoordinates transformedPV = this.evolvingTransform(AbsoluteDate.J2000_EPOCH, dt)
            ...
            for (int i = -3; i < 4; ++i) {
                final Transform t = this.evolvingTransform(AbsoluteDate.J2000_EPOCH, dt + i * h);
            ...
            this.checkVector(rebuiltPV.getPosition(), transformedPV.getPosition(), 4.0e-16);
            this.checkVector(rebuiltPV.getVelocity(), transformedPV.getVelocity(), 2.0e-15);
            this.checkVector(rebuiltPV.getAcceleration(), transformedPV.getAcceleration(), 9.0e-11);
    ...
    public void testAccelerationComposition() {
        ...
        final Vector3D p1 = this.randomVector(1.0e3, random);
        final Vector3D v1 = this.randomVector(1.0, random);
        final Vector3D a1 = this.randomVector(1.0e-3, random);
        final Rotation r1 = this.randomRotation(random);
        final Vector3D o1 = this.randomVector(0.1, random);

        final Vector3D p2 = this.randomVector(1.0e3, random);
        final Vector3D v2 = this.randomVector(1.0, random);
        final Vector3D a2 = this.randomVector(1.0e-3, random);
        final Rotation r2 = this.randomRotation(random);
        final Vector3D o2 = this.randomVector(0.1, random);

        final Transform t1 = new Transform(AbsoluteDate.J2000_EPOCH,
            new Transform(AbsoluteDate.J2000_EPOCH, p1, v1, a1),
            new Transform(AbsoluteDate.J2000_EPOCH, r1, o1), true);
        final Transform t2 = new Transform(AbsoluteDate.J2000_EPOCH,
            new Transform(AbsoluteDate.J2000_EPOCH, p2, v2, a2),
            new Transform(AbsoluteDate.J2000_EPOCH, r2, o2), true);
        final Transform t12 = new Transform(AbsoluteDate.J2000_EPOCH, t1, t2, true);

        final Vector3D q = this.randomVector(1.0e3, random);
        final Vector3D qDot = this.randomVector(1.0, random);
        final Vector3D qDotDot = this.randomVector(1.0e-3, random);
        ...
        this.checkVector(pva2.getPosition(), pvac.getPosition(), 1.0e-15);
        this.checkVector(pva2.getVelocity(), pvac.getVelocity(), 1.0e-15);
        this.checkVector(pva2.getAcceleration(), pvac.getAcceleration(), 1.0e-15);
    ...
    public void testRandomComposition() {
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            for (int k = 0; k < n; ++k) {
                transforms[k] =
                    random.nextBoolean()
                        ? new Transform(AbsoluteDate.J2000_EPOCH, this.randomVector(1.0e3, random),
                            this.randomVector(1.0, random), this.randomVector(1.0e-3, random))
                        : new Transform(AbsoluteDate.J2000_EPOCH, this.randomRotation(random), this.randomVector(0.01,
                            random),
                            this.randomVector(1.0e-4, random));
                combined = new Transform(AbsoluteDate.J2000_EPOCH, combined, transforms[k], true);
            }

            // check the composition
            for (int j = 0; j < 10; ++j) {
                final Vector3D a = this.randomVector(1.0, random);
                final Vector3D b = this.randomVector(1.0e3, random);
                final PVCoordinates c =
                    new PVCoordinates(this.randomVector(1.0e3, random), this.randomVector(1.0, random),
                        this.randomVector(1.0e-3, random));
                Vector3D aRef = a;
                Vector3D bRef = b;
                ...
                this.checkVector(aRef, aCombined, 3.0e-15);
                this.checkVector(bRef, bCombined, 5.0e-14);
                this.checkVector(cRef.getPosition(), cCombined.getPosition(), 1.0e-14);
                this.checkVector(cRef.getVelocity(), cCombined.getVelocity(), 1.0e-14);
                this.checkVector(cRef.getAcceleration(), cCombined.getAcceleration(), 1.0e-14);
    ...
    public void testReverse() {
        ...
        for (int i = 0; i < 20; ++i) {
            final Transform combined = this.randomTransform(random);

            this.checkNoTransform(new Transform(AbsoluteDate.J2000_EPOCH, combined, combined.getInverse()), random);
            this.checkNoTransform(Transform.IDENTITY.getInverse(), random);
    ...
    public void testDecomposeAndRebuild() {
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            this.checkNoTransform(new Transform(AbsoluteDate.J2000_EPOCH, combined, rebuilt.getInverse(true), true),
                random);
    ...
    public void testTranslation() {
        ...
        for (int i = 0; i < 10; ++i) {
            final Vector3D delta = this.randomVector(rnd);
    ...
    public void testRoughTransPV() {
        ...
        final PVCoordinates pointP2 = new PVCoordinates(new Vector3D(0, 0, 0), new Vector3D(0, 0, 0), new Vector3D(0, 0, 0));
        ...
        this.checkVector(pointP2.getPosition(), result1.getPosition(), 1.0e-15);
        this.checkVector(pointP2.getVelocity(), result1.getVelocity(), 1.0e-15);
        this.checkVector(pointP2.getAcceleration(), result1.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP1.getPosition(), invResult1.getPosition(), 1.0e-15);
        this.checkVector(pointP1.getVelocity(), invResult1.getVelocity(), 1.0e-15);
        this.checkVector(pointP1.getAcceleration(), invResult1.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP3.getPosition(), result2.getPosition(), 1.0e-15);
        this.checkVector(pointP3.getVelocity(), result2.getVelocity(), 1.0e-15);
        this.checkVector(pointP3.getAcceleration(), result2.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP1.getPosition(), invResult2.getPosition(), 1.0e-15);
        this.checkVector(pointP1.getVelocity(), invResult2.getVelocity(), 1.0e-15);
        this.checkVector(pointP1.getAcceleration(), invResult2.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP4.getPosition(), compResult.getPosition(), 1.0e-15);
        this.checkVector(pointP4.getVelocity(), compResult.getVelocity(), 1.0e-15);
        this.checkVector(pointP4.getAcceleration(), compResult.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP5.getPosition(), combResult.getPosition(), 1.0e-15);
        this.checkVector(pointP5.getVelocity(), combResult.getVelocity(), 1.0e-15);
        this.checkVector(pointP5.getAcceleration(), combResult.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP3.getPosition(), result.getPosition(), 1.0e-15);
        this.checkVector(pointP3.getVelocity(), result.getVelocity(), 1.0e-15);
        this.checkVector(pointP3.getAcceleration(), result.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP2.getPosition(), result.getPosition(), 1.0e-15);
        this.checkVector(pointP2.getVelocity(), result.getVelocity(), 1.0e-15);
        this.checkVector(pointP2.getAcceleration(), result.getAcceleration(), 1.0e-15);
        ...
        this.checkVector(pointP5.getPosition(), result.getPosition(), 1.0e-15);
        this.checkVector(pointP5.getVelocity(), result.getVelocity(), 1.0e-15);
        this.checkVector(pointP5.getAcceleration(), result.getAcceleration(), 1.0e-15);
    ...
    public void testRotPV() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            this.checkVectors(resultvel, vel);
    ...
    public void testTransPV() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            // random transform
            final Vector3D trans = this.randomVector(rnd);
            final Vector3D transVel = this.randomVector(rnd);
    ...
    public void testRotation() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            final Rotation r = this.randomRotation(rnd);
    ...
    public void testJacobian() {
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            for (int j = 0; j < 100; ++j) {
                ...
                final PVCoordinates pv0 = new PVCoordinates(this.randomVector(random), this.randomVector(random));
    ...
    public void testLine() {
        ...
        for (int i = 0; i < 100; ++i) {
            final Transform transform = this.randomTransform(random);
            for (int j = 0; j < 20; ++j) {
                final Vector3D p0 = this.randomVector(random);
                final Vector3D p1 = this.randomVector(random);
    ...
    public void testLinear() {
        ...
        for (int n = 0; n < 100; ++n) {
            ...
            for (int i = 0; i < 100; ++i) {
                final Vector3D p = this.randomVector(random);
        ...
        for (double dt = -10.0; dt < 10.0; dt += 0.125) {
            ...
            final PVCoordinates expectedFixedPoint =
                shifted.transformPVCoordinates(new PVCoordinates(new Vector3D(1, dt, 0), Vector3D.PLUS_J,
                    Vector3D.ZERO));
            this.checkVector(expectedFixedPoint.getPosition(), Vector3D.ZERO, 1.0e-14);
            this.checkVector(expectedFixedPoint.getVelocity(), Vector3D.ZERO, 1.0e-14);
            this.checkVector(expectedFixedPoint.getAcceleration(), Vector3D.ZERO, 1.0e-14);
            ...
            final Vector3D referenceAcceleration = new Vector3D(omega * (2 + omega) * c - dt * omega * omega * s,
                ...
                0);
            this.checkVector(expectedApparentMotion.getPosition(), referencePosition, 1.0e-14);
            this.checkVector(expectedApparentMotion.getVelocity(), referenceVelocity, 1.0e-14);
            this.checkVector(expectedApparentMotion.getAcceleration(), referenceAcceleration, 1.0e-14);
    ...
    public void testShiftDerivatives() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            for (double dt = -10.0; dt < 10.0; dt += 0.125) {
                ...
                final double numXDot = this.derivative(h,
                ...
                final double numYDot = this.derivative(h,
                ...
                final double numZDot = this.derivative(h,
                ...
                final double numXDot2 = this.derivative(h,
                ...
                final double numYDot2 = this.derivative(h,
                ...
                final double numZDot2 = this.derivative(h,
                ...
                final double numQ0Dot = this.derivative(h,
                ...
                final double numQ1Dot = this.derivative(h,
                ...
                final double numQ2Dot = this.derivative(h,
                ...
                final double numQ3Dot = this.derivative(h,
                ...
                final double numOxDot = this.derivative(h,
                ...
                final double numOyDot = this.derivative(h,
                ...
                final double numOzDot = this.derivative(h,
    ...
    public void testInterpolation() throws PatriusException {
        ...
        final List<Transform> sample = new ArrayList<Transform>();
        for (int i = 0; i < 5; ++i) {
            sample.add(this.evolvingTransform(t0, i * 0.8));
        }

        for (double dt = 0.1; dt <= 3.1; dt += 0.01) {
            final Transform reference = this.evolvingTransform(t0, dt);
    ...
    public void testInterpolationRotationOnly() throws PatriusException {
        ...
        final List<Transform> sample = new ArrayList<Transform>();
        for (int i = 0; i < 5; ++i) {
            sample.add(this.evolvingTransform2(t0, i * 0.8));
        }

        for (double dt = 0.1; dt <= 3.1; dt += 0.01) {
            final Transform reference = this.evolvingTransform2(t0, dt);
            final Transform interpolated = sample.get(0).interpolate(reference.getDate(), sample);
    ...
    private Transform evolvingTransform2(final AbsoluteDate t0, final double dt) {
        final double omega = 0.2;
        final AbsoluteDate date = t0.shiftedBy(dt);
    ...
    private double derivative(final double h,
    ...
    private Transform randomTransform(final Random random) {
        ...
        for (int k = 0; k < 20; ++k) {
            final Transform t =
                random.nextBoolean() ? new Transform(AbsoluteDate.J2000_EPOCH, this.randomVector(random),
                    this.randomVector(random), this.randomVector(random)) : new Transform(AbsoluteDate.J2000_EPOCH,
                    this.randomRotation(random),
                    this.randomVector(random), this.randomVector(random));
            combined = new Transform(AbsoluteDate.J2000_EPOCH, combined, t);
        }
        return combined;
    }

    private Transform randomTransform(final RandomGenerator random) {
        // generate a random transform
        Transform combined = Transform.IDENTITY;
        for (int k = 0; k < 20; ++k) {
            final Transform t =
                random.nextBoolean() ?
                    new Transform(AbsoluteDate.J2000_EPOCH, this.randomVector(1.0e3, random), this.randomVector(1.0,
                        random),
                        this.randomVector(1.0e-3, random)) :
                    new Transform(AbsoluteDate.J2000_EPOCH, this.randomRotation(random),
                        this.randomVector(0.01, random),
                        this.randomVector(1.0e-4, random));
    ...
    private Vector3D randomVector(final Random random) {
        return new Vector3D(random.nextDouble() * 1.0, random.nextDouble() * 1.0, random.nextDouble() * 1.0);
    }

    private Vector3D randomVector(final double scale, final RandomGenerator random) {
        return new Vector3D(random.nextDouble() * scale,
            random.nextDouble() * scale,
            random.nextDouble() * scale);
    }

    private Rotation randomRotation(final Random random) {
    ...
    private void checkNoTransform(final Transform transform, final RandomGenerator random) {
        for (int i = 0; i < 100; ++i) {
            final Vector3D a = this.randomVector(1.0e3, random);
            final Vector3D tA = transform.transformVector(a);
            Assert.assertEquals(0, a.subtract(tA).getNorm(), 1.0e-10 * a.getNorm());
            final Vector3D b = this.randomVector(1.0e3, random);
            final Vector3D tB = transform.transformPosition(b);
            ...
            final PVCoordinates pv =
                new PVCoordinates(this.randomVector(1.0e3, random), this.randomVector(1.0, random), this.randomVector(
                    1.0e-3, random));
            final PVCoordinates tPv = transform.transformPVCoordinates(pv);
            this.checkVector(pv.getPosition(), tPv.getPosition(), 1.0e-10);
            this.checkVector(pv.getVelocity(), tPv.getVelocity(), 3.0e-9);
            this.checkVector(pv.getAcceleration(), tPv.getAcceleration(), 3.0e-9);
        }
    }
    ...
    public void testTransformWrench() {
        ...
        this.checkVectors(force, transformed.getForce());
        this.checkVectors(Vector3D.PLUS_I, transformed.getTorque());
    }
    ...
    private void checkVector(final Vector3D reference, final Vector3D result, final double relativeTolerance) {
        if (reference == null) {
