...
public class FrameTest {
    ...
    public void testFrameReferentialAngularCoordinates() throws PatriusException {
        ...
        CartesianOrbit orbit = new CartesianOrbit(pv, gcrf, AbsoluteDate.J2000_EPOCH.shiftedBy(4*365*86400.), Constants.GRS80_EARTH_MU);
        
        // Checks
        // Position in ITRF is the same independently of referential
        Assert.assertEquals(0., orbit.getPVCoordinates(itrf).getPosition().subtract(orbit.getPVCoordinates(itrf_gcrf).getPosition()).getNorm(), 1E-12);
        // Velocity is the same when referential is the same
        Assert.assertEquals(pv.getVelocity().getNorm(), orbit.getPVCoordinates(itrf_gcrf).getVelocity().getNorm(), 1E-12);
    }
    ...
    public void testGetPVCoordinates() throws PatriusException {
        ...
        final Frame frame2 = new Frame(frame, new Transform(AbsoluteDate.J2000_EPOCH, new PVCoordinates(translation2, velocity)), "child2");
        ...
        final Frame frame3 = new Frame(frame, new Transform(AbsoluteDate.J2000_EPOCH, new PVCoordinates(translation1, Vector3D.ZERO), angular), "child3");
