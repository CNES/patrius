...
//    }
    ...
    @Test
    public void testSetCenter() throws PatriusException {
        ...
        final Vector3D pos2 = new Vector3D(3., Vector3D.PLUS_J);
        final Vector3D vel2 = new Vector3D(7., Vector3D.MINUS_K);
        final PVCoordinates pvc2 = new PVCoordinates(pos2, vel2);

        final PVCoordinatesProvider pvcProv2 = new BasicPVCoordinatesProvider(pvc2, gcrf);
        final Transform t1 = tnw1.getTransformTo(FramesFactory.getGCRF(), initialDate);
        tnw1.setCenter(pvcProv2);
        final Transform t2 = tnw1.getTransformTo(FramesFactory.getGCRF(), initialDate);
        // Check center of transform provider has been updated
        Assert.assertFalse(Vector3D.distance(t1.getCartesian().getPosition(), t2.getCartesian().getPosition()) == 0);
    }
