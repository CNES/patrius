...
public class SynodicFrameTest {
    ...
    private void testSynodicFrame(final LOFType lofType) throws PatriusException {
        ...
        final PVCoordinates pvCoord = pvProv.getPVCoordinates(date, frame2);
        final Vector3D position = pvCoord.getPosition();
        final Vector3D velocity = pvCoord.getVelocity();
        final Vector3D acceleration = pvCoord.getAcceleration();
        Assert.assertEquals(position.getX(), 0.0, Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(position.getY(), 0.0, Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(position.getZ(), 0.0, Precision.DOUBLE_COMPARISON_EPSILON);
