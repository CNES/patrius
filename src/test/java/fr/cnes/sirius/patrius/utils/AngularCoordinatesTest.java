...
public class AngularCoordinatesTest {
    ...
    public void testRoundTripNoOp() {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            final Vector3D oDot1 = this.randomVector(random, 1.0e-2);
            ...
            final Vector3D oDot2 = this.randomVector(random, 1.0e-2);
    ...
    public void testRodriguesSymmetry() {
        ...
        for (int i = 0; i < 1000; ++i) {
            final Rotation rotation = this.randomRotation(random);
            final Vector3D rotationRate = this.randomVector(random, 0.01);
            final Vector3D rotationAcceleration = this.randomVector(random, 0.01);
    ...
    public void testRodriguesSpecialCases() {
        ...
        for (int i = 0; i < 100; ++i) {
            final Vector3D axis = this.randomVector(random, 1.0);
    ...
    @Test
    public void testShiftWithoutAcceleration() throws PatriusException {
        final double rate = 2 * FastMath.PI / (12 * 60);
    ...
    @Test
    public void testShiftWithAcceleration() throws PatriusException {
        final double rate = 2 * FastMath.PI / (12 * 60);
    ...
    public void testInverseCrossProducts()
        ...
        this.checkInverse(Vector3D.PLUS_K, Vector3D.PLUS_I, Vector3D.PLUS_J);
        this.checkInverse(Vector3D.ZERO, Vector3D.ZERO, Vector3D.ZERO);
        this.checkInverse(Vector3D.ZERO, Vector3D.ZERO, Vector3D.PLUS_J);
        this.checkInverse(Vector3D.PLUS_K, Vector3D.PLUS_K, Vector3D.PLUS_J);
        this.checkInverse(Vector3D.ZERO, Vector3D.PLUS_K, Vector3D.ZERO);
        this.checkInverse(Vector3D.PLUS_K, Vector3D.PLUS_I, Vector3D.PLUS_K);
        this.checkInverse(Vector3D.PLUS_K, Vector3D.PLUS_I, Vector3D.PLUS_I);
        this.checkInverse(Vector3D.PLUS_K, Vector3D.PLUS_I, new Vector3D(1, 0, -1).normalize());
        this.checkInverse(Vector3D.ZERO, Vector3D.PLUS_I, Vector3D.ZERO, Vector3D.PLUS_J, Vector3D.ZERO);
    }

    @Test
    public void testInverseCrossProductsFailures() {
        this.checkInverseFailure(Vector3D.PLUS_K, Vector3D.ZERO, Vector3D.PLUS_J, Vector3D.PLUS_I, Vector3D.PLUS_K);
        this.checkInverseFailure(Vector3D.PLUS_K, Vector3D.ZERO, Vector3D.ZERO, Vector3D.ZERO, Vector3D.PLUS_K);
        this.checkInverseFailure(Vector3D.PLUS_I, Vector3D.PLUS_I, Vector3D.ZERO, Vector3D.MINUS_I, Vector3D.PLUS_K);
        this.checkInverseFailure(Vector3D.PLUS_I, Vector3D.PLUS_I, Vector3D.ZERO, Vector3D.PLUS_J, Vector3D.PLUS_J);
        this.checkInverseFailure(Vector3D.PLUS_I, Vector3D.PLUS_I, Vector3D.PLUS_J, Vector3D.PLUS_J, Vector3D.ZERO);
        this.checkInverseFailure(Vector3D.PLUS_I, Vector3D.PLUS_I, Vector3D.PLUS_J, Vector3D.ZERO, Vector3D.PLUS_J);
    }

    @Test
    public void testRandomInverseCrossProducts() throws PatriusException {
        ...
        for (int i = 0; i < 10000; ++i) {
            ...
            final Vector3D v1 = this.randomVector(generator, 10 * generator.nextDouble() + 1.0);
            final Vector3D v2 = this.randomVector(generator, 10 * generator.nextDouble() + 1.0);
            this.checkInverse(omega, v1, v2);
        }
    }
    ...
    public void testRandomPVCoordinates() throws PatriusException {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            for (int j = 0; j < 100; ++j) {
                ...
                final PVCoordinates v1 = this.randomPVCoordinates(generator, 1000, 1.0, 0.001);
                final PVCoordinates v2 = this.randomPVCoordinates(generator, 1000, 1.0, 0.001);
    ...
    public void testAngularCoordException2() throws PatriusException {
        ...
        final RandomGenerator generator = new Well1024a(0x52b29d8f6ac2d64bl);
        final PVCoordinates u1 = this.randomPVCoordinates(generator, 1000, 1.0, 0.001);
        final PVCoordinates u2 = this.randomPVCoordinates(generator, 1000, 1.0, 0.001);
        final PVCoordinates v1 = this.randomPVCoordinates(generator, 1000, 1.0, 0.001);
        final PVCoordinates v2 = this.randomPVCoordinates(generator, 1000, 1.0, 0.001);
    ...
    private void checkInverse(final Vector3D omega, final Vector3D v1, final Vector3D v2) throws PatriusException {
        this.checkInverse(omega,
            v1, Vector3D.crossProduct(omega, v1),
            v2, Vector3D.crossProduct(omega, v2));
    }

    private void checkInverseFailure(final Vector3D omega, final Vector3D v1, final Vector3D c1, final Vector3D v2,
                                     final Vector3D c2) {
        try {
            this.checkInverse(omega, v1, c1, v2, c2);
    ...
    private void checkInverse(final Vector3D omega, final Vector3D v1, final Vector3D c1, final Vector3D v2,
                              final Vector3D c2)
    ...
    private Vector3D randomVector(final Random random, final double norm) {
