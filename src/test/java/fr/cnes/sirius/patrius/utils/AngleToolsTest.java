...
public class AngleToolsTest {
    ...
    public enum features {
         ...
         *
         * @featureDescription Computes an angle between 2 vectors of R2 or R3, from a sine or a
         *                     cosine.
         *
         ...
         *
         * @featureDescription Supplementary, complementary and opposite angles computation in an
         *                     interval.
         *
     ...
     *
     * @description Nominal cases and degraded cases for angle computation between 2 vectors 3D
     *              (without orientation).
     *
     * @input Vecstor3D vector1 : real values, very small values, very large values, NaN value,
     *        infinite value, vector
     *        with a zero norm.
     * @input Vecstor3D vector2 : real values, very small values, very large values, NaN value,
     *        infinite value, vector
     *        with a zero norm.
     *
     ...
     */
    @Test
    public void testGetAngleBetween2Vector3D() {
     ...
     */
    @Test
    public void testGetOrientedAngleBetween2Vector2D() {
     ...
     *
     * @description Nominal cases and degraded cases for oriented angle computation between 2
     *              vectors 3D.
     *
     * @input Vecstor3D vector1 : real values.
     * @input Vecstor3D vector2 : real values.
     * @input Vecstor3D vector3 : real values, NaN value, infinite value, vector with a zero norm,
     *        normal to the plane
     ...
     */
    @Test
    public void testGetOrientedAngleBetween2Vector3D() {
     ...
     *
     * @input The inputs are real numbers given by the cosine and the sine of a random angle. The
     *        case when the
     *        inputs are a cosine and a sine of different angles is tested. The limits cases when
     *        the inputs are the
     ...
     *
     * @testPassCriteria The angle used to compute the cosine and the sine given as entries of the
     *                   tested function.
     *
     * @referenceVersion 1.0
     *
     ...
     */
    @Test
    public void testGetAngleFromCosineAndSine() {
     ...
     *
     * @testedMethod {@link AngleTools#getOrientedAngleBetween2Vector2D(Vector2D, Vector2D)}
     * @testedMethod {@link AngleTools#getOrientedAngleBetween2Vector3D(Vector3D, Vector3D, Vector3D)}
     *
     * @description Nominal cases and degraded cases for angles computed from a cosine and a sine.
     *
     * @input The inputs are simple unit vectors given in 2D, then the same vectors with an
     *        additional Z dimension. the Z vector is used as the orientation vector.
     *
     * @output Double angle : defined between {@code -PI} and {@code PI}.
     *
     * @testPassCriteria The angle is the same expressed in 2D or in 3D in the same plan
     *
     ...
     */
    @Test
    public void testAngle2DAnd3D() {

        // Creation of data in 2D
        final Vector2D first2D = new Vector2D(1, 0);
        final Vector2D second2D = new Vector2D(0, 1);

        final double angle2D = AngleTools.getOrientedAngleBetween2Vector2D(first2D, second2D);

        // Creation of data in 3D (correspond to the same data in 2D)
        final Vector3D first3D = new Vector3D(1, 0, 0);
        final Vector3D second3D = new Vector3D(0, 1, 0);
        final Vector3D orientationVector = new Vector3D(0, 0, 1);

        final double angle3D = AngleTools.getOrientedAngleBewteen2Vector3D(first3D, second3D, orientationVector);

        // both angles must be the same
        Assert.assertEquals(angle2D, angle3D, 0);
    }
     ...
     *
     * @input The inputs are a double (angle) and an interval. Several doubles are tested : one
     *        whose 2PI modulo belongs
     ...
     */
    @Test
    public void angleInIntervalTest() {
     ...
     * @description Test of the modulo function with nominal and error cases
     *
     * @input The inputs are a double (angle) and an interval. Several doubles are tested around
     *        critical values to see
    ...
    public void angleInIntervalTest1() {
        ...
        for (int k = -5; k < 5; k++) {
            // décalage interval
            zero2Pi = new AngleInterval(IntervalEndpointType.CLOSED, deuxPi * k, deuxPi * (k + 1),
     ...
     *
     * @testPassCriteria All the angles must be computed without exceptions being thrown.
     *                   angles lower than 2pi accuracy must be exactly (0 ulp difference) equal to
     *                   0
     *                   angles bigger than 2pi accuracy must be exactly (0 ulp difference) equal to
     *                   2pi - angle.
    ...
    public void angleInIntervalTest2() {
        ...
        final AngleInterval zero2Pi = new AngleInterval(IntervalEndpointType.CLOSED, 0, deuxPi,
                IntervalEndpointType.OPEN);
        ...
        for (final double element : azimuth) {
            try {
                ...
                res = Precision.equals(MathLib.abs(element) < threshold / 2. ? 0 : deuxPi + element, result, 0);
     ...
     *
     * @testPassCriteria All the angles must be computed without exceptions being thrown.
     *                   computed angles must be exactly (0 ulp difference) equal to the expected
     *                   values
     *
     * @referenceVersion 2.1
     *
    ...
    public void angleInIntervalTest3() {
        ...
        final double[] azimuth = { 1e-14, 9e-15, 8e-15, 7e-15, 6e-15, 5e-15, 4e-15, 3e-15, 2e-15, 1e-15, 9e-16, 8e-16,
                7e-16, 6e-16, 5e-16, 4e-16, 3e-16, 2e-16, 1e-16, Precision.SAFE_MIN };
     ...
     *
     * @output Boolean set to true if the angles are equal once expressed in the interval, false
     *         otherwise.
     *
     * @testPassCriteria For the first test, the boolean returned must be true, for the second it
     *                   must be false, for the
     *                   third,
     ...
     */
    @Test
    public void supplementaryAngleTest() {
        double angle;
     ...
     */
    @Test
    public void complementaryAngleTest() {
        double angle;
        final double eps = 1.0e-10;
        // First test
        try {
    ...
    public void testInterval() {
        ...
        final AngleInterval interval = new AngleInterval(value + FastMath.PI, MathUtils.TWO_PI,
    ...
    public void testInterval2() {
        ...
        final AngleInterval interval = new AngleInterval(IntervalEndpointType.CLOSED, ref - FastMath.PI, ref
        ...
        final AngleInterval interval2 = new AngleInterval(IntervalEndpointType.CLOSED,
                valueInRefInterval - FastMath.PI, valueInRefInterval + FastMath.PI, IntervalEndpointType.OPEN);
    ...
    public void testModuloTwoPi() {
        ...
        // Calculate the modulo2PI for all the angles contained in the input list
        final List<Double> result = AngleTools.modulo(input, MathUtils.TWO_PI);

        // All the modulo2PI must be the same
        for (int i = 1; i < 5; i++) {
            Assert.assertEquals(result.get(i), result.get(i - 1), Precision.DOUBLE_COMPARISON_EPSILON);
        }

        // Same if we do it angle by angle
        for (final Double inAng : input) {
            final Double outAng = AngleTools.moduloTwoPi(inAng);
            Assert.assertEquals(outAng, 30 * Math.PI / 180, Precision.DOUBLE_COMPARISON_EPSILON);
        }
    }
    ...
    public void testMeanAngle() {
        ...
        final double quaterPi = Math.PI / 4.;

        // 1) Dummy call to the method:
        Assert.assertEquals(halfPi, AngleTools.meanAngle(halfPi), Precision.DOUBLE_COMPARISON_EPSILON);

        // 2) Two angles
        Assert.assertEquals(quaterPi, AngleTools.meanAngle(0., halfPi), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(0., AngleTools.meanAngle(quaterPi, 7. * quaterPi), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(0., AngleTools.meanAngle(quaterPi, -quaterPi), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(Math.PI, AngleTools.meanAngle(5. * quaterPi, 3. * quaterPi),
                Precision.DOUBLE_COMPARISON_EPSILON);

        // 3) N angles
        Assert.assertEquals(halfPi, AngleTools.meanAngle(halfPi, halfPi, halfPi), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(MathLib.atan2(1., 2.), AngleTools.meanAngle(halfPi, 0., 0.),
                Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(Double.NaN, AngleTools.meanAngle(halfPi, halfPi, -halfPi, -halfPi),
                Precision.DOUBLE_COMPARISON_EPSILON);
    }
     ...
     * @testedFeature {@link features#ANGLE_COMPARISONS}
     *
     * @testedMethod {@link AngleTools#unMod(List, double)}
     *
     *
     * @input a array of data to be unmod.
     *
     *
     * @referenceVersion 4.11
     * @nonRegressionVersion 4.11
     */
    @Test
    public void testUnMod() {
        final List<Double> y = new ArrayList<>(Arrays.asList(0., 0.14, 1.2, -2.85, -0.5, 1.6, 2.47, -2.78));

        AngleTools.unMod(y);

        final List<Double> yUnModExpected = new ArrayList<>(Arrays.asList(0.0, 0.14, 1.2, 3.433185307179586, 5.783185307179586, 7.883185307179586, 8.753185307179587, 9.786370614359173));
        Assert.assertEquals(yUnModExpected, y);
     ...
     */
    @Test
    public void testUnModWithApproxSlope() {
        final List<Double> x = new ArrayList<>(Arrays.asList(0., 1., 2., 4., 5.));
        final List<Double> y = new ArrayList<>(Arrays.asList(0., 2., 4. - MathUtils.TWO_PI, 8 - MathUtils.TWO_PI,
                10 - 2 * MathUtils.TWO_PI));
        AngleTools.unMod(x, y, MathUtils.TWO_PI, 2.);
        final List<Double> yUnModExpected = new ArrayList<>(Arrays.asList(0., 2., 4., 8., 10.));
        Assert.assertEquals(yUnModExpected, y);
