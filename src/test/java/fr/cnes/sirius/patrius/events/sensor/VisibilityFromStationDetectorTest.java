...
public class VisibilityFromStationDetectorTest {
    ...
    public void bodyOccultationTest() throws PatriusException {
        ...
        final double r = 6000000.0;
        final BodyShape earth = new OneAxisEllipsoid(r, 0.0, EME2000Frame);
        final GeodeticPoint point = new GeodeticPoint(0.0, FastMath.PI, 0.1);
        final TopocentricFrame topoFrame = new TopocentricFrame(earth, point, "Gstation");
        ...
        // attitude law
        final AttitudeProvider attitudeProv = new TargetGroundPointing(earth, point);
        ...
        Vector3D[] intersectionsMoon = moon.getShape().getIntersectionPoints(line, moonEme2000, date0);
        ...
        Assert.assertEquals(0., endState.getDate().durationFrom(endDate), datesComparisonEpsilon);
     ...
     * @testedMethod {@link VisibilityFromStationDetector#eventOccurred(SpacecraftState, boolean, boolean)}
     * 
     * @description Check visibility from station with body occultation when light speed is taken into account (downlink and uplink)
     * 
    ...
    public void bodyOccultationLightSpeedTest() throws PatriusException {
        ...
        // Propagator with detector
        Propagator propagator1 = new KeplerianPropagator(orbit, attitudeProv);
        propagator1.addEventDetector(detector1);

        // Propagate
        SpacecraftState endState1 = propagator1.propagate(date0.shiftedBy(orbitPeriod));
        AbsoluteDate endDate1 = endState1.getDate();
        ...
        // Propagator with detector
        Propagator propagator2 = new KeplerianPropagator(orbit, attitudeProv);
        propagator2.addEventDetector(detector2);

        // Propagate
        SpacecraftState endState2 = propagator2.propagate(date0.shiftedBy(orbitPeriod));
        AbsoluteDate endDate2 = endState2.getDate();
     ...
     * @testedMethod {@link VisibilityFromStationDetector#eventOccurred(SpacecraftState, boolean, boolean)}
     * 
     * @description Check visibility from station with spacecraft occultation when light speed is taken into account (downlink and uplink)
     *              This test is the same as bodyOccultationLightSpeedTest, except the occulting body is replaced by a spacecraft
     * 
     * @testPassCriteria dates are coherent (no regression only, FD check has been performed beforehand)
     * 
     * @referenceVersion 4.10.2
    ...
    public void spacecraftOccultationLightSpeedTest() throws PatriusException {
        ...
        builder2.addProperty(new GeometricProperty(new Sphere(Vector3D.ZERO, Constants.IERS92_MOON_EQUATORIAL_RADIUS)), "MaskingSpacecraftMainPart");
        ...
        final SecondarySpacecraft maskingSpacecraft = new SecondarySpacecraft(assembly2, propagatorMasking, "MaskingSpacecraft");
    ...
    private static class TestStepHandler implements PatriusFixedStepHandler {
        ...
        public void handleStep(SpacecraftState currentState, boolean isLast) throws PropagationException {
            ...
            try {
                value = detector.g(currentState);
                final Vector3D sightAxis = sensor.getSightAxis(sFrame, sDate);
                final Vector3D satPos = currentState.getPVCoordinates().getPosition();
                final Line line = Line.createLine(satPos, sightAxis, satPos);
                final Vector3D[] intersectionsMoon = moon.getIntersectionPoints(line, sFrame, sDate);
