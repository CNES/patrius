 ...
 * Specifics :
 * <ul>
 * <li>Each thread asks simultaneously for a the same celestial body : no issues.</li>
 * </ul>
 * 
 * @author cardosop
 * 
 * @version $Id: CelestialBodyFactoryThirdTest.java 17911 2017-09-11 12:02:31Z bignon $
 * 
 * @since 1.2
 * 
 */
public class CelestialBodyFactoryThirdTest {

    /** Orekit data path. */
    private static final String KEY = "orekit.data.path";

    /** Invocation count for the multithreaded tests. */
    private static final int INVOC_COUNT = 50;
    /** Number of threads. */
    ...
    public void celestialBodyConcurrentTest() throws PatriusException, InterruptedException {
        ...
        if (orderCB == -1) {
            ...
            // Pick one celestial body
            final CelestialPoint b1 = CelestialBodyFactory.getEarth();
            // Wait a bit to delay the IO slowdown
            Thread.sleep(1000);
