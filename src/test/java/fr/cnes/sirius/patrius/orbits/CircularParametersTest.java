...
public class CircularParametersTest {
    ...
    public void testCircularToEquinoctialCirc() {
        ...
        final EquinoctialOrbit paramCir = new EquinoctialOrbit(pvCoordinates,
                FramesFactory.getEME2000(), this.date, this.mu);
        checkDouble(paramCir.getA(), circCir.getA(), Utils.epsilonTest * circCir.getA(), "a");
        checkDouble(paramCir.getEquinoctialEx(), circCir.getEquinoctialEx(), Utils.epsilonEcir
                * MathLib.abs(circCir.getE()), "ex");
        checkDouble(paramCir.getEquinoctialEy(), circCir.getEquinoctialEy(), Utils.epsilonEcir
                * MathLib.abs(circCir.getE()), "ey");
        checkDouble(paramCir.getHx(), circCir.getHx(),
                Utils.epsilonAngle * MathLib.abs(circCir.getI()), "Hx");
        checkDouble(paramCir.getHy(), circCir.getHy(),
                Utils.epsilonAngle * MathLib.abs(circCir.getI()), "Hy");
        checkDouble(MathUtils.normalizeAngle(paramCir.getLv(), circCir.getLv()),
	...
	public void testCircularToCartesian() {
		...
		Report.printMethodHeader("testCartesianToEquinoctial",
				"Circular to cartesian", "Orekit", Utils.epsilonTest,
				ComparisonType.RELATIVE);

		final double ix = 1.200e-04;
		final double iy = -1.16e-04;
		final double i = 2 * MathLib
				.asin(MathLib.sqrt((ix * ix + iy * iy) / 4));
		final double raan = MathLib.atan2(iy, ix);
		...
		final CircularOrbit circ = new CircularOrbit(42166.712, ex, ey, i,
				raan, 5.300 - raan, PositionAngle.MEAN,
				FramesFactory.getEME2000(), this.date, this.mu);
		final Vector3D pos = circ.getPVCoordinates().getPosition();
		...
		checkDouble(0.233745668678733e+05, pos.getX(), Utils.epsilonTest * r,
				"X");
		checkDouble(-0.350998914352669e+05, pos.getY(), Utils.epsilonTest * r,
				"Y");
		checkDouble(-0.150053723123334e+01, pos.getZ(), Utils.epsilonTest * r,
				"Z");

		checkDouble(0.809135038364960e+05, vel.getX(), Utils.epsilonTest * v,
				"Vx");
		checkDouble(0.538902268252598e+05, vel.getY(), Utils.epsilonTest * v,
				"Vy");
		checkDouble(0.158527938296630e+02, vel.getZ(), Utils.epsilonTest * v,
				"Vz");
	...
	public void testHyperbola() {
		final CircularOrbit orbit = new CircularOrbit(-10000000.0, 2.5, 0,
				17.188733853924695, 0.0, 0.0, PositionAngle.TRUE,
		...
		final Vector3D focus2 = new Vector3D(-2 * orbit.getA() * orbit.getE(),
				u);
		for (double dt = -5000; dt < 5000; dt += 60) {
			...
			final KeplerianOrbit rebuilt = new KeplerianOrbit(pv,
					orbit.getFrame(), orbit.getDate().shiftedBy(dt), this.mu);
    ...
    public void testCircularToKeplerian() {
        ...
        checkDouble(42166.71200, circ.getA(), Utils.epsilonTest * kep.getA(), "a");
    ...
    public void testOrbitAEMismatch(){
    	new CircularParameters(-8000, 0.12, 0.2, 0.05, 0.5, 2.5,
    			PositionAngle.TRUE, this.mu);
    }
    
	@Test(expected = IllegalArgumentException.class)
	public void testOrbitAnomalyOutOfHyperRange() throws PatriusException {
		new CircularOrbit(-7000434.460140012, 1.1999785407363386*MathLib.cos(1.3962320168955138), 1.1999785407363386*MathLib.sin(1.3962320168955138),
				1.3962787004479158, 0.3490728321331678, 1.3962320168955138-2.55593407037698,
				PositionAngle.TRUE, FramesFactory.getEME2000(), new AbsoluteDate("2000-01-01T12:00:00.391",
                        TimeScalesFactory.getUTC()), this.mu);
    }
    ...
    public void testJacobianFinitedifferences() throws PatriusException {
        ...
        for (final PositionAngle type : PositionAngle.values()) {
            final double hP = 2.0;
            final double[][] finiteDiffJacobian = finiteDifferencesJacobian(type, orbCir, hP);
            final double[][] jacobian = new double[6][6];
    ...
    private static double[][] finiteDifferencesJacobian(final PositionAngle type,
                                                        final CircularOrbit orbit, final double hP) {
        final double[][] jacobian = new double[6][6];
        for (int i = 0; i < 6; ++i) {
    ...
    private static void fillColumn(final PositionAngle type, final int i, final CircularOrbit orbit,
    ...
    @Test
    public void testHypCircToCar(){
    	final CircularParameters circParam = new CircularParameters(-5000, 1.2,
				1.3, 0.3, 0.4, 0.5, PositionAngle.TRUE, this.mu);
    	final CartesianParameters carParam = circParam.getCartesianParameters();
    	final CircularParameters circParam2 = carParam.getCircularParameters();
    	Assert.assertEquals(circParam.getA(), circParam2.getA(), 1E-10);
    	Assert.assertEquals(circParam.getI(), circParam2.getI(), 1E-15);
    	Assert.assertEquals(circParam.getRightAscensionOfAscendingNode(), circParam2.getRightAscensionOfAscendingNode(), 1E-10);
    	Assert.assertEquals(circParam.getCircularEx(), circParam2.getCircularEx(), 1E-15);
    	Assert.assertEquals(circParam.getCircularEy(), circParam2.getCircularEy(), 1E-15);
    	Assert.assertEquals(circParam.getAlphaV(), circParam2.getAlphaV(), 1E-15);
    }
    
    @Test
    public void testHypCircGetCartesian(){
    	final CircularParameters circParam = new CircularParameters(-5000, 1.2,
				1.3, 0.3, 0.4, 0.5, PositionAngle.TRUE, this.mu);
    	final KeplerianParameters kepParam = circParam.getKeplerianParameters();
    	final CartesianParameters carParamFromKep = kepParam.getCartesianParameters();
    	final PVCoordinates PVFromKep = carParamFromKep.getPVCoordinates();
    	final CartesianParameters carParam = circParam.getCartesianParameters();
    	final PVCoordinates PVFromCirc = carParam.getPVCoordinates();
    	Assert.assertEquals(PVFromKep.getAcceleration().getX(), PVFromCirc.getAcceleration().getX(), 1E-15);
    	Assert.assertEquals(PVFromKep.getAcceleration().getY(), PVFromCirc.getAcceleration().getY(), 1E-15);
    	Assert.assertEquals(PVFromKep.getAcceleration().getZ(), PVFromCirc.getAcceleration().getZ(), 1E-15);
    	Assert.assertEquals(PVFromKep.getVelocity().getX(), PVFromCirc.getVelocity().getX(), 1E-15);
    	Assert.assertEquals(PVFromKep.getVelocity().getY(), PVFromCirc.getVelocity().getY(), 1E-15);
    	Assert.assertEquals(PVFromKep.getVelocity().getZ(), PVFromCirc.getVelocity().getZ(), 1E-15);
    	Assert.assertEquals(PVFromKep.getPosition().getX(), PVFromCirc.getPosition().getX(), 1E-15);
    	Assert.assertEquals(PVFromKep.getPosition().getY(), PVFromCirc.getPosition().getY(), 1E-15);
    	Assert.assertEquals(PVFromKep.getPosition().getZ(), PVFromCirc.getPosition().getZ(), 1E-15);
    }

    
	@Test
	public void testHyperbolicTrueCircToKepToCirc() {
		final CircularParameters param = new CircularParameters(-5000, 1.2,
				1.3, 0.3, 0.4, 0.5, PositionAngle.TRUE, this.mu);
		Assert.assertEquals(-5000, param.getA(), 0);
		Assert.assertEquals(1.2, param.getCircularEx(), 0);
		Assert.assertEquals(1.3, param.getCircularEy(), 0);
		Assert.assertEquals(0.3, param.getI(), 0);
		Assert.assertEquals(0.4, param.getRightAscensionOfAscendingNode(), 0);
		Assert.assertEquals(0.5, param.getAlphaV(), 0);
		final KeplerianParameters paramKep = param.getKeplerianParameters();
		final CircularParameters paramCirc = paramKep.getCircularParameters();
		Assert.assertEquals(paramCirc.getA(), param.getA(), 0);
		Assert.assertEquals(paramCirc.getCircularEx(), param.getCircularEx(), 0);
		Assert.assertEquals(paramCirc.getCircularEy(), param.getCircularEy(), 0);
		Assert.assertEquals(paramCirc.getI(), param.getI(), 0);
		Assert.assertEquals(paramCirc.getRightAscensionOfAscendingNode(),
				param.getRightAscensionOfAscendingNode(), 0);
		Assert.assertEquals(paramCirc.getAlphaV(), param.getAlphaV(), 0);
	}
    
	@Test
	public void testHyperbolicTrueKepToCircToKep() {
		final KeplerianParameters param = new KeplerianParameters(-5000, 1.77,
				0.5, 0.6, 0.7, 0.8, PositionAngle.TRUE, this.mu);
		final CircularParameters circParam = param.getCircularParameters();
		final KeplerianParameters kepParam = circParam.getKeplerianParameters();
		Assert.assertEquals(kepParam.getA(), param.getA(), 0);
		Assert.assertEquals(kepParam.getE(), param.getE(), 0);
		Assert.assertEquals(kepParam.getI(), param.getI(), 0);
		Assert.assertEquals(kepParam.getRightAscensionOfAscendingNode(),
				param.getRightAscensionOfAscendingNode(), 0);
		Assert.assertEquals(kepParam.getAnomaly(PositionAngle.TRUE),
				param.getAnomaly(PositionAngle.TRUE), 1E-15);
		Assert.assertEquals(kepParam.getPerigeeArgument(),
				param.getPerigeeArgument(), 0);
	}
	
	@Test
	public void testHyperbolicEccentricKepToCircToKep() {
		final KeplerianParameters param = new KeplerianParameters(-5000, 1.77,
				0.5, 0.6, 0.7, 0.8, PositionAngle.ECCENTRIC, this.mu);
		final CircularParameters circParam = param.getCircularParameters();
		final KeplerianParameters kepParam = circParam.getKeplerianParameters();
		Assert.assertEquals(kepParam.getA(), param.getA(), 0);
		Assert.assertEquals(kepParam.getE(), param.getE(), 0);
		...
		Assert.assertEquals(kepParam.getRightAscensionOfAscendingNode(),
				param.getRightAscensionOfAscendingNode(), 0);
		Assert.assertEquals(kepParam.getAnomaly(PositionAngle.ECCENTRIC),
				param.getAnomaly(PositionAngle.ECCENTRIC), 1E-15);
		Assert.assertEquals(kepParam.getPerigeeArgument(),
				param.getPerigeeArgument(), 0);
	}
	
	@Test
	public void testHyperbolicMeanKepToCircToKep() {
		final KeplerianParameters param = new KeplerianParameters(-5000, 1.77,
				0.5, 0.6, 0.7, 0.8, PositionAngle.MEAN, this.mu);
		final CircularParameters circParam = param.getCircularParameters();
		final KeplerianParameters kepParam = circParam.getKeplerianParameters();
		Assert.assertEquals(kepParam.getA(), param.getA(), 0);
		Assert.assertEquals(kepParam.getE(), param.getE(), 0);
	...
	@Test
	public void testHypKepCircCarKep() {
		// With True anomaly
		final KeplerianOrbit kepOrb = new KeplerianOrbit(-10000000.0, 2.5, 0.3,
				0.5, 0, 0.9, PositionAngle.TRUE, FramesFactory.getEME2000(),
				AbsoluteDate.J2000_EPOCH, this.mu);
		final CircularParameters circParam = kepOrb.getKeplerianParameters()
				.getCircularParameters();
		final CircularOrbit cirOrb = new CircularOrbit(circParam,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH);
		final CartesianParameters cartParam = cirOrb.getCircularParameters()
				.getCartesianParameters();
		final CartesianOrbit cartOrb = new CartesianOrbit(cartParam,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH);
		final KeplerianParameters kepParam = cartOrb.getCartesianParameters()
				.getKeplerianParameters();
		final KeplerianOrbit kepOrbVerif = new KeplerianOrbit(kepParam,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH);
		Assert.assertEquals(kepOrbVerif.getA(), kepOrb.getA(), 1E-8);
		Assert.assertEquals(kepOrbVerif.getI(), kepOrb.getI(), 1E-15);
		Assert.assertEquals(kepOrbVerif.getE(), kepOrb.getE(), 1E-15);
		Assert.assertEquals(kepOrbVerif.getTrueAnomaly(),
				kepOrb.getTrueAnomaly(), 1E-15);
		Assert.assertEquals(kepOrbVerif.getPerigeeArgument(),
				kepOrb.getPerigeeArgument(), 1E-15);
		Assert.assertEquals(kepOrbVerif.getRightAscensionOfAscendingNode(),
				kepOrb.getRightAscensionOfAscendingNode(), 1E-15);
		// With Eccentric anomaly
		final KeplerianOrbit kepOrbEcc = new KeplerianOrbit(-10000000.0, 2.5,
				0.3, 0, 0, 0.0, PositionAngle.ECCENTRIC,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH, this.mu);
		final CircularParameters circParamEcc = kepOrbEcc
				.getKeplerianParameters().getCircularParameters();
		final CircularOrbit cirOrbEcc = new CircularOrbit(circParamEcc.getA(),
				circParamEcc.getCircularEx(), circParamEcc.getCircularEy(),
				circParamEcc.getI(),
				circParamEcc.getRightAscensionOfAscendingNode(),
				circParamEcc.getAlphaE(), PositionAngle.ECCENTRIC,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH, this.mu);
		final CartesianParameters cartParamEcc = cirOrbEcc
				.getCircularParameters().getCartesianParameters();
		final CartesianOrbit cartOrbEcc = new CartesianOrbit(cartParamEcc,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH);
		final KeplerianParameters kepParamEcc = cartOrbEcc
				.getCartesianParameters().getKeplerianParameters();
		final KeplerianOrbit kepOrbVerifEcc = new KeplerianOrbit(kepParamEcc,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH);
		Assert.assertEquals(kepOrbVerifEcc.getA(), kepOrbEcc.getA(), 1E-8);
		Assert.assertEquals(kepOrbVerifEcc.getI(), kepOrbEcc.getI(), 1E-15);
		Assert.assertEquals(kepOrbVerifEcc.getE(), kepOrbEcc.getE(), 1E-15);
		Assert.assertEquals(kepOrbVerifEcc.getEccentricAnomaly(),
				kepOrbEcc.getEccentricAnomaly(), 1E-15);
		Assert.assertEquals(kepOrbVerifEcc.getPerigeeArgument(),
				kepOrbEcc.getPerigeeArgument(), 1E-15);
		Assert.assertEquals(kepOrbVerifEcc.getRightAscensionOfAscendingNode(),
				kepOrbEcc.getRightAscensionOfAscendingNode(), 1E-15);
		// With Mean anomaly
		final KeplerianOrbit kepOrbMean = new KeplerianOrbit(-10000000.0, 2.5,
				0.3, 0, 0, 0.0, PositionAngle.MEAN, FramesFactory.getEME2000(),
				AbsoluteDate.J2000_EPOCH, this.mu);
		final CircularParameters circParamMean = kepOrbMean
				.getKeplerianParameters().getCircularParameters();
		final CircularOrbit cirOrbMean = new CircularOrbit(
				circParamMean.getA(), circParamMean.getCircularEx(),
				circParamMean.getCircularEy(), circParamMean.getI(),
				circParamMean.getRightAscensionOfAscendingNode(),
				circParamMean.getAlphaE(), PositionAngle.MEAN,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH, this.mu);
		final CartesianParameters cartParamMean = cirOrbMean
				.getCircularParameters().getCartesianParameters();
		final CartesianOrbit cartOrbMean = new CartesianOrbit(cartParamMean,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH);
		final KeplerianParameters kepParamMean = cartOrbMean
				.getCartesianParameters().getKeplerianParameters();
		final KeplerianOrbit kepOrbVerifMean = new KeplerianOrbit(kepParamMean,
				FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH);
		Assert.assertEquals(kepOrbVerifMean.getA(), kepOrbMean.getA(), 1E-8);
		Assert.assertEquals(kepOrbVerifMean.getI(), kepOrbMean.getI(), 1E-15);
		Assert.assertEquals(kepOrbVerifMean.getE(), kepOrbMean.getE(), 1E-15);
		Assert.assertEquals(kepOrbVerifMean.getMeanAnomaly(),
				kepOrbMean.getMeanAnomaly(), 1E-15);
		Assert.assertEquals(kepOrbVerifMean.getPerigeeArgument(),
				kepOrbMean.getPerigeeArgument(), 1E-15);
		Assert.assertEquals(kepOrbVerifMean.getRightAscensionOfAscendingNode(),
				kepOrbMean.getRightAscensionOfAscendingNode(), 1E-15);
	}

	@Test
	public void testHypGetAlpha() {
		final CircularParameters paramCircTrue = new CircularParameters(-5000,
				1.2, 1.3, 0.3, 0.4, 0.5, PositionAngle.TRUE, this.mu);
		final double alphaV = paramCircTrue.getAlphaV();
		final double alphaE = paramCircTrue.getAlphaE();
		final double alphaM = paramCircTrue.getAlphaM();
		final KeplerianParameters paramKep = paramCircTrue
				.getKeplerianParameters();
		final double trueAnomaly = paramKep.getTrueAnomaly();
		final double eccentricAnomaly = paramKep.getEccentricAnomaly();
		final double meanAnomaly = paramKep.getMeanAnomaly();
		final double pa = paramKep.getPerigeeArgument();
		Assert.assertEquals(alphaV, trueAnomaly + pa, 0);
		Assert.assertEquals(alphaE, eccentricAnomaly + pa, 0);
		Assert.assertEquals(alphaM, meanAnomaly + pa, 0);
	}

	@Test
	public void testHypAlphaTranform() {
		// test eccentric to true
		final CircularParameters paramCircEcc = new CircularParameters(-5000,
				1.2, 1.3, 0.3, 0.4, 0.5, PositionAngle.ECCENTRIC, this.mu);
		final double alphaEccE = paramCircEcc.getAlphaE();
		final double alphaEccV = paramCircEcc.getAlphaV();
		final CircularParameters paramCircTrue = new CircularParameters(-5000,
				1.2, 1.3, 0.3, 0.4, alphaEccV, PositionAngle.TRUE, this.mu);
		final double alphaEccE2 = paramCircTrue.getAlphaE();
		Assert.assertEquals(alphaEccE, alphaEccE2, 0);
		// test mean to true
		final CircularParameters paramCircMean = new CircularParameters(-5000,
				1.2, 1.3, 0.3, 0.4, 0.5, PositionAngle.MEAN, this.mu);
		final double alphaCircV = paramCircMean.getAlphaV();
		final double alphaCircM = paramCircMean.getAlphaM();
		final CircularParameters paramCircTrue2 = new CircularParameters(-5000,
				1.2, 1.3, 0.3, 0.4, alphaCircV, PositionAngle.TRUE, this.mu);
		final double alphaCircM2 = paramCircTrue2.getAlphaM();
		Assert.assertEquals(alphaCircM, alphaCircM2, 0);
	}
