...
public class ReentryParametersTest {
     ...
     */
    @Test
    public void testReentryParametersCartesianParameters() {
        ...
        checkCartesianParameters(new double[] { 7961E3, 8034E3, 7957E3, 6.04E3, -6.03E3, -5.03E3 },
        ...
        checkRentryParameters(new double[] { 8599616.15015757, 0.839975092194095, 0.789962057936808,
            9906.73508276062,
        ...
        checkRentryParameters(new double[] { 8599616.15015757, 0.839975092194095, 0.789962057936808,
            9906.73508276062, -0.390035484104706, 4.34000921645338 }, reentryParameters2, eps2, true);
     ...
     */
    @Test
    public void testConversionCases() {
        ...
        checkRentryParameters(new double[] { 1E3, 0, 0, 10, FastMath.PI / 2., 0 }, reentryParams1, eps);
        final CartesianParameters reverseCartParams1 = reentryParams1.getCartesianParameters();
        checkCartesianParameters(new double[] { ae + 1E3, 0, 0, 10, 0, 0 }, reverseCartParams1, eps);
        ...
        checkRentryParameters(new double[] { 1E3, 0, 0, 10, -FastMath.PI / 2., 0 }, reentryParams2, eps);
        final CartesianParameters reverseCartParams2 = reentryParams2.getCartesianParameters();
        checkCartesianParameters(new double[] { ae + 1E3, 0, 0, -10, 0, 0 }, reverseCartParams2, eps);
        ...
        checkRentryParameters(new double[] { 1E3, FastMath.PI / 2., 0, 10, FastMath.PI / 2., 0 }, reentryParams3,
            eps);
        final CartesianParameters reverseCartParams3 = reentryParams3.getCartesianParameters();
        checkCartesianParameters(new double[] { 0, 0, ae * (1. - f) + 1E3, 0, 0, 10 }, reverseCartParams3, 1E-9);
        ...
        checkRentryParameters(new double[] { 1E3, FastMath.PI / 2., 0, 10, -FastMath.PI / 2., 0 }, reentryParams4,
            eps);
        final CartesianParameters reverseCartParams4 = reentryParams4.getCartesianParameters();
        checkCartesianParameters(new double[] { 0, 0, ae * (1. - f) + 1E3, 0, 0, -10 }, reverseCartParams4, 1E-9);
    }
     ...
     */
    @Test
    public void testAllConversionTypes() {
        ...
        // Check conversions
        checkRentryParameters(new double[] { 1000000., 0.84, 0.79, 7100, 0.01, 4.34 }, reentryParameters
        ...
        checkRentryParameters(new double[] { 1000000., 0.84, 0.79, 7100, 0.01, 4.34 },
     ...
     */
    private static void checkRentryParameters(final double[] expected, final ReentryParameters reentryParams,
                                       final double eps, final boolean writeToReport) {
        checkRelDiff(expected[0], reentryParams.getAltitude(), eps, writeToReport, "Altitude");
        checkRelDiff(expected[1], reentryParams.getLatitude(), eps, writeToReport, "Latitude");
        checkRelDiff(expected[2], reentryParams.getLongitude(), eps, writeToReport, "Longitude");
        checkRelDiff(expected[3], reentryParams.getVelocity(), eps, writeToReport, "Velocity");
        checkRelDiff(expected[4], reentryParams.getSlope(), eps, writeToReport, "Slope");
        checkRelDiff(expected[5], reentryParams.getAzimuth(), eps, writeToReport, "Azimuth");
    }
     ...
     *        true if write to report
     */
    private static void checkCartesianParameters(final double[] expected, final CartesianParameters cartParams,
                                          final double eps, final boolean writeToReport) {
        checkRelDiff(expected[0], cartParams.getPosition().getX(), eps, writeToReport, "X");
        checkRelDiff(expected[1], cartParams.getPosition().getY(), eps, writeToReport, "Y");
        checkRelDiff(expected[2], cartParams.getPosition().getZ(), eps, writeToReport, "Z");
        checkRelDiff(expected[3], cartParams.getVelocity().getX(), eps, writeToReport, "VX");
        checkRelDiff(expected[4], cartParams.getVelocity().getY(), eps, writeToReport, "VY");
        checkRelDiff(expected[5], cartParams.getVelocity().getZ(), eps, writeToReport, "VZ");
    }
    ...
    private static void
        ...
        checkRentryParameters(expected, reentryParams, eps, false);
    }
    ...
    private static void checkCartesianParameters(final double[] expected, final CartesianParameters cartParams,
                                          final double eps) {
        checkCartesianParameters(expected, cartParams, eps, false);
    }
     ...
     *        report tag
     */
    private static void checkRelDiff(final double expected, final double actual, final double eps,
