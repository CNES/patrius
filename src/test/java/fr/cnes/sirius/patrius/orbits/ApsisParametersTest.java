...
public class ApsisParametersTest {
    ...
    public void testApsisToKeplerian() {
        ...
        final ApsisOrbit paramCir = new ApsisOrbit(new PVCoordinates(posCir, vitCir),
                FramesFactory.getEME2000(), this.date, this.mu);
        checkDouble(param.getPeriapsis(), apsis.getPeriapsis(),
                Utils.epsilonTest * apsis.getPeriapsis(), "Perigee");
        checkDouble(param.getApoapsis(), apsis.getApoapsis(),
                Utils.epsilonTest * apsis.getApoapsis(), "Apogee");
        checkDouble(paramCir.getA(), apsisCir.getA(), Utils.epsilonTest * apsisCir.getA(), "a");
        checkDouble(paramCir.getE(), apsisCir.getE(),
                Utils.epsilonE * MathLib.max(1., MathLib.abs(apsisCir.getE())), "e");
        checkDouble(MathUtils.normalizeAngle(paramCir.getI(), apsisCir.getI()),
                apsisCir.getI(), Utils.epsilonAngle * MathLib.abs(apsisCir.getI()), "i");
        checkDouble(MathUtils.normalizeAngle(paramCir.getLM(), apsisCir.getLM()),
                apsisCir.getLM(), Utils.epsilonAngle * MathLib.abs(apsisCir.getLM()), "LM");
        checkDouble(MathUtils.normalizeAngle(paramCir.getLE(), apsisCir.getLE()),
                apsisCir.getLE(), Utils.epsilonAngle * MathLib.abs(apsisCir.getLE()), "LE");
        checkDouble(MathUtils.normalizeAngle(paramCir.getLv(), apsisCir.getLv()),
    ...
    public void testJacobianFinitedifferencesEllipse() throws PatriusException {
        ...
        for (final PositionAngle type : PositionAngle.values()) {
            final double hP = 2.0;
            final double[][] finiteDiffJacobian = finiteDifferencesJacobian(type, orbAps, hP);
    ...
    private static void fillColumn(final PositionAngle type, final int i, final ApsisOrbit orbit,
