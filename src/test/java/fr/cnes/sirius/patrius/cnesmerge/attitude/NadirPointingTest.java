...
public class NadirPointingTest {
    ...
    public class NadirPointingClassTest extends NadirPointing {
         ...
         *        the shape
         */
        public NadirPointingClassTest(final BodyShape bodyShape) {
            super(bodyShape);
        }
        ...
        public void test() throws PatriusException {
            ...
            final PVCoordinates pvProvM2h =
                circ.getPVCoordinates(NadirPointingTest.this.date.shiftedBy(-2 * h),
                    NadirPointingTest.this.frameITRF2005);
            final PVCoordinates pvProvM1h =
                circ.getPVCoordinates(NadirPointingTest.this.date.shiftedBy(-h), NadirPointingTest.this.frameITRF2005);
            final PVCoordinates pvProvh =
                circ.getPVCoordinates(NadirPointingTest.this.date, NadirPointingTest.this.frameITRF2005);
            final PVCoordinates pvProvP1h =
                circ.getPVCoordinates(NadirPointingTest.this.date.shiftedBy(+h), NadirPointingTest.this.frameITRF2005);
            final PVCoordinates pvProvP2h =
                circ.getPVCoordinates(NadirPointingTest.this.date.shiftedBy(+2 * h),
                    NadirPointingTest.this.frameITRF2005);
            sample.add(NadirPointingTest.this.nadirRef(
                new TimeStampedPVCoordinates(NadirPointingTest.this.date.shiftedBy(-2 * h), pvProvM2h),
                refToBody.shiftedBy(-2 * h), this.getBodyShape(), this.getBodyFrame()));
            sample.add(NadirPointingTest.this.nadirRef(
                new TimeStampedPVCoordinates(NadirPointingTest.this.date.shiftedBy(-h), pvProvM1h),
                refToBody.shiftedBy(-h),
                this.getBodyShape(), this.getBodyFrame()));
            sample.add(NadirPointingTest.this.nadirRef(new TimeStampedPVCoordinates(NadirPointingTest.this.date,
                pvProvh), refToBody, this.getBodyShape(),
                this.getBodyFrame()));
            sample.add(NadirPointingTest.this.nadirRef(
    ...
    private TimeStampedPVCoordinates nadirRef(final TimeStampedPVCoordinates scRef, final Transform refToBody,
                                              final BodyShape bodyShape, final Frame frame)
        ...
        final GeodeticPoint gpSat = bodyShape.transform(satInBodyFrame, frame, scRef.getDate());
        ...
        final Vector3D pNadirBody = bodyShape.transform(gpNadir);
     ...
     */
    @Test
    public final void testGetTargetPV() throws PatriusException {
     ...
     */
    @Test
    public final void testCollision() throws PatriusException {
        final OneAxisEllipsoid earthShape = new OneAxisEllipsoid(6378136, 0.0, FramesFactory.getEME2000());
        final NadirPointingClassTest pointing = new NadirPointingClassTest(earthShape);
