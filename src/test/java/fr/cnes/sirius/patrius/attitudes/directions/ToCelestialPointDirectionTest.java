 ...
 * @since 1.1
 */
public class ToCelestialPointDirectionTest {
    ...
    public static void setUpBeforeClass() {
        Report.printClassHeader(ToCelestialPointDirectionTest.class.getSimpleName(),
            "To celestial point direction");
    }
     ...
     * 
     * @testedMethod {@link ToCelestialPointDirection#getVector(PVCoordinatesProvider, AbsoluteDate, Frame)}
     * 
    ...
    public void testGetVector() {
        ...
        try {
            ...
            final Frame earthFrame = earth.getICRF();
            ...
            // direction creation
            final ToCelestialPointDirection direction = new ToCelestialPointDirection(earth);
     ...
     * @testedFeature {@link features#TO_CELECTIAL_BODY_CENTER_DIRECTION}
     * 
     * @testedMethod {@link ToCelestialPointDirection#getTargetPVCoordinates(AbsoluteDate, Frame)}
     * 
     ...
     * @testedFeature {@link features#TO_CELECTIAL_BODY_CENTER_DIRECTION}
     * 
     * @testedMethod {@link ToCelestialPointDirection#getLine(PVCoordinatesProvider, AbsoluteDate, Frame)}
     * 
    ...
    public void testGetLine() {
        ...
        try {
            ...
            // direction creation
            ToCelestialPointDirection direction = new ToCelestialPointDirection(earth);
            ...
            // direction creation
            direction = new ToCelestialPointDirection(earth);
     ...
     * @testedFeature {@link features#TO_CELECTIAL_BODY_CENTER_DIRECTION}
     * 
     * @testedMethod {@link ToCelestialPointDirection#getVector(PVCoordinatesProvider, SignalDirection, AbsoluteDate, FixedDate, Frame, double)}
     * 
     * @description Instantiation of a direction described by a celestial body set as the target point (the earth is the
     *              celestial body). The origin point is described by a {@link LinearTwoPointsPVProvider}. The output
     *              result is the direction vector connecting both points taking into account the delay of signal
     *              propagation.
     * 
    ...
    public void testGetVectorDelay() throws PatriusException{
        ...
        // position of origin point at emission date
        pvOrigin = origin.getPVCoordinates(t0, gcrf);
        // position of target at reception date in gcrf
        ...
        // position of target at reception date in gcrf
        pvTarget = toEarth.getTargetPVCoordinates(fixedDate.shiftedBy(trueDelay), gcrf);
        expected = pvTarget.getPosition().subtract(pvOrigin.getPosition());
        checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
        ...
        trueDelay = 0.03335640951981521;
        // target position at fixed date
        pvTarget = toEarth.getTargetPVCoordinates(fixedDate, gcrf);
        // origin position at emission date
        ...
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection,
            origin.getPVCoordinates(t0, gcrf).getVelocity(), SignalDirection.TOWARD_TARGET);
        checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
        // inertial velocity of source in ICRF frame
        ...
        pvOrigin = origin.getPVCoordinates(t0, gcrf);
        expectedPreStellarCorrection = pvTarget.getPosition().subtract(pvOrigin.getPosition());
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection.negate(),
            target.getPVCoordinates(t0, gcrf).getVelocity(), SignalDirection.FROM_TARGET).negate();
        checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
        // inertial velocity of source in ICRF frame
        ...
        expectedPreStellarCorrection = pvTarget.getPosition().subtract(pvOrigin.getPosition());
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection,
            target.getPVCoordinates(t0, gcrf).getVelocity(), SignalDirection.FROM_TARGET);
        checkVector3DEquality(expected, actual, 1E-13, 1E-13);
        // inertial velocity of source in ICRF frame
        sourceVelocityInIcrf = target.getPVCoordinates(t0, icrf).getVelocity();
        // source velocity projected in GCRF frame
        sourceVelocityProjInFrame = icrf.getTransformTo(gcrf, t0).transformVector(sourceVelocityInIcrf);
        ...
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection.negate(),
            origin.getPVCoordinates(t0, gcrf).getVelocity(), SignalDirection.FROM_TARGET);
        checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
        // inertial velocity of source in ICRF frame
        sourceVelocityInIcrf = origin.getPVCoordinates(t0, icrf).getVelocity();
        // source velocity projected in GCRF frame
        ...
        expectedPreStellarCorrection = pvTarget.getPosition().subtract(pvOrigin.getPosition());
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection,
            origin.getPVCoordinates(fixedDate, gcrf).getVelocity(), SignalDirection.TOWARD_TARGET);
        checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
        // inertial velocity of source in ICRF frame
        sourceVelocityInIcrf = origin.getPVCoordinates(t0, icrf).getVelocity();
        // source velocity projected in GCRF frame
        ...
        expectedPreStellarCorrection = pvTarget.getPosition().subtract(pvOrigin.getPosition());
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection.negate(),
            target.getPVCoordinates(fixedDate, gcrf).getVelocity(), SignalDirection.TOWARD_TARGET).negate();
        checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
        // inertial velocity of source in ICRF frame
        sourceVelocityInIcrf = target.getPVCoordinates(t0, icrf).getVelocity();
        // source velocity projected in GCRF frame
        ...
        expectedPreStellarCorrection = pvTarget.getPosition().subtract(pvOrigin.getPosition());
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection,
            target.getPVCoordinates(fixedDate, gcrf).getVelocity(), SignalDirection.FROM_TARGET);
        ...
        expectedPreStellarCorrection = pvTarget.getPosition().subtract(pvOrigin.getPosition()).negate();
        expected = LightAberrationTransformation.applyTo(expectedPreStellarCorrection.negate(),
            origin.getPVCoordinates(fixedDate, gcrf).getVelocity(), SignalDirection.FROM_TARGET);
        checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
        // inertial velocity of source in ICRF frame
        sourceVelocityInIcrf = origin.getPVCoordinates(t0, icrf).getVelocity();
        // source velocity projected in GCRF frame
     ...
     * @testedMethod {@link ToCelestialPointDirection#getVector(PVCoordinatesProvider, AbsoluteDate, Frame)}
     * 
     * @description check IAUPole is not needed for ToCelestialBodyCenterDirection.
     * 
     ...
     */
    @Test
    public void testNoIAUPole() throws PatriusException {
        final CelestialBody sun = new UserCelestialBody("Sun", new MeeusSun(), 0, null, FramesFactory.getICRF(), null);
        final ToCelestialPointDirection direction = new ToCelestialPointDirection(sun);
        direction.getVector(new ConstantPVCoordinatesProvider(Vector3D.ZERO, FramesFactory.getGCRF()), AbsoluteDate.J2000_EPOCH, FramesFactory.getGCRF());
    }
     ...
     * 
     * @testedMethod {@link ToCelestialPointDirection#getLine(PVCoordinatesProvider, SignalDirection, AbsoluteDate, FixedDate, Frame, double)}
     * 
     * @description Instantiation of a direction described
     *              a central body (the central body's center is the target point),
     *              and getting the line joining both points taking into account the delay of the signal propagation.
    ...
    public void testGetLineDelayKeplerianMotion() throws PatriusException{
        ...
        // earth direction
        final ToCelestialPointDirection earthDirection = new ToCelestialPointDirection(earth);

        // -------- the origin point with a keplerian motion (circular)
        ...
        // low precision since the expected signal propagation duration is used
        Assert.assertTrue(actual.distance(pvOrigin.getPosition()) < 3E-9);
        Assert.assertTrue(actual.distance(pvTarget.getPosition()) < 1E-1);
        ...
        Assert.assertTrue(actual.contains(Vector3D.ZERO));
        // low precision since the expected signal propagation duration is used
        Assert.assertTrue(actual.distance(pvOrigin.getPosition()) < 1E-2);
