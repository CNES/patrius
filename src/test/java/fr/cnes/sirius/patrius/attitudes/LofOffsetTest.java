...
public class LofOffsetTest {
    ...
    public void testOffset() throws PatriusException {
        ...
        // Attitude law definition from ellipsoid point target
    ...
    private static void checkSatVector(final Orbit o, final Attitude a, final Vector3D satVector,
        ...
        Assert.assertEquals(expectedX, Vector3D.dotProduct(v, xLof), threshold);
        Assert.assertEquals(expectedY, Vector3D.dotProduct(v, yLof), threshold);
        Assert.assertEquals(expectedZ, Vector3D.dotProduct(v, zLof), threshold);
    }
