...
public class GenericTargetDirectionTest {
    ...
    public void testGetVectorDelay() throws PatriusException{
        ...
        final double a = 1E7;
        final KeplerianOrbit origin = new KeplerianOrbit(a, 0, 0, 0, 0, 0, PositionAngle.TRUE, gcrf, t0, Constants.EGM96_EARTH_MU);
        ...
        actual = toTarget.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
        ...
        // direction vector computed in GCRF 
        // from origin to target, fixed date is signal emission
        actual = toTarget.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.RECEPTION, gcrf, eps);
        ...
        actual = toTarget.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.EMISSION, lof, eps);
        ...
        actual = toTarget.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.RECEPTION, lof, eps);
     ...
     * @description Instantiation of a direction described by a target stationnary point in an arbitrary frame defined
     *              by a {@link BasicPVCoordinatesProvider}. The origin object is described by a {@link KeplerianOrbit}.
     *              The output result is the {@link Line} joining both objects positions taking into account the delay of signal propagation.
     * 
     * @input the origin is a spacecraft created as {@link KeplerianOrbit}
     * 
     * @output Vector3D
    ...
    public void testGetLineDelay() throws PatriusException{
        ...
        final Vector3D posInGcrf = new Vector3D(radius * latSinCos[1] * longSinCos[1], radius * latSinCos[1] * longSinCos[0], radius * latSinCos[0]);
        // local frame
        ...
        final BasicPVCoordinatesProvider target = new BasicPVCoordinatesProvider(new PVCoordinates(posInGcrf, Vector3D.ZERO), gcrf);
        
        // the target direction
        final GenericTargetDirection toTarget = new GenericTargetDirection(target);
              
        // the origin object
        final KeplerianOrbit origin = new KeplerianOrbit(radius + 850E3, 0.0, 0, 0., 0., 0., PositionAngle.MEAN, gcrf, t0, mu);
        // local orbital frame
        final Frame lof = new LocalOrbitalFrame(gcrf, LOFType.QSW, origin, "orbital frame");
        final Frame frozenLof = lof.getFrozenFrame(icrf, fixedDate, " ");
        final Transform frozenToLof = frozenLof.getTransformTo(lof, fixedDate);
        ...
        actual = toTarget.getLine(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.EMISSION, stationFrame, eps);
        ...
        checkVector3DEquality(direction.normalize(), actual.getDirection(), 1E-6, 1E-6);
        ...
        actual = toTarget.getLine(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.RECEPTION, lof, eps);
        ...
        Assert.assertTrue(actual.distance(frozenToLof.transformPosition(pvTarget.getPosition())) < 1E-8);
        Assert.assertTrue(actual.distance(frozenToLof.transformPosition(pvOrigin.getPosition())) < 1E-9);
        
        // from target to origin, fixed date is signal emission 
        // computed in LOCAL ORBITAL FRAME
        actual = toTarget.getLine(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.RECEPTION, lof, eps);
        ...
        direction = frozenToLof.transformVector(pvTarget.getPosition().subtract(pvOrigin.getPosition())).normalize();
        
        checkVector3DEquality(direction, actual.getDirection(), this.comparisonEpsilon, this.comparisonEpsilon);
        Assert.assertTrue(actual.distance(pvOrigin.getPosition()) < 1E-9);
        Assert.assertTrue(actual.distance(frozenToLof.transformPosition(pvTarget.getPosition())) < 1E-8);
        
        // from target to origin, fixed date is signal reception
        // computed in GCRF
        actual = toTarget.getLine(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
        ...
        checkVector3DEquality(direction.normalize(), actual.getDirection(), this.comparisonEpsilon, this.comparisonEpsilon);
        Assert.assertTrue(actual.distance(frozenToGcrf.transformPosition(pvTarget.getPosition())) < 1E-9);
        Assert.assertTrue(actual.distance(frozenToGcrf.transformPosition(pvOrigin.getPosition())) < 1E-9);
    }
