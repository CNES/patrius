...
public class NadirPointingTest {
    ...
    public void testDeltaT() throws PatriusException {
        ...
        final NadirPointing law1 = new NadirPointing(new OneAxisEllipsoid(3678000, 0.001, FramesFactory.getGCRF()), Vector3D.PLUS_I, Vector3D.PLUS_J);
        final NadirPointing law2 = new NadirPointing(new OneAxisEllipsoid(3678000, 0.001, FramesFactory.getGCRF()), Vector3D.PLUS_I, Vector3D.PLUS_J, 0.5);
    ...
    public void testVertical() throws PatriusException {
        ...
        final Vector3D pTargetItrf = nadirAttitudeLaw.getTargetPoint(circ, this.date, this.frameITRF2005);

        // Convert to geodetic coordinates
        final GeodeticPoint geoTarget = earthShape.transform(pTargetItrf, this.frameITRF2005, this.date);

        // Compute local vertical axis
        final double xVert = MathLib.cos(geoTarget.getLongitude())
            * MathLib.cos(geoTarget.getLatitude());
        final double yVert = MathLib.sin(geoTarget.getLongitude())
            * MathLib.cos(geoTarget.getLatitude());
        final double zVert = MathLib.sin(geoTarget.getLatitude());
     ...
     */
    @Test
    public final void testFT2104() throws IllegalArgumentException, PatriusException {
