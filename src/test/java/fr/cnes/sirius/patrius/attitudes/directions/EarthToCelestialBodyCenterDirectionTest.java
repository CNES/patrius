...
public class EarthToCelestialBodyCenterDirectionTest {
     ...
     * @description Instantiation of a direction described
     *              by the direction Earth center => celestial body center (the central body's center is the target
     *              point),and getting the relative position vector between both objects taking into
     *              account the delay of the signal propagation.
     * 
     * @output Vector3D
     * 
     * @testPassCriteria the computed vector is oriented from the 
     *                   
    ...
    public void testGetVectorWithDelay() throws PatriusException{      
        ...
        final EarthToCelestialBodyCenterDirection earthToSun = new EarthToCelestialBodyCenterDirection(this.sun);
        ...
        final Frame bodyFrame = this.sun.getTrueEquatorFrame();
        final Frame icrf = FramesFactory.getICRF();
        final Frame frozen = bodyFrame.getFrozenFrame(icrf, t0, " ");
        
        // frame transform
        final Transform frozenToGcrf = frozen.getTransformTo(bodyFrame, t0);
        
        // signal direction from target to origin
        // fixed date is signal emission
        actual = earthToSun.getVector(null, SignalDirection.FROM_TARGET, t0, FixedDate.EMISSION, bodyFrame, eps);
        ...
        direction = frozenToGcrf.transformVector(direction);
        Assert.assertEquals(direction, actual);
        
        // signal direction from target to origin
        // fixed date is signal reception
        actual = earthToSun.getVector(null, SignalDirection.FROM_TARGET, t0, FixedDate.RECEPTION, bodyFrame, eps);
        ...
        // signal direction from origin to target
        // fixed date is signal emission
        actual = earthToSun.getVector(null, SignalDirection.TOWARD_TARGET, t0, FixedDate.EMISSION, bodyFrame, eps);
        ...
        // signal direction from origin to target
        // fixed date is signal reception
        actual = earthToSun.getVector(null, SignalDirection.TOWARD_TARGET, t0, FixedDate.RECEPTION, bodyFrame, eps);
     ...
     * @output Vector3D
     * 
     * @testPassCriteria the direction of the computed line must be equal to the normalized direction returned by getVector. 
     */
    @Test
    public void testGetLineWithDelay() throws PatriusException{
        ...
        for(final SignalDirection signalDirection : SignalDirection.values()){    
            for(final FixedDate fixedDateType : FixedDate.values()){
                ...
                final Line actual = earthToSun.getLine(null, signalDirection, t0, fixedDateType, mod, this.comparisonEpsilon);
                final Vector3D direction = earthToSun.getVector(null, signalDirection, t0, fixedDateType, mod, this.comparisonEpsilon).normalize();
