...
public class EarthCenterDirectionTest {
     ...
     * @testedFeature {@link features#CENTRAL_BODY_CENTER_DIRECTION}
     * 
     * @testedMethod {@link EarthCenterDirection#getVector(PVCoordinatesProvider, SignalDirection, AbsoluteDate, FixedDate, Frame, double)}
     * 
     * @description Instantiation of a direction described
     *              a central body (the central body's center is the target point),
    ...
    public void testGetVectorDelayKeplerianMotion() throws PatriusException {
		...
		final Frame frozenGcrf = gcrf.getFrozenFrame(icrf, fixedDate, "inertialFrozen");
		...
		final Transform frozenToLof = frozenGcrf.getTransformTo(lof, fixedDate);
		...
		// from origin to target, fixed date is signal emission
		actual = earthDirection.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
		...
		pvTarget = earthDirection.getTargetPVCoordinates(fixedDate.shiftedBy(trueDelay), frozenGcrf);
		...
		// direction vector computed in GCRF
		// from origin to target, fixed date is signal reception
		actual = earthDirection.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.RECEPTION, gcrf, eps);
		// true signal propagation duration
		trueDelay = 0.033359727332068156;
		// target position at fixed date
		pvTarget = earthDirection.getTargetPVCoordinates(fixedDate, frozenGcrf);
		...
		// from target to origin, fixed date is signal emission
		actual = earthDirection.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
		// true signal propagation duration
		...
		// from target to origin, fixed date is signal reception
		actual = earthDirection.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.RECEPTION, gcrf, eps);
		// true signal propagation duration
		...
		// from origin to target, fixed date is emission
		actual = earthDirection.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.EMISSION, lof, eps);
		// true signal propagation duration
        trueDelay = 0.03335972734336279;
		pvOrigin = origin.getPVCoordinates(fixedDate, frozenGcrf);
		pvTarget = earthDirection.getTargetPVCoordinates(fixedDate.shiftedBy(trueDelay), frozenGcrf);
		expected = frozenToLof.transformVector(pvTarget.getPosition().subtract(pvOrigin.getPosition()));
		checkVector3DEquality(expected, actual, this.comparisonEpsilon, this.comparisonEpsilon);
		
		// direction vector computed in QSW
		// from origin to target, fixed date is emission
		actual = earthDirection.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.RECEPTION, lof, eps);
        // true signal propagation duration
        trueDelay = 0.033359727332068156;
		pvOrigin = origin.getPVCoordinates(fixedDate.shiftedBy(-trueDelay), frozenGcrf);
		pvTarget = earthDirection.getTargetPVCoordinates(fixedDate, frozenGcrf);
		...
		// direction vector computed in QSW
		// from target to origin, fixed date is emission
		actual = earthDirection.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.EMISSION, lof, eps);
	     // true signal propagation duration
        trueDelay = 0.0333530923552573;
		pvOrigin = origin.getPVCoordinates(fixedDate.shiftedBy(trueDelay), frozenGcrf);
		pvTarget = earthDirection.getTargetPVCoordinates(fixedDate, frozenGcrf);
		expected = frozenToLof.transformVector(pvTarget.getPosition().subtract(pvOrigin.getPosition()));
		checkVector3DEquality(expected, actual, 1E-4, 1E-5);
		
		// direction vector computed in QSW
		// from target to origin, fixed date is reception
		actual = earthDirection.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.RECEPTION, lof, eps);
		// true signal propagation duration
        trueDelay = 0.033353092366599335;
		pvOrigin = origin.getPVCoordinates(fixedDate, frozenGcrf);
		pvTarget = earthDirection.getTargetPVCoordinates(fixedDate.shiftedBy(-trueDelay), frozenGcrf);
     ...
     * @testedFeature {@link features#CENTRAL_BODY_CENTER_DIRECTION}
     * 
     * @testedMethod {@link EarthCenterDirection#getLine(PVCoordinatesProvider, SignalDirection, AbsoluteDate, FixedDate, Frame, double)}
     * 
    ...
    public void testGetLineDelayKeplerianMotion() throws PatriusException {
		...
		// direction vector computed in GCRF 
		// from origin to target, fixed date is signal emission
		actual = earthDirection.getLine(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
		direction = earthDirection.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
	    ...
	    Assert.assertTrue(actual.distance(pvOrigin.getPosition()) < 1E-9);
		...
		// from origin to target, fixed date is signal reception
		actual = earthDirection.getLine(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.RECEPTION, gcrf, eps);
		direction = earthDirection.getVector(origin, SignalDirection.TOWARD_TARGET, fixedDate, FixedDate.RECEPTION, gcrf, eps);
        // position/velocity of objects
        pvOrigin = origin.getPVCoordinates(fixedDate.shiftedBy(-dt), frozenGcrf);
		...
		// from origin to target, fixed date is signal emission
		actual = earthDirection.getLine(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
		direction = earthDirection.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.EMISSION, gcrf, eps);
		pvOrigin = origin.getPVCoordinates(fixedDate.shiftedBy(+dt), frozenGcrf);
		checkVector3DEquality(direction.normalize().negate(), actual.getDirection(), this.comparisonEpsilon, this.comparisonEpsilon);
		// low precision since the expected signal propagation duration is used
		Assert.assertTrue(actual.distance(frozenToGcrf.transformPosition(pvOrigin.getPosition())) < 1E-2);
		...
		// from target to origin, fixed date is signal reception
		actual = earthDirection.getLine(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.RECEPTION, gcrf, eps);
		direction = earthDirection.getVector(origin, SignalDirection.FROM_TARGET, fixedDate, FixedDate.RECEPTION, gcrf, eps);
		// position/velocity of objects
        pvOrigin = origin.getPVCoordinates(fixedDate, frozenGcrf);
        pvTarget = earthDirection.getTargetPVCoordinates(fixedDate.shiftedBy(-dt), frozenGcrf);
		checkVector3DEquality(direction.normalize(), actual.getDirection(), this.comparisonEpsilon, this.comparisonEpsilon);
