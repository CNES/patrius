...
/**
 * @description <p>
 *              This class aims at testing the TwoDirectionsAttitude.
 *              </p>
 * 
 * @author Julie Anton
 * 
 * @version $Id: TwoDirectionsLawTest.java 17910 2017-09-11 11:58:16Z bignon $
 * 
 * @since 1.2
 */
public class TwoDirectionsLawTest {
    ...
    public static void setUpBeforeClass() {
        Report.printClassHeader(TwoDirectionsLawTest.class.getSimpleName(), "Two directions attitude provider");
    }
     ...
     * 
     * @description compare the attitudes given by TwoDirectionsAttitude with those given by YawSteering. The two
     *              directions given at the construction of TwoDirectionsAttitude are the sun-satellite direction and
     *              the earth-satellite direction. The earth-satellite direction should be aligned with the satelltie K
     *              axis and the sun-satellite direction should be aligned at best with the satellite I axis.
     * 
     * @input an elliptic orbit
     * 
     * @output the attitudes are computed with both TwoDirectionsAttitude and YawSteering laws on 100 points equally
     ...
     *                   TwoDirectionsAttitude law and the other one obtained with the YawSteering law) is below 1e-14
     *                   rad. (NB : the spins are different between YawSteering and TwoDirectionsAttitude, they are
    ...
    public final void testYawSteering() throws PatriusException {
        ...
        final BodyShape earthSphere = new OneAxisEllipsoid(6378136.460, 0., earth.getTrueRotatingFrame());
        ...
        final AttitudeProvider siriusYawSteering = new TwoDirectionsAttitude(d1, d2, Vector3D.PLUS_K, Vector3D.PLUS_I);
     ...
     * 
     * @description compare the attitudes given by TwoDirectionsAttitude with those given by YawCompensation. The two
     *              directions given at the construction of TwoDirectionsAttitude are the ground velocity direction and
     *              the nadir direction. The nadir direction should be aligned with the satellite K axis and the ground
     *              velocity direction should be aligned at best with the satellite I axis.
     * 
     * @input a quasi circular orbit
     * 
     * @output the attitudes are computed with both TwoDirectionsAttitude and YawCompensation laws on 100 points equally
     *         distributed over 1 orbit period
     * 
     * @testPassCriteria the angle of the rotation between the two attitudes (one obtained with the
     *                   TwoDirectionsAttitude law and the other one obtained with the YawCompensation law) is below
     *                   1e-12 rad. (NB : the spins are different between YawCompensation and TwoDirectionsAttitude,
    ...
    public final void testYawCompensation() throws PatriusException {
        ...
        final OneAxisEllipsoid body = new OneAxisEllipsoid(Constants.GRIM5C1_EARTH_EQUATORIAL_RADIUS,
            Constants.GRIM5C1_EARTH_FLATTENING, earth.getTrueRotatingFrame());
        ...
        final AttitudeProvider siriusYawCompensation = new TwoDirectionsAttitude(d1, d2, Vector3D.PLUS_K,
            Vector3D.PLUS_I);
        ...
        while (currentDate.durationFrom(date) < period) {
            ...
            Assert.assertEquals(0, Rotation.distance(rotOrekit, rotSirius), 10 * this.epsilon);
            if (i == 0) {
                Report.printToReport("Rotation", rotOrekit, rotSirius);
                i++;
            }
    ...
    public final void testBasic() throws PatriusException {
        ...
        final IDirection d1 = new GenericTargetDirection(earth);
        final IDirection d2 = new GenericTargetDirection(sun);
        final AttitudeLaw siriusYawSteering = new TwoDirectionsAttitude(d1, d2, Vector3D.PLUS_K, Vector3D.PLUS_I);
     ...
     * 
     * @testedMethod {@link TwoDirectionsAttitude#TwoDirectionsAttitude(IDirection, IDirection, Vector3D, Vector3D, double, double)}
     * 
     ...
     */
    @Test
    public final void testFiniteDifferenceThresholds() throws PatriusException {
        ...
        final AttitudeProvider twoDirectionsDefault = new TwoDirectionsAttitude(d1, d2, Vector3D.PLUS_K,
            Vector3D.PLUS_I);
        final AttitudeProvider twoDirectionsDefault2 = new TwoDirectionsAttitude(d1, d2, Vector3D.PLUS_K,
            Vector3D.PLUS_I, 0.2, 0.001);
        final AttitudeProvider twoDirectionsDiffAcc = new TwoDirectionsAttitude(d1, d2, Vector3D.PLUS_K,
            Vector3D.PLUS_I, 0.2, 0.01);
        final AttitudeProvider twoDirectionsDiffSpin = new TwoDirectionsAttitude(d1, d2, Vector3D.PLUS_K,
