...
public final class MathUtilsTest {
    ...
    @Test
    public void testSolveQuadraticEquationMinRoot() {

        // Polynome coefficients
        final double aCoeff = 2;
        final double bCoeff = 7;
        final double cCoeff = 3;

        // Solve second order polynom and get the minimum root
        final double result = MathUtils.solveQuadraticEquationMinRoot(aCoeff, bCoeff, cCoeff);
    ...
    @Test
    public void testSolveQuadraticEquationZeroA() {

        // Polynome coefficients
        final double aCoeff = 0.0;
        final double bCoeff = 7.0;
        final double cCoeff = 3.0;

        // Solve second order polynom and get the minimum root
        final double result = MathUtils.solveQuadraticEquationMaxRoot(aCoeff, bCoeff, cCoeff);

        // Reference
        final double expectedResult = -3.0 / 7.0;

        Assert.assertEquals(expectedResult, result, 0);
    }

    @Test(expected = PatriusRuntimeException.class)
    public void testSolveQuadraticEquationZeroAB() {

        // Polynome coefficients
        final double aCoeff = 0.0;
        final double bCoeff = 0.0;
        final double cCoeff = 3.0;
    ...
    @Test
    public void testFindSmallestOffset() {
        final double refNb = 8.;
        double a = -2.;
        double b = 27;

        Assert.assertEquals(refNb - a, MathUtils.findSmallestOffset(refNb, a, b), Precision.DOUBLE_COMPARISON_EPSILON);
