...
public class UniLinearIntervalsFunctionTest {
    ...
    public void nominalCase() {
        ...
        final ArrayList<Double> fres = new ArrayList<>();
    ...
    public void testSerialization() {
        ...
        final double[] xtab = { 1., 2., 3., 4. };
        final UniLinearIntervalsFunction fct = new UniLinearIntervalsFunction(xtab, xtab);
        final UniLinearIntervalsFunction deserializedFct = TestUtils.serializeAndRecover(fct);

        for (double x = 0.; x <= 5.; x += 0.5) {
            Assert.assertEquals(fct.value(x), deserializedFct.value(x), EPSILON);
        }
    }
}
