...
public class StorelessCovarianceTest {
    ...
    public void testLonglySimpleVar() {
        final double rCov = 12333921.73333333246;
        final StorelessBivariateCovariance cov = new StorelessBivariateCovariance();
        for (final double[] element : this.longleyDataSimple) {
            cov.increment(element[0], element[0]);
        }
    ...
    public void testLonglySimpleCov() {
        final double rCov = 36796.660000;
        final StorelessBivariateCovariance cov = new StorelessBivariateCovariance();
        for (final double[] element : this.longleyDataSimple) {
            cov.increment(element[0], element[1]);
        }
        TestUtils.assertEquals("simple covariance test", rCov, cov.getResult(), 10E-7);
    }
    ...
    public void testLonglyByRow() {
        ...
        final StorelessCovariance covMatrix = new StorelessCovariance(7);
        for (int i = 0; i < matrix.getRowDimension(); i++) {
            covMatrix.increment(matrix.getRow(i));
        }

        final RealMatrix covarianceMatrix = covMatrix.getCovarianceMatrix();

        TestUtils.assertEquals("covariance matrix", this.createRealMatrix(rData, 7, 7), covarianceMatrix, 10E-7);
    ...
    public void testSwissFertilityByRow() {
        ...
        final StorelessCovariance covMatrix = new StorelessCovariance(5);
        for (int i = 0; i < matrix.getRowDimension(); i++) {
            covMatrix.increment(matrix.getRow(i));
        }
        ...
        TestUtils.assertEquals("covariance matrix", this.createRealMatrix(rData, 5, 5), covarianceMatrix, 10E-13);
    }
    ...
    public void testSymmetry() {
        ...
        final StorelessCovariance storelessCov = new StorelessCovariance(dimension);
        for (int i = 0; i < matrix.getRowDimension(); i++) {
            storelessCov.increment(matrix.getRow(i));
        }

        final double[][] covMatrix = storelessCov.getData();
