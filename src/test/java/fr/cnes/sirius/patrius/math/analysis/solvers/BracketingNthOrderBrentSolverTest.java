...
public final class BracketingNthOrderBrentSolverTest extends BaseSecantSolverAbstractTest {
    ...
    public void testIssue716() {
        ...
        final double result = solver.solve(100, sharpTurn, -0.9999999, 30, 15, AllowedSolution.RIGHT_SIDE);
    ...
    public void testULPAccuracy() {
        ...
        final BracketingNthOrderBrentSolver solver = new BracketingNthOrderBrentSolver(1.0e-12, 1.0e-10, oneULP, 3);
        final double anyValueBtwOneULP = oneULP*0.3;
        
        try {
            UnivariateFunction f = new TestStepFunction(ULPtarget, anyValueBtwOneULP);
            double result = solver.solve(20, f, ULPtarget, (ULPtarget + oneULP), ULPtarget, AllowedSolution.BELOW_SIDE);
    ...
    /**
     * This function is specifically developed for {@link BracketingNthOrderBrentSolverTest#testULPAccuracy} test.
     * It represents a step function with a root at <cite>x = target + step</cite>.
     * In the current use, the step should be set as <cite>0 < step < 1ULP</cite> but the actual value doesn't matter.
     * The solver which uses this function, should be set with such limits : min = target / max = target + 1ULP.
     */
    ...
    public void testFasterThanNewton() {
        ...
        this.compare(new TestFunction(0.0, -2, 2){
            @Override
            public DerivativeStructure value(final DerivativeStructure x) {
                return x.sin().subtract(x.multiply(0.5));
            }
        });
        this.compare(new TestFunction(6.3087771299726890947, -5, 10){
            @Override
            public DerivativeStructure value(final DerivativeStructure x) {
                return x.pow(5).add(x).subtract(10000);
            }
        });
        this.compare(new TestFunction(9.6335955628326951924, 0.001, 10){
            @Override
            public DerivativeStructure value(final DerivativeStructure x) {
                return x.sqrt().subtract(x.reciprocal()).subtract(3);
            }
        });
        this.compare(new TestFunction(2.8424389537844470678, -5, 5){
            @Override
            public DerivativeStructure value(final DerivativeStructure x) {
                return x.exp().add(x).subtract(20);
            }
        });
        this.compare(new TestFunction(8.3094326942315717953, 0.001, 10){
            @Override
            public DerivativeStructure value(final DerivativeStructure x) {
                return x.log().add(x.sqrt()).subtract(5);
            }
        });
        this.compare(new TestFunction(1.4655712318767680266, -0.5, 1.5){
            @Override
            public DerivativeStructure value(final DerivativeStructure x) {
    ...
    public void testSerialization() {
        ...
        final BracketingNthOrderBrentSolver solver2 =
            (BracketingNthOrderBrentSolver) TestUtils.serializeAndRecover(solver);

        // Test between the 2 objects
        Assert.assertEquals(solver.getAbsoluteAccuracy(), solver2.getAbsoluteAccuracy(), 0.);
        Assert.assertEquals(solver.getFunctionValueAccuracy(), solver2.getFunctionValueAccuracy(), 0.);
