...
public class FourierSeriesTest {
    ...
    public enum features {
         ...
         * 
         * @featureDescription Add, subtract, multiply, divide FourierSeries by scalars. Test
         *                     negate()
         * 
         * @coveredRequirements DV-CALCUL_30, DV-CALCUL_40
         */
    ...
    public void testValue() {
        ...
        for (final double element : x) {
            Assert.assertEquals(MathLib.sin(element) + MathLib.cos(element), myPol.value(element),
                    EPS);
        }
        ...
        myPol = new FourierSeries(1, a0, a, b);
        for (final double element : x) {
            Assert.assertEquals(MathLib.cos(element) + MathLib.sin(2 * element),
                    myPol.value(element), EPS);
        }
        ...
        for (final double element : x) {
            Assert.assertEquals(a0 + MathLib.cos(2 * element) + 2 * MathLib.sin(4 * element),
                    myPol.value(element), EPS);
        }
    ...
    public void testDerivative() {
        ...
        FourierSeries myPolDVR = new FourierSeries(2, 0, new double[] { 0, 4 }, new double[] { -2,
                0 });
        ...
        myPolDVR = new FourierSeries(3, 0, new double[] { 0, 2 * 3, 6 * 3, -8 * 3, -35 * 3 },
                new double[] { -1 * 3, 0, 9 * 3, 20 * 3, -45 * 3 });
        areFourierSeriesEqual(myPolDV, myPolDVR);
        ...
        myPolDVR = new FourierSeries(3, 0, new double[] { -1 * 9, 0, 27 * 9, 80 * 9, -225 * 9 },
                new double[] { 0, -4 * 9, -18 * 9, 32 * 9, 175 * 9 });
        areFourierSeriesEqual(myPolDV, myPolDVR);
    ...
    public void testPrimitive() {
        ...
        final FourierSeries myPolPrEx = new FourierSeries(2, 0, new double[] { 0, -.25 },
                new double[] { .5, 0 });
    ...
    /**
     * @description Evaluate the finite Fourier series serialization / deserialization process.
     *
     * @testPassCriteria The finite Fourier series can be serialized and deserialized.
     */
    @Test
    public void testSerialization() {
        final double omega = 0.1;
        final double a0 = 1.1;
        final double[] a = new double[] { 1, 2, 3, 4 };
        final double[] b = new double[] { 4, 3, 2, 1 };

        final FourierSeries p = new FourierSeries(omega, a0, a, b);
        final FourierSeries p2 = TestUtils.serializeAndRecover(p);

        Assert.assertEquals(p.getAngularFrequency(), p2.getAngularFrequency(), 1e-12);
        Assert.assertEquals(p.getConstant(), p2.getConstant(), 1e-12);
        Assert.assertTrue(Arrays.equals(p.getCosArray(), p2.getCosArray()));
        Assert.assertTrue(Arrays.equals(p.getSinArray(), p2.getSinArray()));
        Assert.assertEquals(p.getOrder(), p2.getOrder());
