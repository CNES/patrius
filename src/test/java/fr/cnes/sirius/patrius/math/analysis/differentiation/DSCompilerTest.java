...
public class DSCompilerTest {
    ...
    public void testSize() {
        for (int i = 0; i < 6; ++i) {
            for (int j = 0; j < 6; ++j) {
                final long expected = ArithmeticUtils.binomialCoefficient(i + j, i);
    ...
    public void testSymmetry() {
        for (int i = 0; i < 6; ++i) {
            for (int j = 0; j < 6; ++j) {
                final DSCompiler c = DSCompiler.getCompiler(i, j);
    ...
    public void testMultiplicationRules() {
        ...
        final Map<String, String> referenceRules = new HashMap<String, String>();
        ...
        for (int i = 0; i < 5; ++i) {
            for (int j = 0; j < 4; ++j) {
                final DSCompiler compiler = DSCompiler.getCompiler(i, j);
                final int[][][] multIndirection = compiler.getMultIndirection();
                for (int k = 0; k < multIndirection.length; ++k) {
                    final String product = this.ordersToString(compiler.getPartialDerivativeOrders(k),
                        "(f*g)", "x", "y", "z", "t");
                    final StringBuilder rule = new StringBuilder();
                    for (final int[] term : multIndirection[k]) {
                        if (rule.length() > 0) {
                            rule.append(" + ");
                        }
                        if (term[0] > 1) {
                        ...
                        rule.append(this.ordersToString(compiler.getPartialDerivativeOrders(term[1]),
                            "f", "x", "y", "z", "t"));
                        rule.append(" * ");
                        rule.append(this.ordersToString(compiler.getPartialDerivativeOrders(term[2]),
                            "g", "x", "y", "z", "t"));
                    }
    ...
    public void testCompositionRules() {
        ...
        for (int i = 0; i < 5; ++i) {
            for (int j = 0; j < 5; ++j) {
                final DSCompiler compiler = DSCompiler.getCompiler(i, j);
                final int[][][] compIndirection = compiler.getCompIndirection();
                for (int k = 0; k < compIndirection.length; ++k) {
                    final String product = this.ordersToString(compiler.getPartialDerivativeOrders(k),
                        "(f(g))", "x", "y", "z", "t");
                    final StringBuilder rule = new StringBuilder();
                    for (final int[] term : compIndirection[k]) {
                        if (rule.length() > 0) {
                            rule.append(" + ");
                        }
                        if (term[0] > 1) {
                        ...
                        for (int l = 2; l < term.length; ++l) {
                            rule.append(" * ");
                            rule.append(this.ordersToString(compiler.getPartialDerivativeOrders(term[l]),
                                "g", "x", "y", "z", "t"));
                        }
                    }
                    Assert.assertEquals(product, referenceRules.get(product), rule.toString());
    ...
    private void checkIndices(final int[] indices, final int... expected) {
    ...
    private String orderToString(final int order, final String functionName, final String parameterName) {
        if (order == 0) {
            return functionName;
        } else if (order == 1) {
    ...
    private String ordersToString(final int[] orders, final String functionName, final String... parametersNames) {
