    ...
    public static StatisticalReferenceDataset createMGH17()
        ...
        try {
            dataset = new StatisticalReferenceDataset(in){
                ...
                @Override
                public double getModelValue(final double x, final double[] a) {
                    return a[0] + a[1] * MathLib.exp(-a[3] * x) + a[2] *
                        MathLib.exp(-a[4] * x);
                }
                ...
                public double[] getModelDerivatives(final double x,
                    ...
                    final double[] dy = new double[5];
                    dy[0] = 1.0;
                    dy[1] = MathLib.exp(-x * a[3]);
                    dy[2] = MathLib.exp(-x * a[4]);
                    dy[3] = -x * a[1] * dy[1];
                    dy[4] = -x * a[2] * dy[2];
                    return dy;
                }
        ...
        try {
            dataset = new StatisticalReferenceDataset(in){
                ...
                public double getModelValue(final double x, final double[] a) {
                    ...
                    return a[0] * MathLib.exp(-a[3] * x) +
                        a[1] * MathLib.exp(-a[4] * x) +
                        a[2] * MathLib.exp(-a[5] * x);
                }
                ...
                public double[] getModelDerivatives(final double x,
                    ...
                    dy[0] = MathLib.exp(-x * a[3]);
                    dy[1] = MathLib.exp(-x * a[4]);
                    dy[2] = MathLib.exp(-x * a[5]);
                    dy[3] = -x * a[0] * dy[0];
                    dy[4] = -x * a[1] * dy[1];
                    dy[5] = -x * a[2] * dy[2];
                    return dy;
