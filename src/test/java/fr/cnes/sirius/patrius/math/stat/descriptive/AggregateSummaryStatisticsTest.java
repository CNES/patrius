...
public class AggregateSummaryStatisticsTest {
    ...
    public void testAggregationConsistency() {
        ...
        final double[] totalSample = generateSample();
        final double[][] subSamples = generatePartition(totalSample);
    ...
    public void testAggregate() {
        ...
        final Collection<SummaryStatistics> aggregate = new ArrayList<>();
        for (int i = 0; i < nSamples; i++) {
            for (int j = 0; j < subSamples[i].length; j++) {
     ...
     */
    private static double[][] generatePartition(final double[] sample) {
