...
public class Vector2DTest {
     ...
     * @testedMethod {@link Vector2D#dotProduct(Vector2D, Vector2D)}
     * @testedMethod {@link Vector2D#angle(Vector2D, Vector2D)}
     * 
    ...
    public void testMethods() {
        ...
        Assert.assertEquals(6., Vector2D.dotProduct(v1, v2), 0.);
        ...
        // angle method
        final double cosAngle = 2 / v2.getNorm();
        Assert.assertEquals(MathLib.acos(cosAngle), Vector2D.angle(v1, v2), 0.);
        
        // apply various rotation to an original vector
        final double pi = MathLib.PI;
        for(final double angle : new double[] {0, 0.1*pi, 0.5 * pi, 0.75*pi, pi, 1.3*pi, 2*pi}){
            final double actual = Vector2D.angle(v1, rotateVector(v1, angle));
            final double expected = angle <= pi ? angle : 2 * pi - angle;
            Assert.assertEquals(expected, actual, 1E-15);
        }
        
        // null norm
        final Vector2D v = new Vector2D(0, 0);
        try{
            Vector2D.angle(v, v1);
            Assert.fail();
        } catch(final MathArithmeticException m){
     ...
     * 
     * @param v
     *        the vector to rotate on the plane.
     * @param angle
     *        the angle of rotation
     *        
     * @return the rotated vector
     */
    private static Vector2D rotateVector(final Vector2D v, final double angle) {
        
        // convert the input into 3D vector
        final Vector3D v3d = new Vector3D(v.getX(), v.getY(), 0);
        
        // rotation around the [0, 0, 1] axis
        final Rotation rotation = new Rotation(new Vector3D(0, 0, 1), angle);
