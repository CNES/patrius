...
public class PolynomialSplineFunctionTest {
    ...
    public void testConstructor() {
        final PolynomialSplineFunction spline = new PolynomialSplineFunction(this.knots,
                this.polynomials);
    ...
    public void testValues() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Assert.assertEquals("spline function evaluation failed for x=" + x,
                    this.polynomials[index].value(x - this.knots[index]), spline.value(x),
                    this.tolerance);
    ...
    /**
     * @description Evaluate the polynomial spline function serialization / deserialization process.
     *
     * @testPassCriteria The polynomial spline function can be serialized and deserialized.
     */
    @Test
    public void testSerialization() {
        ...
        final PolynomialSplineFunction spline = new PolynomialSplineFunction(this.knots,
                this.polynomials);
        final PolynomialSplineFunction deserializedSpline = TestUtils.serializeAndRecover(spline);

        Assert.assertEquals(spline.getN(), deserializedSpline.getN());
        Assert.assertTrue(Arrays.equals(spline.getKnots(), deserializedSpline.getKnots()));
        Assert.assertTrue(Arrays.equals(spline.getPolynomials(),
                deserializedSpline.getPolynomials()));
        Assert.assertEquals(spline.value(1.2), deserializedSpline.value(1.2), this.tolerance);
