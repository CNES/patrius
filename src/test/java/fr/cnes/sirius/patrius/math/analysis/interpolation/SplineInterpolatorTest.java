...
public class SplineInterpolatorTest {
    ...
    protected void verifyConsistency(final PolynomialSplineFunction f, final double x[])
    {
        ...
        for (int i = 1; i < x.length - 2; i++) {
            ...
            Assert.assertEquals(polynomials[i].univariateDerivative().value(x[i + 1] - x[i]),
                polynomials[i + 1].univariateDerivative().value(0), 0.5);
            Assert.assertEquals(polynomials[i].derivative().univariateDerivative().value(x[i + 1] - x[i]),
                polynomials[i + 1].derivative().univariateDerivative().value(0), 0.5);
        }
    }
