...
public final class PolynomialFunctionNewtonFormTest {
    ...
    public void testSerialization() {
        ...
        final PolynomialFunctionNewtonForm p = new PolynomialFunctionNewtonForm(new double[] { 0.,
                1., 2., 3. }, new double[] { -1., -2., 0. });
        final PolynomialFunctionNewtonForm p2 = TestUtils.serializeAndRecover(p);

        Assert.assertEquals(p.value(1.1), p2.value(1.1), 1e-12);
        Assert.assertTrue(Arrays.equals(p.getCoefficients(), p2.getCoefficients()));
        Assert.assertTrue(Arrays.equals(p.getNewtonCoefficients(), p2.getNewtonCoefficients()));
        Assert.assertTrue(Arrays.equals(p.getCenters(), p2.getCenters()));
    }
}
