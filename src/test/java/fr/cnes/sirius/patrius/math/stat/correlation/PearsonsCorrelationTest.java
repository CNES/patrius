...
public class PearsonsCorrelationTest {
    ...
    public void testLongly() {
        ...
        final PearsonsCorrelation corrInstance = new PearsonsCorrelation(matrix);
        ...
        TestUtils.assertEquals("correlation matrix", this.createRealMatrix(rData, 7, 7), correlationMatrix, 10E-15);

        final double[] rPvalues = new double[] {
            ...
            3.693245043123738e-09, 4.327782576751815e-11, 1.167954621905665e-13, 0.00331028281967516,
            0.1652293725106684,
        ...
        final RealMatrix rPMatrix = this.createLowerTriangularRealMatrix(rPvalues, 7);
        this.fillUpper(rPMatrix, 0d);
    ...
    public void testSwissFertility() {
        ...
        TestUtils.assertEquals("correlation matrix", this.createRealMatrix(rData, 5, 5), correlationMatrix, 10E-15);
        ...
        final RealMatrix rPMatrix = this.createLowerTriangularRealMatrix(rPvalues, 5);
        this.fillUpper(rPMatrix, 0d);
        TestUtils.assertEquals("correlation p values", rPMatrix, corrInstance.getCorrelationPValues(), 10E-15);
    }
    ...
    public void testPValueNearZero() {
         ...
         */
        final int dimension = 120;
        final double[][] data = new double[dimension][2];
        for (int i = 0; i < dimension; i++) {
            data[i][0] = i;
            data[i][1] = i + 1 / ((double) i + 1);
        }
        final PearsonsCorrelation corrInstance = new PearsonsCorrelation(data);
    ...
    public void testStdErrorConsistency() {
        final TDistribution tDistribution = new TDistribution(45);
        final RealMatrix matrix = this.createRealMatrix(this.swissData, 47, 5);
        final PearsonsCorrelation corrInstance = new PearsonsCorrelation(matrix);
        final RealMatrix rValues = corrInstance.getCorrelationMatrix();
        final RealMatrix pValues = corrInstance.getCorrelationPValues();
        final RealMatrix stdErrors = corrInstance.getCorrelationStandardErrors();
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < i; j++) {
                final double t = MathLib.abs(rValues.getEntry(i, j)) / stdErrors.getEntry(i, j);
                final double p = 2 * (1 - tDistribution.cumulativeProbability(t));
    ...
    public void testCovarianceConsistency() {
        final RealMatrix matrix = this.createRealMatrix(this.longleyData, 16, 7);
        final PearsonsCorrelation corrInstance = new PearsonsCorrelation(matrix);
        final Covariance covInstance = new Covariance(matrix);
        final PearsonsCorrelation corrFromCovInstance = new PearsonsCorrelation(covInstance);
        ...
        final PearsonsCorrelation corrFromCovInstance2 =
    ...
    protected RealMatrix createRealMatrix(final double[] data, final int nRows, final int nCols) {
        final double[][] matrixData = new double[nRows][nCols];
        int ptr = 0;
    ...
    protected RealMatrix createLowerTriangularRealMatrix(final double[] data, final int dimension) {
        int ptr = 0;
        final RealMatrix result = new BlockRealMatrix(dimension, dimension);
    ...
    protected void fillUpper(final RealMatrix matrix, final double diagonalValue) {
