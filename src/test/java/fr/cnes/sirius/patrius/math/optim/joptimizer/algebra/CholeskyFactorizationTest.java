...
public class CholeskyFactorizationTest extends TestCase {
    ...
    public void testInvert1() throws PatriusException {
        ...
        // check Q.QInv = 1
        RealMatrix QInv = MatrixUtils.createRealMatrix(myc.getInverse().getData());
    ...
    public void testInvert2() throws PatriusException {
        ...
        myc.factorize();
        RealMatrix L = new Array2DRowRealMatrix(myc.getL().getData());
        RealMatrix LT = new Array2DRowRealMatrix(myc.getLT().getData());
     ...
     */
    public void testInvertNotPositive() throws PatriusException, IOException {
        double[][] G = TestUtils.loadDoubleMatrixFromFile("factorization" + File.separator  + "matrix4.csv");
        
        try{
            CholeskyFactorization cs = new CholeskyFactorization(new BlockRealMatrix(G));
            cs.factorize();
    ...
    public void testScale6() throws PatriusException, IOException {
        ...
        RealMatrix AMatrix = new BlockRealMatrix(A);
        ...
        try{
            ...
            RealVector Uv = rescaler.getMatrixScalingFactorsSymm(AMatrix);
            RealMatrix U = AlgebraUtils.diagonal(Uv);
            
            assertTrue(rescaler.checkScaling(AlgebraUtils.fillSubdiagonalSymmetricMatrix(AMatrix), Uv, Uv));
            
            RealMatrix AScaled = AlgebraUtils.diagonalMatrixMult(Uv, AMatrix, Uv);
            ...
            //solve Q.X = B
            RealMatrix B = Utils.randomValuesMatrix(dim, 5, -1, 1, 12345L);
            RealMatrix X = cs.solve(U.multiply(B));
