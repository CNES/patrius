 ...
 * optimizer implementations in package {@link fr.cnes.sirius.patrius.math.optimization.general}. <br/>
 * Not enabled by default, as the class name does not end with "Test". <br/>
 * Invoke by running
 * 
 * <pre>
 * <code>
...
public class AbstractLeastSquaresOptimizerTestValidation {
     ...
     * The optimization problem solved is defined in class {@link StraightLineProblem}. <br/>
    ...
    public void testParametersErrorMonteCarloObservations() {
        ...
        // Samples generator.
        final RandomStraightLinePointGenerator lineGenerator = new RandomStraightLinePointGenerator(slope, offset,
            yError,
            -1e3, 1e4,
            138577L);
        ...
        while (mcCount < mcRepeat) {
            ...
            // covariance matrix).
            final PointVectorValuePair optimum = optim.optimize(new MaxEval(Integer.MAX_VALUE),
     ...
     * them); the first block will contain all parameter sets for which {@code chi2 < chi2_b + 1} and the second block,
     * all sets for which {@code chi2 >= chi2_b + 1} where {@code chi2_b} is the lowest chi-square (corresponding to the
     * best solution).
     */
    @Test
    public void testParametersErrorMonteCarloParameters() {
        ...
        // observations.
        final double bestChi2N = this.getChi2N(optim, problem, regress);
        final double[] sigma = optim.computeSigma(regress, 1e-14);

        // Monte-Carlo (generates a grid of parameters).
        final int mcRepeat = MONTE_CARLO_RUNS;
        final int gridSize = (int) MathLib.sqrt(mcRepeat);
        ...
        for (int i = 0; i < gridSize; i++) {
            ...
            for (int j = 0; j < gridSize; j++) {
                ...
                final double chi2N = this.getChi2N(optim, problem, new double[] { s, o });
        ...
        for (final double[] d : paramsAndChi2) {
            if (d[2] <= chi2NPlusOne) {
     ...
     */
    private double getChi2N(final AbstractLeastSquaresOptimizer optim,
                            final StraightLineProblem problem,
                            final double[] params) {
