...
public class FieldLUSolverTest {
    ...
    public static FieldMatrix<Fraction> createFractionMatrix(final int[][] data) {
        ...
        m = new Array2DRowFieldMatrix<>(FractionField.getInstance(), numRows, numCols);
    ...
    public void testSingular() {
        ...
        solver = new FieldLUDecomposition<>(createFractionMatrix(this.testData)).getSolver();
        Assert.assertTrue(solver.isNonSingular());
        solver = new FieldLUDecomposition<>(createFractionMatrix(this.singular)).getSolver();
        Assert.assertFalse(solver.isNonSingular());
        solver = new FieldLUDecomposition<>(createFractionMatrix(this.bigSingular)).getSolver();
        Assert.assertFalse(solver.isNonSingular());
    }
    ...
    public void testSolve() {
        ...
        for (int j = 0; j < b.getColumnDimension(); j++) {
            ...
            bj = new ArrayFieldVector<>(FractionField.getInstance(),
                b.getColumn(j));
    ...
    public void testExceptionSolveFieldVector() {
        ...
        final ArrayFieldVector<Fraction> b = new ArrayFieldVector<>(FractionField.getInstance(), d);
    ...
    public void testDeterminant() {
        Assert.assertEquals(-1, getDeterminant(createFractionMatrix(this.testData)), 1E-15);
        Assert.assertEquals(-10, getDeterminant(createFractionMatrix(this.luData)), 1E-14);
        Assert.assertEquals(0, getDeterminant(createFractionMatrix(this.singular)), 1E-15);
        Assert.assertEquals(0, getDeterminant(createFractionMatrix(this.bigSingular)), 1E-15);
    }

    private static double getDeterminant(final FieldMatrix<Fraction> m) {
