...
public class AVLTreeTest {
    ...
    public void testInsert() {
        ...
        final AVLTree<Integer> tree = this.buildTree(array);

        Assert.assertEquals(array.length, tree.size());

        for (final int element : array) {
            Assert.assertEquals(element, this.value(tree.getNotSmaller(new Integer(element))));
        }
    ...
    public void testDelete1() {
        ...
        for (final int[][] array : arrays) {
            final AVLTree<Integer> tree = this.buildTree(array[0]);
    ...
    public void testNavigation() {
        ...
        Assert.assertEquals(array[0], this.value(node));
        for (final int element : array) {
            Assert.assertEquals(element, this.value(node));
            node = node.getNext();
        }
        Assert.assertNull(node);

        node = tree.getLargest();
        Assert.assertEquals(array[array.length - 1], this.value(node));
        for (int i = array.length - 1; i >= 0; --i) {
            Assert.assertEquals(array[i], this.value(node));
    ...
    public void testSearch() {
        ...
        for (final int element : array) {
            Assert.assertEquals(element,
                this.value(tree.getNotSmaller(new Integer(element - 1))));
            Assert.assertEquals(element,
                this.value(tree.getNotLarger(new Integer(element + 1))));
        }
    ...
    public void testRepetition() {
        ...
        Assert.assertEquals(3, this.value(node));
        Assert.assertEquals(1, this.value(node.getPrevious()));
        Assert.assertEquals(3, this.value(node.getNext()));
        Assert.assertEquals(4, this.value(node.getNext().getNext()));

        node = tree.getNotLarger(new Integer(2));
        Assert.assertEquals(1, this.value(node));
        Assert.assertEquals(1, this.value(node.getPrevious()));
        Assert.assertEquals(3, this.value(node.getNext()));
        ...
        Assert.assertEquals(1, this.value(otherNode));
        Assert.assertNull(otherNode.getPrevious());

        node = tree.getNotLarger(new Integer(10));
        Assert.assertEquals(7, this.value(node));
        ...
        Assert.assertEquals(7, this.value(node));
        node = node.getPrevious();
        Assert.assertEquals(6, this.value(node));

        this.checkOrder(tree);

    }

    private AVLTree<Integer> buildTree(final int[] array) {
    ...
    private int value(final AVLTree<Integer>.Node node) {
        return node.getElement().intValue();
    }

    private void checkOrder(final AVLTree<Integer> tree) {
