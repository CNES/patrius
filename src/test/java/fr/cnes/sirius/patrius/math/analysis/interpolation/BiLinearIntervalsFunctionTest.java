...
public class BiLinearIntervalsFunctionTest {
    ...
    public void nominalCase() {
        ...
        final double[][] cxTab = { { 0.10, 0.20, 0.30, 0.40 }, { 0.20, 0.30, 0.40, 0.50 },
                { 0.30, 0.40, 0.50, 0.60 }, };

        final double[][] czTab = { { 0.05, 0.10, 0.10, 0.05 }, { 0.10, 0.15, 0.15, 0.10 },
                { 0.15, 0.20, 0.20, 0.25 }, };

        final BiLinearIntervalsFunction cxInterpolator = new BiLinearIntervalsFunction(aoaTab,
                machTab, cxTab);
        final BiLinearIntervalsFunction czInterpolator = new BiLinearIntervalsFunction(aoaTab,
                machTab, czTab);
        ...
        final ArrayList<Double> xres = new ArrayList<>();
        final ArrayList<Double> yres = new ArrayList<>();

        do {
            final double cx = cxInterpolator.value(alpha, mach);
            final double cz = czInterpolator.value(alpha, mach);
            // System.out.println("alpha : " + alpha + " mach : " + mach + " cx : " + cx + " cz : "
            // + cz + " cz/cx  " +
            // cz / cx);
            xres.add(mach);
            yres.add(cz / cx);
    ...
    public void testGetValues() {
        ...
        final BiLinearIntervalsFunction interp = new BiLinearIntervalsFunction(
                new BinarySearchIndexClosedOpen(xtab), new BinarySearchIndexClosedOpen(ytab), ftab);
    ...
    public void testSerialization() {
        ...
        final BiLinearIntervalsFunction fct = new BiLinearIntervalsFunction(xtab, ytab, ftab);
        final BiLinearIntervalsFunction deserializedFct = TestUtils.serializeAndRecover(fct);

        for (double x = 0.; x <= 5.; x += 0.5) {
            for (double y = 0.; y <= 5.; y += 0.5) {
                Assert.assertEquals(fct.value(x, y), deserializedFct.value(x, y), EPSILON);
            }
        }
    }
}
