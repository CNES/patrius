...
public class TriLinearIntervalsFunctionTest {
        ...
        for (int i = 0; i < dimx; i++) {
            for (int j = 0; j < dimy; j++) {
                for (int k = 0; k < dimz; k++) {
                    fval[i][j][k] = 5 + 2 * xval[i] + 3 * yval[j] + 4 * zval[k]
                        + 0.01 * xval[i] * yval[j] + 0.01 * xval[i] * zval[k]
                        + 0.01 * yval[j] * zval[k];
                }
            }
        }

        final TriLinearIntervalsFunction interp = new TriLinearIntervalsFunction(xval, yval, zval, fval);
     ...
     * No trick.
     * Goes through ALL the loops for the boundary treatments, meaning at least 2^3 cases are required !
     */
    @Test
    ...
    /**
     * Error test where not enough in x, y or z do not have the same length as ftab for the constructor
     */
    ...
    public void testGetValues() {
        ...
        final TriLinearIntervalsFunction interp = new TriLinearIntervalsFunction(new BinarySearchIndexClosedOpen(xtab),
            new BinarySearchIndexClosedOpen(ytab), new BinarySearchIndexClosedOpen(ztab), ftab);
