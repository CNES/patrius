...
public final class PolynomialFunctionTest {
    ...
    public void testConstants() {
        ...
        Assert.assertEquals(f.derivative().value(0), 0, this.tolerance);

        Assert.assertEquals(f.polynomialDerivative().derivative().value(0), 0, this.tolerance);
    }
     ...
     * 
     * <p>
     * This will test the functions <tt>f(x) = x^3 - 2x^2 + 6x + 3, g(x) = 3x^2 - 4x + 6</tt> and <tt>h(x) = 6x - 4</tt>
     */
    @Test
    public void testfirstDerivativeComparison() {
        ...
        // compare f' = g
        Assert.assertEquals(f.derivative().value(0), g.value(0), this.tolerance);
        Assert.assertEquals(f.derivative().value(1), g.value(1), this.tolerance);
        Assert.assertEquals(f.derivative().value(100), g.value(100), this.tolerance);
        Assert.assertEquals(f.derivative().value(4.1), g.value(4.1), this.tolerance);
        Assert.assertEquals(f.derivative().value(-3.25), g.value(-3.25), this.tolerance);

        // compare g' = h
        Assert.assertEquals(g.derivative().value(FastMath.PI), h.value(FastMath.PI), this.tolerance);
        Assert.assertEquals(g.derivative().value(FastMath.E), h.value(FastMath.E), this.tolerance);
    }
    ...
    public void testString() {
        ...
        this.checkPolynomial(new PolynomialFunction(new double[] { 0, -2, 3 }),
            "-2 x + 3 x^2");
        this.checkPolynomial(new PolynomialFunction(new double[] { 1, -2, 3 }),
            "1 - 2 x + 3 x^2");
        this.checkPolynomial(new PolynomialFunction(new double[] { 0, 2, 3 }),
            "2 x + 3 x^2");
        this.checkPolynomial(new PolynomialFunction(new double[] { 1, 2, 3 }),
            "1 + 2 x + 3 x^2");
        this.checkPolynomial(new PolynomialFunction(new double[] { 1, 0, 3 }),
            "1 + 3 x^2");
        this.checkPolynomial(new PolynomialFunction(new double[] { 0 }),
            "0");
    }
    ...
    public void checkPolynomial(final PolynomialFunction p, final String reference) {
