...
public class RandomDataTest {
    ...
    public void checkNextPoissonConsistency(final double mean) {
         ...
         */
        final List<Integer> binBounds = new ArrayList<>();
        binBounds.add(lower);
    ...
    public void testNextSample() {
        ...
        final HashSet<Object> cPop = new HashSet<>(); // {0,1,2,3,4}
        for (int i = 0; i < 5; i++) {
        ...
        for (int i = 0; i < 10; i++) {
            final HashSet<Object> hs = new HashSet<>();
            hs.add(c[i][0]);
            hs.add(c[i][1]);
            sets[i] = hs;
        }
    ...
    private static int findSample(final Object[] u, final Object[] samp) {
        for (int i = 0; i < u.length; i++) {
            final HashSet<Object> set = (HashSet<Object>) u[i];
            final HashSet<Object> sampSet = new HashSet<>();
    ...
    private static int findPerm(final int[][] p, final int[] samp) {
