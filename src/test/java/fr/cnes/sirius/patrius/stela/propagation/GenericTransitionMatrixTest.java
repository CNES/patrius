...
public class GenericTransitionMatrixTest {
    ...
    /** Parameters from specific test case */
    private final StelaConstantDragCoef Cx;
    /** Parameters from specific test case */
    ...
    public GenericTransitionMatrixTest(final String string, final double[] delta, final int jjfin, final int jjdeb,
                                       ...
                                       final double pom, final double gom, final double m, final double mu,
                                       final double[][] tolerance,
                                       final double dragArea2, final double refArea2,
                                       final boolean isOsculating, final double refCoef, final double mass,
                                       final double cx, final double dt,
                                       final double reentryAltitude,
                                       final boolean drag, final boolean tess, final ComparisonTypes[][] comparisonType2)
        throws PatriusException,
        IOException,
        ParseException {
        ...
        this.mass = mass;
        this.Cx = new StelaConstantDragCoef(cx);
        this.dt = dt;
    ...
    public void setup() throws PatriusException, IOException, ParseException {
        ...
        this.parDer = new TreeMap<>();
        ephemeris = new TreeMap<>();
        ephemerisAm = new TreeMap<>();
        ephemerisAp = new TreeMap<>();
        ephemerisLm = new TreeMap<>();
        ephemerisLp = new TreeMap<>();
        ephemerisExp = new TreeMap<>();
        ephemerisExm = new TreeMap<>();
        ephemerisEyp = new TreeMap<>();
        ephemerisEym = new TreeMap<>();
        ephemerisIxp = new TreeMap<>();
        ephemerisIxm = new TreeMap<>();
        ephemerisIyp = new TreeMap<>();
        ephemerisIym = new TreeMap<>();
        ephemerisK1p = new TreeMap<>();
        ephemerisK1m = new TreeMap<>();
        ephemerisK2p = new TreeMap<>();
        ephemerisK2m = new TreeMap<>();
    }
    ...
    public void test() throws PatriusException, IOException, ParseException {
        ...
        if (this.expectedResult == null) {
            ...
            // compute derivatives
            final double[] aDv =
                computeDvs(this.getParams(this.endDate, FD.AP), this.getParams(this.endDate, FD.AM), da0);
            final double[] lmDv =
                computeDvs(this.getParams(this.endDate, FD.LMP), this.getParams(this.endDate, FD.LMM), dksi0);
            final double[] exDv =
                computeDvs(this.getParams(this.endDate, FD.EXP), this.getParams(this.endDate, FD.EXM), dex0);
            final double[] eyDv =
                computeDvs(this.getParams(this.endDate, FD.EYP), this.getParams(this.endDate, FD.EYM), dey0);
            final double[] ixDv =
                computeDvs(this.getParams(this.endDate, FD.IXP), this.getParams(this.endDate, FD.IXM), dix0);
            final double[] iyDv =
                computeDvs(this.getParams(this.endDate, FD.IYP), this.getParams(this.endDate, FD.IYM), diy0);
            final double[] k1Dv =
                computeDvs(this.getParams(this.endDate, FD.SMP), this.getParams(this.endDate, FD.SMM), dK10);
            final double[] k2Dv =
                computeDvs(this.getParams(this.endDate, FD.SRP), this.getParams(this.endDate, FD.SRM), dK20);
     ...
     */
    private static double[] computeDvs(final double[] opp, final double[] opm, final double d) {
    ...
    public void configProp(final FD fd) throws PatriusException, IOException, ParseException {
        ...
        forceModelsNature.add(zonaux);
        forceModelsNature.add(tesseraux);
        forceModelsNature.add(sunPerturbation);
        forceModelsNature.add(moonPerturbation);
        this.propagator.setNatureConverter(forceModelsNature);

        final PatriusFixedStepHandler handler = new PatriusFixedStepHandler(){
            /** Serializable UID. */
            private static final long serialVersionUID = 7616156275549053329L;
