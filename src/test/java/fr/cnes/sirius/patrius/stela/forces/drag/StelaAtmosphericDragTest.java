...
public class StelaAtmosphericDragTest {
     ...
     * The Stela Cd value.
     */
    private static AbstractStelaDragCoef cd;
    ...
    public void testComputePerturbationsGTO1() throws PatriusException {
        ...
        final StelaAeroModel sp = new StelaAeroModel(114.907, new StelaConstantDragCoef(2.2), 1.07, atmosphere, 50);
     ...
     * @testedFeature {@link features#STELA_ATMOSPHERIC_DRAG}
     * 
     * @testedMethod {@link StelaAtmosphericDrag#computeShortPeriods(StelaEquinoctialOrbit, OrbitNatureConverter)}
     * 
     * @description
     *              Test method Drag Short Periods LEO.
     *              Test from STELA 3.4 (AtmosphericDragAccSPTest.computeShortPeriodsDragLEO with US76 atmospheric model instead of J77).
     * 
     * @testPassCriteria relative threshold is 1E-10. Difference is due to different models (between STELA and PATRIUS) from frames CIRF and TIRF and US76 models.
     * 
     ...
     */
    @Test
    public void testShortPeriodDragLEO() throws PatriusException {
        ...
        // Bulletin (mean)
        final AbsoluteDate date = new AbsoluteDate(AbsoluteDate.FIFTIES_EPOCH_TAI, 24472 * Constants.JULIAN_DAY + 35);

        final double a = 7000000.0;
        final double e = 0.00125;
        final double i = 70.0 * MathLib.PI / 180;
        final double raan = 30.0 * MathLib.PI / 180;
        final double w = 80.0 * MathLib.PI / 180;
        final double meanAn = 0.0 * MathLib.PI / 180;
        final KeplerianOrbit meanOrbit = new KeplerianOrbit(a, e, i, w, raan, meanAn,
                PositionAngle.MEAN, FramesFactory.getCIRF(), date, Constants.CNES_STELA_MU);

        // Force
        final DragSensitive spacecraft = new StelaAeroModel(50., new StelaConstantDragCoef(2.9600000381469727), 10);
        final Atmosphere atmosphere = new US76(new OneAxisEllipsoid(Constants.CNES_STELA_AE, 0.0033528058710313043, FramesFactory.getTIRF()));
        final StelaAtmosphericDrag drag = new StelaAtmosphericDrag(spacecraft, atmosphere, 33, Constants.CNES_STELA_AE, 2500E3, 10, 10);

        // Converter
        final List<StelaForceModel> models = new ArrayList<StelaForceModel>();
        models.add(new StelaZonalAttraction(new MyGRGSFormatReader(), 2, false, 2, 0, false));
        models.add(drag);
        final OrbitNatureConverter converter = new OrbitNatureConverter(models);

        // Conversion
        final double[] sp = drag.computeShortPeriods(new StelaEquinoctialOrbit(meanOrbit), converter);

        // Check
        final double tol = 1E-10;
        Assert.assertEquals(0., (-0.04182975434488559 - sp[0]) / sp[0], tol);
        Assert.assertEquals(0., (-1.890718958704836E-9 - sp[1]) / sp[1], tol);
        Assert.assertEquals(0., (-2.7769349166603996E-7 - sp[2]) / sp[2], tol);
        Assert.assertEquals(0., (-1.0876573134374233E-7 - sp[3]) / sp[3], tol);
        Assert.assertEquals(0., (3.9953781221013877E-10 - sp[4]) / sp[4], tol);
        Assert.assertEquals(0., (-1.4747457772495472E-9 - sp[5]) / sp[5], tol);
    }
    ...
    public void setUp() throws PatriusException {
        ...
        // Cd value:
        cd = new StelaConstantDragCoef(2.2);
    ...
    /**
     * Simplest coefficient provider, for J2 use only (from STELA 3.4 values).
     */
    private static class MyGRGSFormatReader implements PotentialCoefficientsProvider {

        /**
         * Constructor.
         */
        public MyGRGSFormatReader() {
        }

        @Override
        public double[] getJ(final boolean normalized,
                final int n) throws PatriusException {
            return new double[] {0, 0, 0.0010826261107488974 };
        }

        @Override
        public double[][] getC(final int n,
                final int m,
                final boolean normalized) throws PatriusException {
            // Unused
            return null;
        }

        @Override
        public double[][] getS(final int n,
                final int m,
                final boolean normalized) throws PatriusException {
            // Unused
            return null;
        ...
        @Override
        public double getMu() {
            return Constants.CNES_STELA_MU;
        }
