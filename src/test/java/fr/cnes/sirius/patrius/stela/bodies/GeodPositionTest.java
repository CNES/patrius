...
public class GeodPositionTest {
    ...
    public static void setUpBeforeClass() {
        Report.printClassHeader(GeodPositionTest.class.getSimpleName(), "STELA Ellipsoid coordinates");
    }
     ...
     * @testedMethod {@link GeodPosition#getGeodeticAltitude(Vector3D)} (Vector3D)}
     * 
     * @description tests the computation of the ellipsoid altitude (used for the atmospheric drag computation)
     * 
     ...
     */
    @Test
    public void testGetEllipsoidAltitude() throws PatriusException {
        Report.printMethodHeader("testGetEllipsoidLatitude", "Ellipsoid altitude computation", "Satlight V0", EPS,
     ...
     * 
     * @testedMethod {@link GeodPosition#GeodPosition(double, double)}
     * @testedMethod {@link GeodPosition#getGeodeticLatitude(Vector3D)} (Vector3D)}
     * 
     * @description tests the computation of the ellipsoid latitude (used for the atmospheric drag computation)
     * 
     * @input a vector representing spacecraft position
     * 
     * @output the latitude
     ...
     */
    @Test
    public void testGetEllipsoidLatitude() throws PatriusException {
        Report.printMethodHeader("testGetEllipsoidLatitude", "Ellipsoid latitude computation", "Satlight V0", EPS,
     ...
     * 
     * @testedMethod {@link GeodPosition#GeodPosition(double, double)}
     * @testedMethod {@link GeodPosition#getGeodeticLongitude(Vector3D, AbsoluteDate)} (Vector3D, AbsoluteDate)}
     * 
     * @description tests the computation of the ellipsoid longitude (used for the atmospheric drag computation)
     * 
     * @input a vector representing spacecraft position
     * 
     ...
     */
    @Test
    public void testGetEllipsoidLongitude() throws PatriusException {
        Report.printMethodHeader("testGetEllipsoidLongitude", "Ellipsoid longitude computation", "STELA 2.6", EPS,
            ComparisonType.ABSOLUTE);
     ...
     */
    @Test
    public void testTLoc() throws PatriusException {
     ...
     */
    @Test
    public void testForCoverage() throws PatriusException {
        final GeodPosition geod = new GeodPosition(6378136.46, 0.0033528058710313043);
        boolean rez = false;
        try {
        ...
        double getEllipsoidLatitudeExepected = FastMath.PI / 2;
        double getEllipsoidLatitudeActual = geod.getGeodeticLatitude(pos);
        Assert.assertEquals(getEllipsoidLatitudeExepected, getEllipsoidLatitudeActual, Precision.EPSILON);

        // latitude very close to 0°:
        pos = new Vector3D(9000000, 0., 0.);
        getEllipsoidLatitudeExepected = 0.0;
        getEllipsoidLatitudeActual = geod.getGeodeticLatitude(pos);
        Assert.assertEquals(getEllipsoidLatitudeExepected, getEllipsoidLatitudeActual, Precision.EPSILON);
