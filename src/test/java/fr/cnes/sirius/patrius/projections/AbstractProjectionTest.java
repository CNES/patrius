...
public class AbstractProjectionTest {
    ...
    public static void setUpBeforeClass() throws PatriusException {
        ...
        final EllipsoidPoint point = new EllipsoidPoint(ellipsoid, ellipsoid.getLLHCoordinatesSystem(), 0.1, 0.2, 0.3, "");
        projection = new Mercator(point, 0.5, true, false);
        projection2 = new GeneralizedFlamsteedSamson(point, 0.5);
    ...
    public void testDiscretize() {
        ...
        final List<Vector2D> actual = AbstractProjection.discretize(proj1, proj2, 1000000., true);
        ...
        Assert.assertEquals(expected.size() - 1, AbstractProjection.discretize(proj1, proj2, 1000000., false).size());
    }
    ...
    public void testDiscretizeAndApplyTo() throws PatriusException {
        ...
        final List<Vector2D> actual1 = projection.discretizeAndApplyTo(listPoints,
            EnumLineProperty.GREAT_CIRCLE, 1000000.);
        final List<Vector2D> actual2 = projection.discretizeAndApplyTo(listPoints, EnumLineProperty.STRAIGHT,
            1000000.);
        final List<Vector2D> actual3 = projection.discretizeAndApplyTo(listPoints,
            EnumLineProperty.STRAIGHT_RHUMB_LINE, 1000000.);

        final List<Vector2D> actual4 = projection.discretizeAndApplyTo(listPoints,
            EnumLineProperty.GREAT_CIRCLE, 0.);
        final List<Vector2D> actual5 = projection.discretizeAndApplyTo(listPoints,
            EnumLineProperty.STRAIGHT_RHUMB_LINE, 0.);

        final List<Vector2D> actual6 = projection2.discretizeAndApplyTo(listPoints,
            EnumLineProperty.STRAIGHT_RHUMB_LINE, 1000000.);
        final List<Vector2D> actual7 = projection2.discretizeAndApplyTo(listPoints,
            EnumLineProperty.STRAIGHT_RHUMB_LINE, 0.);
        ...
        try {
            projection.discretizeAndApplyTo(listPoints, EnumLineProperty.NONE, 1000000.);
            Assert.fail();
     ...
     * @testedFeature {@link features#PROJECTION}
     * 
     * @testedMethod {@link AbstractProjection#applyToAndDiscretize(EllipsoidPoint, EllipsoidPoint, double, boolean)}
     * 
    ...
    public void testApplyTo() throws PatriusException {
        ...
        final List<Vector2D> actual = projection.applyTo(listPoints);
    ...
    public void testApplyInverseTo() throws PatriusException {
        ...
        final List<EllipsoidPoint> actual1 = projection.applyInverseTo(listVector2D);
        ...
        final List<EllipsoidPoint> actual2 = projection.applyInverseTo(x, y);

        final EllipsoidBodyShape ellipsoid = point1.getBodyShape();

        final double eps = 1E-14;
        final List<EllipsoidPoint> expected = new ArrayList<>();
        expected.add(new EllipsoidPoint(ellipsoid, ellipsoid.getLLHCoordinatesSystem(), MathLib
            .toRadians(43.43087666000002), MathLib.toRadians(1.4977419400000034), 0., ""));
        expected.add(new EllipsoidPoint(ellipsoid, ellipsoid.getLLHCoordinatesSystem(), MathLib
            .toRadians(11.45915590261646), MathLib.toRadians(22.918311805232932), 0., ""));
        expected.add(new EllipsoidPoint(ellipsoid, ellipsoid.getLLHCoordinatesSystem(), MathLib
