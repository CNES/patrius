...
public class DateComponentsTest {
    ...
    public void testReferenceDates() {
        ...
        for (final int[] element : reference) {
            final int day = element[3];
            final DateComponents date = new DateComponents(DateComponents.J2000_EPOCH, day);
            Assert.assertEquals(element[0], date.getYear());
            Assert.assertEquals(element[1], date.getMonth());
            Assert.assertEquals(element[2], date.getDay());
        }
    ...
    public void testCalendarWeek() {
        ...
        final DateComponents[] firstWeekMonday = new DateComponents[502];
        for (int i = 0; i < firstWeekMonday.length; ++i) {
            firstWeekMonday[i] = this.firstWeekMonday(1599 + i);
        }
        final int startDay = firstWeekMonday[0].getJ2000Day();
        final int endDay = firstWeekMonday[firstWeekMonday.length - 1].getJ2000Day();
        int index = 0;
        for (int day = startDay; day < endDay; ++day) {
            final DateComponents d = new DateComponents(day);
            if (firstWeekMonday[index + 1].compareTo(d) <= 0) {
                ++index;
            }
            final int delta = d.getJ2000Day() - firstWeekMonday[index].getJ2000Day();
    ...
    public void testWeekComponents() {
        ...
        for (final int[] refI : reference) {
            final DateComponents date = DateComponents.createFromWeekComponents(refI[0], refI[1], refI[2]);
    ...
    // poor man's (slow) implementation of first calendar week computation,
    // using ISO rules
    private DateComponents firstWeekMonday(final int year) {
        int i = 0;
        while (true) {
            final DateComponents d = new DateComponents(year, 1, ++i);
            if (d.getDayOfWeek() == 4) {
                // this is the first Thursday of the year, Monday is 3 days
                // before
    ...
    public void testSymmetry() {
        this.checkSymmetry(-2460000, 20000);
        this.checkSymmetry(-740000, 20000);
        this.checkSymmetry(-185000, 200000);
    }

    private void checkSymmetry(final int start, final int n) {
        for (int i = start; i < start + n; ++i) {
            final DateComponents date1 = new DateComponents(DateComponents.J2000_EPOCH, i);
            Assert.assertEquals(i, date1.getJ2000Day());
            final DateComponents date2 = new DateComponents(date1.getYear(), date1.getMonth(), date1.getDay());
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testConstructorBadDayF() {
        new DateComponents(2004, 1, 40);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testConstructorBadMonthA() {
        new DateComponents(2004, 13, 12);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testConstructorBadMonthB() {
        new DateComponents(2004, 0, 12);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testConstructorBadYear() {
        new DateComponents(-9787654, 2, 12);
    }
    ...
    public void testWellFormed() {
        this.checkWellFormed(-4800, -4700, -2483687, -2446797);
        this.checkWellFormed(-5, 5, -732313, -728296);
        this.checkWellFormed(1580, 1605, -153392, -143906);
        this.checkWellFormed(1695, 1705, -111398, -107382);
        this.checkWellFormed(1795, 1805, -74874, -70858);
        this.checkWellFormed(1895, 1905, -38350, -34334);
        this.checkWellFormed(1995, 2005, -1826, 2191);
    }

    public void checkWellFormed(final int startYear, final int endYear, final int startJ2000, final int endJ2000) {

        final int[] commonLength = { 0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
        final int[] leapLength = { 0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
    ...
    public void testSerialization() {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            final int y = r.nextInt((9999 - -9999) + 1) + -9999;
            final int m = r.nextInt((12 - 1) + 1) + 1;
            final int d = r.nextInt((31 - 1) + 1) + 1;

            try {
                final DateComponents dateComponents = new DateComponents(y, m, d);
                TestUtils.checkSerializedEquality(dateComponents);
            } catch (final IllegalArgumentException iAE) {
                // do nothing, no DateComponent to be serialized.
            }
        }

        // Test constants
        TestUtils.checkSerializedEquality(DateComponents.CCSDS_EPOCH);
        TestUtils.checkSerializedEquality(DateComponents.FIFTIES_EPOCH);
        TestUtils.checkSerializedEquality(DateComponents.GALILEO_EPOCH);
        TestUtils.checkSerializedEquality(DateComponents.GPS_EPOCH);
        TestUtils.checkSerializedEquality(DateComponents.J2000_EPOCH);
        TestUtils.checkSerializedEquality(DateComponents.JAVA_EPOCH);
        TestUtils.checkSerializedEquality(DateComponents.JULIAN_EPOCH);
        TestUtils.checkSerializedEquality(DateComponents.MODIFIED_JULIAN_EPOCH);
    }
