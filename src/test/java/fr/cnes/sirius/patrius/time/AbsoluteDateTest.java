 ...
 *
 * HISTORY
 * VERSION:4.10.4:FA:FA-107:08/08/2023:[PATRIUS] Correction du problème de 
...
//        Assert.assertEquals(expectedEnd.getOffset(), actualOffset, 0.);
    ...
    @Test
    public void testInfinityStrings() {
    ...
    public void testGetSecondsInDay() {
        ...
        Assert.assertEquals(0.0, date1.getSecondsInDay(this.utc), 1.0e-10);
        
        final AbsoluteDate date2 = new AbsoluteDate(new DateComponents(2006, 8, 9),
                new TimeComponents(6, 0, 0), this.utc);
        Assert.assertEquals(21600.0, date2.getSecondsInDay(this.utc), 1.0e-10);
        
        final AbsoluteDate date3 = new AbsoluteDate(new DateComponents(2006, 8, 9),
                new TimeComponents(12, 0, 0), this.utc);
        Assert.assertEquals(43200.0, date3.getSecondsInDay(this.utc), 1.0e-10);
    }
    ...
    @Test
    public void testEpochOffsetCtor() {
    ...
    private static void checkIteration(final double step, final AbsoluteDate t0, final int nMax,
     ...
     * @testType UT
     * 
     * @testedMethod {@link AbsoluteDate#toString(int, TimeScale)}
     * 
     * @description test numerical quality issue with toString(int, TimeScale) method: date string should be exactly as
     *              expected
     * 
     * @input TAI date
     *
     * @output TDB date
     *
     * @testPassCriteria TDB date is exactly as expected
     * 
