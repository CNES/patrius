...
public class IntervalMapSearcherTest {
    ...
    public void testFeatures() {
        ...
        // Test methods
        final AbsoluteDate outOfRangeDate = date.shiftedBy(-duration);
        final AbsoluteDate goodDate = date.shiftedBy(duration / 2);
        final CacheEntry<AbsoluteDateInterval, Double> entry1 = searcher.getEntry(goodDate);
        final CacheEntry<AbsoluteDateInterval, Double> entry2 = searcher.getEntry(goodDate, false);
        final CacheEntry<AbsoluteDateInterval, Double> entry3 = searcher.getEntry(outOfRangeDate, false);
        ...
        Assert.assertEquals(intervals.get(0), entry1.getKey());
    ...
    public void testExceptions() {
        ...
        // (outside the supported interval [dateInf, dateSup]) (should fail)
        try {
            searcher.getEntry(date.shiftedBy(-duration));
            Assert.fail();
        } catch (final IllegalStateException e) {
            // Expected
            Assert.assertEquals(e.getMessage(), "The provided date (2000-01-01T07:59:27.816) is outside the supported" +
                    " intervals.");
        ...
        // (inside the supported interval [dateInf, dateSup]) (should fail)
        try {
            searcher.getEntry(date.shiftedBy(duration + 10.));
            Assert.fail();
        } catch (final IllegalStateException e) {
            // Expected
            Assert.assertEquals(e.getMessage(), "The provided date (2000-01-01T15:59:37.816) does not belong to any " +
                    "of the intervals.");
        }
    }
