...
public class JPLCelestialBodyLoaderTest {
    ...
    public void testFA1777() throws PatriusException {
        ...
        final Frame icrf = CelestialBodyFactory.getSolarSystemBarycenter()
                .getInertialFrameConstantModel();
        final Frame frameSSB = CelestialBodyFactory.getSolarSystemBarycenter()
                .getInertialFrameConstantModel();
    ...
    public void testConstantsInpop() throws PatriusException {
        ...
        final JPLCelestialBodyLoader loader = new JPLCelestialBodyLoader(
            JPLCelestialBodyLoader.DEFAULT_INPOP_SUPPORTED_NAMES,
            EphemerisType.SUN);
        Assert.assertEquals(149597870691.0, loader.getLoadedAstronomicalUnit(), 0.1);
    ...
    public void testEndianness() throws PatriusException {
        ...
        final JPLCelestialBodyLoader loaderInpopTCBBig = new JPLCelestialBodyLoader(
            "^inpop.*_TCB_.*_bigendian\\.dat$", type);
        final CelestialBody bodysInpopTCBBig = loaderInpopTCBBig
        ...
        final CelestialBody bodysInpopTCBLittle = loaderInpopTCBLittle
            .loadCelestialBody(CelestialBodyFactory.MARS);
    ...
    public void testInpopvsJPL() throws PatriusException {
        ...
        final EphemerisType type = EphemerisType.MARS;
        final JPLCelestialBodyLoader loaderDE405 = new JPLCelestialBodyLoader(
            "^unxp(\\d\\d\\d\\d)\\.405$", type);
        final CelestialBody bodysDE405 = loaderDE405.loadCelestialBody(CelestialBodyFactory.MARS);
        final JPLCelestialBodyLoader loaderInpopTDBBig = new JPLCelestialBodyLoader(
            "^inpop.*_TDB_.*_bigendian\\.dat$", type);
        final CelestialBody bodysInpopTDBBig = loaderInpopTDBBig
            .loadCelestialBody(CelestialBodyFactory.MARS);
    ...
    public void testCoverage() throws PatriusException {
        ...
        // Test GetNativeFrame
        Assert.assertEquals(icrfMoon.getParent(),
                CelestialBodyFactory.getMoon().getNativeFrame(null, null));
    }
    ...
    public void testEarth() throws PatriusException {
        ...
        Assert.assertEquals(FramesFactory.getGCRF(), earth.getInertialFrameConstantModel());
        Assert.assertEquals(FramesFactory.getMOD(true), earth.getInertialFrameMeanModel());
        Assert.assertEquals(FramesFactory.getTOD(true), earth.getInertialFrameTrueModel());
        try {
            earth.getRotatingFrameConstantModel();
            Assert.fail();
        } catch (final PatriusException e) {
            Assert.assertTrue(true);
        }
        try {
            earth.getRotatingFrameMeanModel();
            Assert.fail();
        } catch (final PatriusException e) {
            Assert.assertTrue(true);
        }
        Assert.assertEquals(FramesFactory.getITRF(), earth.getRotatingFrameTrueModel());
        Assert.assertEquals(earth.getInertialFrameConstantModel(), earth.getNativeFrame(null, null));
    }
        ...
        for (final Body body : Body.values()) {
            ...
            if (type.equals(EphemerisType.EARTH_MOON)
                ...
                final BodyShape expectedShape = new OneAxisEllipsoid(radius, flatness,
                        celestialBody.getRotatingFrameTrueModel(), "body");

                // arbitrary line expressed in the body frame
                final Line line1 = new Line(new Vector3D(10 * radius, 0, 0), new Vector3D(0,
                        10 * radius, 0));
                ...
                expected = expectedShape.distanceTo(line1,
                        celestialBody.getInertialFrameConstantModel(), date);
                actual = defaultShape.distanceTo(line1,
                        celestialBody.getInertialFrameConstantModel(), date);
                ...
                final PVCoordinatesProvider provider = new ConstantPVCoordinatesProvider(pv,
                        celestialBody.getInertialFrameTrueModel());
                    expected = expectedShape.getApparentRadius(new ConstantPVCoordinatesProvider(posInInertialFrame.scalarMultiply(2.5), celestialBody.getInertialFrameConstantModel()),
                    date, provider, PropagationDelayType.INSTANTANEOUS);
                actual = defaultShape.getApparentRadius(new ConstantPVCoordinatesProvider(posInInertialFrame.scalarMultiply(2.5), celestialBody.getInertialFrameConstantModel()),
                    date, provider, PropagationDelayType.INSTANTANEOUS);

                // test getIntersections method
                final Line line2 = new Line(new Vector3D(0.8 * radius, 0, 0), new Vector3D(0,
                        0.8 * radius, 0));
                final Vector3D[] actualInter = defaultShape.getIntersectionPoints(line2,
                        celestialBody.getInertialFrameConstantModel(), date);
                ...
                Assert.assertEquals(expectedInter[expectedInter.length - 1],
                        actualInter[actualInter.length - 1]);

                // test the setters with slightly modified parameters
                celestialBody.setShape(new OneAxisEllipsoid(radius * 1.05, flatness * 0.9,
                        celestialBody.getRotatingFrameTrueModel(), "body modified"));
    ...
    /**
     * Check the default instance {@link AbstractAttractionModel} for
     * each celestial bodies.
     * 
     * @throws PatriusException
     */
    ...
    public void testAttractionModel() throws PatriusException {
        ...
        for (final EphemerisType type : EphemerisType.values()) {
            ...
            final AbstractAttractionModel actual = celestialBody.getAttractionModel();

            Assert.assertTrue(actual.getClass().equals(NewtonianAttractionModel.class));
    ...
    /**
     * Check the constructor with an {@link AbstractAttractionModel} in input.
     * 
     * @throws PatriusException
     */
    @Test
    public void testAttractionModelConstructor() throws PatriusException {
        // data to load
        Utils.setDataRoot("regular-dataPBASE");
        final AbstractAttractionModel attractionModel = new NewtonianAttractionModel(
    ...
    private static void checkDerivative(final String supportedNames, final AbsoluteDate date)
            throws PatriusException {
        final JPLCelestialBodyLoader loader = new JPLCelestialBodyLoader(supportedNames,
