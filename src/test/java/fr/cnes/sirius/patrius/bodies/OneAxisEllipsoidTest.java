...
public class OneAxisEllipsoidTest {
    ...
    public void testGetNormal() throws PatriusException {
        ...
        final GeodeticPoint target0 = new GeodeticPoint(MathLib.toRadians(15), MathLib.toRadians(30), 0.);
        final GeodeticPoint target1000 = new GeodeticPoint(MathLib.toRadians(15), MathLib.toRadians(30), 1000.);
        // Compute the normal direction on both targets
        final Vector3D normal0 = earthShape.getNormal(earthShape.transform(target0));
        final Vector3D normal1000 = earthShape.getNormal(earthShape.transform(target1000));
        // Check the results
        Assert.assertEquals(normal0.getX(), target0.getZenith().getX(), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(normal0.getY(), target0.getZenith().getY(), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(normal0.getZ(), target0.getZenith().getZ(), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(normal1000.getX(), target1000.getZenith().getX(), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(normal1000.getY(), target1000.getZenith().getY(), Precision.DOUBLE_COMPARISON_EPSILON);
        Assert.assertEquals(normal1000.getZ(), target1000.getZenith().getZ(), Precision.DOUBLE_COMPARISON_EPSILON);
    }
