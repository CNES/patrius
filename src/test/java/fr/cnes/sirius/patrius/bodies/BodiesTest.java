...
public class BodiesTest {
    ...
    public void testFrameShift() throws PatriusException {
        ...
        final Frame moon = CelestialBodyFactory.getMoon().getRotatingFrameTrueModel();
        final Frame earth = CelestialBodyFactory.getEarth().getRotatingFrameTrueModel();
    ...
    private static void checkKepler(final PVCoordinatesProvider orbiting, final CelestialBody central,
        ...
        final Orbit orbit = new KeplerianOrbit(orbiting.getPVCoordinates(start, central.getInertialFrameConstantModel()),
            central.getInertialFrameConstantModel(), start, central.getGM());
        ...
        for (AbsoluteDate date = start; date.durationFrom(start) < duration; date = date.shiftedBy(duration / 100)) {
            final PVCoordinates ephemPV = orbiting.getPVCoordinates(date, central.getInertialFrameConstantModel());
    ...
    public void testEarthMoonBarycenter() throws PatriusException {
        ...
        final List<Frame> frames = Arrays.asList(FramesFactory.getEME2000(),
            ...
            sun.getInertialFrameConstantModel(),
            mars.getInertialFrameConstantModel(),
            earth.getInertialFrameConstantModel());
    ...
    public void testMoonSpinDerivatives() throws PatriusException {
        ...
        // Body-oriented frame
        final Transform tBody = moon.getRotatingFrameTrueModel().getTransformTo(moon.getInertialFrameConstantModel(), date);
        final Transform tBodydt = moon.getRotatingFrameTrueModel().getTransformTo(moon.getInertialFrameConstantModel(), date.shiftedBy(10.));
        ...
        // Inertial frame
        final Transform tInertial = moon.getInertialFrameTrueModel().getTransformTo(FramesFactory.getGCRF(), date);
        final Transform tInertialdt = moon.getInertialFrameTrueModel().getTransformTo(FramesFactory.getGCRF(), date.shiftedBy(10.));
    ...
    public void testCelestialBodyFrames() throws PatriusException {
        ...
        Assert.assertEquals("Moon Inertial frame (constant model)", body.getInertialFrameConstantModel().toString());
        Assert.assertEquals("Moon Inertial frame (mean model)", body.getInertialFrameMeanModel().toString());
        Assert.assertEquals("Moon Inertial frame (true model)", body.getInertialFrameTrueModel().toString());
        Assert.assertEquals("Moon Rotating frame (constant model)", body.getRotatingFrameConstantModel().toString());
        Assert.assertEquals("Moon Rotating frame (mean model)", body.getRotatingFrameMeanModel().toString());
        Assert.assertEquals("Moon Rotating frame (true model)", body.getRotatingFrameTrueModel().toString());
        
        // Build all transformations wrt to ICRF
        final AbsoluteDate date = new AbsoluteDate(2020, 1, 1, 0, 0, 0, TimeScalesFactory.getTAI());
        final Transform t1 = body.getICRF().getTransformTo(body.getInertialFrameConstantModel(), date);
        final Transform t2 = body.getICRF().getTransformTo(body.getInertialFrameMeanModel(), date);
        final Transform t3 = body.getICRF().getTransformTo(body.getInertialFrameTrueModel(), date);
        final Transform t4 = body.getICRF().getTransformTo(body.getRotatingFrameConstantModel(), date);
        final Transform t5 = body.getICRF().getTransformTo(body.getRotatingFrameMeanModel(), date);
        final Transform t6 = body.getICRF().getTransformTo(body.getRotatingFrameTrueModel(), date);
