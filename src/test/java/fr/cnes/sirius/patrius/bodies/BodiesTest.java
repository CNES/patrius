...
public class BodiesTest {
    ...
    public void IAUPole() throws PatriusException {
        ...
        Assert.assertNotNull(((AbstractCelestialBody) CelestialBodyFactory.getSun()).getIAUPole());
    }
    ...
    public void testFrameShift() throws PatriusException {
        ...
        final Frame moon = CelestialBodyFactory.getMoon().getTrueRotatingFrame();
        final Frame earth = CelestialBodyFactory.getEarth().getTrueRotatingFrame();
    ...
    private void checkKepler(final PVCoordinatesProvider orbiting, final CelestialBody central,
        ...
        final Orbit orbit = new KeplerianOrbit(orbiting.getPVCoordinates(start, central.getInertialEquatorFrame()),
            central.getInertialEquatorFrame(), start, central.getGM());
        ...
        for (AbsoluteDate date = start; date.durationFrom(start) < duration; date = date.shiftedBy(duration / 100)) {
            final PVCoordinates ephemPV = orbiting.getPVCoordinates(date, central.getInertialEquatorFrame());
    ...
    public void testEarthMoonBarycenter() throws PatriusException {
        ...
        final List<Frame> frames = Arrays.asList(FramesFactory.getEME2000(),
            ...
            sun.getInertialEquatorFrame(),
            mars.getInertialEquatorFrame(),
            earth.getInertialEquatorFrame());
    ...
    public final void testMoonSpinDerivatives() throws PatriusException {
        ...
        // Body-oriented frame
        final Transform tBody = moon.getTrueRotatingFrame().getTransformTo(moon.getInertialEquatorFrame(), date);
        final Transform tBodydt = moon.getTrueRotatingFrame().getTransformTo(moon.getInertialEquatorFrame(), date.shiftedBy(10.));
        ...
        // Inertial frame
        final Transform tInertial = moon.getTrueEquatorFrame().getTransformTo(FramesFactory.getGCRF(), date);
        final Transform tInertialdt = moon.getTrueEquatorFrame().getTransformTo(FramesFactory.getGCRF(), date.shiftedBy(10.));
    ...
    public void testCelestialBodyFrames() throws PatriusException {
        ...
        Assert.assertEquals("Moon Inertial equator frame", body.getInertialEquatorFrame().toString());
        Assert.assertEquals("Moon Mean equator frame", body.getMeanEquatorFrame().toString());
        Assert.assertEquals("Moon True equator frame", body.getTrueEquatorFrame().toString());
        Assert.assertEquals("Moon Constant rotating frame", body.getConstantRotatingFrame().toString());
        Assert.assertEquals("Moon Mean rotating frame", body.getMeanRotatingFrame().toString());
        Assert.assertEquals("Moon True rotating frame", body.getTrueRotatingFrame().toString());
        
        // Build all transformations wrt to ICRF
        final AbsoluteDate date = new AbsoluteDate(2020, 1, 1, 0, 0, 0, TimeScalesFactory.getTAI());
        final Transform t1 = body.getICRF().getTransformTo(body.getInertialEquatorFrame(), date);
        final Transform t2 = body.getICRF().getTransformTo(body.getMeanEquatorFrame(), date);
        final Transform t3 = body.getICRF().getTransformTo(body.getTrueEquatorFrame(), date);
        final Transform t4 = body.getICRF().getTransformTo(body.getConstantRotatingFrame(), date);
        final Transform t5 = body.getICRF().getTransformTo(body.getMeanRotatingFrame(), date);
        final Transform t6 = body.getICRF().getTransformTo(body.getTrueRotatingFrame(), date);
