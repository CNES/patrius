 ...
 * @since 4.6
 */
public class FacetCelestialBodyTest {
    ...
    private FacetCelestialBody body;
    ...
    public void setUpBeforeClass() throws PatriusException, IOException {
        ...
        body = new FacetCelestialBody("My body", pvCoordinates, 0, IAUPoleFactory.getIAUPole(null), new ObjMeshLoader(modelFile));
    }
    ...
    public final void intersectionTest() throws PatriusException {
        ...
        final Intersection actual4 = body.getIntersection(lineOfSight4, position4, body.getBodyFrame(), date);
    ...
    public final void intersectionGeodeticTest() throws PatriusException {
        ...
        final Vector3D position4 = new Vector3D(0, 0, 10000);
        final Line lineOfSight4 = new Line(position4, position4.add(Vector3D.PLUS_I));
        final GeodeticPoint actual4 = body.getIntersectionPoint(lineOfSight4, position4, body.getBodyFrame(), date);
    ...
    public final void intersectionsTest() throws PatriusException {
        ...
        final Vector3D position4 = new Vector3D(0, 0, 10000);
        final Line lineOfSight4 = new Line(position4, position4.add(Vector3D.PLUS_I));
        final Vector3D[] actual4 = body.getIntersectionPoints(lineOfSight4, body.getBodyFrame(), date);
    ...
    public final void intersectionPointsAltitudeTest() throws PatriusException {
        ...
        final GeodeticPoint gp2 = body.getIntersectionPoint(line2, point2, body.getBodyFrame(), date, 0);
        final GeodeticPoint gpRef = body.getIntersectionPoint(line2, point2, body.getBodyFrame(), date);
    ...
    public final void transformTest() throws PatriusException {
        ...
        final Frame frame = FramesFactory.getEME2000();
        final ExtendedOneAxisEllipsoid referenceEllipsoid = body.getInnerSphere();

        // Check cartesian => geodetic coordinates
        final GeodeticPoint actual1 = body.transform(position, frame, date);
        final GeodeticPoint expected1 = referenceEllipsoid.transform(position, frame, date);
        ...
        final Vector3D actual2 = body.transform(actual1);
        final Vector3D expected2 = referenceEllipsoid.transform(actual1);
    ...
    public final void exceptionsTest() {
        try {
            new FacetCelestialBody("", null, 0, IAUPoleFactory.getIAUPole(null), new ObjMeshLoader("Dummy.obj"));
            Assert.fail();
        } catch (final PatriusException e) {
            Assert.assertTrue(true);
        }
        try {
            new FacetCelestialBody("", null, 0, IAUPoleFactory.getIAUPole(null), new GeodeticMeshLoader("Dummy.tab"));
            Assert.fail();
        } catch (final PatriusException e) {
            Assert.assertTrue(true);
        }
     ...
     * @testType UT
     * 
     * @description check that {@link EclipseDetector} can be used together with {@link Propagator} and {@link FacetCelestialBody}.
     *              Detected eclipse dates must be similar to dates obtained with a same-size {@link OneAxisEllipsoid}.
     * 
     * @testPassCriteria eclipse dates are as expected (reference: propagation with {@link OneAxisEllipsoid}, relative threshold: 0)
     * 
     * @referenceVersion 4.6
     * 
     * @nonRegressionVersion 4.6
    ...
    public final void eclipseDetectorTest() throws PatriusException {
        ...
        for (int i = 0; i < referenceDates.size(); i++) {
            Assert.assertEquals(0, referenceDates.get(i).durationFrom(actualDates.get(i)), 0.);
        }
    }
     ...
     * 
     * @testPassCriteria inner sphere is a sphere of radius, the radius of the spherical body (reference math)
     * 
     * @referenceVersion 4.6
     * 
     * @nonRegressionVersion 4.6
     */
    @Test
    public final void innerSphereTest() throws PatriusException {
        final ExtendedOneAxisEllipsoid referenceEllipsoid = body.getInnerSphere();
        Assert.assertEquals(10000, referenceEllipsoid.getEquatorialRadius(), 1E-11);
        Assert.assertEquals(0., referenceEllipsoid.getFlattening(), 0);
    }
