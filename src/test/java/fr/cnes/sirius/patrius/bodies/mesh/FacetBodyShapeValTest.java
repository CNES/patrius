...
public class FacetBodyShapeValTest {
    ...
    public void neighborsTest() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            // getNeighbors(BodyPoint)
            final FacetPoint point2 = body1.buildPoint(body1.getLLHCoordinatesSystem(), random.nextDouble()
                    * MathLib.PI - MathLib.PI / 2., random.nextDouble() * 2. * MathLib.PI, random.nextDouble() * 10000,
                "");
        ...
        for (int i = 0; i < 10; i++) {
            ...
            for (final Triangle t : actual1.getVisibleTriangles()) {
                ...
                for (final Vertex v : t.getVertices()) {
                    ...
                    final FacetPoint[] intersections = body1.getIntersectionPoints(line, bodyFrame, null);
                    boolean res = false;
                    for (final FacetPoint intersection : intersections) {
                        // Discard current vertex which is an obvious intersection point
                        if (intersection.getPosition().distance(v.getPosition()) > 1E-6) {
                            ...
                            final Vector3D v2 = posInBodyFrame.subtract(intersection.getPosition());
            ...
            final List<FacetPoint> contour2 = actual2.getContour();
            Assert.assertEquals(contour1.size(), contour2.size());
            for (final FacetPoint point : contour1) {
                final Vector3D p3D = point.getPosition();
            ...
            for (final FacetPoint point : contour2) {
                final Vector3D p3D = point.getPosition();
                final Vector3D vertexDirInBodyFrame = p3D.subtract(transform.transformPosition(position));
                final Vector3D vertexDirInSensorFrame = transform2.transformVector(vertexDirInBodyFrame);
                Assert.assertTrue(fieldOfView.isInTheField(vertexDirInSensorFrame));
     ...
     * @param maxDistance max distance
     */
    private static void checkNeighborsTriangles(final List<Triangle> triangles, final FacetPoint point,
