...
public class FacetBodyShapeValTest {
    ...
    private static FacetBodyShape body1;
    ...
    public static void setUpBeforeClass() throws PatriusException, IOException, URISyntaxException {
        ...
        celestialBody1 = new UserCelestialBody("", CelestialBodyFactory.getMoon(), 0, IAUPoleFactory.getIAUPole(null), FramesFactory.getGCRF(), null);
        body1 = new FacetBodyShape("", celestialBody1.getTrueRotatingFrame(), CelestialBodyFactory.getMoon(), EllipsoidType.INNER_SPHERE, new ObjMeshLoader(fullName1));
        
        // Load geodetic mesh and validate .obj file writing
        final String modelFile2 = "mnt" + File.separator + "m1phobos.tab";
        final String fullName2 = FacetBodyShape.class.getClassLoader().getResource(modelFile2).toURI().getPath();
        ...
        celestialBody2 = new UserCelestialBody("", CelestialBodyFactory.getMoon(), 0, IAUPoleFactory.getIAUPole(null), FramesFactory.getGCRF(), null);
        body2 = new FacetBodyShape("", celestialBody2.getTrueRotatingFrame(), CelestialBodyFactory.getMoon(), EllipsoidType.INNER_SPHERE, loader3);
    ...
    public final void surfacePointedDataTest() throws PatriusException {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            // Transform position in body frame
            final Vector3D posInBodyFrame = t.transformPosition(position);
            final Intersection intersection = body1.getIntersection(lineOfSight, posInBodyFrame, frame, date);
    ...
    public final void localRadiusTest() throws PatriusException {
        ...
        final double apparentRadiusFarthestPoint = body2.getLocalRadius(farthestPoint, frame, date,
            sunPVCoordProvider);
        ...
        // default one
        final double apparentRadiusFarthestPoint2 = body2.getLocalRadius(farthestPoint, frame, date,
            sunPVCoordProvider);
    ...
    public final void innerEllipsoidTest() {
        // Check that the equatorial radius of the inner ellipsoid coincides with the expected minimum value of the
        // distance between the vertices and the center of the body, i.e. with the norm of the position of the closest
        // vertex
        Assert.assertEquals(0., (body1.getInnerEllipsoid().getEquatorialRadius() - EXPECTED_MIN) / EXPECTED_MIN, EPS);
        // Check that the flattening of the inner ellipsoid has the same flattening as the fitted ellipsoid
        Assert.assertEquals(0., (body1.getInnerEllipsoid().getFlattening() - body1.getFittedEllipsoid()
            .getFlattening()) / body1.getFittedEllipsoid().getFlattening(), EPS);
        ...
        for (Vertex vertex : verticesMap.values()) {
            // Check that the inner ellipsoid is inscribed within the current vertex
            Assert.assertTrue(body1.getInnerEllipsoid().getEquatorialRadius() <= vertex.getPosition().getNorm());
        }
    }
    ...
    public final void outerEllipsoidTest() {
        // Check that the polar radius of the outer ellipsoid coincides with the expected maximum value of the distance
        // between the vertices and the center of the body, i.e. with the norm of the position of the farthest
        // vertex
        Assert.assertEquals(0., (body1.getOuterEllipsoid().getEquatorialRadius()
                * (1 - body1.getOuterEllipsoid().getFlattening()) - EXPECTED_MAX)
                / EXPECTED_MAX, EPS);
        // Check that the flattening of the outer ellipsoid has the same flattening as the fitted ellipsoid
        Assert.assertEquals(0., (body1.getOuterEllipsoid().getFlattening() - body1.getFittedEllipsoid()
            .getFlattening()) / body1.getFittedEllipsoid().getFlattening(), EPS);
        ...
        // Loop on all the vertices
        for (Vertex vertex : verticesMap.values()) {
            // Check that the outer ellipsoid is englobing the current vertex
            Assert.assertTrue(body1.getOuterEllipsoid().getEquatorialRadius() >= vertex.getPosition().getNorm());
        }
    }
    ...
    public final void innerSphereTest() {
        ...
        Assert.assertEquals(0., (body1.getInnerSphere().getEquatorialRadius() - EXPECTED_MIN) / EXPECTED_MIN, EPS);
        // Check that the flattening of the inner sphere is zero
        Assert.assertEquals(0., body1.getInnerSphere().getFlattening(), EPS);
        ...
        for (Vertex vertex : verticesMap.values()) {
            ...
            Assert.assertTrue(body1.getInnerSphere().getEquatorialRadius() <= vertex.getPosition().getNorm());
        }
    }
    ...
    public final void outerSphereTest() {
        ...
        Assert.assertEquals(0., (body1.getOuterSphere().getEquatorialRadius() - EXPECTED_MAX) / EXPECTED_MAX, EPS);
        // Check that the flattening of the outer sphere is zero
        Assert.assertEquals(0., body1.getOuterSphere().getFlattening(), EPS);
        ...
        for (Vertex vertex : verticesMap.values()) {
            ...
            Assert.assertTrue(body1.getOuterSphere().getEquatorialRadius() >= vertex.getPosition().getNorm());
        }
    }
     ...
     * @param maxDistance max distance
     */
    private void checkNeighborsTriangles(final List<Triangle> triangles, final GeodeticPoint point, final FacetBodyShape body, final double maxDistance) {
     ...
     * @param maxDistance max distance
     */
    private void checkNeighborsTriangles(final List<Triangle> triangles, final Vector3D point, final FacetBodyShape body, final double maxDistance) {
        for (final Triangle t : triangles) {
