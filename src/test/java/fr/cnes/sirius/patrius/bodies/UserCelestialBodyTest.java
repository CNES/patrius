...
public class UserCelestialBodyTest {
    ...
    public void testUserDefinedCelestialBody() throws PatriusException {
        ...
        final PVCoordinatesProvider provider = new ConstantPVCoordinatesProvider(pv,
         ...
         actual = neptuneShape.getApparentRadius(new ConstantPVCoordinatesProvider(posInFrame.scalarMultiply(5), neptuneExpected.getInertialFrame(IAUPoleModelType.CONSTANT)),
            date, provider, PropagationDelayType.INSTANTANEOUS);
        expected = expectedShape.getApparentRadius(new ConstantPVCoordinatesProvider(posInFrame.scalarMultiply(5), neptuneExpected.getInertialFrame(IAUPoleModelType.CONSTANT)),
            date, provider, PropagationDelayType.INSTANTANEOUS);
        Assert.assertEquals(expected, actual, 0.);
