...
public class UserCelestialBodyTest {
    ...
    public void testUserDefinedCelestialBody() throws PatriusException {
        ...
        final BodyShape neptuneShape = new OneAxisEllipsoid(ae, (ae - ap) / ae,
                neptuneExpected.getRotatingFrameTrueModel(), "Neptune");
        ...
        final UserCelestialBody neptuneActualAttractionModel = new UserCelestialBody("Neptune", pvProvider,
            neptuneExpected.getAttractionModel(), IAUPoleFactory.getIAUPole(EphemerisType.NEPTUNE),
            FramesFactory.getICRF(), neptuneShape);
        ...
        final Transform t1 = neptuneExpected.getRotatingFrameTrueModel().getTransformTo(
                neptuneActual.getRotatingFrameTrueModel(), date);
        final Transform t2 = neptuneExpected.getInertialFrameConstantModel().getTransformTo(
                neptuneActual.getInertialFrameConstantModel(), date);
        ...
        actual = neptuneShape.distanceTo(line1, neptuneExpected.getInertialFrameConstantModel(),
                date);
        expected = expectedShape.distanceTo(line1, neptuneExpected.getInertialFrameConstantModel(),
                date);
        ...
        final PVCoordinatesProvider provider = new ConstantPVCoordinatesProvider(pv,
                neptuneExpected.getInertialFrameConstantModel());
         actual = neptuneShape.getApparentRadius(new ConstantPVCoordinatesProvider(posInFrame.scalarMultiply(5), neptuneExpected.getInertialFrameConstantModel()),
            date, provider, PropagationDelayType.INSTANTANEOUS);
        expected = expectedShape.getApparentRadius(new ConstantPVCoordinatesProvider(posInFrame.scalarMultiply(5), neptuneExpected.getInertialFrameConstantModel()),
            date, provider, PropagationDelayType.INSTANTANEOUS);
        Assert.assertEquals(expected, actual, 0.);
        ...
        final Vector3D[] intersections = neptuneShape.getIntersectionPoints(line2,
                neptuneExpected.getInertialFrameConstantModel(), date);
        ...
        Assert.assertEquals(intersections[intersections.length - 1],
                expectedInter[expectedInter.length - 1]);

        // set a new body shape
        final BodyShape newShape = new OneAxisEllipsoid(1.05 * ae, 0.001,
                neptuneExpected.getRotatingFrameTrueModel(), "new Neptune");
        ...
        final AbstractAttractionModel model = neptuneActual.getAttractionModel();
        final AbstractAttractionModel modelAttractionModel = neptuneActualAttractionModel.getAttractionModel();
        Assert.assertEquals(mu, neptuneActual.getGM(), 0.);
        Assert.assertEquals(mu, neptuneActualAttractionModel.getGM(), 0.);
        Assert.assertTrue(model.getClass().equals(NewtonianAttractionModel.class));
        Assert.assertTrue(modelAttractionModel.getClass().equals(NewtonianAttractionModel.class));
        ...
        neptuneActualAttractionModel.setAttractionModel(newModel);
        Assert.assertEquals(2 * mu, neptuneActual.getGM(), 0.);
        Assert.assertEquals(2 * mu, neptuneActualAttractionModel.getGM(), 0.);
        Assert.assertTrue(neptuneActual.getAttractionModel().getClass()
                .equals(NewtonianAttractionModel.class));
        Assert.assertTrue(neptuneActualAttractionModel.getAttractionModel().getClass()
            .equals(NewtonianAttractionModel.class));
        ...
        Assert.assertEquals(1.5 * mu, neptuneActualAttractionModel.getAttractionModel().getMu(), 0.);
        Assert.assertTrue(neptuneActual.getAttractionModel().getClass()
                .equals(NewtonianAttractionModel.class));
        Assert.assertTrue(neptuneActualAttractionModel.getAttractionModel().getClass()
            .equals(NewtonianAttractionModel.class));
        ...
        // Test GetNativeFrame
        Assert.assertEquals(pvProvider.getNativeFrame(null, null),
                neptuneActual.getNativeFrame(null, null));
        Assert.assertEquals(pvProvider.getNativeFrame(null, null),
            neptuneActualAttractionModel.getNativeFrame(null, null));
    }
    ...
    public void testPropagation() throws PatriusException, IOException {
        ...
        final Orbit orbit = new KeplerianOrbit(5000000, 0, 0, 0, 0, 0, PositionAngle.TRUE,
                mars.getInertialFrameConstantModel(), initialDate, mars.getGM());
        ...
        final CelestialBody marsAttractionModel = new UserCelestialBody("Mars", pvProvider, CelestialBodyFactory
            .getMars().getAttractionModel(), IAUPoleFactory.getIAUPole(EphemerisType.MARS),
            FramesFactory.getICRF(), CelestialBodyFactory.getMars().getShape());
    ...
    public void testToString() throws PatriusException {
        ...
        final CelestialBody body5 = new UserCelestialBody("My body", new MeeusMoon(), new NewtonianAttractionModel(
     ...
     * @testType UT
     * 
     * @description check that the getPVCoordinatesProvider() method of the UserCelestialBody
     *              returns the correct
     ...
     * @nonRegressionVersion 4.9
     * 
     * @throws PatriusException if the celestial body cannot be built or if solar system ephemerides
     *         cannot be loaded
     */
    @Test
    public void testGetPVCoordinatesProvider() throws PatriusException {
        ...
        final PVCoordinatesProvider pvProvider = new PVCoordinatesProvider() {
            /** Serializable UID. */
            private static final long serialVersionUID = -8014507977350961447L;
        ...
        final UserCelestialBody neptuneActual = new UserCelestialBody("Neptune", pvProvider,
                neptuneExpected.getGM(), IAUPoleFactory.getIAUPole(EphemerisType.NEPTUNE),
                FramesFactory.getICRF(), neptuneExpected.getShape());
        ...
        final UserCelestialBody neptuneActualAttractionModel = new UserCelestialBody("Neptune", pvProvider,
            ...
            FramesFactory.getICRF(), neptuneExpected.getShape());
        // Check getPVCoordinatesProvider()
        Assert.assertEquals(pvProvider, neptuneActualAttractionModel.getPVCoordinatesProvider());
    }
