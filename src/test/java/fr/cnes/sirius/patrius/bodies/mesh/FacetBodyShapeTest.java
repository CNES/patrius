...
public class FacetBodyShapeTest {
    ...
    public void intersectionGeodeticTest() throws PatriusException {
        ...
        final Vector3D position1 = new Vector3D(100E3, 200E3, 300E3);
        final Line lineOfSight1 = new Line(position1, Vector3D.ZERO);
        final GeodeticPoint actual1 = this.body.getIntersectionPoint(lineOfSight1, position1, frame, date);
        ...
        final Vector3D position2 = new Vector3D(100E3, 200E3, 300E3);
        final Line lineOfSight2 = new Line(position2, position2.scalarMultiply(2.));
        final GeodeticPoint actual2 = this.body.getIntersectionPoint(lineOfSight2, position2, frame, date);
        ...
        final Vector3D position3 = new Vector3D(10001, 0, 0);
        final Line lineOfSight3 = new Line(position3, position3.add(Vector3D.PLUS_J));
        final GeodeticPoint actual3 = this.body.getIntersectionPoint(lineOfSight3, position3, frame, date);
        Assert.assertNull(actual3);
        ...
        final Line lineOfSight4 = new Line(position4, position4.add(Vector3D.PLUS_I));
        final Frame bodyFrame = this.body.getBodyFrame();
        final GeodeticPoint actual4 = this.body.getIntersectionPoint(lineOfSight4, position4, bodyFrame, date);
        final GeodeticPoint reference4 = this.body.transform(position4, this.body.getBodyFrame(), date);
    ...
    public void neighborsTest() throws PatriusException {
        ...
        final List<Triangle> actual7 = this.body.getNeighbors(point7, 1500.);
        Assert.assertEquals(400, actual7.size());
        checkTriangles(actual7, point7, 1500);
        ...
        // Max distance = 27 order: only closest triangle is included
        final GeodeticPoint point12 = new GeodeticPoint(-FastMath.PI / 2., 1E-9, 10);

        final List<Triangle> actual12 = this.body.getNeighbors(point12, 0);
        Assert.assertEquals(1, actual12.size());
        checkTriangles(actual12, triangle, 27);
        ...
        final List<Triangle> actual18 = this.body.getNeighbors(point18, 1700.);
        Assert.assertEquals(500, actual18.size());
        checkTriangles(actual18, point18, 1700);

        // Max distance = 27 order: only closest triangle is included
        // Pos is exactly the same as point12.
        final Vector3D point19 = new Vector3D(6.1293572297325E-13, 6.129357229732501E-22, -10009.999999999996);
        final List<Triangle> actual19 = this.body.getNeighbors(point19, 0);
        Assert.assertEquals(1, actual19.size());
        checkTriangles(actual19, triangle, 27);
    ...
    public void contourTest() throws PatriusException {
        ...
        final List<GeodeticPoint> contour = actualBis.getContour();
        ...
        // Convert them in GeodeticPoint as the expected contour
        final List<GeodeticPoint> expectedContour = new ArrayList<>();
        for (final Vertex vertex : expectedVectex) {
            final GeodeticPoint point = this.body.transform(vertex.getPosition(), this.body.getBodyFrame(), null);
            expectedContour.add(point);
        }

        // Check that the computed coutour defines all the points in the expected one
        // Implementation note: as GeodeticPoint doesn't override equals, we can't use containAll as the points in the
        // two lists are stored in different instances
        Assert.assertEquals(expectedContour.size(), contour.size());
        for (final GeodeticPoint contour1 : contour) {
            boolean equals = false;
            for (final GeodeticPoint contour2 : expectedContour) {
                if (Math.abs(contour1.getLatitude() - contour2.getLatitude()) < EPS
                        && Math.abs(contour1.getLongitude() - contour2.getLongitude()) < EPS
                        && Math.abs(contour1.getAltitude() - contour2.getAltitude()) < EPS) {
                    equals = true;
                    break;
                }
            }
    ...
    public void resizeTest() throws PatriusException {
        ...
        final StarConvexFacetBodyShape body2 = this.body.resize(MarginType.DISTANCE, marginValue2);
        ...
        final StarConvexFacetBodyShape body3 = this.body.resize(MarginType.DISTANCE, marginValue3);
        ...
        final StarConvexFacetBodyShape body4 = this.body.resize(MarginType.DISTANCE, marginValue4);
        ...
        final StarConvexFacetBodyShape body6 = this.body.resize(MarginType.SCALE_FACTOR, marginValue6);
        ...
        Assert.assertEquals(this.body.getEllipsoid(EllipsoidType.INNER_ELLIPSOID).getFlattening(),
            body6.getEllipsoid(EllipsoidType.INNER_ELLIPSOID).getFlattening(),
            EPS);
        Assert.assertEquals(this.body.getEllipsoid(EllipsoidType.OUTER_ELLIPSOID).getEquatorialRadius() * marginValue6,
            body6
                .getEllipsoid(EllipsoidType.OUTER_ELLIPSOID).getEquatorialRadius());
        Assert.assertEquals(this.body.getEllipsoid(EllipsoidType.OUTER_ELLIPSOID).getFlattening(),
            body6.getEllipsoid(EllipsoidType.OUTER_ELLIPSOID).getFlattening(),
            EPS);
        // Case with a positive margin scale factor larger than 1
        final double marginValue7 = 2.;
        final StarConvexFacetBodyShape body7 = this.body.resize(MarginType.SCALE_FACTOR, marginValue7);
        ...
        Assert.assertEquals(this.body.getEllipsoid(EllipsoidType.INNER_ELLIPSOID).getFlattening(),
            body7.getEllipsoid(EllipsoidType.INNER_ELLIPSOID).getFlattening(),
            EPS);
        ...
        Assert.assertEquals(this.body.getEllipsoid(EllipsoidType.OUTER_ELLIPSOID).getFlattening(),
            body7.getEllipsoid(EllipsoidType.OUTER_ELLIPSOID).getFlattening(),
            EPS);
    ...
    public void closestPointToTest() throws PatriusException {
        ...
        final Vector3D[] actual3 = this.body.closestPointTo(lineOfSight3, this.body.getBodyFrame(), date);
    ...
    public void closestPointToNoFrameNoDateTest() throws PatriusException {
        ...
        final Vector3D position1 = new Vector3D(10E3, 20E3, 30E3);
        final Line lineOfSight1 = new Line(position1, Vector3D.ZERO);
        final Vector3D[] actual1 = this.body.closestPointTo(lineOfSight1);
        final Vector3D[] reference1 = this.body.closestPointTo(lineOfSight1, frame, date);
        Assert.assertEquals(0., Vector3D.distance(reference1[0], actual1[0]) / reference1[0].getNorm(), 1E-3);
        Assert.assertEquals(0., Vector3D.distance(reference1[1], actual1[1]) / reference1[1].getNorm(), 1E-3);
        ...
        // shall be the same)
        final Line lineOfSight2 = new Line(position1, Vector3D.ZERO, position1);
        final Vector3D[] actual2 = this.body.closestPointTo(lineOfSight2);
        final Vector3D[] reference2 = this.body.closestPointTo(lineOfSight2, frame, date);
        Assert.assertEquals(0., Vector3D.distance(reference2[0], actual2[0]) / reference2[0].getNorm(), 1E-3);
        Assert.assertEquals(0., Vector3D.distance(reference2[1], actual2[1]) / reference2[1].getNorm(), 1E-3);
        ...
        final Line lineOfSight3 = new Line(Vector3D.ZERO, position2, position2);
        final Vector3D[] actual3 = this.body.closestPointTo(lineOfSight3, this.body.getBodyFrame(), date);
        Assert.assertEquals(0., reference3.subtract(actual3[1]).getNorm(), 0.);
    }
