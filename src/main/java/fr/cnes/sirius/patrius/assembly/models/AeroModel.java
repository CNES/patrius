...
public final class AeroModel extends Parameterizable implements DragSensitive {
    ...
    /**
     * Aero model (the acceleration is computed from all the sub parts of the vehicle) with an {@link Atmosphere} model
     * and a {@link OneAxisEllipsoid} for acceleration partial derivatives computation with respect to state by finite
    ...
    /**
     * This method tests if the required properties exist (AERO_CROSS_SECTION or AERO_FACET). At least one part with a
     * mass is required. It also store jacobians parameters handled in the parts
     *
     * @param assemblyIn
     *        The considered vehicle.
     */
    ...
    private void getCoefsParameters(final IPart part) {
        ...
        final IParamDiffFunction aeroTangentialCoefFunction =
            ((AeroFacetProperty) part.getProperty(PropertyType.AERO_FACET)).getTangentialCoef();
        // adding parameters to internal map
    ...
    private void getDragForceParameters(final IPart part) {
        final IParamDiffFunction aeroDragForceFunction =
            ((AeroCrossSectionProperty) part.getProperty(PropertyType.AERO_CROSS_SECTION)).getDragForce();
     ...
     */
    protected static Vector3D forceOnFacet(final SpacecraftState state, final IPart part, final Assembly assembly,
        ...
        if (relativeVelocity.getNorm() > Precision.EPSILON) {
            ...
            if (orientation < 0.) {
                ...
                final double coeff = HALF * density * velonorm * velonorm * fArea * cosAlpha;
                // Always positive due to the angle value
                final double normalForceNorm = coeff * cNorm * cosAlpha;
                // Always negative due to the angle value
                final double tangentForceNorm = coeff * cTang * sinAlpha;
     ...
     */
    protected static Vector3D forceOnSphere(final SpacecraftState state, final IPart part, final double density,
                                            final Vector3D relativeVelocity, final Frame mainPartFrame)
        throws PatriusException {
    ...
    public void addDDragAccDParam(final SpacecraftState s, final Parameter param, final double density,
        ...
        for (final IPart part : this.aeroFacetParts) {
            ...
            dAdT = computeDAccDParamFacet(s, param, relativeVelocity, coeff, part);
     ...
     *         orekit frame exception
     */
    private double[] computeDAccDParamFacet(final SpacecraftState s, final Parameter param,
        ...
        if (orientation < 0.) {
            ...
            if (aeroNormalCoefFunction.supportsParameter(param)) {
                ...
                final double normalDerivativeValue = ((AeroFacetProperty) part.getProperty(PropertyType.AERO_FACET))
                    .getNormalCoef().derivativeValue(param, s);
                ...
                final double tangentialDerivativeValue = ((AeroFacetProperty) part.getProperty(PropertyType.AERO_FACET))
                    .getTangentialCoef().derivativeValue(param, s);
    ...
    public void addDDragAccDState(final SpacecraftState s, final double[][] dAccdPos, final double[][] dAccdVel,
        ...
        for (final IPart part : this.aeroSphereParts) {
            ...
            computeDAccSphere(s, density, acceleration, relativeVelocity, dAdPos, dAdVel, part,
        ...
        for (final IPart part : this.aeroFacetParts) {
            ...
            if (computeGradientVelocity) {
                ...
                computeDAccFacet(s, density, acceleration, relativeVelocity, dAdPos, dAdVel, part);
            }
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    // add new position and new velocities
     ...
     *        double array
     */
    private static void addZeroToArray(final double[][] dAd) {
    ...
    private void computeDAccFacet(final SpacecraftState s, final double density, final Vector3D acceleration,
        ...
        if (orientation < 0.) {
            ...
            if (tangentAccNorm > Precision.EPSILON) {
                ...
                // vector elements
                dTdV = computeTangentVectDerivativeWRTvelocity(dCosAdV, vx, vy, vz, cosA, vRelNorm, fNorm);
            }
            ...
            final Vector3D v1Sat = new Vector3D(dAdDt.getX(), this.tangentVector)
                .add(new Vector3D(dAdDn.getX(), fNorm)).subtract(dtdv1);
            final Vector3D v2Sat = new Vector3D(dAdDt.getY(), this.tangentVector)
                .add(new Vector3D(dAdDn.getY(), fNorm)).subtract(dtdv2);
            final Vector3D v3Sat = new Vector3D(dAdDt.getZ(), this.tangentVector)
                .add(new Vector3D(dAdDn.getZ(), fNorm)).subtract(dtdv3);
    ...
    private double calculateTangentAccNorm(final Vector3D dAdDt, final Vector3D relativeVelocitySat,
        ...
        // If the norm for the tangent vector is big enough, we try to compute the tangent vector.
        if (dAdDt.getNorm() > Precision.EPSILON) {
    ...
    private Vector3D[] computeTangentVectDerivativeWRTvelocity(final double[] dCosAdV, final double vx,
                                                               ...
                                                               final double cosA, final double vRelNorm,
                                                               final Vector3D fNorm) {

        // common parameters
        this.vCosA = vRelNorm * cosA;
        this.vCosA2 = vRelNorm * cosA * cosA;
        this.v3CosA = vRelNorm * vRelNorm * vRelNorm * cosA;
        this.tRelNorm = MathLib.sqrt((fNorm.getX() - MathLib.divide(vx, this.vCosA))
                * (fNorm.getX() - MathLib.divide(vx, this.vCosA))
        ...
        final double dXtdXv = computDE(dCosAdV[0], vx, vx, v, this.tangentVector.getX(), lastTermXX);

        // derivative dXt / dYv
        final double[] lastTermXY = { 0.0, 0.0, 1.0, 0.0 };
        final double dXtdYv = computDE(dCosAdV[1], vx, vy, v, this.tangentVector.getX(), lastTermXY);

        // derivative dXt / dZv
        final double[] lastTermXZ = { 0.0, 0.0, 0.0, 1.0 };
        final double dXtdZv = computDE(dCosAdV[2], vx, vz, v, this.tangentVector.getX(), lastTermXZ);

        // derivative dYt / dXv
        final double[] lastTermYX = { 0.0, 1.0, 0.0, 0.0 };
        final double dYtdXv = computDE(dCosAdV[0], vy, vx, v, this.tangentVector.getY(), lastTermYX);

        // derivative dYt / dYv
        final double[] lastTermYY = { 1.0, 0.0, 1.0, 0.0 };
        final double dYtdYv = computDE(dCosAdV[1], vy, vy, v, this.tangentVector.getY(), lastTermYY);

        // derivative dYt / dZv
        final double[] lastTermYZ = { 0.0, 0.0, 0.0, 1.0 };
        final double dYtdZv = computDE(dCosAdV[2], vy, vz, v, this.tangentVector.getY(), lastTermYZ);

        // derivative dZt / dXv
        final double[] lastTermZX = { 0.0, 1.0, 0.0, 0.0 };
        final double dZtdXv = computDE(dCosAdV[0], vz, vx, v, this.tangentVector.getZ(), lastTermZX);

        // derivative dZt / dYv
        final double[] lastTermZY = { 0.0, 0.0, 1.0, 0.0 };
        final double dZtdYv = computDE(dCosAdV[1], vz, vy, v, this.tangentVector.getZ(), lastTermZY);

        // derivative dZt / dZv
        final double[] lastTermZZ = { 1.0, 0.0, 0.0, 1.0 };
        final double dZtdZv = computDE(dCosAdV[2], vz, vz, v, this.tangentVector.getZ(), lastTermZZ);
    ...
    private double computDE(final double cosDeriv, final double v1, final double v2, final double[] v,
                            final double tangentVEctEl,
                            final double[] lastTerm) {

        // creation of each term of the sum
        final double de1 =
            MathLib.divide(v1 * cosDeriv, this.vCosA2) + MathLib.divide(v1 * v2, this.v3CosA)
                    - MathLib.divide(lastTerm[0], this.vCosA);
        final double de2 =
            MathLib.divide(v[0] * cosDeriv, this.vCosA2) + MathLib.divide(v[0] * v2, this.v3CosA)
                    - MathLib.divide(lastTerm[1], this.vCosA);
        final double de3 =
            MathLib.divide(v[1] * cosDeriv, this.vCosA2) + MathLib.divide(v[1] * v2, this.v3CosA)
                    - MathLib.divide(lastTerm[2], this.vCosA);
        final double de4 =
            MathLib.divide(v[2] * cosDeriv, this.vCosA2) + MathLib.divide(v[2] * v2, this.v3CosA)
                    - MathLib.divide(lastTerm[3], this.vCosA);

        // one derivative element
        return MathLib.divide(de1, this.tRelNorm)
                - MathLib.divide(
                    (de2 * this.tangentVector.getX() + de3 * this.tangentVector.getY() + de4
                            * this.tangentVector.getZ())
                            * tangentVEctEl,
                    this.tRelNorm);
    }

    /**
     * Compute the partial derivatives with respect to position and velocity for the part (cylinder, parallelepiped,
     * sphere).
     *
     * @param s
     *        the spacecraft state.
     ...
     */
    private void computeDAccSphere(final SpacecraftState s, final double density, final Vector3D acceleration,
                                   final Vector3D relativeVelocity, final double[][] dAdPos, final double[][] dAdVel,
                                   final IPart part, final boolean computeGradientPosition,
                                   final boolean computeGradientVelocity)
        throws PatriusException {
        ...
        if (computeGradientPosition) {
            ...
            final Vector3D posNewAlt = position.scalarMultiply(1.0 + MathLib.divide(this.altitudeStep,
                position.getNorm()));
            ...
            if (!computeGradientVelocity) {
                ...
                throw PatriusException
                    .createIllegalArgumentException(PatriusMessages.PDB_AERO_DERIVATIVES_COMPUTATION_ERROR);
            }
