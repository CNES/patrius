...
public class RFAntennaProperty implements IPartProperty {
     ...
     *        antenna pattern values of the polar angle (1 x N); this array can't contain 2 values
     *        (but it can
     ...
     *        antenna pattern values of the azimuthal angle (1 x M); this array can't contain 2
     *        values (but it can
     *        contain only one value) because of the spline interpolation: at least 3 values are
     *        required.<br>
    ...
    public RFAntennaProperty(final double inOutputPower, final double[] inPatternPolarAngle,
            ...
            final double[][] inEllipticityFactor, final double inTechnoLoss,
            final double inCircuitLoss, final double inBitRate, final double inFrequency) {
        ...
        checkInputArrays(inGainPattern, inEllipticityFactor, polLength, azLength);
        ...
        this.interpFunctionGain = computeInterpolatingFunction(this.patternPolarAngle,
                this.patternAzimuth, inGainPattern.clone());
        this.interpFunctionEllipticity = computeInterpolatingFunction(this.patternPolarAngle,
                this.patternAzimuth, inEllipticityFactor.clone());
     ...
     */
    private static void checkInputArrays(final double[][] gainPatternIn,
            final double[][] ellipticityFactorIn, final int polLength, final int azLength) {
        ...
        if ((polLength != gainLengthX) || (polLength != ellLengthX)) {
            ...
            if (polLength == gainLengthX) {
                throw new DimensionMismatchException(PatriusMessages.DIMENSIONS_MISMATCH,
                        polLength, ellLengthX);
            }
            throw new DimensionMismatchException(PatriusMessages.DIMENSIONS_MISMATCH, polLength,
                    gainLengthX);
        }
        // check the rows dimension of the matrices:
        for (int j = 0; j < polLength; j++) {
            if ((gainPatternIn[j].length != azLength)
                    || (ellipticityFactorIn[j].length != azLength)) {
                // the y-dimensions do not correspond:
                if (azLength == gainPatternIn[j].length) {
                    throw new DimensionMismatchException(PatriusMessages.DIMENSIONS_MISMATCH,
                            azLength, ellipticityFactorIn[j].length);
                }
                throw new DimensionMismatchException(PatriusMessages.DIMENSIONS_MISMATCH, azLength,
                        gainPatternIn[j].length);
            }
        }
    }

    /**
     * Computes the bivariate function interpolating the input values. The interpolation is done
     * using splines.
     * 
     * @param polarAngle
     *        the polar angle
     ...
     */
    private static BivariateFunction computeInterpolatingFunction(final double[] polarAngle,
            final double[] azimuth, final double[][] values) {
        ...
        if (na >= 2 && np >= 2) {
            ...
            final UnivariateFunction univariateFunction = new LinearInterpolator().interpolate(
                    azimuth, vector);
            function = new BivariateFunction() {
                /** Serializable UID. */
                private static final long serialVersionUID = -8180685511484231248L;
            ...
            function = new BivariateFunction() {
                /** Serializable UID. */
                private static final long serialVersionUID = -2652658049450115668L;
            ...
            function = new BivariateFunction() {
                /** Serializable UID. */
                private static final long serialVersionUID = 6199509861643527370L;
    ...
    public final double getGain(final double polarAngle, final double azimuth) {
        ...
        if ((polarAngle < 0 || polarAngle > FastMath.PI)
                || (azimuth < 0 || azimuth > 2 * FastMath.PI)) {
            throw new MathIllegalArgumentException(PatriusMessages.PDB_ANGLE_OUTSIDE_INTERVAL);
        }
        final double pol = checkInputAngles(polarAngle, this.patternPolarAngle);
        final double az = checkInputAngles(azimuth, this.patternAzimuth);
        return this.interpFunctionGain.value(pol, az);
    }

    /**
     * Gets the factor of ellipticity using a spline interpolation. <br>
     * The ellipticity factor is a function of the station direction (ellipticity = F(polarAngle,
     * azimuth))
    ...
    /**
     * Verifies if the input angle is contained in the array of angles; if not, returns the value of
     * the
     ...
     */
    private static double checkInputAngles(final double angle, final double[] array) {
