...
public final class SensorModel implements PVCoordinatesProvider {
     ...
     *         (if the assembly is not linked to the tree of frame in witch the target is defined)
     */
    public double getInhibitionTargetAngularRadius(final AbsoluteDate inhibitionDate,
            final AbsoluteDate sensorDate,
...
//        final BasicPVCoordinatesProvider pvProv = new BasicPVCoordinatesProvider(pvDir, sensorFrame);
        ...
        final PVCoordinatesProvider occultedBody = new PVCoordinatesProvider() {
            ...
            public PVCoordinates getPVCoordinates(final AbsoluteDate date,
                ...
                final PVCoordinates pvInhibition = inhibitionTargets[inhibitionFieldNumber - 1].getPVCoordinates(date,
                        frame);
            ...
            public Frame getNativeFrame(final AbsoluteDate date,
                    final Frame frame) throws PatriusException {
                return inhibitionTargets[inhibitionFieldNumber - 1].getNativeFrame(date, frame);
            }
        };
     ...
     * @param propagationDelayType propagation delay type
     * @param linkType link type (uplink or downlink)
     * @return the minimal distance to the spacecraft's shapes
     * @throws PatriusException
     *         if a problem occurs in frames transformations
     */
    public double spacecraftsMaskingDistance(final AbsoluteDate spacecraftDate,
        ...
        for (int i = 0; i < this.maskingAssemblies.size(); i++) {
            final AbsoluteDate assemblyDate = getPartDate(spacecraftDate, propagationDelayType,
                    linkType, maskingAssemblies.get(i).getPropagator(), maskingAssemblies.get(i).getPropagator()
                            .getFrame());
     ...
     * @param pvProvider PV coordinates provider
     * @param pvProviderFrame PV coordinates provider reference frame
     * @return part date
    ...
    private AbsoluteDate getPartDate(final AbsoluteDate spacecraftDate,
            ...
            final Frame pvProviderFrame) throws PatriusException {
     ...
     *         if a problem occurs in frames transformations
     */
    public double celestialBodiesMaskingDistance(final AbsoluteDate spacecraftDate,
            final AbsoluteDate targetDate,
            final PropagationDelayType propagationDelayType,
            final LinkType linkType) throws PatriusException {
        ...
        if (maskingBodiesNumber > 0) {
            ...
            for (int i = 0; i < maskingBodiesNumber; i++) {
                ...
                if (propagationDelayType.equals(PropagationDelayType.LIGHT_SPEED)) {
                    // Signal propagation frame
                    final Frame nativeFrameBody = maskingBodies.get(i).getNativeFrame(spacecraftDate,
                            maskingBodies.get(i).getBodyFrame());
                    final Frame nativeFrameSensor = getNativeFrame(spacecraftDate, null);
                    final Frame bodyInertialFrame = nativeFrameBody
                            .getFirstCommonPseudoInertialAncestor(nativeFrameSensor);
                    // Handle link type
                    if (linkType.equals(LinkType.DOWNLINK)) {
                        bodyDate = VacuumSignalPropagationModel.getSignalReceptionDate(maskingBodies.get(i),
                                this, spacecraftDate, this.epsSignalPropagation, propagationDelayType,
                                bodyInertialFrame);
                    } else {
                        bodyDate = VacuumSignalPropagationModel.getSignalEmissionDate(maskingBodies.get(i), this,
    ...
    private static double
            maskingDistFromShape(final Line line, final Transform trans,
    ...
    private static double bodyShapeMaskingDistance(final BodyShape body, final AbsoluteDate date,
            final Line line, final Frame sensorFrame, final double targetDistance, final Vector3D targetInSensorFrame)
            throws PatriusException {
