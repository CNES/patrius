...
/**
 * Aerodynamic coefficient function of the spacecraft angle of attack. Aerodynamic coefficient is then retrieved by
 * linear interpolation of an <angle of attack, aerodynamic coefficient> array.
 *
 * @author Marie Capot
 *
 * @version $Id$
...
public class AeroCoeffByAoA extends AbstractAeroCoeff1D {
     ...
     *
     * @param xVariables
     *        Array of x variables (angle of attack (rad))
     * @param yVariables
    ...
    public static double angleOfAttackFromSpacecraftState(final SpacecraftState state,
        ...
        final EllipsoidPoint point = earthShape.buildPoint(state.getPVCoordinates().getPosition(), state.getFrame(),
            state.getDate(), BodyPointName.DEFAULT);
        final TopocentricFrame topocentricFrame = new TopocentricFrame(point, 0, "TopoPATRIUS");

        // JPSIMU topocentric frame.
        final Transform topoPatriusToTopoPSIMU = new Transform(state.getDate(), new Rotation(RotationOrder.XYZ,
            FastMath.PI, 0, 0));
        // Topocentric frame
        final Frame topocentricFramePsimu = new Frame(topocentricFrame, topoPatriusToTopoPSIMU, "TopoPSimu");

        // Azimuth and elevation of the velocity in body frame
        final CartesianParameters carPar = new CartesianParameters(state.getPVCoordinates(earthShape.getBodyFrame()),
            state.getMu());
        final ReentryParameters reentryPar = carPar.getReentryParameters(earthShape.getEquatorialRadius(),
            earthShape.getFlattening());
        ...
        // Yaw, pitch, roll
        final double[] yawPitchRoll =
            state.getAttitude().withReferenceFrame(topocentricFramePsimu).getRotation().getAngles(RotationOrder.ZYX);

        // Rotation Aircraft-carried normal earth axis system frame -> Aircraft frame
        final Rotation earthToAircraft = new Rotation(RotationOrder.ZYX, yawPitchRoll[0], yawPitchRoll[1],
            yawPitchRoll[2]);
