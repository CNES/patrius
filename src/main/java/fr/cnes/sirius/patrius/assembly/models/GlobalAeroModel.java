...
public final class GlobalAeroModel implements DragSensitive {
    ...
    public GlobalAeroModel(final Assembly assemblyIn, final DragCoefficientProvider dragCoefficientProviderIn,
        ...
        this.checkProperties(assemblyIn);
    ...
    public
        ...
        if (computeGradientPosition) {
            // dPx
            final SpacecraftState stateXp = this.shiftState(s, new Vector3D(this.hPos, Vector3D.PLUS_I), Vector3D.ZERO);
            final SpacecraftState stateXm =
                this.shiftState(s, new Vector3D(this.hPos, Vector3D.MINUS_I), Vector3D.ZERO);
            ...
            // dPy
            final SpacecraftState stateYp = this.shiftState(s, new Vector3D(this.hPos, Vector3D.PLUS_J), Vector3D.ZERO);
            final SpacecraftState stateYm =
                this.shiftState(s, new Vector3D(this.hPos, Vector3D.MINUS_J), Vector3D.ZERO);
            ...
            // dPz
            final SpacecraftState stateZp = this.shiftState(s, new Vector3D(this.hPos, Vector3D.PLUS_K), Vector3D.ZERO);
            final SpacecraftState stateZm =
                this.shiftState(s, new Vector3D(this.hPos, Vector3D.MINUS_K), Vector3D.ZERO);
    ...
    public DragSensitive copy(final Assembly newAssembly) {
        return new GlobalAeroModel(newAssembly, dragCoefficientProvider, atmosphere, hPos);
    }
