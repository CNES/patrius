 ...
 * @version $Id$
 */
public class GinsWallGasTemperature implements WallGasTemperatureProvider {

    /** Serializable UID. */
    private static final long serialVersionUID = 1346973727259864386L;
    ...
    /** Surface temperature square root (square root stored for optimization). */
     ...
     *        surface temperature
     */
    public GinsWallGasTemperature(final ExtendedAtmosphere atmos, final AlphaProvider alpha, final double surfaceTemp) {
    ...
    public double getWallGasTemperature(final SpacecraftState state, final Vector3D relativeVelocity,
        ...
        try {
            ...
            final double s = theta > 0 ? MathLib.sqrt(vrelnorm2 / (2. * r * tAtmo)) : 1.;
            final double alpha = this.alphaProvider.getAlpha(state);

            // Compute temperature
            final double sqrtRes =
                this.sqrtSurfaceTemperature + MathLib.sqrt(1. - alpha) * (MathLib.max(s * sintheta, 1.)
                    * MathLib.sqrt(tAtmo) - this.sqrtSurfaceTemperature);
