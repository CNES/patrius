...
public class SaastamoinenModel implements TroposphericCorrection {
     ...
     * @deprecated as of release 4.11, better use {@link #SaastamoinenModel(MeteorologicalConditionsProvider, double)}.
     */
    @Deprecated
    public SaastamoinenModel(final double temperature, final double pressure,
                             final double humidity, final double altitude) {
        this(new ConstantMeteorologicalConditionsProvider(new MeteorologicalConditions(pressure,
     ...
     *        the altitude above sea level of the station [m]
     */
    public SaastamoinenModel(final MeteorologicalConditionsProvider meteoConditionsProvider,
        ...
        this.b = Functions.INSTANCE.b.value(altitude / M_TO_KM);
    }
    ...
    public static SaastamoinenModel getStandardModel(final double altitude) {
        final MeteorologicalConditions standardMeteoConditions = MeteorologicalConditions
            .computeStandardValues(altitude);
        final MeteorologicalConditionsProvider standardMeteoConditionsProvider =
            new ConstantMeteorologicalConditionsProvider(standardMeteoConditions);

        return new SaastamoinenModel(standardMeteoConditionsProvider, altitude);
    }
     ...
     * @deprecated as of release 4.11, better use {@link #calculatePathDelay(AbsoluteDate, double)}. A
    ...
    public double calculatePathDelay(final double elevation) {
        return this.calculatePathDelay(new AbsoluteDate(), elevation);
    }
     ...
     * 
     * @param date
     *        date of the meteorological conditions
     * @param elevation
     *        the elevation of the satellite in radians
     * @return the path delay due to the troposphere in m
     */
    public double calculatePathDelay(final AbsoluteDate date, final double elevation) {
        ...
        final double deltaR = getDeltaR(this.altitudeStation, z);
        ...
        final double pressure = meteoConditionsAtDate.getPressure() * PA_TO_MBAR;
        final double temperature = meteoConditionsAtDate.getTemperature();
        // humidity [0; 1]
        final double humidity = meteoConditionsAtDate.getHumidity() * PERCENTAGE_TO_FRACTION;

        // Implementation note: kept in milibar to use the constants of the article
        final double waterVaporPartialPressure = humidity * MathLib.exp(Functions.INSTANCE.e.value(temperature));

        // Calculate the path delay in m - Equation (5.98)
        final double tan = MathLib.tan(z);
        return 2.277e-3 / MathLib.cos(z) *
                (pressure + (1255d / temperature + 5e-2) * waterVaporPartialPressure - this.b * tan * tan) + deltaR;
    }
     ...
     * @deprecated as of release 4.11, better use {@link #computeSignalDelay(AbsoluteDate, double)}. A
     *             date has to be provided for the computations of tropospheric effects.
     */
    @Override
    @Deprecated
    public double computeSignalDelay(final double elevation) {
        ...
        return this.computeSignalDelay(defaultDate, elevation);
    }

    /**
     * Calculates the tropospheric signal delay for the signal path from a ground station to a
     * satellite at a given date. This method exists only for convenience reasons and returns the same as
     * 
     * <pre>
     *   {@link SaastamoinenModel#calculatePathDelay(double)}/
     *   {@link fr.cnes.sirius.patrius.utils.Constants#SPEED_OF_LIGHT}
    ...
    public double computeSignalDelay(final AbsoluteDate date, final double elevation) {
        return this.calculatePathDelay(date, elevation) / Constants.SPEED_OF_LIGHT;
    }
     ...
     */
    private static double getDeltaR(final double height, final double zenith) {
