...
public class BentModel implements IonosphericCorrection {
     ...
     * @throws PatriusException if point cannot be converted to body frame
     */
    public BentModel(final R12Provider r12Provider, final SolarActivityDataProvider solarActivity,
                     final USKProvider uskProvider, final BodyShape earth, final Vector3D station, final Frame frameSta)
        throws PatriusException {
        this(r12Provider, solarActivity, uskProvider, new TopocentricFrame(earth, earth.transform(station, frameSta,
            null), "earth"));
    }
    ...
    public BentModel(final R12Provider r12Provider, final SolarActivityDataProvider solarActivity,
        ...
        this.flat = stationTopoFrame.getPoint().getLatitude();
        // Normalization on [0;2PI] as in the original algorithm
        this.flon = MathUtils.normalizeAngle(stationTopoFrame.getPoint().getLongitude(), FastMath.PI);
    }
    ...
    /** {@inheritDoc} */
    @Override
    public double computeSignalDelay(final double freq, final AbsoluteDate date,
                                     final Vector3D satellite, final Frame frameSat)
        throws PatriusException {
     ...
     */
    public double computeElectronicCont(final AbsoluteDate date,
                                        final Vector3D satellite, final Frame frameSat)
        throws PatriusException {
        ...
        final double rsat = MathLib.sqrt(rat * rat + dist * dist
                + 2.0 * rat * dist * MathLib.sin(elev));
        ...
        if (lat1 != lat2) {
            cnt = cnt + MathLib.divide((CENT[lat2] - CENT[lat1]) * (DG[lat1] - hlat),
                (DG[lat1] - DG[lat2]));
        }
     ...
     * @return om
     */
    private static double uskf(final int[] kd,
     ...
     *        g
     */
    private static void completeAs(final double[] initAs, final int m, final double[][] d,
     ...
     *        g
     */
    private static void completeBs(final double[] initBs, final int m, final double[][] d,
                                   final int nff, final double[] gG) {
     ...
     * @return modified YM
     */
    private static double computeYM(final double ymIn, final int if1, final int if2, final double t1,
                                    final int lt1, final int lt2, final double fof2) {
        double ym = ymIn;
        if (if1 != if2) {
     ...
     *        flux
     */
    private static void fillXK(final int j, final int i1,
     ...
     * @return XNTNM
     */
    private static double computeXNTNM(final double ym, final double[] xk,
                                       final double yt, final double hm, final double hs, final double distIn) {
        // initializations
