...
public class MariniMurrayModel implements TroposphericCorrection {
     ...
     *             {@link #MariniMurrayModel(MeteorologicalConditionsProvider, double, double, double)}.
     */
    // CHECKSTYLE: stop MagicNumber check
    // Reason: numerous empirical model parameters
    @Deprecated
    public MariniMurrayModel(final double pressure, final double phi, final double humidity,
                             final double temperature, final double lambda, final double alt) {

        this(new ConstantMeteorologicalConditionsProvider(new MeteorologicalConditions(pressure,
            temperature, humidity)), phi, lambda, alt);
    ...
    public double computeSignalDelay(final double elevation) {
        ...
        return computeSignalDelay(defaultDate, elevation);
    }
