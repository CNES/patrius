...
public class MariniMurrayModel implements TroposphericCorrection {
    ...
    public MariniMurrayModel(final double p0,
        ...
        // Computation of a
        a = 0.002357 * p0Millibar + 0.000141 * e0;

        // Computation of b
        final double bCoeff = 2.0 * MathLib.pow(p0Millibar, 2) / (t0 * (3.0 - 1.0 / k));
        b = 1.084e-8 * p0Millibar * t0 * k + 4.734e-8 * bCoeff;

        // Computation of fLambda
        final double lambdaMicrons = lambda / NM_TO_MICROM;
        fLambda = 0.9650 + 0.0164 / MathLib.pow(lambdaMicrons, 2) + 0.000228 / MathLib.pow(lambdaMicrons, 4);

        // Computation of fPhiH
        final double altKM = alt / M_TO_KM;
        fPhiH = 1 - 0.0026 * cos2Phi - 0.00031 * altKM;
    ...
    public double computeSignalDelay(final double elev) {
        ...
        final double deltaR1 = fLambda / fPhiH;
    ...
    public int hashCode() {
        ...
        result = prime * result + (int) (temp ^ temp >>> 32);
        temp = Double.doubleToLongBits(fLambda);
        result = prime * result + (int) (temp ^ temp >>> 32);
        temp = Double.doubleToLongBits(fPhiH);
        result = prime * result + (int) (temp ^ temp >>> 32);
        // Return result
        return result;
    }
    ...
    public boolean equals(final Object obj) {
        ...
        if (this == obj) {
            ...
            res &= a == other.a;
            res &= b == other.b;
            res &= fLambda == other.fLambda;
            res &= fPhiH == other.fPhiH;
        } else {
            // Different classes
            res = false;
        }
