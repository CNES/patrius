 ...
 * 
 * <p>
 * Each time the integrator proposes a step, the event handler switching function should be checked. This class handles
 * the state of one handler during one integration step, with references to the state at the end of the preceding step.
 * This information is used to decide if the handler should trigger an event or not during the proposed step.
 * </p>
 * 
...
public class EventState {
     ...
     *        Root-finding algorithm to use to detect state events
     */
    public EventState(final EventHandler handlerIn,
        final double maxCheckIntervalIn,
        final double convergenceIn,
        final int maxIterationCountIn,
     ...
     */
    public void storeState(final double t,
                           final double[] y,
     ...
     */
    public boolean evaluateStep(final double t,
                                final double[] y) {
    ...
    public boolean evaluateStep(final StepInterpolator interpolator) {
        ...
        try {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                final boolean eventAtFirstStep = (this.g0 == 0) && (t00 == this.initialDate) && (ta == t00);
                    ...
                    if (slope == 2 || ((this.forward ^ !this.increasing) ^ slope == 1)) {
                        ...
                        if ((ga2 >= 0) ^ (gb >= 0)) {
                            if (this.solver instanceof BracketedUnivariateSolver<?>) {
                                ...
                                root = this.forward ? bracketing.solve(this.maxIterationCount, f, ta, tb,
                                    AllowedSolution.RIGHT_SIDE) : bracketing.solve(
                                    this.maxIterationCount, f, tb, ta, AllowedSolution.LEFT_SIDE);
                            } else {
                                final double baseRoot = this.forward ? this.solver.solve(this.maxIterationCount,
                                    f, ta, tb) : this.solver.solve(this.maxIterationCount, f, tb, ta);
                                final int remainingEval = this.maxIterationCount
                                    - this.solver.getEvaluations();
                                final BracketedUnivariateSolver<UnivariateFunction> bracketing =
                                    new PegasusSolver(this.solver.getRelativeAccuracy(),
                                        this.solver.getAbsoluteAccuracy());
                                root = this.forward ? UnivariateSolverUtils.forceSide(remainingEval, f,
                                    bracketing, baseRoot, ta, tb, AllowedSolution.RIGHT_SIDE)
                                    : UnivariateSolverUtils.forceSide(remainingEval, f,
                                        bracketing, baseRoot, tb, ta,
                        ...
                        if ((!Double.isNaN(this.previousEventTime))
                            ...
                            ta = this.forward ? ta + this.stepConvergence : ta - this.stepConvergence;
                            ga = f.value(ta);
     ...
     */
    public void stepAccepted(final double t,
                             final double[] y) {
     ...
     */
    public boolean reset(final double t,
                         final double[] y) {
