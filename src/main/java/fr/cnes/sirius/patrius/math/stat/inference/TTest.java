 ...
 * methods return t-statistics. Among the "Test" methods, <code>double-</code>valued methods return p-values;
 * <code>boolean-</code>valued methods perform fixed significance level tests. Significance levels are always specified
 * as numbers between 0 and 0.5 (e.g. tests at the 95% level use <code>alpha=0.05</code>).
 * </p>
 * <p>
 * Input to tests can be either <code>double[]</code> arrays or {@link StatisticalSummary} instances.
 * </p>
 * <p>
 * Uses commons-math {@link fr.cnes.sirius.patrius.math.distribution.TDistribution} implementation to estimate exact
 * p-values.
 * </p>
 * 
 * @version $Id: TTest.java 18108 2017-10-04 06:45:27Z bignon $
 */
...
public class TTest {
     ...
     * computing the one-sample t-statistic {@link #t(double, double[])}, with <code>mu = 0</code> and the sample array
    ...
    public double pairedT(final double[] sample1, final double[] sample2) {
        ...
        return this.t(meanDifference, 0,
     ...
     * of the paired differences is 0 in favor of the two-sided alternative that the mean paired difference is not equal
     * to 0. For a one-sided test, divide the returned value by 2.
     * </p>
     * <p>
     * This test is equivalent to a one-sample t-test computed using {@link #tTest(double, double[])} with
     * <code>mu = 0</code> and the sample array consisting of the signed differences between corresponding elements of
    ...
    public double pairedTTest(final double[] sample1, final double[] sample2) {
        ...
        final double meanDifference = StatUtils.meanDifference(sample1, sample2);
        return this.tTest(meanDifference, 0,
            StatUtils.varianceDifference(sample1, sample2, meanDifference),
            sample1.length);
     ...
     * mean of the paired differences between <code>sample1</code> and <code>sample2</code> is 0 in favor of the
     * two-sided alternative that the
     * mean paired difference is not equal to 0, with significance level <code>alpha</code>.
     * <p>
     * Returns <code>true</code> iff the null hypothesis can be rejected with confidence <code>1 - alpha</code>. To
    ...
    public double t(final double mu, final double[] observed) {
        ...
        return this.t(StatUtils.mean(observed), mu, StatUtils.variance(observed),
            observed.length);
    ...
    /**
     * Computes a <a href="http://www.itl.nist.gov/div898/handbook/prc/section2/prc22.htm#formula">
     * t statistic </a> to use in comparing the mean of the dataset described by <code>sampleStats</code> to
     * <code>mu</code>.
     * <p>
     * This statistic can be used to perform a one sample t-test for the mean.
     * </p>
    ...
    public double t(final double mu, final StatisticalSummary sampleStats) {
        ...
        return this.t(sampleStats.getMean(), mu, sampleStats.getVariance(),
            sampleStats.getN());
     ...
     * </p>
     * <p>
     * where <strong><code>n1</code></strong> is the size of first sample; <strong><code> n2</code></strong> is the size
     * of second sample; <strong><code> m1</code></strong> is the mean of first sample; <strong><code> m2</code>
     * </strong> is the mean of second sample</li> </ul> and <strong><code>var</code></strong> is the pooled variance
     * estimate:
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * with <strong><code>var1</code></strong> the variance of the first sample and <strong><code>var2</code></strong>
    ...
    public double homoscedasticT(final double[] sample1, final double[] sample2) {
        ...
        this.checkSampleData(sample2);
        // No try-catch or advertised exception because args have just been checked
        return this.homoscedasticT(StatUtils.mean(sample1), StatUtils.mean(sample2),
     ...
     * </p>
     * <p>
     * where <strong><code>n1</code></strong> is the size of the first sample <strong><code> n2</code></strong> is the
     * size of the second sample; <strong><code> m1</code></strong> is the mean of the first sample; <strong>
     * <code> m2</code></strong> is the mean of the second sample; <strong><code> var1</code></strong> is the variance
     * of the first sample; <strong><code> var2</code></strong> is the variance of the second sample;
     * </p>
     * <p>
     * <strong>Preconditions</strong>:
     * <ul>
    ...
    public double t(final double[] sample1, final double[] sample2) {
        ...
        this.checkSampleData(sample2);
        // No try-catch or advertised exception because args have just been checked
        return this.t(StatUtils.mean(sample1), StatUtils.mean(sample2),
            StatUtils.variance(sample1), StatUtils.variance(sample2),
            sample1.length, sample2.length);
    }
     ...
     * assumption of equal subpopulation variances. Use {@link #homoscedasticT(StatisticalSummary, StatisticalSummary)}
     * to
     ...
     * where <strong><code>n1</code></strong> is the size of the first sample; <strong><code> n2</code></strong> is the
     * size of the second sample; <strong><code> m1</code></strong> is the mean of the first sample; <strong>
     * <code> m2</code></strong> is the mean of the second sample <strong><code> var1</code></strong> is the variance of
     * the first sample; <strong><code> var2</code></strong> is the variance of the second sample
     * </p>
     * <p>
     * <strong>Preconditions</strong>:
     * <ul>
    ...
    public double t(final StatisticalSummary sampleStats1,
        ...
        return this.t(sampleStats1.getMean(), sampleStats2.getMean(),
     ...
     * where <strong><code>n1</code></strong> is the size of first sample; <strong><code> n2</code></strong> is the size
     * of second sample; <strong><code> m1</code></strong> is the mean of first sample; <strong><code> m2</code>
     * </strong> is the mean of second sample and <strong><code>var</code></strong> is the pooled variance estimate:
     * </p>
     * <p>
     * <code>var = sqrt(((n1 - 1)var1 + (n2 - 1)var2) / ((n1-1) + (n2-1)))</code>
     * </p>
    ...
    public double homoscedasticT(final StatisticalSummary sampleStats1,
        ...
        this.checkSampleData(sampleStats1);
        this.checkSampleData(sampleStats2);
        return this.homoscedasticT(sampleStats1.getMean(), sampleStats2.getMean(),
            sampleStats1.getVariance(), sampleStats2.getVariance(),
            sampleStats1.getN(), sampleStats2.getN());
     ...
     * <p>
     * The number returned is the smallest significance level at which one can reject the null hypothesis that the mean
     * equals <code>mu</code> in favor of the two-sided alternative that the mean is different from <code>mu</code>. For
    ...
    public double tTest(final double mu, final double[] sample) {
        ...
        // No try-catch or advertised exception because args have just been checked
        return this.tTest(StatUtils.mean(sample), mu, StatUtils.variance(sample),
            sample.length);
     ...
     * <li>To test the (one-sided) hypothesis <code> sample mean < mu </code> at the 99% level, first verify that the
     * measured sample mean is less than <code>mu</code> and then use <br>
    ...
    public double tTest(final double mu, final StatisticalSummary sampleStats) {
        ...
        this.checkSampleData(sampleStats);
        return this.tTest(sampleStats.getMean(), mu, sampleStats.getVariance(),
            sampleStats.getN());
     ...
     * <p>
     * The number returned is the smallest significance level at which one can reject the null hypothesis that the two
     * means are equal in favor of the two-sided alternative that they are different. For a one-sided test, divide the
     * returned value by 2.
     * </p>
     * <p>
     ...
     * {@link #t(double[], double[])} and the Welch-Satterthwaite approximation to the degrees of freedom is used, as
     * described <a href="http://www.itl.nist.gov/div898/handbook/prc/section3/prc31.htm"> here.</a> To perform the test
     * under the assumption of equal subpopulation variances, use {@link #homoscedasticTTest(double[], double[])}.
     * </p>
     * <p>
     * <strong>Usage Note:</strong><br>
     * The validity of the p-value depends on the assumptions of the parametric t-test procedure, as discussed <a
    ...
    public double tTest(final double[] sample1, final double[] sample2) {
        ...
        this.checkSampleData(sample2);
        // No try-catch or advertised exception because args have just been checked
        return this.tTest(StatUtils.mean(sample1), StatUtils.mean(sample2),
            StatUtils.variance(sample1), StatUtils.variance(sample2),
            sample1.length, sample2.length);
     ...
     * </p>
     * <p>
     * A pooled variance estimate is used to compute the t-statistic. See {@link #homoscedasticT(double[], double[])}.
    ...
    public double homoscedasticTTest(final double[] sample1, final double[] sample2) {
        ...
        this.checkSampleData(sample2);
        // No try-catch or advertised exception because args have just been checked
        return this.homoscedasticTTest(StatUtils.mean(sample1),
     ...
     * two-sided t-test</a> evaluating the null hypothesis that <code>sample1</code> and <code>sample2</code> are drawn
     ...
     * <p>
     * Returns <code>true</code> iff the null hypothesis that the means are equal can be rejected with confidence
     * <code>1 - alpha</code>. To perform a 1-sided test, use <code>alpha * 2</code>
     * </p>
     * <p>
     * See {@link #t(double[], double[])} for the formula used to compute the t-statistic. Degrees of freedom are
     * approximated using the <a href="http://www.itl.nist.gov/div898/handbook/prc/section3/prc31.htm">
     ...
     * <li>To test the (one-sided) hypothesis <code> mean 1 < mean 2 </code>, at the 99% level, first verify that the
     * measured mean of <code>sample 1</code> is less than the mean of <code>sample 2</code> and then use <br>
     ...
     * <p>
     * Returns <code>true</code> iff the null hypothesis that the means are equal can be rejected with confidence
     * <code>1 - alpha</code>. To perform a 1-sided test, use <code>alpha * 2.</code> To perform the test without the
     * assumption of equal subpopulation variances, use {@link #tTest(double[], double[], double)}.
     * </p>
     * <p>
     * A pooled variance estimate is used to compute the t-statistic. See {@link #t(double[], double[])} for the
    ...
    public double tTest(final StatisticalSummary sampleStats1,
        ...
        this.checkSampleData(sampleStats1);
        this.checkSampleData(sampleStats2);
        return this.tTest(sampleStats1.getMean(), sampleStats2.getMean(),
            sampleStats1.getVariance(), sampleStats2.getVariance(),
            sampleStats1.getN(), sampleStats2.getN());
     ...
     * </p>
     * <p>
     * See {@link #homoscedasticT(double[], double[])} for the formula used to compute the t-statistic. The sum of the
    ...
    public double homoscedasticTTest(final StatisticalSummary sampleStats1,
        ...
        this.checkSampleData(sampleStats1);
        this.checkSampleData(sampleStats2);
        return this.homoscedasticTTest(sampleStats1.getMean(),
     ...
     * Performs a
     * <a href="http://www.itl.nist.gov/div898/handbook/eda/section3/eda353.htm">
     * two-sided t-test</a> evaluating the null hypothesis that <code>sampleStats1</code> and <code>sampleStats2</code>
     * describe
    ...
    protected double t(final double m1, final double m2,
        ...
        return (m1 - m2) / MathLib.sqrt((v1 / n1) + (v2 / n2));
    }
    ...
    protected double homoscedasticT(final double m1, final double m2,
        ...
        return (m1 - m2) / MathLib.sqrt(pooledVariance * (1d / n1 + 1d / n2));
    }
    ...
    protected double tTest(final double m, final double mu,
        ...
        final double t = MathLib.abs(this.t(m, mu, v, n));
    ...
    protected double tTest(final double m1, final double m2,
        ...
        final double t = MathLib.abs(this.t(m1, m2, v1, v2, n1, n2));
     ...
     *         strictly positive
     */
    protected double homoscedasticTTest(final double m1, final double m2,
                                        final double v1, final double v2,
                                        final double n1, final double n2) {

        final double t = MathLib.abs(this.homoscedasticT(m1, m2, v1, v2, n1, n2));
    ...
    private void checkSampleData(final double[] data) {
        ...
        if (data.length < 2) {
            throw new NumberIsTooSmallException(
                PatriusMessages.INSUFFICIENT_DATA_FOR_T_STATISTIC,
                data.length, 2, true);
        }
