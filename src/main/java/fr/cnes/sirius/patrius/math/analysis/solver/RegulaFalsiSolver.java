 ...
 * 
 * <p>
 * The <em>Regula Falsi</em> method is included for completeness, for testing purposes, for educational purposes, for
 * comparison to other algorithms, etc. It is however <strong>not</strong> intended to be used for actual problems, as
 ...
 * <em>Illinois</em>} or {@link PegasusSolver <em>Pegasus</em>}). These two algorithms solve the fundamental issues of
 * the original <em>Regula
 * Falsi</em> algorithm, and greatly out-performs it for most, if not all, (practical) functions.
 * 
 * <p>
 * Unlike the <em>Secant</em> method, the <em>Regula Falsi</em> guarantees convergence, by maintaining a bracketed
 * solution. Note however, that due to the finite/limited precision of Java's {@link Double double} type, which is used
 * in this implementation, the algorithm may get stuck in a situation where it no longer makes any progress. Such cases
 * are detected and result in a {@code ConvergenceException} exception being thrown. In other words, the algorithm
 ...
 * 
 * @since 3.0
 * @version $Id: RegulaFalsiSolver.java 18108 2017-10-04 06:45:27Z bignon $
 */
public class RegulaFalsiSolver extends BaseSecantSolver {

    /** Serializable UID. */
    private static final long serialVersionUID = -8595890603822977833L;
