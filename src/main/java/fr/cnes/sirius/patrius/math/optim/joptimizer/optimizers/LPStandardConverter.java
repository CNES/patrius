...
public class LPStandardConverter {
    ...
    private final List<Integer> splittedVariablesList = new ArrayList<>();
    ...
    public void toStandardForm(final RealVector originalC,
        ...
        /** lower bound slack */
        final boolean[] lbSlack = new boolean[this.originalN];
        /** upper bound slack */
        final boolean[] ubSlack = new boolean[this.originalN];
        ...
        if (originalA != null) {
            this.standardA = new BlockRealMatrix(this.standardS + originalA.getRowDimension(), this.standardN);
            this.standardB = new ArrayRealVector(this.standardS + originalB.getDimension());
        } else {
            this.standardA = new BlockRealMatrix(this.standardS, this.standardN);
            this.standardB = new ArrayRealVector(this.standardS);
        }

        // filling with original G values
        for (int i = 0; i < nOfSlackG; i++) {
            this.standardA.setEntry(i, i, 1);// slack variable position
            this.standardB.setEntry(i, originalH.getEntry(i));
        }
        for (int i = 0; i < nOfSlackG; i++) {
            for (int j = 0; j < this.originalN; j++) {
                this.standardA.setEntry(i, this.standardS + j, originalG.getEntry(i, j));
            }
        }
        ...
        for (int i = 0; i < this.originalN; i++) {
            if (lbSlack[i]) {
                this.standardA.setEntry(nOfSlackG + cntSlackLB, nOfSlackG + cntSlackLB, 1);// slack
                                                                                      // variable
                                                                                      // position
                this.standardA.setEntry(nOfSlackG + cntSlackLB, this.standardS + i, -1);
                this.standardB.setEntry(nOfSlackG + cntSlackLB, -originalLBCopy.getEntry(i));
                cntSlackLB++;
            }
            if (ubSlack[i]) {
                this.standardA.setEntry(nOfSlackG + nOfSlackLB + cntSlackUB, nOfSlackG + nOfSlackLB
                        + cntSlackUB, 1);// slack
                // variable
                // position
                this.standardA.setEntry(nOfSlackG + nOfSlackLB + cntSlackUB, this.standardS + i, 1);
                this.standardB.setEntry(nOfSlackG + nOfSlackLB + cntSlackUB, originalUBCopy.getEntry(i));
                cntSlackUB++;
            }
        }
        ...
        for (int i = 0; originalA != null && i < originalA.getRowDimension(); i++) {
            for (int j = 0; j < this.originalN; j++) {
                this.standardA.setEntry(this.standardS + i, this.standardS + j, originalA.getEntry(i, j));
            }
            this.standardB.setEntry(this.standardS + i, originalB.getEntry(i));
        }
        ...
        for (int sv = 0; sv < nOfSplittedVariables; sv++) {
            final int i = this.splittedVariablesList.get(sv);
            // variable i was splitted
            for (int r = 0; r < nOfSlackG; r++) {// n of rows of G
                this.standardA.setEntry(r, this.standardS + this.originalN + sv, -originalG.getEntry(r, i));
            }
            ...
            if (lbSlack[i]) {
                ...
                this.standardA.setEntry(nOfSlackG + cntSlackLB, this.standardS + this.originalN + sv, 1);// lower
                                                                                          // bound
            }
            ...
            if (ubSlack[i]) {
                ...
                this.standardA.setEntry(nOfSlackG + nOfSlackLB + cntSlackUB, this.standardS + this.originalN + sv,
                        -1);// upper
                            // bound
            }
            ...
            for (int r = 0; originalA != null && r < originalA.getRowDimension(); r++) {
                this.standardA.setEntry(this.standardS + r, this.standardS + this.originalN + sv,
                        -originalA.getEntry(r, i));
            }
        }

        this.standardC = new ArrayRealVector(this.standardN);
        this.standardLB = new ArrayRealVector(this.standardN);
        this.standardUB = new ArrayRealVector(this.standardN, this.unboundedUBValue);// the slacks are upper
                                                                      // unbounded
        for (int i = 0; i < this.originalN; i++) {
            this.standardC.setEntry(this.standardS + i, originalC.getEntry(i));
        }
        for (int i = 0; i < this.standardS; i++) {
            this.standardLB.setEntry(i, 0.);
        }
        for (int i = 0; i < this.originalN; i++) {
            this.standardLB.setEntry(this.standardS + i, originalLBCopy.getEntry(i));
        }
        for (int i = 0; i < this.originalN; i++) {
            this.standardUB.setEntry(this.standardS + i, originalUBCopy.getEntry(i));
        }

        if (this.strictlyStandardForm) {
            this.standardLB = new ArrayRealVector(this.standardN, 0.);// brand new lb
            this.standardUB = null;// no ub for the strictly standard form
        }
    }
    ...
    public double[] postConvert(final double[] x) {
        ...
        final double[] ret = new double[this.originalN];
        int cntSplitted = 0;
    ...
    public double[] getStandardComponents(final double[] x) {
        ...
        final double[] ret = new double[this.standardN];
            ...
            if (this.splittedVariablesList.contains(i)) {
                ...
                if (x[i] >= 0) {
                    ...
                    for (int k = 0; k < this.splittedVariablesList.size(); k++) {
                        if (this.splittedVariablesList.get(k) == i) {
                            pos = k;
        ...
        if (this.standardS > 0) {
            final RealVector residuals = AlgebraUtils.zMult(this.standardA, new ArrayRealVector(ret),
                this.standardB, -1);
            for (int i = 0; i < this.standardS; i++) {
