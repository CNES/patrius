...
//CHECKSTYLE: stop ParameterAssignment check
 ...
 * </p>
 * <p>
 * FastMath speed is achieved by relying heavily on optimizing compilers to native code present in many JVMs today and
 * use of large tables. The larger tables are lazily initialised on first use, so that the setup time does not penalise
 * methods that don't need them.
 * </p>
 * <p>
 * Note that FastMath is extensively used inside Apache Commons Math, so by calling some algorithms, the overhead when
 * the the tables need to be intialised will occur regardless of the end-user calling FastMath methods directly or not.
 * Performance figures for a specific JVM and hardware can be evaluated by running the FastMathTestPerformance tests in
 * the test directory of the source distribution.
 * </p>
 * <p>
 * FastMath accuracy should be mostly independent of the JVM as it relies only on IEEE-754 basic operations and on
 * embedded tables. Almost all operations are accurate to about 0.5 ulp throughout the domain range. This statement, of
 * course is only a rough global observed behavior, it is <em>not</em> a guarantee for <em>every</em> double numbers
 * input (see William Kahan's <a href="http://en.wikipedia.org/wiki/Rounding#The_table-maker.27s_dilemma">Table Maker's
 * Dilemma</a>).
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * This version of FastMath never returns Double.NaN values. If result cannot be returned because undefined, an
 * ArithmeticException is returned.
 * </p>
 * 
 * <p>This class is up-to-date with commons-math 3.6.1.</p>
 * 
 * @version $Id: FastMath.java 18108 2017-10-04 06:45:27Z bignon $
 * @since 2.2
 */
...
public final class FastMath {
     ...
     * <p>
     * This compile-time constant should be set to true only if one explicitly wants to compute the tables at class
     * loading time instead of using the already computed ones provided as literal arrays below.
     * </p>
     */
     ...
     * @throws ArithmeticException
     *         thrown if a > 1 or a < -1 or a is NaN
     */
    ...
    private static double exp(final double x, final double extra, final double[] hiPrec) {
        ...
        final double intPartA = ExpIntTable.EXP_INT_TABLE_A[EXP_INT_TABLE_MAX_INDEX + intVal];
        final double intPartB = ExpIntTable.EXP_INT_TABLE_B[EXP_INT_TABLE_MAX_INDEX + intVal];
    ...
    private static double expm1(double x, final double[] hiPrecOut) {
        ...
        double tempAB = tempA + tempB;
        tempB = -(tempAB - tempA - tempB);
        tempA = tempAB;

        tempAB = tempA * HEX_40000000;
        baseA = tempA + tempAB - tempAB;
    ...
    private static double log(final double x, final double[] hiPrec) {
        ...
        final boolean cond = x < 1.01 && x > 0.99 && hiPrec == null;
        if ((exp == -1 || exp == 0) && cond) {
            /*
            ...
            final double[] lnCoefLast = LN_QUICK_COEF[LN_QUICK_COEF.length - 1];
            double ya = lnCoefLast[0];
            double yb = lnCoefLast[1];
            ...
            for (int i = LN_QUICK_COEF.length - 2; i >= 0; i--) {
                ...
                /* Add a = y + lnQuickCoef */
                final double[] lnCoefi = LN_QUICK_COEF[i];
                aa = ya + lnCoefi[0];
                ab = yb + lnCoefi[1];
                /* Split y = a */
                tmp = aa * HEX_40000000;
        ...
        if (hiPrec == null) {
            ...
            final double[] lnCoefLast = LN_HI_PREC_COEF[LN_HI_PREC_COEF.length - 1];
            double ya = lnCoefLast[0];
            double yb = lnCoefLast[1];
            ...
            for (int i = LN_HI_PREC_COEF.length - 2; i >= 0; i--) {
                ...
                final double[] lnCoefi = LN_HI_PREC_COEF[i];
                aa = ya + lnCoefi[0];
                ab = yb + lnCoefi[1];
                /* Split y = a */
                tmp = aa * HEX_40000000;
     ...
     * @throws ArithmeticException
     *         thrown if x < 0 or (base or x) is NaN or base and x = 0
     */
    public static double log(final double base, final double x) {
        if (Double.isNaN(x) || Double.isNaN(base)) {
            throw new ArithmeticException("Input of log(base, x) is NaN.");
        }
        if (base == 0 && x == 0) {
            throw new ArithmeticException("log(0, 0) is undefined.");
        }
        if (base == 1) {
            throw new ArithmeticException("log(x, 1) is undefined.");
        }
     ...
     * @return double
     * @throws ArithmeticException
     *         thrown if y < 0 and y not an integer or x or y is NaN or x = -1
     */
    public static double pow(final double x, final double y) {
        ...
        if (x < 0) {
            ...
            if (y == (long) y) {
                ...
                throw new ArithmeticException("x^y with y < 0 and y not an integer is not defined.");
            }
        }
        ...
        final double yb;
        if (y < 8e298 && y > -8e298) {
            final double tmp1 = y * HEX_40000000;
            ya = y + tmp1 - tmp1;
            yb = y - ya;
        } else {
            final double tmp1 = y * 9.31322574615478515625E-10;
            final double tmp2 = tmp1 * 9.31322574615478515625E-10;
    ...
    private static void reducePayneHanek(final double x, final double[] result) {
        ...
        /* Carry */
        final boolean cond = ((bita || bitb) && !bitsum);
        if ((bita && bitb) || cond) {
            prodA++;
        }
        ...
        /* Carry */
        final boolean cond2 = ((bita || bitb) && !bitsum);
        if ((bita && bitb) || cond2) {
            prodA++;
        }
        ...
        bitsum = (prodB & 0x8000000000000000L) != 0;
        /* Carry */
        final boolean cond3 = ((bita || bitb) && !bitsum);
        if ((bita && bitb) || cond3) {
            prodA++;
        }
        ...
        /* Carry */
        final boolean cond4 = ((bita || bitb) && !bitsum);
        if ((bita && bitb) || cond4) {
            prod2A++;
        }
        ...
        /* Carry */
        final boolean cond5 = ((bita || bitb) && !bitsum);
        if ((bita && bitb) || cond5) {
            prod2A++;
        }
        ...
        bitsum = (prod2B & 0x8000000000000000L) != 0;
        /* Carry */
        final boolean cond6 = ((bita || bitb) && !bitsum);
        if ((bita && bitb) || cond6) {
            prod2A++;
        }
        ...
        bitsum = (prod2B & 0x8000000000000000L) != 0;
        /* Carry */
        final boolean cond7 = ((bita || bitb) && !bitsum);
        if ((bita && bitb) || cond7) {
            prod2A++;
        }
        ...
        // Low bits
        final double tmpB = (((prod2A & 0xfffL) << 40) + (prod2B >>> 24)) / TWO_POWER_52 / TWO_POWER_52;
     ...
     * @return sin(x)
     * @throws ArithmeticException
     *         thrown if x is NaN or Infinity
     */
     ...
     * @throws ArithmeticException
     *         thrown if x < -1 or x > 1 or x is NaN
     */
    ...
    public static double cbrt(double x) {
        ...
        final double p2 = Double.longBitsToDouble((inbits & 0x8000000000000000L) |
     ...
     * @throws ArithmeticException
     *         thrown if d or direction is NaN
     */
     ...
     * @return the next machine representable number in the specified direction
     * @throws ArithmeticException
     *         thrown if f or direction is NaN
     */
     ...
     * The remainder value is mathematically equal to {@code x - y*n} where {@code n} is the mathematical integer
     * closest to the exact mathematical value
     ...
     * @throws ArithmeticException
     *         thrown if input cannot allow to compute remainder
     */
    ...
    public static double IEEEremainder(final double dividend, final double divisor) {
        // CHECKSTYLE: resume MethodName
        final double res = StrictMath.IEEEremainder(dividend, divisor);
        if (Double.isNaN(res)) {
    ...
    public static int toIntExact(final long n) {
        if (n < Integer.MIN_VALUE || n > Integer.MAX_VALUE) {
            throw new ArithmeticException("n is out of int range.");
        }
        return (int) n;
    }
    ...
    public static int incrementExact(final int n) throws MathArithmeticException {
        ...
        if (n == Integer.MAX_VALUE) {
            throw new MathArithmeticException(PatriusMessages.OVERFLOW_IN_ADDITION, n, 1);
        }
    ...
    public static int decrementExact(final int n) throws MathArithmeticException {
        ...
        if (n == Integer.MIN_VALUE) {
            throw new MathArithmeticException(PatriusMessages.OVERFLOW_IN_SUBTRACTION, n, 1);
        }
    ...
    public static int subtractExact(final int a, final int b) {
        ...
        // check for overflow
        if ((a ^ b) < 0 && (sub ^ b) >= 0) {
            throw new MathArithmeticException(PatriusMessages.OVERFLOW_IN_SUBTRACTION, a, b);
        }
    ...
    public static int multiplyExact(final int a, final int b) {
        final boolean c1 = ((b > 0) && (a > Integer.MAX_VALUE / b || a < Integer.MIN_VALUE / b));
        final boolean c2 = ((b < -1) && (a > Integer.MIN_VALUE / b || a < Integer.MAX_VALUE / b));
        final boolean c3 = ((b == -1) && (a == Integer.MIN_VALUE));
        if (c1 || c2 || c3) {
            throw new MathArithmeticException(PatriusMessages.OVERFLOW_IN_MULTIPLICATION, a, b);
        }
        return a * b;
    }
    ...
    public static long multiplyExact(final long a, final long b) {
        final boolean c1 = ((b > 0l) && (a > Long.MAX_VALUE / b || a < Long.MIN_VALUE / b));
        final boolean c2 = ((b < -1l) && (a > Long.MIN_VALUE / b || a < Long.MAX_VALUE / b));
        final boolean c3 = ((b == -1l) && (a == Long.MIN_VALUE));
        if (c1 || c2 || c3) {
            throw new MathArithmeticException(PatriusMessages.OVERFLOW_IN_MULTIPLICATION, a, b);
        }
        return a * b;
     ...
     * <p>
     * This methods returns the same value as integer division when a and b are same signs, but returns a different
     * value when they are opposite (i.e. q is negative).
     * </p>
     * 
     * @param a dividend
     * @param b divisor
     ...
     * <p>
     * This methods returns the same value as integer modulo when a and b are same signs, but returns a different value
     * when they are opposite (i.e. q is negative).
     * </p>
     * 
     * @param a dividend
     * @param b divisor
    ...
    private static final class CodyWaite {
        /** k */
         ...
         * @param xa Argument.
         */
        private CodyWaite(final double xa) {
            // Estimate k.
    ...
    // CHECKSTYLE: resume ParameterAssignment check
