...
public final class FourierSeries implements DifferentiableIntegrableUnivariateFunction, Serializable {
    ...
    public String toString() {
        ...
        if (this.getConstant() != 0) {
            buf.append(this.addNumber(this.getConstant()));
        }
        // Append arrays for sine and cosine coeffcicients
        for (int i = 0; i < this.getOrder() - 1; i++) {
            if (this.getCosArray()[i] != 0) {
                buf.append(this.addNumber(this.getCosArray()[i]) + " cos("
                    + this.addNumber((i + 1) * this.getAngularFrequency()) + x);
            }
            if (this.getSinArray()[i] != 0) {
                buf.append(this.addNumber(this.getSinArray()[i]) + " sin("
                    + this.addNumber((i + 1) * this.getAngularFrequency()) + x);
            }
        }
