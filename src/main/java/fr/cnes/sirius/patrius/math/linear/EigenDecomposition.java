...
public class EigenDecomposition implements Decomposition {
    ...
    private void eigenDecompose(final RealMatrix matrix) {
        ...
        if (matrix instanceof DiagonalMatrix) {
            ...
            this.symmetric = matrix.isSymmetric(relativeSymmetryThreshold);
            if (this.symmetric) {
     ...
     *
     * @return a solver
     * @throws MathUnsupportedOperationException if the matrix is not symmetric or not positive definite or if the 
     *          decomposition resulted in complex eigenvalues
     */
    @Override
    public DecompositionSolver getSolver() {
        if (!this.symmetric || this.hasComplexEigenvalues()) {
            throw new MathUnsupportedOperationException();
        }
    ...
    private void findEigenVectors(final double[][] householderMatrix) {
        ...
        for (int i = 0; i < n; i++) {
            if (MathLib.abs(realEigenValues[i]) > maxAbsoluteValue) {
                maxAbsoluteValue = MathLib.abs(realEigenValues[i]);
            }
        ...
        if (maxAbsoluteValue != 0) {
            for (int i = 0; i < n; i++) {
                if (MathLib.abs(realEigenValues[i]) <= Precision.EPSILON * maxAbsoluteValue) {
                    realEigenValues[i] = 0;
                }
        ...
        for (int j = 0; j < n; j++) {
            ...
            do {
                for (m = j; m < n - 1; m++) {
                    final double delta = MathLib.abs(realEigenValues[m]) + MathLib.abs(realEigenValues[m + 1]);
     ...
     */
    private Complex cdiv(final double xr,
            final double xi,
            final double yr,
            final double yi) {
    ...
    private void findEigenVectorsFromSchur(final SchurTransformer schur) {
        ...
        for (int idx = n - 1; idx >= 0; idx--) {
            final double p = realEigenValues[idx];
                ...
                for (int i = idx - 1; i >= 0; i--) {
                    ...
                    if (Precision.compareTo(imagEigenValues[i], 0.0, EPSILON) < 0) {
                        ...
                        if (Precision.equals(imagEigenValues[i], 0.0)) {
                            ...
                            q = (realEigenValues[i] - p) * (realEigenValues[i] - p) + imagEigenValues[i]
                                    * imagEigenValues[i];
                ...
                for (int i = idx - 2; i >= 0; i--) {
                    ...
                    if (Precision.compareTo(imagEigenValues[i], 0.0, EPSILON) < 0) {
                        ...
                        if (Precision.equals(imagEigenValues[i], 0.0)) {
                            ...
                            final double x = matrixT[i][i + 1];
                            final double y = matrixT[i + 1][i];
                            double vr = (realEigenValues[i] - p) * (realEigenValues[i] - p) + imagEigenValues[i]
                                    * imagEigenValues[i] - q * q;
