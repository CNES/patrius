...
public final class AngleTools {
     ...
     * <p>
     * For the case of more than two input angles, since the arithmetic mean is not appropriate for
     * circular quantities due to angular wrapping, the angles are converted to their corresponding
     * points on the unit circle, i.e. converting its polar coordinates to cartesian, and then
     * computing the arithmetic mean of those two points.
     * <p>
     * <b>Note:</b> Results produced by this computation may seem counterintuitive since they are
     * different from what would be obtained using a standard arithmetic mean of the values, with
     * this difference being greater when the angles are widely distributed.
     *
     * @param angles
     *        set of input angles [rad]
     * @return mean angular value
     */
    public static double meanAngle(final double... angles) {

        final double mean;
        if (angles.length == 1) {
            mean = angles[0];
        } else if (angles.length == 2) {
            mean = angles[0] + MathUtils.normalizeAngle(angles[1] - angles[0], 0.) / 2.;
        } else {
            // Get the sum of x-axis and y-axis components of the unit vectors
            double sumX = 0;
            double sumY = 0;
            for (final double ang : angles) {
                sumX += MathLib.cos(ang);
                sumY += MathLib.sin(ang);
            }

            // Check for the special cases
            if (MathLib.abs(sumX) <= Precision.DOUBLE_COMPARISON_EPSILON
                    && MathLib.abs(sumY) <= Precision.DOUBLE_COMPARISON_EPSILON) {
                mean = Double.NaN;
            } else {
                // The scaling does not matter for atan2, thus it can be omitted. Angle is in [0 ;
                // 2*PI[
                mean = MathUtils.normalizeAngle(MathLib.atan2(sumY, sumX), FastMath.PI);
            }
        }
        return mean;
    }
     ...
     * @param moduloMax
     *        maximum value
     * @return the modulo value in [0 ; moduloMax[
     */
    public static double modulo(final double value, final double moduloMax) {
        final double offset = MathLib.floor(value / moduloMax) * moduloMax;
        return value - offset;
    }
     ...
     * @param value
     *        input value
     * @return the modulo value in [0 ; 2PI[
     */
    public static double moduloTwoPi(final double value) {
        return modulo(value, MathUtils.TWO_PI);
    }
     ...
     * @param moduloMax
     *        maximum value
     * @return the list of modulo values in [0 ; moduloMax[
     */
    public static List<Double> modulo(final List<Double> values, final double moduloMax) {
        final List<Double> moduloValues = new ArrayList<Double>();
        for (final Double value : values) {
            moduloValues.add(modulo(value, moduloMax));
        }
        return moduloValues;
    }
     ...
     * @param a
     *        Size of jump to be corrected (the absolute value will be always used)
     */
    public static void unMod(final List<Double> y, final double a) {

        final List<Double> y2 = new ArrayList<Double>();

        // First element is identical
        y2.add(y.get(0));

        // Correcting next values
        final double absA = MathLib.abs(a);
        double sum = 0;
        for (int i = 1; i < y.size(); i++) {

            // Computing correction if jump detected
            if (MathLib.abs(y.get(i) - y.get(i - 1)) > FastMath.PI) {
                sum = sum - MathLib.round((y.get(i) - y.get(i - 1)) / absA) * absA;
            }
     ...
     *        Array of data to be unmod
     */
    public static void unMod(final List<Double> y) {
        unMod(y, 2 * FastMath.PI);
    }

    /**
     * Make angular data continuous by removing modulo every time a jump larger than pi is detected
     * <p>
     * A user-input shift is added when necessary when a jump is detected. This function is valid
     * even if the "distance" between two point is larger than a/2. In that case the approximate
     * slope shall be provided to enable reconstruction by the algorithm.
     *
     * @param x
     *        Array of abscissas
     * @param y
     ...
     *        Size of jump to be corrected (the absolute value will be always used)
     * @param slope
     *        Approximate value of dy/dx
     */
    public static void unMod(final List<Double> x, final List<Double> y, final double a, final double slope) {
        ...
        for (int i = 1; i < y.size(); i++) {
            ...
            final double aux = -y.get(i) + y.get(i - 1) + (x.get(i) - x.get(i - 1)) * slope;
            sum = sum + MathLib.round(aux / absA) * absA;
