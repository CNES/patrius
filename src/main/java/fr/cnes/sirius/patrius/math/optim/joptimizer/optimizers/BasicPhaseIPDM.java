...
public class BasicPhaseIPDM {
    ...
    public RealVector findFeasibleInitialPoint() throws PatriusException {
        ...
        final ConvexMultivariateRealFunction[] inequalities = new ConvexMultivariateRealFunction[originalProblem
                .getMieq()];
        for (int i = 0; i < inequalities.length; i++) {
            ...
            final ConvexMultivariateRealFunction fi = new ConvexMultivariateRealFunction() {
                ...
                public double value(final double[] val) {
                    ...
                    final RealVector x = y.getSubVector(0, originalDim);
                    return originalFi.value(x.toArray()) - y.getEntry(dim - 1);
                }
        ...
        for (int i = 0; i < originalFiX0.getDimension(); i++) {
            initialPoint.setEntry(
                    dim - 1,
                    MathLib.max(initialPoint.getEntry(dim - 1),
                            originalFiX0.getEntry(i) * MathLib.pow(originalProblem.getToleranceFeas(), SCALAR)));
        }
        ...
        final RealVector ineq = originalProblem.getFi(ret); // inequalities
