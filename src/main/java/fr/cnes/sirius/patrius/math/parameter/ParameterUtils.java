...
/**
 * This utility class defines static methods to manage {@link Parameter parameters} and {@link ParameterDescriptor
...
public final class ParameterUtils {
     ...
     * ignored).
     * <p>
     * Note that this method will not have any effect if {@linkplain IParameterizable#getParameters() getParameters()}
     ...
     */
    public static void removeFieldFromParameters(final IParameterizable parameterizable,
                                                 final FieldDescriptor<?> fieldDescriptor) {
        if (parameterizable != null) {
            removeFieldFromParameters(parameterizable.getParameters(), fieldDescriptor);
        }
    }
     ...
     *        the field descriptor to be removed
     */
    public static void removeFieldFromParameters(final Collection<Parameter> parameters,
                                                 final FieldDescriptor<?> fieldDescriptor) {
        if (parameters != null) {
            for (final Parameter parameter : parameters) {
                if (parameter != null) {
                    ...
                    // Check if the descriptor is initialized and mutable
                    if (descriptor != null && descriptor.isMutable()) {
                        descriptor.removeField(fieldDescriptor);
                    }
                }
            }
        }
     ...
     * {@linkplain StandardFieldDescriptors#ORBITAL_COORDINATE orbital coordinate descriptor} mapped to the described
     * coordinate (for example, {@linkplain CartesianCoordinate#X X}, {@linkplain CartesianCoordinate#Y Y},
     * {@linkplain CartesianCoordinate#Z Z}, {@linkplain CartesianCoordinate#VX VX}, {@linkplain CartesianCoordinate#VY
     * VY}, {@linkplain CartesianCoordinate#VZ VZ} if the orbit type is {@linkplain OrbitType#CARTESIAN
     * CARTESIAN}).
     * </p>
     *
     * @param orbitType
    ...
    public static String concatenateParameterNames(final Collection<Parameter> parameters,
                                                   final String nameSeparator, final String fieldSeparator,
