...
/**
 * This class, with PACKAGE visibility, implements an algorithm for the Fast Fourier Transform for powers of two orders,
 * based on the already existing class FastFourierTransform of version anterior to 2.3
 * It should remain protected, otherwise some exception have to be created to prevent the user from using it for odd
 * orders.
 * 
 ...
 * specified by the parameter {@link DftNormalization}.
 * <p>
 * The current implementation of the discrete Fourier transform as a fast Fourier transform requires the length of the
 * data set to be a power of 2. This greatly simplifies and speeds up the code. Users can pad the data with zeros to
 * meet this requirement. There are other flavors of FFT, for reference, see S. Winograd, <i>On computing the discrete
 * Fourier transform</i>, Mathematics of Computation, 32 (1978), 175 - 199.
 * 
 * @see DftNormalization
 * 
 * @version $Id: FFTpowerOfTwoOrder.java 18108 2017-10-04 06:45:27Z bignon $
 * 
 * @since 2.3
 */
class FFTpowerOfTwoOrder extends AbstractFastFourierTransformer {

    /**
     * {@code W_SUB_N_R[i]} is the real part of {@code exp(- 2 * i * pi / n)}: {@code W_SUB_N_R[i] = cos(2 * pi/ n)},
     * where {@code n = 2^i}.
     */
    ...
    /**
     * {@code W_SUB_N_I[i]} is the imaginary part of {@code exp(- 2 * i * pi / n)}:
    ...
    /**
     * Constructor of the class FFTevenOrder, inherited from the one of the abstract class
     * AbstractFastFourierTransformer
     * 
     * @param dftNormalization
     *        an enum with two possible values : STANDARD or UNITARY
     */
    public FFTpowerOfTwoOrder(final DftNormalization dftNormalization) {
    ...
    public Complex[] transform(final double[] f, final TransformType type) {
        ...
        transformInPlace(dataRI, this.getNormalization(), type);
    ...
    public void transformInPlace(final double[][] dataRI, final DftNormalization normalization,
