...
public class BasicPhaseIBM {
    ...
    public double[] findFeasibleInitialPoint() throws PatriusException {
        ...
        if (matAEorig != null) {
            ...
            if (originalRPriX0.getNorm() > this.originalProblem.getToleranceFeas()) {
        ...
        final double s = this.originalProblem.getBarrierFunction().calculatePhase1InitialFeasiblePoint(x0.toArray(),
            this.originalProblem.getToleranceFeas());
        ...
        // optimization
        final BarrierFunction bfPh1 = this.originalProblem.getBarrierFunction().createPhase1BarrierFunction();
        ...
        final double[] originalSolution = new double[this.originalDim];
        System.arraycopy(response.getSolution(), 0, originalSolution, 0, this.originalDim);
    ...
    private class PhaseIBarrierMethod extends BarrierMethod {
        ...
        protected boolean checkCustomExitConditions(final RealVector y) {
            ...
            if (getA() != null) {
                originalRPriX = AlgebraUtils.zMult(BasicPhaseIBM.this.originalProblem.getA(), x,
                    BasicPhaseIBM.this.originalProblem.getB(), -1.);
            }
            final boolean b2 = originalRPriX.getNorm() < BasicPhaseIBM.this.originalProblem.getToleranceFeas();

            // inequalities
            final boolean b1 = !Double
                .isNaN(BasicPhaseIBM.this.originalProblem.getBarrierFunction().value(x.toArray()))
