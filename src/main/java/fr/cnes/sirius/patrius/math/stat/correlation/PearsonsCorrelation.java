...
public class PearsonsCorrelation {
    ...
    public double correlation(final double[] xArray, final double[] yArray) {
        ...
        if (xArray.length != yArray.length) {
            // Exception
            throw new DimensionMismatchException(PatriusMessages.INCOMPATIBLE_ARRAYS_DIMENSIONS, xArray.length,
                yArray.length);
        } else if (xArray.length < 2) {
            // Exception
            throw new MathIllegalArgumentException(PatriusMessages.INSUFFICIENT_DIMENSION, xArray.length, 2);
        } else {
