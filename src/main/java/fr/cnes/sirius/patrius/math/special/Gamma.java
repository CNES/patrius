 ...
 * </p>
 * <p>
 * Implementation of {@link #invGamma1pm1(double)} and {@link #logGamma1p(double)} is based on the algorithms described
 * in
 * <ul>
 * <li><a href="http://dx.doi.org/10.1145/22721.23109">Didonato and Morris (1986)</a>,
 * <em>Computation of the Incomplete Gamma Function Ratios and
 ...
 * </p>
 * 
 * @version $Id: Gamma.java 18108 2017-10-04 06:45:27Z bignon $
 */
public final class Gamma {
     ...
     * @since 2.0
     */
    public static final double GAMMA_CST = 0.577215664901532860606512090082;
    ...
    private static final double[] LANCZOS_COEFS = {
     ...
     * </p>
     * <p>
     * For x &le; 8, the implementation is based on the double precision implementation in the
     * <em>NSWC Library of Mathematics Subroutines</em>, {@code DGAMLN}. For x &gt; 8, the implementation is based on
     * </p>
     * <ul>
    ...
    public static double logGamma(final double x) {
        ...
        if (Double.isNaN(x) || (x <= 0.0)) {
            ...
            return logGamma1p(x) - MathLib.log(x);
        } else if (x <= 2.5) {
            return logGamma1p((x - 0.5) - 0.5);
        } else if (x <= 8.0) {
            final int n = (int) MathLib.floor(x - 1.5);
            double prod = 1.0;
            for (int i = 1; i <= n; i++) {
                prod *= x - i;
            }
            return logGamma1p(x - (n + 1)) + MathLib.log(prod);
        } else {
            // Generic case
     ...
     * <ul>
     * <li>
     * <a href="http://mathworld.wolfram.com/RegularizedGammaFunction.html"> Regularized Gamma Function</a>, equation
     * (1)</li>
     * <li>
     * <a href="http://mathworld.wolfram.com/IncompleteGammaFunction.html"> Incomplete Gamma Function</a>, equation (4).
     * </li>
     * <li>
     * <a href="http://mathworld.wolfram.com/ConfluentHypergeometricFunctionoftheFirstKind.html"> Confluent
    ...
    // Reason: Commons-Math code kept as such
    public static double regularizedGammaP(final double a,
                                           final double x,
        ...
        if (Double.isNaN(a) || Double.isNaN(x) || (a <= 0.0) || (x < 0.0)) {
            ...
            // partial sum
            double sum = an;
            while (MathLib.abs(an / sum) > epsilon &&
                n < maxIterations &&
            ...
            if (n >= maxIterations) {
                ...
                ret = MathLib.exp(-x + (a * MathLib.log(x)) - logGamma(a)) * sum;
            }
        }
     ...
     * (1).</li>
     * <li>
     * <a href="http://functions.wolfram.com/GammaBetaErf/GammaRegularized/10/0003/"> Regularized incomplete gamma
    ...
    public static double regularizedGammaQ(final double a,
        ...
        if (Double.isNaN(a) || Double.isNaN(x) || (a <= 0.0) || (x < 0.0)) {
            ...
            ret = MathLib.exp(-x + (a * MathLib.log(x)) - logGamma(a)) * ret;
        }
    ...
    public static double digamma(final double x) {
        ...
        if (x >= C_LIMIT) {
            ...
            res = MathLib.log(x) - 0.5 / x - inv * ((1.0 / 12) + inv * (1.0 / 120 - inv / 252));
        } else {
            res = digamma(x + 1) - 1 / x;
        }
    ...
    public static double trigamma(final double x) {
        ...
        if (x >= C_LIMIT) {
            ...
            // x 2 3 5 7
            // 2 x 6 x 30 x 42 x
            res = 1 / x + inv / 2 + inv / x * (1.0 / 6 - inv * (1.0 / 30 + inv / 42));
        } else {
            res = trigamma(x + 1) + 1 / (x * x);
        }
    ...
    /**
     * <p>
     * Returns the Lanczos approximation used to compute the gamma function. The Lanczos approximation is related to the
     * Gamma function by the following equation <center> {@code gamma(x) = sqrt(2 * pi) / x * (x + g + 0.5) ^ (x + 0.5)
    ...
    public static double lanczos(final double x) {
        double sum = 0.0;
        for (int i = LANCZOS_COEFS.length - 1; i > 0; --i) {
            sum = sum + (LANCZOS_COEFS[i] / (x + i));
        }
        return sum + LANCZOS_COEFS[0];
    }
    ...
    public static double logGamma1p(final double x) {
        ...
        return -MathLib.log1p(invGamma1pm1(x));
    }
    ...
    public static double gamma(final double x) {
        ...
        if (absX <= 20.0) {
            ...
            final double gammaAbs = SQRT_TWO_PI / x *
                MathLib.pow(y, absX + 0.5) *
                MathLib.exp(-y) * lanczos(absX);
            if (x > 0.0) {
                ret = gammaAbs;
            } else {
                /*
                 ...
                 */
                ret = -FastMath.PI /
                    (x * MathLib.sin(FastMath.PI * x) * gammaAbs);
            }
        }
        return ret;
    }
