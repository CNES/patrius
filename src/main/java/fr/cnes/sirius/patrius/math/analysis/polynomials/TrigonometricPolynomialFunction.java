 ...
 */
public final class TrigonometricPolynomialFunction implements
    ...
    public String toString() {
        ...
        if (this.getA0() != 0) {
            buf.append(addNumber(this.getA0()));
        }
        // Append arrays of sine and cosine coefficients
        for (int i = 0; i < this.getDegree(); i++) {
            if (this.getA()[i] != 0) {
                buf.append(addNumber(this.getA()[i]) + " cos(" + (i + 1) + x);
            }
            if (this.getB()[i] != 0) {
                buf.append(addNumber(this.getB()[i]) + " sin(" + (i + 1) + x);
            }
        }
