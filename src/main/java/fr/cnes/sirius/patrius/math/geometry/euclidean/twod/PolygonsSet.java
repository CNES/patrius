...
public class PolygonsSet extends AbstractRegion<Euclidean2D, Euclidean1D> {
    ...
    private static List<SubHyperplane<Euclidean2D>> buildSet(final Vector2D[][] vertices) {
        // Initialization
        final List<SubHyperplane<Euclidean2D>> edges = new ArrayList<>();
    ...
    public Vector2D[][] getVertices() {
        ...
        if (this.vertices == null) {
            if (this.getTree(false).getCut() == null) {
                ...
                final ArrayList<List<ComparableSegment>> loops = new ArrayList<>();
                while (!sorted.isEmpty()) {
                    final AVLTree<ComparableSegment>.Node node = sorted.getSmallest();
                    final List<ComparableSegment> loop = PolygonsSet.followLoop(node, sorted);
                    if (loop != null) {
                        loops.add(loop);
                    }
                }
    ...
    public static Vector2D[][] sortVerticies(final Vector2D[][] v, final boolean trigo) {
        ...
        final Map<Double, Vector2D> sortedMap = new TreeMap<>();
    ...
    // Reason: Commons-Math code kept as such
    private static List<ComparableSegment> followLoop(final AVLTree<ComparableSegment>.Node node,
                                               final AVLTree<ComparableSegment> sorted) {
        // CHECKSTYLE: resume CyclomaticComplexity check
        // CHECKSTYLE: resume ReturnCount check

        final ArrayList<ComparableSegment> loop = new ArrayList<>();
