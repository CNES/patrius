...
public class BasicPhaseILPPDM {
    ...
    public RealVector findFeasibleInitialPoint() throws PatriusException {
        ...
        final RealVector ineq = this.originalProblem.getFi(ret);
    ...
    private class PhaseILPPrimalDualMethod extends LPPrimalDualMethod {
        ...
        PhaseILPPrimalDualMethod(final double maxS) {
            ...
            this.mieq = BasicPhaseILPPDM.this.originalProblem.getMieq() + 1;// the variable s has an upper bound
        }
        ...
        protected RealVector getFi(final RealVector vecXs) {
            final double[] ret = new double[BasicPhaseILPPDM.this.originalProblem.getMieq() + 1];
            ...
            for (int i = 0; i < BasicPhaseILPPDM.this.originalProblem.getDim(); i++) {
                ret[i] = -vecXs.getEntry(i) - s + BasicPhaseILPPDM.this.originalProblem.getLb().getEntry(i);// -x -s +lb
                                                                                                            // < 0
                ret[BasicPhaseILPPDM.this.originalProblem.getDim() + i] = vecXs.getEntry(i) -
                        s - BasicPhaseILPPDM.this.originalProblem.getUb().getEntry(i);// x -s -ub
                                                                                                             // < 0
            }
            ret[BasicPhaseILPPDM.this.originalProblem.getMieq()] = s - this.maxSValue;
        ...
        protected RealVector rDual(final RealVector gradF0X,
            ...
            for (int i = 0; i < BasicPhaseILPPDM.this.originalProblem.getDim(); i++) {
                double m = 0;
                m += -l.getEntry(i);
                m += l.getEntry(BasicPhaseILPPDM.this.originalProblem.getDim() + i);
                m1.setEntry(i, m);
            }
        ...
        protected RealVector gradSum(final double t,
            ...
            for (int i = 0; i < BasicPhaseILPPDM.this.originalProblem.getDim(); i++) {
                ...
                di += 1. / (t * fiXL);
                ddim += 1. / (t * fiXL);
                final double fiXU = fiX.getEntry(BasicPhaseILPPDM.this.originalProblem.getDim() + i);
                di += -1. / (t * fiXU);
                ddim += 1. / (t * fiXU);
        ...
        protected RealMatrix gradLSum(final RealVector l,
            ...
            for (int i = 0; i < BasicPhaseILPPDM.this.originalProblem.getDim(); i++) {
                ...
                didim += lIFiXL;
                ddimdim += lIFiXL;
                final double lDimI = l.getEntry(BasicPhaseILPPDM.this.originalProblem.getDim() + i);
                final double fiXU = fiX.getEntry(BasicPhaseILPPDM.this.originalProblem.getDim() + i);
        ...
        protected RealVector gradFiStepX(final RealVector stepX) {
            ...
            for (int i = 0; i < BasicPhaseILPPDM.this.originalProblem.getDim(); i++) {
                ...
                ret.setEntry(BasicPhaseILPPDM.this.originalProblem.getDim() + i,
                    stepX.getEntry(i) - stepX.getEntry(getDim() - 1));
            }
            // counts for the new upper bound on s
        ...
        protected boolean checkCustomExitConditions(final RealVector xS) {
            ...
            final RealVector ineqX = BasicPhaseILPPDM.this.originalProblem.getFi(x);
            ...
            if (getA() != null) {
                originalRPriX = AlgebraUtils.zMult(BasicPhaseILPPDM.this.originalProblem.getA(), x,
                    BasicPhaseILPPDM.this.originalProblem.getB(), -1);
            }
            final boolean isPrimalFeas = originalRPriX.getNorm() < BasicPhaseILPPDM.this.originalProblem
                .getToleranceFeas();
