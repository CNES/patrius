...
public class OptimizationRequestHandler {
    ...
    protected int forwardOptimizationRequest() throws PatriusException {
        if (this.successor != null) {
            ...
            final int retCode = this.successor.optimize();
    ...
    protected boolean isInDomainF0(final RealVector x) {
        final double f0X = this.request.getF0().value(x.toArray());
    ...
    protected RealVector getGradF0(final RealVector x) {
        return new ArrayRealVector(this.request.getF0().gradient(x.toArray()));
    }
    ...
    protected RealMatrix getHessF0(final RealVector x) {
        final double[][] hess = this.request.getF0().hessian(x.toArray());
        if (hess == null) {
    ...
    protected RealVector getFi(final RealVector vecX) {
        ...
        final ConvexMultivariateRealFunction[] fis = this.request.getFi();
        if (fis == null) {
            return null;
        }
    ...
    protected RealMatrix getGradFi(final RealVector vecX) {
        final RealMatrix ret = new BlockRealMatrix(this.request.getFi().length, vecX.getDimension());
        final double[] x = vecX.toArray();
        for (int i = 0; i < this.request.getFi().length; i++) {
            ret.setRow(i, this.request.getFi()[i].gradient(x));
        }
        return ret;
    }
    ...
    protected RealMatrix[] getHessFi(final RealVector vecX) {
        ...
        final RealMatrix[] ret = new RealMatrix[this.request.getFi().length];
        final double[] x = vecX.toArray();
        for (int i = 0; i < this.request.getFi().length; i++) {
            // take the hessian value
            final double[][] hess = this.request.getFi()[i].hessian(x);
            if (hess == null) {
                ret[i] = null;
            } else {
