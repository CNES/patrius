...
public class PolynomialChebyshevFunction implements UnivariateFunction,
        ParametricUnivariateFunction, Serializable {
    ...
    public PolynomialChebyshevFunction(final double start, final double end, final double[] c) {
        ...
        this.start = start;
        this.end = end;
        // Don't consider the 0 (< 1e-14) last terms
        while ((n > 1) && (MathLib.abs(c[n - 1]) < UtilsPatrius.DOUBLE_COMPARISON_EPSILON)) {
            --n;
        }
    ...
    protected static double evaluate(final double start, final double end,
        ...
        if ((argument - start) * (argument - end) > 0.) {
            throw PatriusException.createIllegalArgumentException(
                    PatriusMessages.CHEBYCHEV_POLYNOMIAL_OUT_OF_RANGE, argument, start, end);
        }
    ...
    public PolynomialChebyshevFunction polynomialDerivative() {
        ...
        final double[] derivativeCoefficients = new double[n];

        if (n > 1) { // n == 1 -> derivativeCoefficients = { 0 }
            final int degree = n - 1; // For optimization
            // "n-1" and "n-2" are special cases
            derivativeCoefficients[degree] = 0.;
            derivativeCoefficients[n - 2] = 2. * degree * this.coefficients[degree];
            ...
            final double k = 2. / (this.end - this.start);
            for (int i = 0; i < n; i++) {
