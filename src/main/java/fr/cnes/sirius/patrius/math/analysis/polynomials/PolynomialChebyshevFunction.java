 ...
 * <ul>
 * <li>Defining the first coefficient similarly to other coefficients. Chebyshev Polynomial is then
 * &Sigma;c<sub>i</sub>T<sub>i</sub> (for i in [0, n])</li>
 * <li>Adding a 0.5 multiplier to first coefficient. Chebyshev Polynomial is then c<sub>0</sub>/2 +
 * &Sigma;c<sub>i</sub>T<sub>i</sub> (for i in [1, n])</li>
 * </ul>
 ...
 * @author bonitt
 */
public class PolynomialChebyshevFunction implements UnivariateFunction, ParametricUnivariateFunction,
     ...
     *         if the argument isn't in the Chebyshev polynomial range
     */
    protected static double evaluate(final double start, final double end, final double[] coefficients,
        ...
        if (n > 1) {
            ...
            if (degree > 0) {
                ...
                for (int i = degree - 1; i > 0; i--) {
                    derivativeCoefficients[i - 1] = derivativeCoefficients[i + 1] + 2 * i * this.coefficients[i];
                }
            }
    ...
    /**
     * Returns the primitive of the Chebyshev polynomial. The constant term is computed so the
     * primitive polynomial is matching the given (x0,y0) point.
     * @param x0
     *        the absicssa at which the polynomial value is known
     * @param y0
     *        the value of the polynomial at x0
     * @return the primitive Chebyshev polynomial
     */
    @Override
    public PolynomialChebyshevFunction primitive(final double x0, final double y0) {
        ...
        // Norm.
        final double norm = 2. / (this.end - this.start);

        // Compute the primitive coefficients (general case)
        for (int i = 2; i < n - 1; i++) {
            resultCoeff[i] = ((this.coefficients[i - 1]) - (this.coefficients[i + 1])) / (2 * i * norm);
        }

        // Coefficient of index 1 is a particular case for normalization
        resultCoeff[1] = ((this.coefficients[0] / (norm / 2)) - (this.coefficients[2] / norm)) / 2;

        // Coefficients of indices n and n-1 are particular cases
        resultCoeff[n - 1] = this.coefficients[n - 2] / (2 * (n - 1) * norm);
        resultCoeff[n] = this.coefficients[n - 1] / (2 * n * norm);

        // All the known coefficients are from index 1 to n. For now, the first coefficient is set
        // to 0. So at this step, "result" is the result polynomial without the first coefficient.
        final PolynomialChebyshevFunction knownPartOfPolynomial = new PolynomialChebyshevFunction(this.start, this.end,
                resultCoeff);
        ...
        return new PolynomialChebyshevFunction(this.start, this.end, resultCoeff);
    ...
    private void checkSameRange(final PolynomialChebyshevFunction p) {
        if (this.start != p.start || this.end != p.end) {
            throw PatriusException.createIllegalArgumentException(PatriusMessages.CHEBYCHEV_POLYNOMIALS_NOT_SAME_RANGE,
                    this.start, this.end, p.start, p.end);
        }
    }
}
