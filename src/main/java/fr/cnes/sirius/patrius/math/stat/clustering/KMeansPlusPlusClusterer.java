 ...
 * @version $Id: KMeansPlusPlusClusterer.java 18108 2017-10-04 06:45:27Z bignon $
 * @since 2.0
 */
public class KMeansPlusPlusClusterer<T extends Clusterable<T>> {
     ...
     * Build a clusterer.
     * <p>
     * The default strategy for handling empty clusters that may appear during algorithm iterations is to split the
     * cluster with largest distance variance.
     * </p>
     * 
     * @param randomIn
     *        random generator to use for choosing initial centers
     */
    public KMeansPlusPlusClusterer(final Random randomIn) {
        this(randomIn, EmptyClusterStrategy.LARGEST_VARIANCE);
    }
     ...
     * @param randomIn
     *        random generator to use for choosing initial centers
     * @param emptyStrategyIn
     *        strategy to use for handling empty clusters that
     *        may appear during algorithm iterations
     * @since 2.2
     */
    public KMeansPlusPlusClusterer(final Random randomIn, final EmptyClusterStrategy emptyStrategyIn) {
        this.random = randomIn;
        this.emptyStrategy = emptyStrategyIn;
    }
    ...
    public List<Cluster<T>> cluster(final Collection<T> points, final int k,
                                    final int numTrials, final int maxIterationsPerTrial) {
                ...
                if (cluster.getPoints().isEmpty()) {
                    switch (this.emptyStrategy) {
                        ...
                        case FARTHEST_POINT:
                            ...
                            break;
                        default:
                            throw new ConvergenceException(PatriusMessages.EMPTY_CLUSTER_IN_K_MEANS);
                    }
                    emptyCluster = true;
                } else {
     ...
     */
    private static <T extends Clusterable<T>> int assignPointsToClusters(
            ...
            if (nextPointIndex >= 0) {
                ...
                if (resultSet.size() < k) {
                    ...
                    for (int j = 0; j < numPoints; j++) {
                        ...
                        if (!taken[j]) {
                            final double d = p.distanceFrom(pointList.get(j));
                            final double d2 = d * d;
