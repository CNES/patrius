...
public final class Sphere implements IEllipsoid, CrossSectionProvider, Serializable {
    ...
    private double absoluteDistanceTo(final Line line) {
        ...
        return line.distance(this.center) - this.radius;
    }
    ...
    public Vector3D[] getIntersectionPoints(final Line line) {
        ...
        if (line.getMinAbscissa() == Double.NEGATIVE_INFINITY) {
            ...
            final ArrayList<Vector3D> filteredIntersections = new ArrayList<>();
            for (final Vector3D point : intersections) {
    ...
    public Vector3D[] closestPointTo(final Line line) {
        ...
        if (this.intersects(line)) {
            // Intersection is found: points are identical
            // Intersections
            final Vector3D[] iscs = this.getIntersectionPoints(line);

            // Return intersection with lowest abscissa on line
            points[1] = line.pointOfMinAbscissa(iscs);
            points[0] = points[1];
        } else {
            // At first compute regardless line's min abscissa
            ...
            // Check line's point abscissa if line is semi-finite
            if (line.getMinAbscissa() != Double.NEGATIVE_INFINITY) {
                // Semi-finite line: calculate line's point abscissa and recompute points if needed
                if (line.getAbscissa(points[0]) < line.getMinAbscissa()) {
                    // Line's point does not belong to the semi-finite line: recompute it
