...
public class Ellipsoid implements IEllipsoid, Serializable {
    ...
    public Vector3D[] getIntersectionPoints(final Line line) {
        ...
        // Filter intersections if line is semi-finite
        if (line.getMinAbscissa() == Double.NEGATIVE_INFINITY) {
            // Infinite line: return array
            return result;
        } else {
            // Semi-finite line: calculate intersections abscissa and filter if needed
            final List<Vector3D> filteredIntersections = new ArrayList<>();
            for (final Vector3D point : result) {
                if (line.getAbscissa(point) > line.getMinAbscissa()) {
                    filteredIntersections.add(point);
                }
            }

            return filteredIntersections.toArray(new Vector3D[filteredIntersections.size()]);
        }
    }
    ...
    public boolean intersects(final Line line) {
        // Calculations are different whether the line is semi-finite or not
        if (line.getMinAbscissa() == Double.NEGATIVE_INFINITY) {
            // Classic case with infinite line: get discriminant
            final double[] delta = this.getDiscriminantAndCoeffs(new LineCoefficients(line));
            return (delta[0] > 0 || Precision.equals(delta[0], 0));
        } else {
            // Case with semi-finite line: return true if an intersection is found
    ...
    private PointLocation getPointLocation(final Vector3D point) {
        ...
        if (zEllipsoidSquare >= 0. && !Double.isNaN(zEllipsoidSquare)) {
            ...
            if (Precision.equals(absZ, zEllipsoid, EPS)) {
                ...
                result = Ellipsoid.getPointLocationPart2(x, y, z);
            }
        } else {
     ...
     */
    private static PointLocation getPointLocationPart2(final double x, final double y, final double z) {
     ...
     * @return triplet of ±1
     */
    private static double[] getOctantTransformation(final Vector3D point) {
    ...
    public Vector3D closestPointTo(final Vector3D point) {
        ...
        final double[] t = getOctantTransformation(localPoint);
        final double xT = t[0];
        final double yT = t[1];
        final double zT = t[2];
    ...
    public Vector3D[] closestPointTo(final Line line) {
        ...
        // Check if line is semi-finite or not
        final boolean isNotInfinite = (line.getMinAbscissa() != Double.NEGATIVE_INFINITY);

        // Instantiate infinite version of line if line is semi-finite
        // Semi-finite line could lead to exceptions thrown by Newton Algorithm
        Line infiniteLine = line;
        if (isNotInfinite) {
            infiniteLine = Line.createLine(line.getOrigin(), line.getDirection());
        }

        // Intersections with the infinite line
        final Vector3D[] iscs = this.getIntersectionPoints(infiniteLine);

        if (iscs.length == 0) {

            // No intersection found with an infinite line
            final LineCoefficients userLine = new LineCoefficients(infiniteLine);
            final double[] res = this.runNewtonAlgorithmLine(userLine);

            points[0] = this.getAffineStandardExpression(userLine.getPoint(res[3]));
            points[1] = this.getAffineStandardExpression(new Vector3D(res[0], res[1], res[2]));

            if (isNotInfinite) {
                // Intersection cannot be found with a semi-finite line
                // Check infinite line's closest point has accurate abscissa
                final boolean wrongAbscissa = (line.getAbscissa(points[0]) < line.getMinAbscissa());

                if (wrongAbscissa) {
                    // Recompute line's closest point: corresponds to min abscissa point
                    points[0] = line.pointAt(line.getMinAbscissa());
                    // Recompute the point on the ellipsoid as well
            ...
            // Intersections are found with infinite line: check abscissas and filter points
            double minIscs = Double.POSITIVE_INFINITY;
            for (final Vector3D point : iscs) {
                final double pointAbscissa = line.getAbscissa(point);
                if (pointAbscissa >= line.getMinAbscissa() && pointAbscissa < minIscs) {
                    minIscs = pointAbscissa;
                }
            }

            // Return filtered intersection with lowest abscissa on line if semi-finite line intersects
            // Recompute points if semi-finite line does not intersect (using line's min abscissa)
            if (minIscs == Double.POSITIVE_INFINITY) {
                points[0] = line.pointAt(line.getMinAbscissa());
                // Recompute the point on the ellipsoid as well
                points[1] = this.closestPointTo(points[0]);
            } else {
                // Get the first point that has its abscissa greater than the minimum abscissa
