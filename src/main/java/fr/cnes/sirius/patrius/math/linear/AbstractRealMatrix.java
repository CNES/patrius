...
public abstract class AbstractRealMatrix extends RealLinearOperator implements RealMatrix {
    ...
    public double[] getDiagonal() {
        if (!this.isSquare()) {
            throw new NonSquareMatrixException(this.getColumnDimension(), this.getRowDimension());
        }

        final int dimension = this.getRowDimension();
        final double[] diagonal = new double[dimension];
        for (int i = 0; i < dimension; i++) {
            diagonal[i] = this.getEntry(i, i);
        }
        return diagonal;
    }
