...
public class UniformCrossover<T> implements CrossoverPolicy {
    ...
    private ChromosomePair mate(final AbstractListChromosome<T> first,
        ...
        if (length != second.getLength()) {
            throw new DimensionMismatchException(PatriusMessages.INCOMPATIBLE_CHROMOSOME_ARRAYS_DIMENSION, length,
                second.getLength());
        }
        ...
        // and of the children
        final List<T> child1Rep = new ArrayList<>(first.getLength());
        final List<T> child2Rep = new ArrayList<>(second.getLength());
