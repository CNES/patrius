 ...
 * Ssee E.D. Andersen, K.D. Andersen, "Presolving in linear programming"
 * See Jacek Gondzio
...
public class LPPresolver {
    ...
    public void presolve(final RealVector originalC, final RealMatrix originalA, final RealVector originalB,
        ...
        if (originalLB == null) {
            originalLBCopy = new ArrayRealVector(this.originalN, this.unboundedLBValue);
        } else {
            originalLBCopy = originalLB.copy();
        }
        if (originalUB == null) {
            originalUBCopy = new ArrayRealVector(this.originalN, this.unboundedUBValue);
        } else {
        ...
        this.vRowLengthMap = new int[this.originalN + 1][];// the first position is for 0-length rows
        this.vColLengthMap = new int[this.originalMeq + 1][];// the first position is for 0-length columns
        final int[] vColCounter = new int[this.originalN];// counter of non-zero values in each column
        this.vRowPositions = new int[this.originalMeq][0];
        for (int i = 0; i < this.originalMeq; i++) {
            ...
            this.vRowPositions[i] = vRowPositionsI;
            if (this.vRowLengthMap[vRowPositionsI.length] == null) {
                this.vRowLengthMap[vRowPositionsI.length] = new int[] { i };
            } else {
                this.vRowLengthMap[vRowPositionsI.length] = addToSortedArray(this.vRowLengthMap[vRowPositionsI.length],
                    i);
            }
        }
        ...
        for (int j = 0; j < this.originalN; j++) {
            ...
            if (this.vColLengthMap[length] == null) {
                this.vColLengthMap[length] = new int[] { j };
            } else {
                this.vColLengthMap[length] = addToSortedArray(this.vColLengthMap[length], j);
            }
        }
        for (int i = 0; i < this.vRowPositions.length; i++) {
            final int[] vRowPositionsI = this.vRowPositions[i];
            for (int j = 0; j < vRowPositionsI.length; j++) {
                final int col = vRowPositionsI[j];
                this.vColPositions[col][this.vColPositions[col].length - vColCounter[col]] = i;
        ...
        final RealVector ylb = new ArrayRealVector(this.originalMeq, this.unboundedLBValue);
        // lagrangian upper bounds for linear constraints (A rows)
        final RealVector yub = new ArrayRealVector(this.originalMeq, this.unboundedUBValue);
        // lagrangian lower bounds for lb constraints
        final RealVector zlb = new ArrayRealVector(this.originalN, this.unboundedLBValue);
        // lagrangian upper bounds for ub constraints
        final RealVector zub = new ArrayRealVector(this.originalN, this.unboundedUBValue);
        ...
        while (this.someReductionDone) {
            iteration++;
            this.someReductionDone = false;// reset
            checkProgress(c, a, b, lb, ub, ylb, yub, zlb, zub);
            // Check rows
        ...
        this.presolvedN = 0;
        this.presolvedX = new int[this.originalN];// longer than it needs
        Arrays.fill(this.presolvedX, -1);
        final int[] presolvedPositions = new int[this.originalN];
        Arrays.fill(presolvedPositions, -1);
        for (int j = 0; j < this.indipendentVariables.length; j++) {
            if (this.indipendentVariables[j]) {
                this.presolvedX[this.presolvedN] = j;
                presolvedPositions[j] = this.presolvedN;
                this.presolvedN++;
            }
        }
        ...
        int cntR = 0;
        for (int i = 0; this.presolvedA != null && i < this.vRowPositions.length; i++) {
            final int[] vRowPositionsI = this.vRowPositions[i];
            if (vRowPositionsI.length > 0) {
                for (int j = 0; j < vRowPositionsI.length; j++) {
                    final int jnz = vRowPositionsI[j];
                    final int col = presolvedPositions[jnz];
                    this.presolvedA.setEntry(cntR, col, a.getEntry(i, jnz));
                    this.presolvedB.setEntry(cntR, b.getEntry(i));
                }
                cntR++;
            }
        }
        ...
        for (int i = 0; i < this.vRowPositions.length; i++) {
            if (this.vRowPositions[i].length > 0) {
                this.presolvedYlb.setEntry(cntR, ylb.getEntry(i));
                this.presolvedYub.setEntry(cntR, yub.getEntry(i));
                cntR++;
            }
        }
        for (int j = 0; j < this.presolvedN; j++) {
            final int col = this.presolvedX[j];
            this.presolvedC.setEntry(j, c.getEntry(col));
            this.presolvedLB.setEntry(j, lb.getEntry(col));
            this.presolvedUB.setEntry(j, ub.getEntry(col));
            this.presolvedZlb.setEntry(j, zlb.getEntry(col));
            this.presolvedZub.setEntry(j, zub.getEntry(col));
        }
    ...
    private void objectiveFunctionNormalization() {
        if (this.presolvedC != null && this.presolvedC.getDimension() > 0) {
            final double normC = this.presolvedC.getNorm();
            if (normC > 0) {
    ...
    public double[] presolve(final double[] x) {
        ...
        for (int i = 0; i < this.presolvingStack.size(); i++) {
            this.presolvingStack.get(i).preSolve(presolX);
        }
        final double[] ret = new double[this.presolvedN];
        int cntPosition = 0;
        for (int i = 0; i < presolX.length; i++) {
            if (this.indipendentVariables[i]) {
    ...
    public double[] postsolve(final double[] x) {
        ...
        final double[] postsolvedX = new double[this.originalN];

        if (this.t != null) {
            // rescaling has been done: x = T.x1
            for (int i = 0; i < x.length; i++) {
                x[i] = this.t.getEntry(i) * x[i];
            }
        }
        ...
        for (int i = this.presolvingStack.size() - 1; i > -1; i--) {
            ...
            if (this.indipendentVariables[j]) {
                ...
                if (!isLBUnbounded(lb.getEntry(j)) && isZero(lb.getEntry(j) - ub.getEntry(j))) {
                    ...
                    for (int k = 0; k < this.vRowPositions.length; k++) {
                        final int[] vRowPositionsK = this.vRowPositions[k];
                        for (int i = 0; i < vRowPositionsK.length; i++) {
                            if (vRowPositionsK[i] == j) {
                                ...
                                this.vRowPositions[k] = removeElementFromSortedArray(vRowPositionsK, j);
                                changeRowsLengthPosition(k, this.vRowPositions[k].length + 1,
                                    this.vRowPositions[k].length);
                                a.setEntry(k, j, 0);
                                break;
                            }
                            if (vRowPositionsK[i] > j) {
                    ...
                    changeColumnsLengthPosition(j, this.vColPositions[j].length, 0);
                    this.vColPositions[j] = new int[] {};
    ...
    private void removeSingletonRows(final RealVector c, final RealMatrix a, final RealVector b, final RealVector lb,
        final RealVector ub, final RealVector ylb, final RealVector yub, final RealVector zlb, final RealVector zub) {
        int i = 0;
        while (i < this.vRowPositions.length) {
            final int[] vRowPositionsI = this.vRowPositions[i];
            if (vRowPositionsI.length == 1) {
                    ...
                    if (k != i) {
                        ...
                        for (int nz = 0; nz < vRowPositionsK.length; nz++) {
                            if (vRowPositionsK[nz] == j) {
                                ...
                                a.setEntry(k, j, 0.);
                                this.vRowPositions[k] = (int[]) ArrayUtils.remove(vRowPositionsK, nz);
                ...
                changeColumnsLengthPosition(j, this.vColPositions[j].length, 0);
                this.vColPositions[j] = new int[] {};
                this.vRowPositions[i] = (int[]) ArrayUtils.remove(vRowPositionsI, 0);// this row has
                                                                                // only this
                                                                                // nz-entry
                changeRowsLengthPosition(i, this.vRowPositions[i].length + 1, this.vRowPositions[i].length);
                this.someReductionDone = true;
                i = -1;// restart
            }
            i++;
        ...
        for (int i = 0; i < this.vRowPositions.length; i++) {
            ...
            if (vRowPositionsI.length > 0) {
                ...
                for (int nz = 0; nz < vRowPositionsI.length; nz++) {
                    ...
                    if (aij > 0) {
                        // j in P
                        this.g[i] += aij * lb.getEntry(j);
                        this.h[i] += aij * ub.getEntry(j);
                        allNegative = false;
                    } else if (aij < 0) {
                        // j in M
                        this.g[i] += aij * ub.getEntry(j);
                        this.h[i] += aij * lb.getEntry(j);
                        allPositive = false;
                    }
                ...
                if (this.h[i] < b.getEntry(i) || b.getEntry(i) < this.g[i]) {
                ...
                if (isZero(this.g[i] - b.getEntry(i))) {
                    // forcing constraint
                ...
                } else if (isZero(this.h[i] - b.getEntry(i))) {
                    // forcing constraint
                ...
                if (forcedVariablesI.length > 0) {
                    ...
                    for (int fv = 0; fv < forcedVariablesI.length; fv++) {
                            ...
                            if (k != i) {
                                ...
                                for (int nz = 0; nz < vRowPositionsK.length; nz++) {
                                    if (vRowPositionsK[nz] == j) {
                                        ...
                                        changeRowsLengthPosition(k, this.vRowPositions[k].length + 1,
                                            this.vRowPositions[k].length);
                                        changeColumnsLengthPosition(j, this.vColPositions[j].length,
                                            this.vColPositions[j].length - 1);
                                        // ordered row loop
                                        this.vColPositions[j] = (int[]) ArrayUtils.remove(this.vColPositions[j], 0);
                                        // Exit
                                        break;
                                    } else if (vRowPositionsK[nz] > j) {
                                        // Exit
                        ...
                        a.setEntry(i, j, 0.);
                        b.setEntry(i, 0.);
                        // this row has only this nz-entry
                        this.vRowPositions[i] = removeElementFromSortedArray(this.vRowPositions[i], j);
                        changeRowsLengthPosition(i, this.vRowPositions[i].length + 1, this.vRowPositions[i].length);
                ...
                final boolean aa = this.g[i] >= 0 && b.getEntry(i) >= 0;
                final boolean bb = this.h[i] <= 0 && b.getEntry(i) <= 0;
                final boolean t1 = aa && allPositive;
                // same as above with a sign change
                final boolean t2 = bb && allNegative;
                if (t1 || t2) {
    ...
    private void checkColumnSingletons(final RealVector c, final RealMatrix a, final RealVector b, final RealVector lb,
        ...
        for (int col = 0; col < this.vColPositions.length; col++) {
            if (this.vColPositions[col].length == 1) {
                final int row = this.vColPositions[col][0];
                int[] vRowPositionsRow = this.vRowPositions[row];
                ...
                if (isLBUnbounded || isUBUnbounded) {
                    ...
                    if (isLBUnbounded && isUBUnbounded) {
                        ...
                        for (int j = 0; j < vRowPositionsRow.length; j++) {
                            ...
                            changeColumnsLengthPosition(column, this.vColPositions[column].length,
                                this.vColPositions[column].length - 1);
                            this.vColPositions[column] = removeElementFromSortedArray(this.vColPositions[column], row);
                            a.setEntry(row, column, 0.);
                        }
                        changeRowsLengthPosition(row, this.vRowPositions[row].length, 0);
                        this.vRowPositions[row] = new int[] {};
                        this.vColPositions[col] = new int[] {};
                ...
                if (arCol > 0) {
                    impliedL = (b.getEntry(row) - this.h[row]) / arCol + ubCol;
                    impliedU = (b.getEntry(row) - this.g[row]) / arCol + lbCol;
                } else {
                    impliedL = (b.getEntry(row) - this.g[row]) / arCol + ubCol;
                    impliedU = (b.getEntry(row) - this.h[row]) / arCol + lbCol;
                }
                ...
                if (vRowPositionsRow.length == 2 || isImpliedFree) {
                    ...
                    if (vRowPositionsRow.length == 2) {
                        ...
                        a.setEntry(row, y, 0.);
                        b.setEntry(row, 0.);
                        changeColumnsLengthPosition(col, this.vColPositions[col].length, 0);
                        this.vColPositions[col] = new int[] {};
                        ...
                        changeColumnsLengthPosition(vRowPositionsRow[0],
                            this.vColPositions[vRowPositionsRow[0]].length,
                            this.vColPositions[vRowPositionsRow[0]].length - 1);
                        this.vColPositions[vRowPositionsRow[0]] =
                            removeElementFromSortedArray(this.vColPositions[vRowPositionsRow[0]], row);
                        this.vRowPositions[row] = new int[] {};
                        this.someReductionDone = true;
                        continue;
                    }
                    ...
                    yub.setEntry(row, c.getEntry(col) / a.getEntry(row, col));// ok, but already
                                                                              // stated above
                    for (int cc = 0; cc < this.vRowPositions[row].length; cc++) {
                        final int column = this.vRowPositions[row][cc];
                        if (column == col) {
                            continue;
                        }
    ...
    private void removeDominatedColumns(final RealVector c, final RealMatrix a, final RealVector b, final RealVector lb,
        final RealVector ub, final RealVector ylb, final RealVector yub, final RealVector zlb, final RealVector zub) {
        for (int col = 0; col < this.vColPositions.length; col++) {
            final int[] vColPositionsCol = this.vColPositions[col];
    ...
    private void removeDuplicateRow(final RealVector c, final RealMatrix a, final RealVector b, final RealVector lb,
        ...
        for (int i = startingLength; i < this.vRowLengthMap.length; i++) {
            final int[] vRowLengthMapI = this.vRowLengthMap[i];
            ...
            for (int j = 0; !stop && j < vRowLengthMapI.length; j++) {
                ...
                if (this.vRowPositions[prow].length == 0 || prow < this.nOfSlackVariables) {
                    // the upper left part of A is diagonal if there are the slack variables:
                    // there is no sparsity superset possible
                    continue;
                }
                final int[] vRowPositionsProw = this.vRowPositions[prow];
                for (int si = i; !stop && si < this.vRowLengthMap.length; si++) {
                    // look into rows with superset sparsity pattern
                    final int[] vRowLengthMapSI = this.vRowLengthMap[si];
                    ...
                    for (int sj = 0; sj < vRowLengthMapSI.length; sj++) {
                        ...
                        final int[] vRowPositionsSrow = this.vRowPositions[srow];
                        ...
                        if (isSubsetSparsityPattern(vRowPositionsProw, vRowPositionsSrow)) {
                            ...
                            for (int k = 0; k < candidatedColumns.size(); k++) {
                                ...
                                if (this.vColPositions[col].length > 1
                                        && this.vColPositions[col].length < lessFilledColumnLength) {
                            ...
                            for (int ti = 0; ti < vRowPositionsProw.length; ti++) {
                                ...
                                if (isZero(nv)) {
                                    this.vRowPositions[srow] = removeElementFromSortedArray(this.vRowPositions[srow],
                                        cc);
                                    changeColumnsLengthPosition(cc, this.vColPositions[cc].length,
                                        this.vColPositions[cc].length - 1);
                                    this.vColPositions[cc] = removeElementFromSortedArray(this.vColPositions[cc], srow);
                                    changeRowsLengthPosition(srow, this.vRowPositions[srow].length + 1,
                                        this.vRowPositions[srow].length);
    ...
    private void removeDuplicateColumn(final RealVector c, final RealMatrix a, final RealVector b, final RealVector lb,
        ...
        for (int i = startingLength; i < this.vColLengthMap.length; i++) {
            final int[] vColLengthMapI = this.vColLengthMap[i];
            ...
            for (int j = 0; !stop && j < vColLengthMapI.length; j++) {
                ...
                final int[] vColPositionsPcol = this.vColPositions[pcol];
                if (pcol < this.nOfSlackVariables) {
                    // the upper left part of A is diagonal if there are the slack variables:
                ...
                for (int sj = j + 1; !stop && sj < vColLengthMapI.length; sj++) {
                    ...
                    final int[] vColPositionsScol = this.vColPositions[scol];
                        ...
                        if (!isZero(cAlfaC)) {
                                ...
                                if (v >= 0 && cAlfaC > 0) {
                                    ...
                                    if (!isLBPUnbounded) {// check table 1, row 1
                                        if (isUBPUnbounded) {
                                            ...
                                            addToPresolvingStack(new LinearDependency(pcol, null, null,
                                                lb.getEntry(pcol)));
                            ...
                            if (vp || vm) {
                                ...
                                for (int rr = 0; rr < vColPositionsPcol.length; rr++) {
                                    ...
                                    this.vRowPositions[row] = removeElementFromSortedArray(this.vRowPositions[row],
                                        pcol);
                                    a.setEntry(row, pcol, 0.);
                                    changeRowsLengthPosition(row, this.vRowPositions[row].length + 1,
                                        this.vRowPositions[row].length);
                                }
                                changeColumnsLengthPosition(pcol, this.vColPositions[pcol].length, 0);
                                this.vColPositions[pcol] = new int[] {};
    ...
    private void removeDoubletonRow(final RealVector c, final RealMatrix a, final RealVector b, final RealVector lb,
        ...
        for (int i = 0; i < this.vRowPositions.length; i++) {
            ...
            if (vRowPositionsI.length == 2) {
                            ...
                            if (vRowPositionsK[j] == x) {
                                ...
                                if (!isZero(akYnew)) {
                                    ...
                                    if (!ArrayUtils.contains(vRowPositionsK, y)) {
                                        this.vRowPositions[k] = addToSortedArray(vRowPositionsK, y);
                                        changeRowsLengthPosition(k, this.vRowPositions[k].length - 1,
                                            this.vRowPositions[k].length);
                                        changeColumnsLengthPosition(y, this.vColPositions[y].length,
                                            this.vColPositions[y].length + 1);
                                        this.vColPositions[y] = addToSortedArray(this.vColPositions[y], k);
                                    }
                                } else {
                                    // remove
                                    this.vRowPositions[k] = removeElementFromSortedArray(vRowPositionsK, y);
                                    changeRowsLengthPosition(k, this.vRowPositions[k].length + 1,
                                        this.vRowPositions[k].length);
                                    changeColumnsLengthPosition(y, this.vColPositions[y].length,
                                        this.vColPositions[y].length - 1);
                                    this.vColPositions[y] = removeElementFromSortedArray(this.vColPositions[y], k);
                                    a.setEntry(k, y, 0.);
                                }
                                b.setEntry(k, b.getEntry(k) - akX * q);
                                a.setEntry(k, x, 0.);
                                this.vRowPositions[k] = removeElementFromSortedArray(this.vRowPositions[k], x);
                                changeRowsLengthPosition(k, this.vRowPositions[k].length + 1,
                                    this.vRowPositions[k].length);
                                changeColumnsLengthPosition(x, this.vColPositions[x].length,
                                    this.vColPositions[x].length - 1);
                                this.vColPositions[x] = removeElementFromSortedArray(this.vColPositions[x], k);
                                break;
                ...
                changeColumnsLengthPosition(x, this.vColPositions[x].length, 0);
                this.vColPositions[x] = new int[] {};
                changeColumnsLengthPosition(y, this.vColPositions[y].length, this.vColPositions[y].length - 1);
                this.vColPositions[y] = removeElementFromSortedArray(this.vColPositions[y], i);
                a.setEntry(i, x, 0.);
                a.setEntry(i, y, 0.);
                b.setEntry(i, 0);
    ...
    private void scaling() {
        if (this.presolvedA != null) {
            ...
            final RealVector[] uV = rescaler.getMatrixScalingFactors(this.presolvedA);
            this.r = uV[0];
            this.t = uV[1];
            // to check rescaling:
            ...
            for (int i = 0; i < this.r.getDimension(); i++) {
                final double ri = this.r.getEntry(i);
                this.presolvedB.setEntry(i, this.presolvedB.getEntry(i) * ri);
            }
            // Assign min and max rescaled bounds
            this.minRescaledLB = Double.MAX_VALUE;
            this.maxRescaledUB = -Double.MAX_VALUE;
            for (int i = 0; i < this.t.getDimension(); i++) {
                final double ti = this.t.getEntry(i);

                this.presolvedC.setEntry(i, this.presolvedC.getEntry(i) * ti); // c -> T.c

                final double lbi = this.presolvedLB.getEntry(i) / ti;
                this.presolvedLB.setEntry(i, lbi);
                this.minRescaledLB = MathLib.min(this.minRescaledLB, lbi);

                final double ubi = this.presolvedUB.getEntry(i) / ti;
                this.presolvedUB.setEntry(i, ubi);
    ...
    private void pruneFixedVariable(final int x, final RealVector c, final RealMatrix a, final RealVector b,
        ...
        for (int i = 0; i < this.vRowPositions.length; i++) {
            if (ArrayUtils.contains(this.vRowPositions[i], x)) {
                // remove elements
                this.vRowPositions[i] = removeElementFromSortedArray(this.vRowPositions[i], x);
                changeRowsLengthPosition(i, this.vRowPositions[i].length + 1, this.vRowPositions[i].length);
    ...
    private void checkProgress(final RealVector c, final RealMatrix a, final RealVector b, final RealVector lb,
        ...
        if (Double.isNaN(this.expectedTolerance)) {
            ...
            // start
            final RealVector x = MatrixUtils.createRealVector(this.expectedSolution);
    ...
    private void changeRowsLengthPosition(final int rowIndex, final int lengthIndexFrom, final int lengthIndexTo) {
        ...
        if (this.vRowLengthMap[lengthIndexTo] == null) {
            this.vRowLengthMap[lengthIndexTo] = new int[] {};
        }
        this.vRowLengthMap[lengthIndexTo] = addToSortedArray(this.vRowLengthMap[lengthIndexTo], rowIndex);
        this.vRowLengthMap[lengthIndexFrom] = removeElementFromSortedArray(this.vRowLengthMap[lengthIndexFrom],
            rowIndex);
    }
    ...
    private void changeColumnsLengthPosition(final int colIndex, final int lengthIndexFrom, final int lengthIndexTo) {
        ...
        if (this.vColLengthMap[lengthIndexTo] == null) {
            this.vColLengthMap[lengthIndexTo] = new int[] {};
        }
        this.vColLengthMap[lengthIndexTo] = addToSortedArray(this.vColLengthMap[lengthIndexTo], colIndex);
        this.vColLengthMap[lengthIndexFrom] = removeElementFromSortedArray(this.vColLengthMap[lengthIndexFrom],
            colIndex);
    }
    ...
    private class LinearDependency extends AbstractPresolvingStackElement {
        ...
        public String toString() {
            ...
            sb.append(BRACKET6 + this.x + BRACKET3);
            for (int i = 0; this.xi != null && i < this.xi.length; i++) {
                sb.append(PLUS + this.mi[i] + BRACKET4 + this.xi[i] + BRACKET1);
            }
            sb.append(PLUS + this.q);
            return sb.toString();
        }
    }
    ...
    private class DuplicatedColumn extends AbstractPresolvingStackElement {
        ...
        protected void postSolve(final double[] postsolvedX) {
            ...
            if (this.v > 0) {
                ...
                final double p = postsolvedX[this.xkPrime];
                postsolvedX[this.xk] = MathLib.max(this.lbk, p - this.v * this.ubj);
                ...
                final double p = postsolvedX[this.xkPrime];
                postsolvedX[this.xk] = MathLib.max(this.lbk, p - this.v * this.lbj);
                postsolvedX[this.xj] = (p - postsolvedX[this.xk]) / this.v;
            } else {
        ...
        public String toString() {
            final StringBuffer sb = new StringBuffer();
            sb.append(BRACKET6 + this.xk + "]=-" + this.v + BRACKET4 + this.xj + "] + xPrime[" + this.xkPrime
                    + BRACKET1);
            return sb.toString();
        }
    }
