 ...
 * 
 * @version $Id: DormandPrince853StepInterpolator.java 18108 2017-10-04 06:45:27Z bignon $
 * @since 1.2
 */
...
class DormandPrince853StepInterpolator
    ...
    public DormandPrince853StepInterpolator(final DormandPrince853StepInterpolator interpolator) {
        ...
        if (interpolator.currentState == null) {
            ...
            for (int k = 0; k < this.yDotKLast.length; ++k) {
                ...
                System.arraycopy(interpolator.yDotKLast[k], 0, this.yDotKLast[k], 0,
                    dimension);
            }
    ...
    protected void computeInterpolatedStateAndDerivatives(final double theta,
        ...
        if (!this.vectorsInitialized) {
            ...
            for (int i = 0; i < this.interpolatedState.length; ++i) {
                ...
                final double yDot8 = this.yDotK[7][i];
                final double yDot9 = this.yDotK[8][i];
                final double yDot10 = this.yDotK[9][i];
                final double yDot11 = this.yDotK[10][i];
                final double yDot12 = this.yDotK[ELEVEN][i];
                final double yDot13 = this.yDotK[TWELVE][i];
                final double yDot14 = this.yDotKLast[0][i];
                final double yDot15 = this.yDotKLast[1][i];
                final double yDot16 = this.yDotKLast[2][i];
                this.v[0][i] = B_01 * yDot1 + B_06 * yDot6 + B_07 * yDot7 +
                    B_08 * yDot8 + B_09 * yDot9 + B_10 * yDot10 +
                    B_11 * yDot11 + B_12 * yDot12;
                this.v[1][i] = yDot1 - this.v[0][i];
                this.v[2][i] = this.v[0][i] - this.v[1][i] - this.yDotK[TWELVE][i];
                for (int k = 0; k < D.length; ++k) {
                    this.v[k + 3][i] = D[k][0] * yDot1 + D[k][1] * yDot6 + D[k][2] * yDot7 +
                        D[k][3] * yDot8 + D[k][4] * yDot9 + D[k][5] * yDot10 +
                        D[k][6] * yDot11 + D[k][7] * yDot12 + D[k][8] * yDot13 +
                        D[k][9] * yDot14 + D[k][10] * yDot15 + D[k][ELEVEN] * yDot16;
                }
            }
        ...
        if ((this.previousState != null) && (theta <= HALF)) {
            for (int i = 0; i < this.interpolatedState.length; ++i) {
                this.interpolatedState[i] = this.previousState[i] +
                    theta * this.h * (this.v[0][i] +
                    eta * (this.v[1][i] +
                        theta * (this.v[2][i] +
                            eta * (this.v[3][i] +
                                theta * (this.v[4][i] +
                                    eta * (this.v[5][i] +
                                        theta * (this.v[6][i])))))));
                this.interpolatedDerivatives[i] = this.v[0][i] + dot1 * this.v[1][i] + dot2 * this.v[2][i] +
                    dot3 * this.v[3][i] + dot4 * this.v[4][i] +
                    dot5 * this.v[5][i] + dot6 * this.v[6][i];
            }
        } else {
            for (int i = 0; i < this.interpolatedState.length; ++i) {
                this.interpolatedState[i] = this.currentState[i] -
                    oneMinusThetaH * (this.v[0][i] -
                    theta * (this.v[1][i] +
                        theta * (this.v[2][i] +
                            eta * (this.v[3][i] +
                                theta * (this.v[4][i] +
    ...
    protected void doFinalize() {
        ...
        for (int j = 0; j < this.currentState.length; ++j) {
            s = K14_01 * this.yDotK[0][j] + K14_06 * this.yDotK[5][j] + K14_07 * this.yDotK[6][j] +
                K14_08 * this.yDotK[7][j] + K14_09 * this.yDotK[8][j] + K14_10 * this.yDotK[9][j] +
                K14_11 * this.yDotK[10][j] + K14_12 * this.yDotK[ELEVEN][j] + K14_13 * this.yDotK[TWELVE][j];
            yTmp[j] = this.currentState[j] + this.h * s;
        }
        this.integrator.computeDerivatives(pT + C14 * this.h, yTmp, this.yDotKLast[0]);

        // k15
        for (int j = 0; j < this.currentState.length; ++j) {
            s = K15_01 * this.yDotK[0][j] + K15_06 * this.yDotK[5][j] + K15_07 * this.yDotK[6][j] +
                K15_08 * this.yDotK[7][j] + K15_09 * this.yDotK[8][j] + K15_10 * this.yDotK[9][j] +
                K15_11 * this.yDotK[10][j] + K15_12 * this.yDotK[ELEVEN][j] + K15_13 * this.yDotK[TWELVE][j] +
                K15_14 * this.yDotKLast[0][j];
            yTmp[j] = this.currentState[j] + this.h * s;
        }
        this.integrator.computeDerivatives(pT + C15 * this.h, yTmp, this.yDotKLast[1]);

        // k16
        for (int j = 0; j < this.currentState.length; ++j) {
            s = K16_01 * this.yDotK[0][j] + K16_06 * this.yDotK[5][j] + K16_07 * this.yDotK[6][j] +
                K16_08 * this.yDotK[7][j] + K16_09 * this.yDotK[8][j] + K16_10 * this.yDotK[9][j] +
                K16_11 * this.yDotK[10][j] + K16_12 * this.yDotK[ELEVEN][j] + K16_13 * this.yDotK[TWELVE][j] +
                K16_14 * this.yDotKLast[0][j] + K16_15 * this.yDotKLast[1][j];
            yTmp[j] = this.currentState[j] + this.h * s;
        }
        this.integrator.computeDerivatives(pT + C16 * this.h, yTmp, this.yDotKLast[2]);
    ...
    public void writeExternal(final ObjectOutput oo) throws IOException {
        ...
        oo.writeInt(dimension);
        for (int i = 0; i < dimension; ++i) {
            oo.writeDouble(this.yDotKLast[0][i]);
            oo.writeDouble(this.yDotKLast[1][i]);
            oo.writeDouble(this.yDotKLast[2][i]);
        }
    ...
    public void readExternal(final ObjectInput oi) throws IOException, ClassNotFoundException {
        ...
        for (int i = 0; i < dimension; ++i) {
            this.yDotKLast[0][i] = oi.readDouble();
            this.yDotKLast[1][i] = oi.readDouble();
            this.yDotKLast[2][i] = oi.readDouble();
        }
