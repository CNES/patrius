...
public class Vector2D implements Vector<Euclidean2D> {
     ...
     */
    public static double dotProduct(final Vector2D p1, final Vector2D p2) {
     ...
     */
    public static double angle(final Vector2D p1, final Vector2D p2) {

        // compute the norm product
        final double normProduct = p1.getNorm() * p2.getNorm();
        if (normProduct == 0) {
            throw new MathArithmeticException(PatriusMessages.ZERO_NORM);
        }

        final double dot = p1.dotProduct(p2);
        ...
        if ((dot < -threshold) || (dot > threshold)) {
            ...
            if (dot >= 0) {
                angle = MathLib.asin(v3.getNorm() / normProduct);
            } else {
