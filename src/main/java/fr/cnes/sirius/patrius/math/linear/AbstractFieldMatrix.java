...
public abstract class AbstractFieldMatrix<T extends FieldElement<T>>
     ...
     * @return a new array
     * @deprecated as of 4.5, replaced by {@link MathArrays#buildArray(Field, int, int)}
     */
    @Deprecated
    ...
    public FieldMatrix<T> power(final int p) {
        ...
        final ArrayList<FieldMatrix<T>> results = new ArrayList<FieldMatrix<T>>(
    ...
    public FieldVector<T> operate(final FieldVector<T> v) {
        try {
            return new ArrayFieldVector<T>(this.field, this.operate(((ArrayFieldVector<T>) v).getDataRef()), false);
    ...
    public FieldVector<T> preMultiply(final FieldVector<T> v) {
        try {
            return new ArrayFieldVector<T>(this.field, this.preMultiply(((ArrayFieldVector<T>) v).getDataRef()), false);
        } catch (final ClassCastException cce) {
            final int nRows = this.getRowDimension();
