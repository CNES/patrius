 ...
 * <p>
 * In addition to transformation of real vectors, the Hadamard transform can transform integer vectors into integer
 * vectors. However, this integer transform cannot be inverted directly. Due to a scaling factor it may lead to rational
 * results. As an example, the inverse transform of integer vector (0, 1, 0, 1) is rational vector (1/2, -1/2, 0, 0).
 * 
 * @version $Id: FastHadamardTransformer.java 18108 2017-10-04 06:45:27Z bignon $
 * @since 2.0
 */
public class FastHadamardTransformer implements RealTransformer, Serializable {
     ...
     * </ul>
     * </li>
     * <li>The consputation of {@code D_top} and {@code D_bottom} are best understood with the above example (for
     * {@code N = 8}).
     ...
     *         if the length of the data array is not a power of two
     */
    protected double[] fht(final double[] x) {

        final int n = x.length;

        if (!ArithmeticUtils.isPowerOfTwo(n)) {
            throw new MathIllegalArgumentException(
                PatriusMessages.NOT_POWER_OF_TWO,
                Integer.valueOf(n));
        }
        final int halfN = n / 2;
