    ...
    public void append(final ContinuousOutputModel model) {
        ...
        if (this.steps.isEmpty()) {
            ...
            if (this.getInterpolatedState().length != model.getInterpolatedState().length) {
                // Exception
                throw new DimensionMismatchException(PatriusMessages.INCOMPATIBLE_STATE_VECTOR_DIMENSIONS,
                    model.getInterpolatedState().length, this.getInterpolatedState().length);
            }
