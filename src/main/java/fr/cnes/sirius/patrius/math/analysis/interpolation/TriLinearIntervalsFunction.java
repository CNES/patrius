...
public class TriLinearIntervalsFunction extends AbstractLinearIntervalsFunction implements TrivariateFunction {
    ...
    public double value(final double x, final double y, final double z) {
        ...
        valXYZ[0] = checkInterpValues(this.xtab, x, xindex);
        valXYZ[1] = checkInterpValues(this.ytab, y, yindex);
        valXYZ[2] = checkInterpValues(this.ztab, z, zindex);
        ...
        return controlOnInterpolation(x, y, z, x0, x1, y0, y1, z0, z1, f000, f001, f010, f011, f100, f101, f110, f111);
    }
     ...
     */
    private static double checkInterpValues(final double[] tab, final double x, final int xindex) {
    ...
    private double controlOnInterpolation(final double x, final double y, final double z, final double x0,
                                          ...
                                          final double f101, final double f110, final double f111) {
        ...
        if (x0 == x1) {
            if (y0 == y1) {
                // z0 != z1
                fxyz = interp1D(f000, f001, z0, z1, z);
            } else {
                // y0 != y1
                // Split into 2 cases : z0 = z1 or not
                if (z0 == z1) {
                    fxyz = interp1D(f000, f010, y0, y1, y);
                } else {
                    fxyz = interp2D(f000, f010, f001, f011, y0, y1, z0, z1, y, z);
                }
            }
        }
        ...
        if (y0 == y1) {
            if (z0 == z1) {
                fxyz = interp1D(f000, f100, x0, x1, x);
            } else {
                final double fval1 = interp1D(f000, f100, x0, x1, x);
                final double fval2 = interp1D(f001, f101, x0, x1, x);
                fxyz = interp1D(fval1, fval2, z0, z1, z);
            }
        }
        ...
        if (z0 == z1 && Double.isNaN(fxyz)) {
            final double fval1 = interp1D(f000, f100, x0, x1, x);
            final double fval2 = interp1D(f010, f110, x0, x1, x);
            fxyz = interp1D(fval1, fval2, y0, y1, y);
        }

        // Return fvalue if defined, interpolate otherwise
        if (Double.isNaN(fxyz)) {
            // interpolation 1D in the x direction
            final double fxy0z0 = interp1D(f000, f100, x0, x1, x);
            final double fxy1z0 = interp1D(f010, f110, x0, x1, x);
            final double fxy0z1 = interp1D(f001, f101, x0, x1, x);
            final double fxy1z1 = interp1D(f011, f111, x0, x1, x);

            // interpolation 2D in the y,z directions, with the already interpolated values in x
            fxyz = interp2D(fxy0z0, fxy1z0, fxy0z1, fxy1z1, y0, y1, z0, z1, y, z);
        }
