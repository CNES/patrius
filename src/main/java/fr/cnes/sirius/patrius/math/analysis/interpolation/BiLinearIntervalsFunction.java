 ...
 * If double[] are given to the constructor, they should be sorted by increasing order (duplicates
 * are allowed).
 ...
 * @since 2.3
 */
public class BiLinearIntervalsFunction extends AbstractLinearIntervalsFunction implements
        BivariateFunction {
    ...
    /**
     * Constructor, in two dimensions. The search index algorithm is set by default to an instance
     * of
     ...
     * 
     * @precondition : xval, yval, and fval are increasingly sorted. No test to ensure that will be
     *               made,
     *               therefore, if these tab are not correctly sorted, results can be totally wrong.
     *               No exception will
     *               be thrown.
     * 
     * @param xval
     ...
     *               created with a sorted tab also. No test to ensure that will be made, therefore,
     *               if these
     *               tab are not correctly sorted, results can be totally wrong. No exception will
     *               be thrown.
     * 
     * @param algoX
     ...
     *        : function values array increasingly sorted of length >= 2.
     */
    public BiLinearIntervalsFunction(final ISearchIndex algoX, final ISearchIndex algoY,
            final double[][] fval) {
        super();
    ...
    public double value(final double x, final double y) {
        ...
        // if x (resp. y) does not belong to xtab (resp. ytab), or with convention OPEN_CLOSED, if x
        // = tab[0][:]
        // or y = tab[:][0]
        if (xindex == -1) {
            xindex = 0;
        ...
        // if x (resp. y) does not belong to xtab (resp. ytab), or with convention CLOSED_OPEN, if x
        // = tab[nxmax-1][:]
        // or y = tab[:][nymax-1]
     ...
     * @return fxy : the value of the function at (x, y)
     */
    private double controlOnInterpolation(final double x, final double y, final double x0,
            final double x1, final double y0, final double y1, final double f00, final double f01,
    ...
    private void checkLength2D() {
        ...
        // abscissas vector and function values vectors for the first component should have the same
        // length
        ...
        if (xLength < 2) {
            throw new NumberIsTooSmallException(PatriusMessages.WRONG_NUMBER_OF_POINTS, 2, xLength,
                    true);
        }

        // ordinates vector and function values vectors for all the lines should have the same
        // length
        ...
        if (yLength < 2) {
            throw new NumberIsTooSmallException(PatriusMessages.WRONG_NUMBER_OF_POINTS, 2, yLength,
                    true);
        }
        // maximal indices
    ...
    private void writeObject(final ObjectOutputStream stream) throws IOException {
        ...
        // manually serialize the needed superclass parameters
        stream.writeObject(this.xtab);
        stream.writeObject(this.ytab);
        stream.writeObject(this.searchXIndex);
        stream.writeObject(this.searchYIndex);
        stream.writeObject(this.nxmax);
        stream.writeObject(this.nymax);
    }
    ...
    private void readObject(final ObjectInputStream stream) throws IOException,
        ...
        // manually deserialize and initialize the needed superclass parameters
        this.xtab = (double[]) stream.readObject();
        this.ytab = (double[]) stream.readObject();
        this.searchXIndex = (ISearchIndex) stream.readObject();
        this.searchYIndex = (ISearchIndex) stream.readObject();
        this.nxmax = (int) stream.readObject();
        this.nymax = (int) stream.readObject();
    }
}
