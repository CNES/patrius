...
public class RelativeTabulatedAttitudeLeg implements AttitudeLeg {
    ...
    /** Serializable UID. */
    private static final long serialVersionUID = -3194005903484124498L;
    ...
    public RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate,
            final List<Pair<Double, Rotation>> orientations, final Frame frame)
            throws PatriusException {
        this(referenceDate, orientations, frame, TabulatedAttitude.DEFAULT_INTERP_ORDER,
                DEFAULT_NATURE);
    }
    ...
    public RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate,
        ...
        this(referenceDate, buildAngularCoordinatesMap(orientations), frame, false,
                nbInterpolationPoints, DEFAULT_NATURE);
    }
    ...
    public RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate, final Frame frame,
            ...
            throws PatriusException {
        this(referenceDate, angularCoordinates, TabulatedAttitude.DEFAULT_INTERP_ORDER, frame,
                DEFAULT_NATURE);
    }
    ...
    public RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate,
            final List<Pair<Double, AngularCoordinates>> angularCoordinates,
            final int nbInterpolationPoints, final Frame frame) throws PatriusException {
    ...
    public RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate,
            final List<Pair<Double, Rotation>> orientations, final Frame frame,
            final int nbInterpolationPoints, final String natureIn) throws PatriusException {
        this(referenceDate, buildAngularCoordinatesMap(orientations), frame, false,
                nbInterpolationPoints, natureIn);
    }
     ...
     */
    public RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate, final Frame frame,
            final List<Pair<Double, AngularCoordinates>> angularCoordinates, final String natureIn)
            throws PatriusException {
        this(referenceDate, angularCoordinates, TabulatedAttitude.DEFAULT_INTERP_ORDER, frame,
                natureIn);
    }
    ...
    public RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate,
            final List<Pair<Double, AngularCoordinates>> angularCoordinates,
            final int nbInterpolationPoints, final Frame frame, final String natureIn)
            throws PatriusException {
    ...
    private RelativeTabulatedAttitudeLeg(final AbsoluteDate referenceDate,
            final List<Pair<Double, AngularCoordinates>> angularCoordinates, final Frame frame,
        ...
        this.validityInterval = new AbsoluteDateInterval(IntervalEndpointType.CLOSED,
                this.refDate.shiftedBy(this.durations[0]),
                this.refDate.shiftedBy(this.durations[arraySize - 1]), IntervalEndpointType.CLOSED);
        this.refFrame = frame;
     ...
     * @since 3.1
     */
    private static List<Pair<Double, AngularCoordinates>> buildAngularCoordinatesMap(
            final List<Pair<Double, Rotation>> orientations) {
        final List<Pair<Double, AngularCoordinates>> res = new ArrayList<>();
        for (int i = 0; i < orientations.size(); i++) {
            final Pair<Double, AngularCoordinates> pairToAdd = new Pair<>(
                    orientations.get(i).getFirst(), new AngularCoordinates(orientations.get(i)
                            .getSecond(), Vector3D.ZERO));
            res.add(pairToAdd);
        }
        return res;
    }
    ...
    public Attitude getAttitude(final PVCoordinatesProvider pvProv, final AbsoluteDate date,
        ...
        if (!this.validityInterval.contains(date)) {
            throw PatriusException
                    .createIllegalArgumentException(PatriusMessages.OUT_OF_RANGE_DATE_FOR_ATTITUDE_LAW);
        }
        ...
        if (pos == -1) {
            ...
            attitude = interpolateAttitude(date, frame);
        }
     ...
     * @since 3.1
     */
    private Attitude interpolateAttitude(final AbsoluteDate date, final Frame frame)
            throws PatriusException {
        ...
        final List<TimeStampedAngularCoordinates> attitudesToHermite = new ArrayList<>();

        // interpolation date in seconds
        final double interplationDateSec = date.durationFrom(this.refDate);

        // main algorithm
        final int posMin = new BinarySearchIndexOpenClosed(this.durations)
                .getIndex(interplationDateSec);
        ...
        // - we are constrained by beginning of array
        debTab = ((debTab + this.interpOrder) > sizeAttitude) ? sizeAttitude - this.interpOrder
                : debTab;

        // fill attitudes to Hermite
        for (int i = 0; i < this.interpOrder; i++) {
            final Vector3D acc = this.spinDerivativesComputation ? this.attitudes[debTab + i]
                    .getRotationAcceleration() : null;
            attitudesToHermite.add(new TimeStampedAngularCoordinates(this.refDate
                    .shiftedBy(this.durations[debTab + i]), this.attitudes[debTab + i]
                    .getRotation(), this.attitudes[debTab + i].getRotationRate(), acc));
        }

        // Call Hermite interpolation
        final Attitude interpolatedAttitude = new Attitude(this.refFrame,
                TimeStampedAngularCoordinates.interpolate(date, this.filter, attitudesToHermite,
                        this.spinDerivativesComputation));
    ...
    public RelativeTabulatedAttitudeLeg copy(final AbsoluteDateInterval newIntervalOfValidity) {
        ...
        // Build new instance
        final List<Pair<Double, AngularCoordinates>> newList = new ArrayList<>();
        for (int i = 0; i < this.attitudes.length; i++) {
            newList.add(new Pair<>(this.durations[i],
                    new AngularCoordinates(this.attitudes[i].getRotation(), this.attitudes[i]
