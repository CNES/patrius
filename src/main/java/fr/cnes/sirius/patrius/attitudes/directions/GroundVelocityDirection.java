...
public class GroundVelocityDirection implements IDirection {
    ...
    public Line getLine(final PVCoordinatesProvider pvCoord, final AbsoluteDate date,
        ...
        return Line.createLine(toProjectionFrame.transformPosition(surfacePointLocation),
            toProjectionFrame.transformVector(relativeVelocity));
    }
    ...
    private Vector3D getTargetPoint(final PVCoordinatesProvider pvProv, final AbsoluteDate date,
        ...
        final GeodeticPoint groundPoint = this.shape.getIntersectionPoint(line, satPosition, frame, date);
        ...
        if (groundPoint == null) {
            ...
            return this.shape.getBodyFrame().getTransformTo(frame, date)
                .transformPosition(this.shape.transform(groundPoint));
        }
    }
}
