 ...
 * VERSION::FA:87:05/08/2013:added the BodyShape attribute
...
public abstract class AbstractGroundPointing extends AbstractAttitudeLaw {
    ...
    public Attitude getAttitude(final PVCoordinatesProvider pvProv,
        ...
        try {
            ...
            final Rotation rot = new Rotation(losInSatFrame.getPosition(), losNormalInSatFrame.getPosition(),
    ...
    protected TimeStampedPVCoordinates getTargetPV(final PVCoordinatesProvider pvProv,
        ...
        final Vector3D intersectionP = this.getTargetPoint(pvProv, date, frame);

        // velocity of target point due to satellite and target motions
        final double h = targetVelocityDeltaT;
        ...
        try {
            ...
            intersectionM1h = this.getTargetPoint(pvProv, date.shiftedBy(-h), frame);
        } catch (final PropagationException exc) {
            intersectionM1h = this.getTargetPoint(pvProv, date, frame);
            scale = 1.0 / h;
        }

        try {
            // try to compute the target point M1h
            intersectionP1h = this.getTargetPoint(pvProv, date.shiftedBy(h), frame);
        } catch (final PropagationException exc) {
            intersectionP1h = this.getTargetPoint(pvProv, date, frame);
            scale = 1.0 / h;
        }
