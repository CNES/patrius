...
public class QuaternionPolynomialProfile extends AbstractAttitudeProfile {
     ...
     *        nature
     */
    public QuaternionPolynomialProfile(final Frame frame, final AbsoluteDateInterval timeInterval,
            final List<QuaternionPolynomialSegment> polynomials, final String nature) {
     ...
     */
    public QuaternionPolynomialProfile(final Frame frame, final AbsoluteDateInterval timeInterval,
            final List<QuaternionPolynomialSegment> polynomials, final double spinDeltaT) {
    ...
    /**
     * Returns the segment at provided date.
     * @param date
     *        the date
    ...
    public Attitude getAttitude(final PVCoordinatesProvider pvProv, final AbsoluteDate date,
        ...
        final Attitude attitudeNoAcceleration = attitudeProviderNoAcceleration.getAttitude(pvProv,
                date, frame);

        // Spin derivative
        Vector3D acc = null;
        if (this.computeSpinDerivativesFlag) {
            acc = attitudeProviderNoAcceleration.computeSpinDerivativeByFD(pvProv, frame, date,
                    getSpinDeltaT());
        }

        return new Attitude(date, frame, attitudeNoAcceleration.getRotation(),
                attitudeNoAcceleration.getSpin(), acc);
    }
    ...
    public QuaternionPolynomialProfile copy(final AbsoluteDateInterval newInterval) {
        ...
        final QuaternionPolynomialProfile res = new QuaternionPolynomialProfile(this.referenceFrame, newInterval,
                this.segments, getNature(), getSpinDeltaT());
    ...
    private class AttitudeProviderNoAcceleration implements AttitudeLeg {
        ...
        public Attitude getAttitude(final PVCoordinatesProvider pvProv, final AbsoluteDate date,
            ...
            final QuaternionPolynomialSegment segment = getSegment(date);
            ...
            final Transform toReferenceFrame = frame.getTransformTo(
                    QuaternionPolynomialProfile.this.referenceFrame, date);
