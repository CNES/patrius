...
public final class GlintApproximatePointingDirection implements IDirection {
    ...
    public GlintApproximatePointingDirection(final EllipsoidBodyShape shape,
        final PVCoordinatesProvider sunPV, final UnivariateSolver univariateSolver) {
    ...
    protected Vector3D getGlintPosition(final Vector3D satPos, final Vector3D sunPos,
            final AbsoluteDate date) throws PatriusException {

        // To compute eclipse detection :
        // Earth radius
        final double occultingRadius = this.bodyShape.getLocalRadius(satPos, this.bodyShape.getBodyFrame(),
            date, this.sunPVCoordinates);
        // Apparent radius
        ...
        // If we are out of an eclipse : Compute the glint position - velocity
    ...
    private void computeLine(final PVCoordinatesProvider origin, final AbsoluteDate date,
        ...
        if (!origin.equals(this.cachedOrigin) || !date.equals(this.cachedDate) || !frame.equals(this.cachedFrame)) {
            ...
            Line result = new Line(satPos, this.cachedGlintPoint);
