...
public class AeroAttitudeLaw extends AbstractAttitudeLaw {
     ...
     * Constructor.
     *
     * @param angleofattack angle of attack in [-Pi;Pi]
     * @param sideslip sideslip angle in ]-Pi/2; Pi/2[
     * @param rollVel roll velocity in [-Pi;Pi]
     * @param earthShape Earth shape
     ...
     * @param rollVel roll velocity
     * @param earthShape Earth shape
     * @throws PatriusException if some specific error occurs
     */
    public AeroAttitudeLaw(final IParameterizableFunction angleofattack,
        final IParameterizableFunction sideslip, final IParameterizableFunction rollVel,
        final EllipsoidBodyShape earthShape) throws PatriusException {
        this(angleofattack, sideslip, rollVel, earthShape, DEFAULT_STEP_VEL, DEFAULT_STEP_ACC);
    }
     ...
     * @param rollVel roll velocity
     * @param earthShape Earth shape
     * @param dtSpin step for finite differences spin computation
     * @param dtAcc step for finite differences spin derivative computation
     * @throws PatriusException if some specific error occurs
     */
    public AeroAttitudeLaw(final double angleofattack, final double sideslip, final double rollVel,
        final EllipsoidBodyShape earthShape, final double dtSpin, final double dtAcc)
        throws PatriusException {
        this(new ConstantFunction(angleofattack), new ConstantFunction(sideslip),
            new ConstantFunction(rollVel), earthShape, dtSpin, dtAcc);
    }
    ...
    public Attitude getAttitude(final PVCoordinatesProvider pvProv, final AbsoluteDate date,
        ...
        // Get rotation
        final Rotation rot = this.computeOrientation(pvProv, date, frame);
        ...
        return new Attitude(frame, new TimeStampedAngularCoordinates(date, rot, spin,
            spinDerivative));
    }
     ...
     * @param frame reference frame from which orientation function is computed
     * @param date date
     * @return orientation at date
     * @throws PatriusException thrown if direction could not be computed
     */
    private Rotation computeOrientation(final PVCoordinatesProvider pvProv,
                                        final AbsoluteDate date, final Frame frame) throws PatriusException {

        // PV coordinates expressed in ITRF
        final PVCoordinates pvCoordinates = pvProv.getPVCoordinates(date,
            this.earthBodyShape.getBodyFrame());

        // Cartesian Parameters
        final CartesianParameters cartParam = new CartesianParameters(pvCoordinates,
        ...
        final GeodeticPoint geodeticPoint = new GeodeticPoint(reentryParam.getLatitude(),
            reentryParam.getLongitude(), reentryParam.getAltitude());

        // Topocentric frame
        final Frame topocentricFrame = new TopocentricFrame(this.earthBodyShape, geodeticPoint, 0,
            "TopoFrame");

        // Rotation 1: from reference Frame to Topocentric frame
        final Rotation fromRefFrameToTopoFrame = frame.getTransformTo(topocentricFrame, date)
            .getRotation();

        // Rotation 2: from Topocentric frame to Aircraft-carried normal earth
        // axis system frame
        final Rotation fromTopoFrameToAircraftNormalEarthAxisFrame = new Rotation(
        ...
        final double[] aircraftAngles = this.computeYawPitchRoll(state, reentryParam);

        final Rotation fromAircraftNormalEarthAxisFrameToAircraftFrame = new Rotation(
            RotationOrder.ZYX, aircraftAngles[0], aircraftAngles[1], aircraftAngles[2]);
    ...
    /**
     * Method to compute aircraft orientation angles with respect the Aircraft-carried normal earth
     * axis system frame: OX towards local North and OY towards local East.
     *
     * @param state state
     * @param reentryParam reentry parameters
     * @return yaw, pitch and roll
     */
    private double[] computeYawPitchRoll(final SpacecraftState state,
                                         final ReentryParameters reentryParam) {

        // Rotation 1: Aircraft-carried normal earth axis system frame ->
        // Aerodynamic frame
        final Rotation earthToAero = new Rotation(RotationOrder.ZYX, reentryParam.getAzimuth(),
            reentryParam.getSlope(), this.rollVelocity.value(state));

        // Rotation 2: Aircraft frame -> Aerodynamic frame
        final Rotation aircraftToAero = new Rotation(RotationOrder.YZX,
            -this.angleOfAttack.value(state), this.sideSlip.value(state), 0);
        ...
        if (Precision.equals(angles[1], FastMath.PI / 2.)
            || Precision.equals(angles[1], -FastMath.PI / 2.)) {
            angles[0] = 0.;
        }
     ...
     * @param frame reference frame from which orientation function is computed
     * @param zeroAbscissa the date for which x=0 for orientation function of date
     * @return the orientation (quaternion) function of the date
     */
    private OrientationFunction getOrientationFunction(final PVCoordinatesProvider pvProv,
    ...
    private Vector3DFunction getSpinFunction(final PVCoordinatesProvider pvProv, final Frame frame,
        ...
        return new AbstractVector3DFunction(zeroAbscissa, new FiniteDifferencesDifferentiator(4,
            this.stepAcc)){
            /** {@inheritDoc} */
            @Override
     ...
     *
     * @param slopeVel Slope velocity
     * @param azimuthVel Azimuth velocity
     * @param yaw Aircraft yaw
     * @param pitch Aircraft pitch
     * @param roll Aircraft roll
     * @return Angle of Attack, Side slip angle, Bank angle
     * @throws PatriusException side slip angle doesn't permit to compute bank angle
     */
    public static double[] aircraftToAero(final double slopeVel, final double azimuthVel, final double yaw,
                                          final double pitch, final double roll) throws PatriusException {
        ...
        // Side slip angle
        final double sideSlipAngle = MathLib
            .asin(velAircraftVec.getY() / velAircraftVec.getNorm());

        // Exception
        if (Comparators.equals(sideSlipAngle, FastMath.PI / 2)
            || (Comparators.equals(sideSlipAngle, -FastMath.PI / 2))) {
        ...
        final Rotation aircraftToAero = new Rotation(RotationOrder.YZX, -angleOfAttack,
            sideSlipAngle, 0);
    ...
    public String toString() {
        return String.format("%s: angleOfAttack=%s, sideSlip=%s, rollVelocity=%s", this.getClass()
            .getSimpleName(), this.angleOfAttack, this.sideSlip, this.rollVelocity);
    }
}
