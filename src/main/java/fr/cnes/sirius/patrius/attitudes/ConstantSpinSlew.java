 ...
 * VERSION:4.10:FA:FA-3234:03/11/2022:[PATRIUS] La classe ConstantSpinSlewCalculator impose le calcul de la rotation a realiser en GCRF
...
public class ConstantSpinSlew implements Slew {
    ...
    private final AbsoluteDateInterval intervalOfValidity;

    /** Nature. */
    private final String nature;

    /** Pivot frame used for building slew. */
    private final Frame pivotFrame;

    /**
     * Constructor for slew with a duration constraint.
     * Slew is computed in GCRF frame.
    ...
    public ConstantSpinSlew(final Attitude initAtt,
        ...
        this(initAtt, finalAtt, natureIn, FramesFactory.getGCRF());
    }
     ...
     * @param finalAtt final attitude at end date
     * @param natureIn nature
     * @param pivotFrame pivot frame, slew will be computed in this frame
     * @throws PatriusException thrown if some frame transformation failed
     */
    public ConstantSpinSlew(final Attitude initAtt,
            final Attitude finalAtt,
            final String natureIn,
            final Frame pivotFrame) throws PatriusException {
        this.intervalOfValidity = new AbsoluteDateInterval(initAtt.getDate(), finalAtt.getDate());
        this.nature = natureIn;
        this.initAtt = initAtt.withReferenceFrame(pivotFrame);
        this.finalAtt = finalAtt.withReferenceFrame(pivotFrame);
    ...
    public ConstantSpinSlew(final Attitude initAtt,
            ...
            final double spin,
            final String natureIn) throws PatriusException {
        this(initAtt, finalAtt, spin, natureIn, FramesFactory.getGCRF());
    }
    ...
    public Attitude getAttitude(final AbsoluteDate date, final Frame frame) throws PatriusException {
        if (!this.intervalOfValidity.contains(date)) {
            ...
            // Attitude is defined in pivot frame
            final Attitude att = new Attitude(pivotFrame,
