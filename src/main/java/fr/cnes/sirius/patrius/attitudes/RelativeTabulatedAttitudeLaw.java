...
public class RelativeTabulatedAttitudeLaw implements AttitudeLaw {
    ...
    public RelativeTabulatedAttitudeLaw(final Frame frame, final AbsoluteDate refDate,
        ...
        final List<Pair<Double, AngularCoordinates>> listAr = this.buildAngularCoordinatesMap(orientations);

        // Law before
        final AttitudeLaw attitudeLawBefore = this.buildAttitudeLaw(lawBefore, listAr.get(0), frame, refDate);

        // Law after
        final AttitudeLaw attitudeLawAfter =
            this.buildAttitudeLaw(lawAfter, listAr.get(listAr.size() - 1), frame, refDate);
    ...
    public RelativeTabulatedAttitudeLaw(final AbsoluteDate refDate,
        ...
        // Law before
        final AttitudeLaw attitudeLawBefore = this.buildAttitudeLaw(lawBefore,
            angularCoordinates.get(0), frame, refDate);

        // Law after
        final AttitudeLaw attitudeLawAfter = this.buildAttitudeLaw(lawAfter,
     ...
     * @since 3.1
     */
    private List<Pair<Double, AngularCoordinates>>
            buildAngularCoordinatesMap(
                                       final List<Pair<Double, Rotation>> orientations) {
        ...
        for (int i = 0; i < orientations.size(); i++) {
            ...
            res.add(new Pair<Double, AngularCoordinates>(orientations.get(i).getFirst(), ac));
        }
        return res;
    }
     ...
     * @since 3.1
     */
    private AttitudeLaw buildAttitudeLaw(final AroundAttitudeType law,
