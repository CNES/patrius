 ...
 * 
 */
public class EarthToCelestialPointDirection implements ITargetDirection {
     ...
     *        celestial point
     */
    public EarthToCelestialPointDirection(final CelestialPoint celestialPoint) {
    ...
    public Vector3D getVector(final PVCoordinatesProvider pvCoord, final AbsoluteDate date, final Frame frame)
        ...
        // Expressed celestial point center direction in output frame
        // Return Earth => celestial point vector in frame
        return t.transformVector(this.celestialPoint.getPVCoordinates(date, EARTH_FRAME).getPosition());
    }
    ...
    public Line getLine(final PVCoordinatesProvider pvCoord, final AbsoluteDate date, final Frame frame)
        ...
        // Celestial point center in frame
        final Vector3D celestialPointPosInFrame = t
            .transformVector(this.celestialPoint.getPVCoordinates(date, EARTH_FRAME).getPosition());

        // creation of the line
        try {
            return new Line(celestialPointPosInFrame, earthPosInFrame, celestialPointPosInFrame);
    ...
    public PVCoordinates getTargetPVCoordinates(final AbsoluteDate date, final Frame frame) throws PatriusException {
        return new ToCelestialPointDirection(this.celestialPoint).getTargetPVCoordinates(date, frame);
    }
    ...
    public Line getLine(final PVCoordinatesProvider origin, final SignalDirection signalDirection,
        ...
        // Freeze the celestial point center frame at the given date wrt to ICRF frame
        final Frame frozenFrame = this.celestialPoint.getICRF().getFrozenFrame(FramesFactory.getICRF(), date,
            INERTIAL_FROZEN);
    ...
    public Vector3D getVector(final PVCoordinatesProvider origin, final SignalDirection signalDirection,
        ...
        final EarthCenterDirection earthCenter = new EarthCenterDirection();
        final PVCoordinatesProvider earthProvider = earthCenter.getTargetPvProvider();
        return ITargetDirection.super.getVector(earthProvider, signalDirection, correction, date, fixedDateType, frame,
                epsilon);
    }
    ...
    private class TargetPVCoordinatesProvider implements PVCoordinatesProvider {
        ...
        public Frame getNativeFrame(final AbsoluteDate date) throws PatriusException {
            return EarthToCelestialPointDirection.this.celestialPoint.getNativeFrame(date);
        }
    }
}
