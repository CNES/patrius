...
public class AngularCoordinates implements TimeShiftable<AngularCoordinates>, Serializable {
    ...
    /** Constant number: -2 */
    public static final double MINUS_TWO = -2.;
    ...
    /** Flag indicating if velocity and acceleration should be projected. */
    private final boolean projectVelocityAndAcceleration;
    ...
    public AngularCoordinates() {
        ...
        this.projectVelocityAndAcceleration = false;
    }
    ...
    public AngularCoordinates(final Rotation rotationIn,
        ...
        this.rotationRate = rotationRateIn;
        this.rotationAcceleration = rotationAccelerationIn;
        this.projectVelocityAndAcceleration = projectVelocityAndAcceleration;
    }
    ...
    public AngularCoordinates(final PVCoordinates u1, final PVCoordinates u2,
        ...
        try {
            ...
            if (spinDerivativesComputation && isu1u2NotNull && isv1v2NotNull) {
                // find rotation acceleration dot(Ω) such that
                // dot(Ω) ⨯ v₁ = inv(r)(dotdot(u₁)) - 2 Ω ⨯ dot(v₁) - Ω ⨯ (Ω ⨯ v₁) - dotdot(v₁)
                // dot(Ω) ⨯ v₂ = inv(r)(dotdot(u₂)) - 2 Ω ⨯ dot(v₂) - Ω ⨯ (Ω ⨯ v₂) - dotdot(v₂)
                final Vector3D ru1DotDot = this.rotation.applyInverseTo(u1.getAcceleration());
                ...
                final Vector3D ru2DotDot = this.rotation.applyInverseTo(u2.getAcceleration());
    ...
    public AngularCoordinates addOffset(final AngularCoordinates offset, final boolean computeSpinDerivatives) {
        final Vector3D rOmega = offset.rotation.applyInverseTo(this.rotationRate);
        ...
        if (computeSpinDerivatives && this.rotationAcceleration != null && offset.rotationAcceleration != null) {
            final Vector3D rOmegaDot = offset.rotation.applyInverseTo(this.rotationAcceleration);
            newAcc = new Vector3D(1.0, offset.rotationAcceleration, 1.0, rOmegaDot, -1.0, Vector3D.crossProduct(
                offset.rotationRate, rOmega));
        }
    ...
    public PVCoordinates applyTo(final PVCoordinates pv) {
        ...
        if (pv.getAcceleration() != null) {
            transformedA = new Vector3D(1, this.rotation.applyInverseTo(pv.getAcceleration()), MINUS_TWO, crossV, -1,
