...
public abstract class AbstractStelaGaussContribution implements StelaForceModel {
     ...
     * @param coeffs
     *        list of coefficients of the Fourier Series
     * @param orderFS
     *        order of the Fourier Series
     * @param xi
     *        mean longitude at the considered state
     * @return short periods terms
     */
    protected double[] evaluateFS(final List<double[][]> coeffs,
            final int orderFS,
            final double xi) {

        // Initialization
        final double[] shortPeriods = new double[6];
        final double[][] coeffCos = coeffs.get(0);
        final double[][] coeffSin = coeffs.get(1);

        // Build short periods
        for (int j = 0; j < orderFS; j++) {
            final double sin = MathLib.sin((j + 1) * xi);
            final double cos = MathLib.cos((j + 1) * xi);
            shortPeriods[0] += coeffCos[0][j] * sin - coeffSin[0][j] * cos;
            shortPeriods[1] += coeffCos[1][j] * sin - coeffSin[1][j] * cos;
            shortPeriods[2] += coeffCos[2][j] * sin - coeffSin[2][j] * cos;
            shortPeriods[3] += coeffCos[3][j] * sin - coeffSin[3][j] * cos;
            shortPeriods[4] += coeffCos[4][j] * sin - coeffSin[4][j] * cos;
            shortPeriods[5] += coeffCos[5][j] * sin - coeffSin[5][j] * cos;
        }
     ...
     * @param dOrbParams
     *        (6 x QuadPoints) matrix containing dE/dt (computed by using Gauss Eqs.).
     * @param lmb
     *        Vector of mean longitude at each quadrature point.
     * @param orderFS
     *        order of the Fourier Series
     * @param quadPoints
     *        number of quadrature points
     * @return elements for each quadrature point
     */
    protected List<double[][][]> computeQuadElements(final double[][] dOrbParams,
            final double[] lmb,
            final int orderFS,
        ...
        // Build cos and sin integrand
        for (int i = 0; i < orderFS; i++) {
            for (int k = 0; k < quadPoints; k++) {
                final double cos = MathLib.cos((i + 1) * lmb[k]);
                final double sin = MathLib.sin((i + 1) * lmb[k]);
                for (int j = 0; j < 6; j++) {
                    intgdCos[j][k][i] = dOrbParams[j][k] * cos;
                    intgdSin[j][k][i] = dOrbParams[j][k] * sin;
                }
            }
        }
        // Add quadrature elements
        listQuadElements.add(intgdCos);
        listQuadElements.add(intgdSin);

        // Return result
        return listQuadElements;
    }

    /**
     * Compute elements under integral for coefficients evaluation.
     * @param listQuadElements
     *        list of quadrature elements under integral [(6 x QuadPoints x orderFS) matrices]
     * @param orderFS
     *        order of the Fourier Series
     * @param quadPoints
     *        number of quadrature points
     ...
     *        the mean motion
     * @param deltaAngle
     *        angular step related to quadrature points discretization
     * @return coefficients
     * @throws PatriusException thrown if computation failed
     */
    protected List<double[][]> computeCoeffs(final List<double[][][]> listQuadElements,
            final int orderFS,
            final int quadPoints,
        ...
        final double[][] coeffCos = new double[6][orderFS];
        final double[][] coeffSin = new double[6][orderFS];
        final double[] simpVectorCos = new double[quadPoints];
        final double[] simpVectorSin = new double[quadPoints];

        // Build coefficients
        for (int i = 0; i < orderFS; i++) {
            // Common factor
            final double fact = deltaAngle * (quadPoints - 1) / ((i + 1) * meanMvt * MathLib.PI);

            for (int j = 0; j < 6; j++) {
                for (int k = 0; k < quadPoints; k++) {
                    simpVectorCos[k] = intgdCos[j][k][i];
                    simpVectorSin[k] = intgdSin[j][k][i];
                }
                // Compute quadrature
                coeffCos[j][i] = fact * Squaring.simpsonMean(simpVectorCos);
                coeffSin[j][i] = fact * Squaring.simpsonMean(simpVectorSin);
            }
        }
        coeffs.add(coeffCos);
        coeffs.add(coeffSin);
