...
public final class StelaAeroModel extends Parameterizable implements DragSensitive {
     ...
     */
    private static Vector3D computeVelocity(final SpacecraftState state) throws PatriusException {
    ...
    public void addDDragAccDState(final SpacecraftState s, final double[][] dAccdPos, final double[][] dAccdVel,
        ...
        if (computeGradientPosition || computeGradientVelocity) {
            ...
            final Vector3D[] tDeriv = computeTDerivatives(vel, velNorm);
            final Vector3D[] wDeriv = computeWDerivatives(pos, vel, posCrossVel, posCrossVelNorm);
            final Vector3D[] nDeriv = computeNDerivatives(t, w, tDeriv, wDeriv);
     ...
     */
    private static Vector3D[] computeTDerivatives(final Vector3D vel, final double velNorm) {
