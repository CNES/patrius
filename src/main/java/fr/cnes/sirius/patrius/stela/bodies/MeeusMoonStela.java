 ...
 * the position. Note that it is not possible to build this Moon from the CelestialBodyFactory.
 * </p>
...
public class MeeusMoonStela extends AbstractCelestialBody {
    ...
    /**
     * <p>
     * This class implements the Moon ephemerides according to the algorithm of Meeus, it only
     * provides the position. Note that it is not possible to build this Moon from the
     * CelestialBodyFactory.
     * </p>
     * See Stela's implementation of this model
     * <p>
     * This class contains methods to store {@link #getInertialFrame(IAUPoleModelType)} to
     * integration frame (CIRF) transform to speed up computation during the integration process. As
     * this transform varies slowly through time, it has been demonstrated it is not necessary to
     * recompute it every time. Warning: these methods should not be used in a stand-alone use
     * (unless you known what you are doing). There are two methods:
     * <ul>
     * <li>{@link #updateTransform(AbsoluteDate, Frame)}: store transform from
     * {@link #getInertialFrame(IAUPoleModelType)} to provided frame at provided date.</li>
     * <li>{@link #resetTransform()}: reset stored transform</li>
     * </ul>
     * </p>
     * 
    ...
    private class MeeusMoonStelaEphemeris implements CelestialBodyEphemeris {
        ...
        public PVCoordinates getPVCoordinates(final AbsoluteDate date, final Frame frame) throws PatriusException {
            ...
            final double epsMoon = MathUtils.DEG_TO_RAD
                    * (OBLIQUITY_0 + OBLIQUITY_1 * t + OBLIQUITY_2 * t2 + OBLIQUITY_3 * t3);
            ...
            // Mean Anomalies
            final double ml = MathUtils.DEG_TO_RAD
                    * (MEAN_ANOMALY_0 + MEAN_ANOMALY_1 * t + MEAN_ANOMALY_2 * t2 + MEAN_ANOMALY_3 * t3 - MEAN_ANOMALY_4
                            * t4);

            final double ms = MathUtils.DEG_TO_RAD
                    * (MS_MEAN_ANOMALY_0 + MS_MEAN_ANOMALY_1 * t - MS_MEAN_ANOMALY_2 * t2 - MS_MEAN_ANOMALY_3 * t3
                    - MS_MEAN_ANOMALY_4 * t4);

            final double ds = MathUtils.DEG_TO_RAD
                    * (MS_MEAN_LONGITUDE_0 + MS_MEAN_LONGITUDE_1 * t - MS_MEAN_LONGITUDE_2 * t2 + MS_MEAN_LONGITUDE_3
                            * t3 - MS_MEAN_LONGITUDE_4 * t4);

            final double ul = MathUtils.DEG_TO_RAD
                    * (MEAN_ARGUMENT_0 + MEAN_ARGUMENT_1 * t - MEAN_ARGUMENT_2 * t2 + MEAN_ARGUMENT_3 * t3
                    + MEAN_ARGUMENT_4 * t4);

            // Longitude and latitude of the ecliptic
            final double lonl = MathUtils.DEG_TO_RAD
                    * (LONG_ECL_0 + LONG_ECL_1 * t + LONG_ECL_2 * MathLib.sin(ml) - LONG_ECL_3
                            * MathLib.sin(ml - 2.0 * ds) + LONG_ECL_4 * MathLib.sin(2.0 * ds) + LONG_ECL_5
                            * MathLib.sin(2.0 * ml) - LONG_ECL_6 * MathLib.sin(ms)
                            - LONG_ECL_7 * MathLib.sin(2.0 * ul));

            final double latl = MathUtils.DEG_TO_RAD
                    * (LAT_ECL_0 * MathLib.sin(ul) + LAT_ECL_1 * MathLib.sin(ml + ul) - LAT_ECL_2
                            * MathLib.sin(ul - ml) - LAT_ECL_3 * MathLib.sin(ul - 2.0 * ds));

            // Parallax in the ecliptic
            final double parl = MathUtils.DEG_TO_RAD
                    * (PAR_ECL_0 + PAR_ECL_1 * MathLib.cos(ml) + PAR_ECL_2 * MathLib.cos(ml - 2.0 * ds) + PAR_ECL_3
                            * MathLib.cos(2.0 * ds) + PAR_ECL_4 * MathLib.cos(2.0 * ml));
            ...
            // return transform.transformPVCoordinates(pv);
            // }
            // // Cached transform is used (integration process): return PV obtained with cached
            // // transform
