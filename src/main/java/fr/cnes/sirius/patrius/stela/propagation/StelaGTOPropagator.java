...
public class StelaGTOPropagator extends StelaAbstractPropagator {
     ...
     * <p>
     * After creation, there is no orbit, method {@link #setInitialState setInitialState} must be called. This
     * constructor uses a Default Law as AttitudeProvider. For a specific Attitude Provider please use the other
     * constructor. After creation, there are no perturbing forces at all. This means that if {@link #addForceModel
     * addForceModel} is not called after creation, the integrated orbit will follow a simple Keplerian evolution.
     * </p>
     * <p>
     * During reentry, no correction is performed if some results are not physical. As a result, an exception will be
     * thrown if obtained results are not physical.
     * </p>
     * 
     * @param integr
     ...
     * This means that if {@link #addForceModel addForceModel} is not called after creation, the integrated orbit will
     * follow a simple Keplerian evolution.
     * </p>
     * <p>
     * No attitude provider is defined. The user should call {@link #setAttitudeProvider(AttitudeProvider)} or
     * {@link #setAttitudeProviderEvents(AttitudeProvider)} or {@link #setAttitudeProviderForces(AttitudeProvider)}
     * </p>
     * 
     * @param integr
     *        the first order integrator to use for extrapolation
     ...
     * all. This means that if {@link #addForceModel addForceModel} is not called after creation, the integrated orbit
     ...
     *         exception in reset initial state
     */
    public StelaGTOPropagator(final FirstOrderIntegrator integr, final AttitudeProvider inAttitudeProvider,
        final StelaBasicInterpolator inInter, final double maxShiftIn,
        final double minStepSizeIn) throws PatriusException {
        // a single attitude provider is defined.
        super(inAttitudeProvider, inInter);

        this.gaussForces = new ArrayList<AbstractStelaGaussContribution>();
        this.lagrangeForces = new ArrayList<AbstractStelaLagrangeContribution>();
        this.addEquationsList = new ArrayList<StelaAdditionalEquations>();
     ...
     * no perturbing forces at all. This means that if {@link #addForceModel addForceModel} is not called after
     ...
     *         exception in reset initial state
     */
    public StelaGTOPropagator(final FirstOrderIntegrator integr, final AttitudeProvider inAttitudeProviderForces,
    ...
    public void setInitialState(final SpacecraftState initialState, final double massIn,
        ...
        if (isOsculatingIn) {
            ...
            final StelaEquinoctialOrbit equiOrbit = new StelaEquinoctialOrbit(initialState.getOrbit());
        ...
        if (this.integrationFrame.getName().equals(initialState2.getFrame().getName())) {
            ...
            // Compute attitudes using AttitudeProvider
            if (this.attitudeProviderByDefault != null) {
                attForces =
                    this.attitudeProviderByDefault.getAttitude(initStateTransform, this.referenceDate,
                ...
                if (this.attitudeProviderForces != null) {
                    attForces =
                        this.attitudeProviderForces.getAttitude(initStateTransform, this.referenceDate,
                            this.integrationFrame);
                }
                if (this.attitudeProviderEvents != null) {
                    attEvents =
                        this.attitudeProviderEvents.getAttitude(initStateTransform, this.referenceDate,
                            this.integrationFrame);
                }
            }
            // If no AttitudeProvider defined : attitudes are null

            // Build initial state
            iniStateTransf = new SpacecraftState(initStateTransform, attForces, attEvents,
     ...
     * If the set of attitude equations is already registered for the current attitude, it is replaced by the new one.
     * 
     * @param addEqu
    ...
    public void setAttitudeProviderForces(final AttitudeProvider attitudeForcesProvider) {
        this.checkTwoAttitudesTreatment();
        for (int i = 0; i < this.addEquationsList.size(); i++) {
            if (this.addEquationsList.get(i).getName().equals(AttitudeType.ATTITUDE_FORCES.toString())) {
                // An additional equation is already defined for this Attitude
                throw PatriusException.createIllegalStateException(PatriusMessages.ATTITUDE_ADD_EQ_ALREADY_DEFINED);
            }
        }
    ...
    public void setAttitudeProviderEvents(final AttitudeProvider attitudeEventsProvider) {
        this.checkTwoAttitudesTreatment();
        for (int i = 0; i < this.addEquationsList.size(); i++) {
            if (this.addEquationsList.get(i).getName().equals(AttitudeType.ATTITUDE_EVENTS.toString())) {
                // An additional equation is already defined for this Attitude
                throw PatriusException.createIllegalStateException(PatriusMessages.ATTITUDE_ADD_EQ_ALREADY_DEFINED);
            }
        }
     ...
     *         If an attitude provider or an additional equation is already defined for a specific attitude (for
    ...
    protected List<PatriusStepHandler> getStepHandlers() {
        final List<PatriusStepHandler> stepHandlers = new ArrayList<PatriusStepHandler>();
    ...
    protected SpacecraftState propagateSpacecraftState(final AbsoluteDate date) throws PatriusException {
        ...
        if (this.wasException && (date.compareTo(this.maxDate) > 0)) {
            throw new PropagationException(PatriusMessages.PDB_PROPAGATION_NO_REENTRY, this.maxShift);
        }

        // get current initial state and date
        final StelaEquinoctialOrbit equiOrbit = new StelaEquinoctialOrbit(this.getInitialState().getOrbit());
        ...
        for (int i = 0; i < this.addEquationsList.size(); i++) {
            ...
            if (name.equals(AttitudeType.ATTITUDE.toString())) {
                ...
                if (name.equals(AttitudeType.ATTITUDE_FORCES.toString())) {
                    initState = initState.addAttitudeToAdditionalStates(AttitudeType.ATTITUDE_FORCES);
                }
                if (name.equals(AttitudeType.ATTITUDE_EVENTS.toString())) {
                    initState = initState.addAttitudeToAdditionalStates(AttitudeType.ATTITUDE_EVENTS);
                }
            }
        }
     ...
     * 
     * @throws PatriusException
     *         if additional states from initial state and additional equations added in the propagator does not
     *         correspond
     */
    private void checkStatesEquations(final SpacecraftState state) throws PatriusException {
        ...
        if (additionalStates.isEmpty() ^ this.addEquationsList.isEmpty()) {
            // Either additional states from SpacecraftStare are empty or additional equations are empty
            throw new PatriusException(PatriusMessages.WRONG_CORRESPONDENCE_STATES_EQUATIONS);
        }
    }
     ...
     * @since 3.0
     */
    private SpacecraftState mapArrayToStelaState(final double[] stateVector,
                                                 final AbsoluteDate date) throws PatriusException {
        // Get the additional state data from state vector
        int index = STATE_SIZE;
        final Map<String, double[]> addStates = new ConcurrentHashMap<String, double[]>();
        for (final StelaAdditionalEquations eq : this.addEquationsList) {
        ...
        // Get Stela orbit
        final StelaEquinoctialOrbit newOrbit = new StelaEquinoctialOrbit(stateVector[0], stateVector[2],
            stateVector[3], stateVector[4], stateVector[5], stateVector[1], this.integrationFrame, date, this.mu);
    ...
    private Attitude[] mapAttitude(final Map<String, double[]> addStates,
        ...
        if (addStates.containsKey(AttitudeType.ATTITUDE.toString())) {
            // One attitude
            attForces = new Attitude(addStates.get(AttitudeType.ATTITUDE.toString()), newOrbit.getDate(),
                newOrbit.getFrame());
        } else {
            // Two attitudes
            if (addStates.containsKey(AttitudeType.ATTITUDE_FORCES.toString())) {
                attForces = new Attitude(addStates.get(AttitudeType.ATTITUDE_FORCES.toString()), newOrbit.getDate(),
                    newOrbit.getFrame());
            }
            if (addStates.containsKey(AttitudeType.ATTITUDE_EVENTS.toString())) {
                attEvents = new Attitude(addStates.get(AttitudeType.ATTITUDE_EVENTS.toString()), newOrbit.getDate(),
                    newOrbit.getFrame());
            }
        }
    ...
    protected SpacecraftState propagationManagement(final SpacecraftState state, final double stepSize,
        ...
        if (this.maxShift == 0 || this.minStepSize >= stepSize / 2.) {
            ...
            try {
                ...
                final IllegalArgumentException exception = PatriusException.createIllegalArgumentException(
                    PatriusMessages.HYPERBOLIC_ORBIT_NOT_HANDLED_AS, StelaEquinoctialParameters.class.getName());
                ...
                if (e.getMessage().equalsIgnoreCase(message)) {
                    ...
                    if (finalState == null) {
                        // Throw exception
                        throw PatriusException.createIllegalArgumentException(
                            PatriusMessages.STELA_INTEGRATION_FAILED, StelaEquinoctialParameters.class.getName());
                    }
                }
            }
     ...
     * @since 1.3
     */
    protected SpacecraftState propagationManagementLoop(final double currentStepSize, final AbsoluteDate target,
            final double dt, final String message) throws PropagationException {
