...
public abstract class StelaAbstractPropagator implements Propagator {
    ...
    protected StelaAbstractPropagator(final AttitudeProvider attitudeProviderIn,
        ...
        this.additionalStateProviders = new ArrayList<AdditionalStateProvider>();
    ...
    public void addAdditionalStateProvider(
            final AdditionalStateProvider additionalStateProvider) throws PatriusException {
    ...
    private void initTimeDerivatives() {
        if (this.isStoreTimeDerivatives()) {
            this.timeDerivativesList = new ArrayList<TimeDerivativeData>();
    ...
    private SpacecraftState resetState(final SpacecraftState oldState,
        ...
        if (this.attitudeProviderByDefault == null) {
            // If two attitudes treatment
            if (this.attitudeProviderForces != null) {
                attitudeForce = this.attitudeProviderForces.getAttitude(this.pvProvider, oldState.getDate(),
                    oldState.getFrame());
            }
            if (this.attitudeProviderEvents != null) {
                attitudeEvent = this.attitudeProviderEvents.getAttitude(this.pvProvider, oldState.getDate(),
                    oldState.getFrame());
            }
        } else {
            // If single attitude treatment
            attitudeForce = this.attitudeProviderByDefault.getAttitude(this.pvProvider, oldState.getDate(),
                oldState.getFrame());
            attitudeEvent = null;
        }
        final SpacecraftState resetState = new SpacecraftState(oldState.getOrbit(), attitudeForce, attitudeEvent,
    ...
    private class BoundedPropagatorView extends StelaAbstractPropagator implements BoundedPropagator {
        ...
        protected List<PatriusStepHandler> getStepHandlers() {
            ...
            return new ArrayList<PatriusStepHandler>();
        }
