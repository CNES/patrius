 ...
 * VERSION:4.11:DM:DM-3235:22/05/2023:[PATRIUS][TEMPS_CALCUL] Attitude des spacecraft state lazy
...
public class StelaAtmosphericDrag extends AbstractStelaGaussContribution {
    ...
    /**
     * Transition matrix computation flag. Set to true if parameters required to get the transition
     * matrix have to be
     * computed.
     */
    ...
    public StelaAtmosphericDrag(final DragSensitive inSpacecraft, final Atmosphere inAtmosphere,
        ...
        this(inSpacecraft, inAtmosphere, inSquaringPoints, inEarthRadius, inAtmoThreshold, inDragRecomputeStep,
                DEFAULT_ORDER_FS);
    }
    ...
    public StelaAtmosphericDrag(final DragSensitive inSpacecraft, final Atmosphere inAtmosphere,
            final int inSquaringPoints, final double inEarthRadius, final double inAtmoThreshold,
            final int inDragRecomputeStep, final int orderFS) {
        super();
    ...
    /** {@inheritDoc} */
    @Override
    public double[] computePerturbation(final StelaEquinoctialOrbit orbit, final OrbitNatureConverter converter)
        ...
        if (zp <= this.atmoThreshold) {
            // At least one point of the orbit is below the upper atmospheric boundary but a part of
            // the orbit remains
            // above

            // Computation of true and eccentric anomaly bounds of the part of the orbit inside the
            // atmosphere
            ...
            for (int i = 0; i < this.squaringPoints; i++) {
                ...
                final StelaEquinoctialOrbit orbitOsc = converterNoDrag.toOsculating(orbitMean);
                ...
                if (this.transMatFlag) {
                    final double[] dGFDESquaringCurrent = this.computeDGFDESquaring(orbitMean, orbitOsc, dragMOD,
                            dragTNW, gaussEq, density);
                    for (int j = 0; j < 6 * 6; j++) {
    ...
    public double[] computeShortPeriods(final StelaEquinoctialOrbit orbit, final OrbitNatureConverter converter)
        ...
        // Compute bounds for Drag
        final double za = orbitCIRF.getA() * (1. + orbitCIRF.getE()) - Constants.CNES_STELA_AE;
        final double[] bounds = computeAnomalyBounds(orbitCIRF.getA(), orbitCIRF.getE(), za);
        final double ve = bounds[0];
        final double vs = bounds[1];
    ...
    /**
     * Computation of the temporary d(GF)/dE term of the transition matrix, for the current squaring
     * point.
     *
     * @param orbitMean
     ...
     * @return the temporary array containing the d(GF)/dE term of the transition matrix for the
     *         current squaring point.
     * @throws PatriusException
    ...
    private double[] computeDGFDESquaring(final StelaEquinoctialOrbit orbitMean, final StelaEquinoctialOrbit orbitOsc,
            final Vector3D dragMOD, final Vector3D dragTNW, final double[][] gaussEq, final double density)
            throws PatriusException {

        // Attitude is not computed: we use the default attitude law.
        final AttitudeProvider attitudeLaw = new ConstantAttitudeLaw(FramesFactory.getEME2000(), Rotation.IDENTITY);
        final SpacecraftState stateOsc = new SpacecraftState(attitudeLaw, orbitOsc);
        final double[][] dAccdPos = new double[3][3];
        final double[][] dAccdVel = new double[3][3];
        final Vector3D vAtm = this.atmosphere.getVelocity(stateOsc.getDate(),
                stateOsc.getPVCoordinates().getPosition(), stateOsc.getFrame());
        final Vector3D relativeVelocity = vAtm.subtract(stateOsc.getPVCoordinates().getVelocity());
        // Call the DragSensitive method to compute partial derivatives with respect to
        // position/velocity:
        ...
        double[][] result = JavaMathAdapter.matrixAdd(
                JavaMathAdapter.threeDMatrixVectorMultiply(computeGaussDerivativeEquations(orbitMean),
                        dragTNW.toArray()),
        ...
        // Temporary array containing the d(GF)/dE term of the transition matrix (vectorized form),
        // for the current
        // squaring point.
    ...
    /**
     * Compute short periods drag coefficients.
     * @param orbitCIRF orbit in CIRF frame
     * @param dOrbParams dOrb/dParams for each squaring points
     * @param nSquaringPointsDrag number of squaring points
     * @param ksi ksi array of squaring points
     * @param deltavi delta anomaly between drag entry/exit
     * @return short periods drag coefficients
     * @throws PatriusException thrown if computation failed
     */
    private List<double[][]> computeSPDragCoefficients(final StelaEquinoctialOrbit orbitCIRF,
            final double[][] dOrbParams, final int nSquaringPointsDrag, final double[] ksi, final double deltavi)
            throws PatriusException {
     ...
     * @throws PatriusException thrown if computation failed
     */
    private double[][] computeDOrbParams(final StelaEquinoctialOrbit orbit, final double[] bounds,
        ...
        for (int i = 0; i < this.squaringPoints; i++) {
            ...
            final SpacecraftState stateOsc = new SpacecraftState(orbitOsc);
            // Compute the drag in the MOD frame:
            final Vector3D dragMOD = this.spacecraft
                    .dragAcceleration(stateOsc, density, velocity.subtract(velocityOsc));

            // Transform the drag vector in the TNW frame:
            final LocalOrbitalFrame tnw = new LocalOrbitalFrame(frame, LOFType.TNW, orbitOsc, "TNW_SP");
            final Transform transform = tnw.getParent().getTransformTo(tnw, squaringDate);
            final Vector3D dragTNW = transform.transformVector(dragMOD);
            ...
            final double[] instantGaussEq = JavaMathAdapter.matrixVectorMultiply(gaussEq, dragTNW.toArray());
            dOrbParamsComputed[0][i] = adjustCoef * instantGaussEq[0];
            dOrbParamsComputed[1][i] = adjustCoef * instantGaussEq[1];
            dOrbParamsComputed[2][i] = adjustCoef * instantGaussEq[2];
            dOrbParamsComputed[3][i] = adjustCoef * instantGaussEq[3];
            dOrbParamsComputed[4][i] = adjustCoef * instantGaussEq[4];
            dOrbParamsComputed[5][i] = adjustCoef * instantGaussEq[5];
        }

        // Return result
        return dOrbParamsComputed;
    }

    /**
     * Build nature converter without drag.
     * @param converter current converter
     * @return nature converter without drag
     */
    private OrbitNatureConverter getConverterWithoutDrag(final OrbitNatureConverter converter) {
        ...
        return new OrbitNatureConverter(forceModelsNoDrag);
    }
}
