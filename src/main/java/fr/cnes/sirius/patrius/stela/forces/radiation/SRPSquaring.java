...
public class SRPSquaring extends AbstractStelaGaussContribution {
    ...
    public double[] computePerturbation(final StelaEquinoctialOrbit orbit,
        ...
        for (int i = 0; i < this.quadPoints; i++) {
            ...
            final double[] instantGaussEq = JavaMathAdapter.matrixVectorMultiply(this.computeGaussEquations(quadOrbit),
                srpTNW.toArray());

            // Perform change of variable
            final double adjustCoef = this.computeAdjustCoef(quadOrbit, eccAnomalies[i]);
    ...
    protected double[] computeInOutTrueAnom(final StelaEquinoctialOrbit orbit,
        ...
        final double[] sunPhiBeta = this.computeSunBetaPhi(orbit, sunPV);
        ...
        if (MathLib.abs(sinBeta) > this.radius / (a * (1. - e))) {
            ...
            // Polynomial order
            final int polyOrder = this.getPolyOrder(a0, a1, a2, a3, a4);
            ...
            switch (polyOrder) {
                ...
                case 2:
                    ...
                    roots = this.solvePolyDeg2(new Complex(a2, 0), new Complex(a1, 0), new Complex(a0, 0));
                    break;
                case 3:
                    // Third-order polynomial
                    roots = this.solvePolyDeg3(a3, a2, a1, a0);
                    break;
                case 4:
                    if (MathLib.abs(a3) < TOL_MAX_DEG && MathLib.abs(a1) < TOL_MAX_DEG) {
                        // Biquadratic polynomial
                        roots = this.solveBiquadratic(a4, a2, a0);
                    } else {
                        // Classical fourth-order polynomial
                        roots = this.solvePolyDeg4(a4, a3, a2, a1, a0);
                    }
                    break;
                default:
            ...
            final double[] alphaInOut = this.rootsFiltering(roots);
    ...
    protected double[] rootsFiltering(final Complex[] roots) {
        final ArrayList<Double> filteredRoots = new ArrayList<Double>();
    ...
    protected Complex[] solveBiquadratic(final double c4, final double c2, final double c0) {
        final Complex[] y = this.solvePolyDeg2(new Complex(c4, 0), new Complex(c2, 0), new Complex(c0, 0));
    ...
    protected Complex[] solvePolyDeg4(final double c4, final double c3, final double c2,
        ...
        if (b == 0) {
            final Complex[] z = this.solveBiquadratic(1, a, c);
            z0 = z[0];
            z1 = z[1];
            z2 = z[2];
            z3 = z[3];
            ...
            final Complex[] phiRoot = this.solvePolyDeg3(1., -a, -4 * c, 4 * a * c - b * b);
            ...
            final Complex[] z01 = this.solvePolyDeg2(Complex.ONE, ig, ib2g.add(phiRoot[0].multiply(1. / 2.)));
            final Complex[] z23 =
                this.solvePolyDeg2(Complex.ONE, ig.negate(), phiRoot[0].multiply(1. / 2.).subtract(ib2g));
            // temporary result
            z0 = z01[0];
            z1 = z01[1];
            z2 = z23[0];
     ...
     */
    protected int getPolyOrder(final double a0, final double a1, final double a2, final double a3, final double a4) {
        // Initialization
        final int res;
