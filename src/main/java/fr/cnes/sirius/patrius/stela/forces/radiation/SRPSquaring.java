...
public class SRPSquaring extends AbstractStelaGaussContribution {
    ...
    /** Default Fourier Series order for short periods computation. */
    private static final int DEFAULT_ORDER_FS = 10;
    ...
    /** Fourier Series order for short periods computation. */
    private final int orderFS;

    /**
     * Create an instance with an assembly and a number of points. Reference normalized flux is that of STELA.
     * <p>Fourier Series order for short periods computation is set to default value: 10.</p> 
     * 
    ...
    public SRPSquaring(final RadiationSensitive radiativeSpacecraft, final int quadraturePoints,
        ...
        this(radiativeSpacecraft, quadraturePoints, sunBody, earthRadius, dRef, pRef, DEFAULT_ORDER_FS);
    }
     ...
     * @param pRef
     *        reference solar radiation pressure at dRef (N/m<sup>2</sup>)
     * @param orderFS Fourier Series order for short periods computation
     */
    public SRPSquaring(final RadiationSensitive radiativeSpacecraft, final int quadraturePoints,
        final PVCoordinatesProvider sunBody, final double earthRadius, final double dRef, final double pRef,
        final int orderFS) {
...
//        Code in order to comply with STELA 3.4 and current short periods computation
//        Computation of quadrature points and Gauss equations is slightly different
//        Left for future use

//        // Compute quadrature points
//        final List<double[]> points = getQuadraturePoints(orbit);
//
//        // Compute Gauss Eqs for SRP
//        final double[][] dOrbParams = computeDOrbParams(orbit, points);
//
//        // Compute delta anomalies
//        
...
//        result[0] = Squaring.simpsonMean(dOrbParams[0], deltaEi);
//        result[1] = Squaring.simpsonMean(dOrbParams[1], deltaEi);
//        result[2] = Squaring.simpsonMean(dOrbParams[2], deltaEi);
//        result[3] = Squaring.simpsonMean(dOrbParams[3], deltaEi);
//        result[4] = Squaring.simpsonMean(dOrbParams[4], deltaEi);
//        result[5] = Squaring.simpsonMean(dOrbParams[5], deltaEi);
//        this.dPert = result;
//        return result;
     ...
     * @return In/Out eccentric anomalies
     */
    protected double[] computeInOutEccentricAnom(final StelaEquinoctialOrbit orbit,
        ...
        final double e2 = ex * ex + ey * ey;
        final double e = MathLib.sqrt(e2);
        final double sqrt1minusE2 = MathLib.sqrt(1. - e2);
        // Cos of in-out true anomalies
        final double cosNuIn = MathLib.cos(startPoint);
        final double cosNuOut = MathLib.cos(endPoint);
        // Sin of in-out eccentric anomalies
        final double sinEeccIn = sqrt1minusE2 * MathLib.sin(startPoint) / (1. + e * cosNuIn);
        final double sinEeccOut = sqrt1minusE2 * MathLib.sin(endPoint) / (1. + e * cosNuOut);
        // Corresponding eccentric anomalies
        double anomEccIn = MathLib.atan2(sinEeccIn, (cosNuIn + e) / (1. + e * cosNuIn));
        double anomEccOut = MathLib.atan2(sinEeccOut, (cosNuOut + e) / (1. + e * cosNuOut));
        anomEccIn = JavaMathAdapter.mod(anomEccIn, TWO_PI);
        ...
        // Return result
        return new double[] { anomEccIn, anomEccOut };
    }
    ...
    public double[] computeShortPeriods(final StelaEquinoctialOrbit orbit,
        ...
        // Eccentric anomalies span
        final double deltaEi;
        final double anomEccIn = points.get(points.size() - 1)[1];
        final double anomEccOut = points.get(0)[1];

        if (anomEccIn <= anomEccOut) {
            // Code from STELA 3.4. Unreachable code because quadrature points computation has changed
            // in the meantime from STELA 3.X to STELA 3.4
            deltaEi = (2. * MathLib.PI - (anomEccOut - anomEccIn)) / (quadPoints - 1);
        } else {
            deltaEi = (anomEccIn - anomEccOut) / (quadPoints - 1);
        }

        // Compute ksi
        final KeplerianParameters kepParams = orbit.getParameters().getKeplerianParameters();
        final double raan = JavaMathAdapter.mod(kepParams.getRightAscensionOfAscendingNode(), 2. * MathLib.PI);
        final double w = JavaMathAdapter.mod(kepParams.getPerigeeArgument(), 2. * MathLib.PI);

        final double[] ksi = new double[quadPoints];
        for (int i = 0; i < points.size(); i++) {
            final double m = JavaMathAdapter.mod(points.get(i)[2], 2. * MathLib.PI);
            ksi[i] = JavaMathAdapter.mod(raan + w + m, 2. * MathLib.PI);
        }

        // Computation of coefficients
        // Compute functions under integral
        final double sma = orbit.getA();
        final double sma2 = sma * sma;
        final double meanMvt = MathLib.sqrt(Constants.CNES_STELA_MU / (sma * sma2));
        final List<double[][][]> listQuadElements = computeQuadElements(dOrbParams, ksi, orderFS,
                quadPoints);
        // Compute coefficients
        final List<double[][]> coeffs = computeCoeffs(listQuadElements, orderFS, quadPoints, meanMvt,
                deltaEi);

        // Mean Longitude xi
        final double xi = orbit.getLM();

        // Short periods SRP through evaluation of Fourier series
        return evaluateFS(coeffs, orderFS, xi);
    }
     ...
     */
    private List<double[]> getQuadraturePoints(final StelaEquinoctialOrbit orbit) throws PatriusException {
        // Initialization
        ...
        // Determination of shadowed part of the orbit (in and out true anomalies)
        final double[] nuInOut = computeInOutTrueAnom(orbit, sunPV);

        // Determination of enlightened part of the orbit (in and out true anomalies)
        final double[] nuInOut2;
        if (nuInOut[0] == 0 && nuInOut[1] == 0) {
            // No eclipse

            // Compute eccentric anomaly from mean anomaly
            final double ecc = kepParam.getEccentricAnomaly();

            // Compute true anomaly from eccentric anomaly
            final double beta = e / (1 + MathLib.sqrt((1 - e) * (1 + e)));
            final double v = ecc + 2 * MathLib.atan(beta * MathLib.sin(ecc) / (1 - beta * MathLib.cos(ecc)));

            nuInOut2 = new double[] { v - MathLib.PI, v + MathLib.PI };
        } else {
            // Eclipse
            nuInOut2 = new double[] { nuInOut[1], nuInOut[0] };
            if (nuInOut2[1] < nuInOut2[0]) {
                nuInOut2[1] += 2. * MathLib.PI;
            }
        }

        final double[] eccInOut = computeInOutEccentricAnom(orbit, nuInOut2[0], nuInOut2[1]);
        final double anomEccIn = eccInOut[0];
        final double anomEccOut = eccInOut[1];

        // Quadrature points
        return Squaring.computeSquaringPointsEccentric2(quadPoints, orbit, anomEccIn, anomEccOut);
    }

    /**
     * Compute Gauss Equations (first order partial derivatives dE'/dt) to state E' for the Solar Radiation Pressure,
     * in TNW frame.
     * @param orbit
     *            orbit
     * @param points
     *            list of quadrature points
     * @return dE'/dt at each quad point.
     * @throws PatriusException thrown if computation failed
     */
    private double[][] computeDOrbParams(final StelaEquinoctialOrbit orbit,
            final List<double[]> points) throws PatriusException {

        // Initialization
        final double[][] gauss = new double[6][quadPoints];

        // Eccentricity + AOP + RAAN
        final double ex = orbit.getEquinoctialEx();
        final double ey = orbit.getEquinoctialEy();
        final double aopPlusRaan = MathLib.atan2(ey, ex);
        ...
        for (int i = 0; i < this.quadPoints; i++) {
            ...
            final AbsoluteDate date = orbit.getDate().shiftedBy(point[0]);

            // LOF for quadrature point
            final StelaEquinoctialOrbit quadOrbit = new StelaEquinoctialOrbit(orbit.getA(), orbit.getEquinoctialEx(),
                orbit.getEquinoctialEy(), orbit.getIx(), orbit.getIy(), point[2] + aopPlusRaan,
                orbit.getFrame(), date, orbit.getMu());
            final LocalOrbitalFrame lof = new LocalOrbitalFrame(quadOrbit.getFrame(), LOFType.TNW, quadOrbit,
                "LOF_SP");
            ...
            // Perform change of variable
            final double adjustCoef = computeAdjustCoef(quadOrbit, point[1]);
            gauss[0][i] = adjustCoef * instantGaussEq[0];
            gauss[1][i] = adjustCoef * instantGaussEq[1];
            gauss[2][i] = adjustCoef * instantGaussEq[2];
            gauss[3][i] = adjustCoef * instantGaussEq[3];
            gauss[4][i] = adjustCoef * instantGaussEq[4];
            gauss[5][i] = adjustCoef * instantGaussEq[5];
        }
