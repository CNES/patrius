...
public final class TopocentricFrame extends Frame {
     ...
     *        local surface cartesian point (expressed in body frame) where topocentric frame is defined
     * @param name
     *        the string representation
     *        
     * @throws PatriusException 
     */
    public TopocentricFrame(final BodyShape bodyParentShape, final Vector3D cartesianPoint,
            final String name) throws PatriusException {

        this(bodyParentShape, cartesianPoint, null, -MathUtils.HALF_PI, name);
    }
    
    /**
     * This constructor builds a topocentric frame whose Z-axis is the zenith.<br>
     * The angle in radians trigowise between the North direction (= "Reference azymuth") and the x-axis of the
     * Topocentric Frame is frameOrientation.<br>
     * <br>
     * 
     * @param bodyParentShape
     *        body shape on which the local point is defined
     ...
     * 
     * @throws PatriusException
     *         if cartesian point cannot be converted into a geodetic point
     */
    public TopocentricFrame(final BodyShape bodyParentShape, final Vector3D cartesianPoint,
            final double frameOrientation, final String name) throws PatriusException {

        this(bodyParentShape, cartesianPoint, null, frameOrientation, name);
    }
    
    /**
     * This constructor builds an East oriented topocentric frame. Its axis are (x = East, y = North, z = Zenith).
     * With this constructor, the zenith is defined by the user.
     * 
     * @param bodyParentShape
     *        body shape on which the local point is defined
     * @param cartesianPoint
     *        local surface cartesian point (expressed in body frame) where topocentric frame is defined
     * @param zenith
     *        local surface normal (expressed in body frame) used as topocentric zenith
     * @param name
     *        the string representation
     * 
     * @throws PatriusException
     *         if cartesian point cannot be converted into a geodetic point
     */
    public TopocentricFrame(final BodyShape bodyParentShape, final Vector3D cartesianPoint, final Vector3D zenith,
            final String name) throws PatriusException {

        this(bodyParentShape, cartesianPoint, zenith, -MathUtils.HALF_PI, name);
    }
    
    /**
     * This constructor builds a topocentric frame whose zenith (topocentric Z-axis) is defined by the user.<br>
     * The angle in radians trigowise between the North direction (= "Reference azimuth") and the x-axis of the
     * topocentric frame is frameOrientation<br>
     * <br>
     * 
     * @param bodyParentShape
    ...
    public TopocentricFrame(final BodyShape bodyParentShape, final Vector3D cartesianPoint, final Vector3D zenith,
        ...
        super(bodyParentShape.getBodyFrame(), createTransform(bodyParentShape, cartesianPoint, zenith,
            frameOrientation), name, false);
        this.parentShape = bodyParentShape;
        this.point = bodyParentShape.transformFromZenith(cartesianPoint, zenith, bodyParentShape.getBodyFrame(),
    ...
    private static Transform createTransform(final BodyShape bodyParentShape, final GeodeticPoint geodeticPoint,
        ...
        final Transform rotation = computeTransformRotation(geodeticPoint, frameOrientation);
     ...
     * @return the transformation from body centered frame to topocentric frame
     * 
     * @throws PatriusException if cartesian point cannot be converted into a geodetic point
     */
    private static Transform createTransform(final BodyShape bodyParentShape, final Vector3D cartesianPoint,
                                             final Vector3D zenith, final double frameOrientation)
        throws PatriusException {
        ...
        // 2. Rotate axes
        // Compute the rotation from the body centered frame to the topocentric frame
        final GeodeticPoint geodeticPoint = bodyParentShape.transformFromZenith(cartesianPoint, zenith,
            bodyParentShape.getBodyFrame(), AbsoluteDate.J2000_EPOCH);
        final Transform rotation = computeTransformRotation(geodeticPoint, frameOrientation);
     ...
     */
    private static Transform computeTransformRotation(final GeodeticPoint geodeticPoint,
                                                      final double frameOrientation) {
        ...
        return new Transform(AbsoluteDate.J2000_EPOCH, firstRotation.applyTo(secondRotation),
            Vector3D.ZERO);
    }

    /**
     * Returns the oriented angle between the local north and the x axis (trigowise).
     * 
