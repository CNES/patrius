...
public class EOP2000HistoryConstantOutsideInterval extends EOP2000History {
    ...
    public PoleCorrection getPoleCorrection(final AbsoluteDate date) {
        if (this.isEmpty()) {
            return PoleCorrection.NULL_CORRECTION; // NO EOP
        }
        final EOPEntry bound = this.getBound(date);
    ...
    public NutationCorrection getNutationCorrection(final AbsoluteDate date) {
        if (this.isEmpty()) {
            return NutationCorrection.NULL_CORRECTION; // NO EOP
        }
        final EOPEntry bound = this.getBound(date);
        return bound == null ? super.getNutationCorrection(date) : new NutationCorrection(bound.getDX(), bound.getDY());
    }
