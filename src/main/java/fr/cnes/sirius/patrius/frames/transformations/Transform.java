...
public class Transform implements TimeStamped, TimeShiftable<Transform>, TimeInterpolable<Transform>, Serializable {
    ...
    private static PVCoordinates compositePVCoordinates(final Transform first, final Transform second,
        ...
        Vector3D a1 = null;
        if (first.cartesian.getAcceleration() != null) {
            a1 = first.cartesian.getAcceleration();
        }
        final Vector3D oDot1 = first.angular.getRotationAcceleration();
        Vector3D a2 = null;
        if (second.cartesian.getAcceleration() != null) {
            a2 = second.cartesian.getAcceleration();
        }

        // Check if data is null or zero
        final boolean nullp1 = isNullOrZero(p1);
        final boolean nullp2 = isNullOrZero(p2);
        final boolean nullv1 = isNullOrZero(v1);
        final boolean nullv2 = isNullOrZero(v2);
        final boolean nulla1 = isNullOrZero(a1);
        final boolean nulla2 = isNullOrZero(a2);

        // Position
        Vector3D p = Vector3D.ZERO;
        if (!nullp1 || !nullp2) {
            // Compute position if none of the elements are null
            p = p1.add(r1.applyTo(p2));
        }

        // Velocity
        Vector3D v = Vector3D.ZERO;
        // Check whether the velocity should be simply projected
        if (projectVelocityAndAcceleration) {
            // Compute velocity if none of the needed elements are null
            if (!nullv1 || !nullv2) {
                v = v1.add(r1.applyTo(v2));
            }
        } else {
            // Compute velocity if none of the needed elements are null
            if (!nullv1 || !nullp2 || !nullv2) {
                v = v1.add(r1.applyTo(v2.add(Vector3D.crossProduct(o1, p2))));
            }
        }
        
        // Acceleration (null if not provided)
        Vector3D a = null;
        // Check that both accelerations are not null
        if (a1 != null && a2 != null) {
            // Check whether the acceleration should be simply projected
            if (projectVelocityAndAcceleration) {
                // Compute acceleration if none of the needed elements are null
                if (!nulla1 || !nulla2) {
                    a = a1.add(r1.applyTo(a2));
                } else {
                    // Computation times optimization: acceleration has been provided and is zero
                    a = Vector3D.ZERO;
                }
            } else {
                // Compute acceleration if none of the needed elements are null
                if (!nulla1 || !nullp2 || !nullv2 || !nulla2) {
                    ...
                    final Vector3D crossDotP = oDot1 == null ? Vector3D.ZERO : Vector3D.crossProduct(oDot1, p2);
                    a = a1.add(r1.applyTo(new Vector3D(1, a2, 2, crossV, 1, crossCrossP, 1, crossDotP)));
                } else {
                    // Computation times optimization: acceleration has been provided and is zero
                    a = Vector3D.ZERO;
                }
    ...
    /**
     * Check if provided vector is null.
     * 
     * @param v
     *        a vector
     * @return true if vector is null
     */
    private static boolean isNullOrZero(final Vector3D v) {
        return v == null || (v.getX() == 0 && v.getY() == 0 && v.getZ() == 0);
    }
    ...
    private static AngularCoordinates compositeAngularCoordinates(final Transform first, final Transform second,
        ...
        AngularCoordinates res = r1.addOffset(r2, computeSpinDerivatives);
        res = new AngularCoordinates(res.getRotation(), res.getRotationRate(), res.getRotationAcceleration(),
                projectVelocityAndAcceleration);

        if (computeSpinDerivatives) {
            return new AngularCoordinates(res.getRotation(), res.getRotationRate(), compositeRotationAcceleration(
                first, second), projectVelocityAndAcceleration);
        }
        return res;
    }
    ...
    private static Vector3D compositeRotationAcceleration(final Transform first, final Transform second) {
        ...
        final Vector3D o1 = first.angular.getRotationRate();
        final Vector3D oDot1 = first.angular.getRotationAcceleration() == null ? Vector3D.ZERO : first.angular
            .getRotationAcceleration();
        final Rotation r2 = second.angular.getRotation();
        final Vector3D o2 = second.angular.getRotationRate();
        final Vector3D oDot2 = second.angular.getRotationAcceleration() == null ? Vector3D.ZERO : second.angular
            .getRotationAcceleration();

        Vector3D res = Vector3D.ZERO;

        // Check if elements are null
        final boolean nulloDot2 = isNullOrZero(oDot2);
        final boolean nulloDot1 = isNullOrZero(oDot1);
        final boolean nullo2 = isNullOrZero(o2);
        final boolean nullo1 = isNullOrZero(o1);

        if (!nulloDot2 || !nulloDot1 || (!nullo2 && !nullo1)) {
            // Compute result if none of the elements are null
            res = new Vector3D(1, oDot2, 1, r2.applyInverseTo(oDot1), -1, Vector3D.crossProduct(o2,
                r2.applyInverseTo(o1)));
        }
    ...
    public Transform getInverse(final boolean computeSpinDerivatives) {
        ...
        // Retrieve attributes
        final Vector3D p = this.cartesian.getPosition();
        final Vector3D v = this.cartesian.getVelocity();
        final Vector3D a = this.cartesian.getAcceleration();
        ...
        // Return result
        return new Transform(this.date, new PVCoordinates(pInv, vInv, aInv),
            this.angular.revert(computeSpinDerivatives));
    }
