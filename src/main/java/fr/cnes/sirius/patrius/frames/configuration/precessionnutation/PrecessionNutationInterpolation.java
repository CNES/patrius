 ...
 *
 * HISTORY
 * VERSION:4.14.1:OPENFD-394:10/09/2024:[PATRIUS] Erreur de la conversion
 * en CIRF avec une AbsoluteDate dont l'offset est négatif
 * VERSION:4.14:OPENFD-367:22/08/2024: [PATRIUS] Incompatibilite de model de precession avec le mode comptibilite 4.12
 ...
 * improve efficiency. The IAU-2000 model involves lots of terms (1600
 * components for x, 1275 components for y and 66 components for s). Recomputing
 * all these components for each point is really slow. The shortest period for
 * these components is about 5.5 days (one fifth of the moon revolution period),
 * hence the pole motion is smooth at the day or week scale. This implies that
 * these motions can be computed accurately using a few reference points per day
 * or week and interpolated between these points. This implementation uses 4
 * points (CIP and CIP velocities) separated by 1/2 day (43200 seconds) each,
 * the resulting maximal interpolation error on the frame is about
 * 1.3&times;10<sup>-10</sup> arcseconds.
 * </p>
 * 
 * <p>
 * Some information about performance: The CIP coordinates are interpolated
 * thanks to reference CIPCoordinates that we will call "references".<br>
 ...
 * {@link #allowedExtensionBeforeEphemerisReset} holes to be filled), the
 ...
 * {@link #initializeCipEphemeris} in order to avoid too many re-initializations
 * of the references.<br>
 * 
 * Another aspect is that if the required dates are separated of more than the
 * {@link #getInterpolationStep()}, the interpolation management will not be
 * efficient and it is advised to use a direct {@link PrecessionNutationModel}
 * instead.
 * </p>
 *
 * @author veuillh
...
public class PrecessionNutationInterpolation implements PrecessionNutationModel {
	...
	/**
	 * Time span between 2 samples of the interpolation (default value:
	 * {@link #DEFAULT_INTERP_STEP}).
	 */
	private final int interpolationStep;

	/**
	 * Ephemeris max size for memory usage control (default value:
	 * {@link #DEFAULT_EPHEM_MAX_SIZE}).
	 */
	private final int ephemerisMaxSize;

	/**
	 * Corresponds to the allowed extra CIPCoordinates to compute in order to keep
	 * the same ephemeris (default value:
	...
	// Reason: volatile so that all threads have the most recent value of this
	// attribute
	 ...
	 *
	 * @param pnModel Precession nutation model to use (must be a
	 *                {@link PrecessionNutationModel#isDirect() direct)}.
	 * @throws IllegalArgumentException if the precession nutation model is not
	 *                                  direct
	 */
	 ...
	 * Main constructor.
	 *
	 * @param pnModel                              Precession nutation model to use
	 *                                             (must be a
	 *                                             {@link PrecessionNutationModel#isDirect()
	 *                                             direct)}.
	 * @param interpolationStep                    time span between interpolation
	 *                                             points
	 * @param interpolationOrder                   number of interpolation points to
	 *                                             use
	 * @param ephemerisMaxSize                     Max size of the internal
	 *                                             ephemeris before reset (too avoid
	 *                                             memory leaks)
	 * @param allowedExtensionBeforeEphemerisReset When a new CIP coordinates needs
	 ...
	 *                                             the required date.
	 * @throws IllegalArgumentException if the number of interpolation points is
	 *                                  {@code < 2}<br>
	 *                                  if the number of interpolation points is not
	 *                                  even<br>
	 *                                  if the precession nutation model is not
	 *                                  direct
	 */
	...
	public PrecessionNutationInterpolation(final PrecessionNutationModel pnModel, final int interpolationStep,
			final int interpolationOrder, final int ephemerisMaxSize, final int allowedExtensionBeforeEphemerisReset) {
	 ...
	 * {@link #allowedExtensionBeforeEphemerisReset}.<br>
	 ...
	 * </p>
	 *
	 * @param firstUsableDate First usable date of the ephemeris
	 * @param lastUsableDate  Last usable date of the ephemeris
	 * @throws IllegalArgumentException if the ephemeris exceeds the maximum allowed
	 *                                  size
	 */
	public void initializeCIPEphemeris(final AbsoluteDate firstUsableDate, final AbsoluteDate lastUsableDate) {
		...
		try {
			final int nbSteps = (int) MathLib
					.ceil(lastUsableDate.durationFrom(firstUsableDate) / this.interpolationStep) - 1;
	...
	public CIPCoordinates getCIPCoordinates(final AbsoluteDate date) {
		...
		// Check that this class is never used with Patrius 4.12 or older versions
        if (PatriusConfiguration.getPatriusCompatibilityMode() != PatriusVersionCompatibility.NEW_MODELS) {
			throw new IllegalStateException(
					"Patrius version compatibility problem: class not allowed to run with Patrius 4.12 or older. The class PrecessionNutationCache should be used instead.");
		}

		// Implementation note: the thread safety management is based on the
		// "CachedData" example of the
		...
		if (needToUpdateCipEphemeris(date)) {
			...
			try {
				...
				if (this.cipEphemeris == null || this.cipEphemeris.getSampleSize() > this.ephemerisMaxSize) {
					...
					// A CIP ephemeris already exists. Check if it needs to be extended to cover the
					// required date.
					...
					if (durationWrtFirstUsableDate < 0) {
						...
						final int stepsBefore = (int) MathLib
								.ceil(-durationWrtFirstUsableDate / this.interpolationStep);
						if (stepsBefore > 2 * this.halfInterpolationOrder + this.allowedExtensionBeforeEphemerisReset) {
							...
							final CIPCoordinates[] extraSamples = generateCIPCoordinates(
									this.cipEphemeris.getFirstDate(), stepsBefore, -1);
							this.cipEphemeris = this.cipEphemeris.extendInterpolableEphemeris(extraSamples, false,
									false);
						}
					} else {
						// Check with respect to the end of the ephemeris
						final double durationWrtLastUsableDate = date
								.durationFrom(this.cipEphemeris.getLastUsableDate());
						if (durationWrtLastUsableDate > 0) {
							// The required date is after the already available ephemeris dates
							final int stepsAfter = (int) MathLib
									.ceil(durationWrtLastUsableDate / this.interpolationStep);
							if (stepsAfter > 2 * this.halfInterpolationOrder
								...
								final CIPCoordinates[] extraSamples = generateCIPCoordinates(
										this.cipEphemeris.getLastDate(), -1, stepsAfter);
								this.cipEphemeris = this.cipEphemeris.extendInterpolableEphemeris(extraSamples, true,
										false);
							}
						}
					}
	...
	private boolean needToUpdateCipEphemeris(final AbsoluteDate date) {
		return this.cipEphemeris == null || date.compareTo(this.cipEphemeris.getFirstUsableDate()) < 0
	 ...
	 *
	 * <p>
	 * Note that the ephemeris is generated using dates belonging to a specific mesh
	 * in order to avoid having too erratic reference dates.
	 * </p>
	 *
	 * @param date Date for which the ephemeris should be initialized
	 * @return the generated ephemeris
	 */
	private TimeStampedInterpolableEphemeris<CIPCoordinates, CIPCoordinates> initializeEphemeris(
	...
	/**
	 * Generate a date belonging to a specific mesh (multiples of
	 * {@link #interpolationStep}) so that the set of CIPCooridnates reference dates
	 * does not depend of the specific dates used to initialize the ephemeris.
	 * 
	 * @param date Date used to generate the mesh date
	 * @return the mesh date (multiple of the interpolation step)
	 */
	private AbsoluteDate generateMeshDate(final AbsoluteDate date) {
		final long epoch = date.getEpoch() + (long) Math.floor(date.getOffset());
		final long floorDivision;
		if (epoch >= 0) {
			// Integer division is equivalent to do a division as doubles and then call
			// floor function
			floorDivision = epoch / this.interpolationStep;
		} else {
			// If epoch is negative, we need to remove 1 to the integer division to have the
	 ...
	 * Initialize an ephemeris of the correct interpolation order.
	 *
	 * @param cipCoordinatesArray CIP coordinates
	 * @return the generated ephemeris
	 */
	private TimeStampedInterpolableEphemeris<CIPCoordinates, CIPCoordinates> generateEphemeris(
			final CIPCoordinates[] cipCoordinatesArray) {
	 ...
	 *
	 * @param refDate     Reference date to start with
	 * @param stepsBefore Number of steps before the reference date. Can be negative
	 *                    to not include the refDate.
	 * @param stepsAfter  Number of steps after the reference date. Can be negative
	 *                    to not include the ref date.
	 ...
	 *
	 * @return the current usable interval of the ephemeris. Can be {@code null} if
	 *         the ephemeris has not been initialized yet.
	 */
	 ...
	 * See {@link TimeStampedInterpolableEphemeris#getCacheReusabilityRatio()} for
	 * more information.
	 * 
	...
	private static class HermiteTimeStampedCIPCoordinatesInterpolationFunction
		...
		/**
		 * Hermite interpolator used to interpolate the
		 ...
		 *
		 * @param samples  Samples of time stamped PV data. It should be sorted.
		 * @param indexInf Inferior index
		 * @param indexSup Superior index
		 */
		public HermiteTimeStampedCIPCoordinatesInterpolationFunction(final CIPCoordinates[] samples, final int indexInf,
				final int indexSup) {
