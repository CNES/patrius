...
public class TLE implements TimeStamped, Serializable {
    ...
    public TLE(final String line1In, final String line2In) throws PatriusException {
        ...
        // identification
        this.satelliteNumber = this.parseInteger(line1In, 2, 5);
        final int satNum2 = this.parseInteger(line2In, 2, 5);
        ...
        if (nbDigit > 6) {
            isBigYear = true;
            this.launchYear = this.parseYear(line1In, 9, isBigYear);
            this.launchNumber = this.parseInteger(line1In, 13, 3);
            this.launchPiece = line1In.substring(16, 17).trim();
        } else {
            // Only two last number of the year
            this.launchYear = this.parseYear(line1In, 9, isBigYear);
            this.launchNumber = this.parseInteger(line1In, 11, 3);
            this.launchPiece = line1In.substring(14, 17).trim();
        }

        this.ephemerisType = this.parseInteger(line1In, 62, 1);
        this.elementNumber = this.parseInteger(line1In, 64, 4);

        // Date format transform (nota: 27/31250 == 86400/100000000)
        final int year = this.parseYear(line1In, 18, false);
        final int dayInYear = this.parseInteger(line1In, 20, 3);
        final long df = 27L * this.parseInteger(line1In, 24, 8);
        ...
        // rad/s^2 and rad/s^3
        this.meanMotion = this.parseDouble(line2In, 52, 11) * FastMath.PI
                / (Constants.JULIAN_DAY / 2.);
        this.meanMotionFirstDerivative = this.parseDouble(line1In, 33, 10) * FastMath.PI
                / 1.86624e9;
        ...
        this.inclination = MathLib.toRadians(this.parseDouble(line2In, 8, 8));
        this.pa = MathLib.toRadians(this.parseDouble(line2In, 34, 8));
        this.raan = MathLib.toRadians(Double.parseDouble(line2In.substring(17, 25)
                .replace(' ', '0')));
        this.meanAnomaly = MathLib.toRadians(this.parseDouble(line2In, 43, 8));

        this.revolutionNumberAtEpoch = this.parseInteger(line2In, 63, 5);
    ...
    private void buildLine1() throws PatriusException {
        ...
        buffer.append(' ');
        buffer.append(this.addPadding(this.satelliteNumber, '0', 5, true));
        buffer.append(this.classification);

        buffer.append(' ');
        buffer.append(this.addPadding(this.launchYear % YEARS_IN_CENTURY, '0', 2, true));
        buffer.append(this.addPadding(this.launchNumber, '0', 3, true));
        buffer.append(this.addPadding(this.launchPiece, ' ', 3, false));

        buffer.append(' ');
        final TimeScale utc = TimeScalesFactory.getUTC();
        final int year = this.epoch.getComponents(utc).getDate().getYear();
        buffer.append(this.addPadding(year % YEARS_IN_CENTURY, '0', 2, true));
        ...
        final String sn1 = this.addPadding(new DecimalFormat(".00000000", SYMBOLS).format(n1), ' ',
                10, true);
        buffer.append(sn1);

        buffer.append(' ');
        final double n2 = this.meanMotionSecondDerivative * 5.3747712e13 / FastMath.PI;
        buffer.append(this.addPadding(this.formatExponentMarkerFree(n2, 5), ' ', 8, true));

        buffer.append(' ');
        buffer.append(this.addPadding(this.formatExponentMarkerFree(this.bStar, 5), ' ', 8, true));

        buffer.append(' ');
        buffer.append(this.ephemerisType);

        buffer.append(' ');
        buffer.append(this.addPadding(this.elementNumber, ' ', 4, true));
    ...
    private String formatExponentMarkerFree(final double d, final int mantissaSize) {
        ...
        final String sMantissa = this.addPadding((int) mantissa, '0', mantissaSize, true);
    ...
    private void buildLine2() {
        ...
        buffer.append(' ');
        buffer.append(this.addPadding(f34.format(MathLib.toDegrees(this.inclination)), ' ', 8, true));
        buffer.append(' ');
        buffer.append(this.addPadding(f34.format(MathLib.toDegrees(this.raan)), ' ', 8, true));
        buffer.append(' ');
        buffer.append(this.addPadding((int) MathLib.rint(this.eccentricity * 1.0e7), '0', 7, true));
        buffer.append(' ');
        buffer.append(this.addPadding(f34.format(MathLib.toDegrees(this.pa)), ' ', 8, true));
        buffer.append(' ');
        buffer.append(this.addPadding(f34.format(MathLib.toDegrees(this.meanAnomaly)), ' ', 8, true));

        buffer.append(' ');
        buffer.append(this.addPadding(
                f211.format(this.meanMotion * Constants.JULIAN_DAY / (2. * FastMath.PI)), ' ', 11,
                true));
        buffer.append(this.addPadding(this.revolutionNumberAtEpoch, ' ', 5, true));
     ...
     */
    private String addPadding(final int k, final char c, final int size,
            final boolean rightJustified) {
        return this.addPadding(Integer.toString(k), c, size, rightJustified);
    }
     ...
     * @return padded string
     */
    private String addPadding(final String string, final char c, final int size,
            final boolean rightJustified) {
     ...
     */
    private int parseYear(final String line, final int start, final boolean isBigYear) {
        if (isBigYear) {
            return this.parseInteger(line, start, 4);
        } else {
            final int year = 2000 + this.parseInteger(line, start, 2);
