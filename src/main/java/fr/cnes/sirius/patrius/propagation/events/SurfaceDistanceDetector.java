...
public class SurfaceDistanceDetector extends DistanceDetector {
    ...
    public enum BodyDistanceType {
        ...
        CLOSEST,
        /** Distance from the closest point of the body surface aligned with the spacecraft and the body centre */
        RADIAL;
    }
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        if (this.bodyDistanceType.equals(BodyDistanceType.CLOSEST)) {
            // Compute the geodetic point corresponding to the satellite position
            final GeodeticPoint satPosGeodeticPoint = this.bodyShape.transform(satPos, this.bodyFixedFrame, date);
            // Compute the closest distance as the altitude (which can be negative) of the geodetic point
            // corresponding to the satellite position
            final double closestDistance = satPosGeodeticPoint.getAltitude();
            ...
            final GeodeticPoint intersectionGeodeticPoint = this.bodyShape.getIntersectionPoint(line, satPos,
                this.bodyFixedFrame, date);
            // Transform the intersection point from a geodetic point to a Vector3D
            final Vector3D intersectionPoint = this.bodyShape.transform(intersectionGeodeticPoint);
    ...
    /**
     * Returns the chosen body distance type for the detector.
     * 
