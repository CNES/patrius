...
public class AltitudeDetector extends AbstractDetector {
     ...
     *        body shape with respect to which altitude should be evaluated
     */
    public AltitudeDetector(final double altitudeIn, final EllipsoidBodyShape bodyShapeIn) {
        this(altitudeIn, bodyShapeIn, DEFAULT_MAXCHECK, DEFAULT_THRESHOLD);
    }
     ...
     */
    public AltitudeDetector(final double altitudeIn, final EllipsoidBodyShape bodyShapeIn, final double maxCheck,
     ...
     * @param descending
     *        action performed when descending
     * @param removeAscending
     *        true if detector should be removed at ascending altitude detection
     * @param removeDescending
     *        true if detector should be removed at descending altitude detection
     * @since 3.1
     */
    public AltitudeDetector(final double altitudeIn, final EllipsoidBodyShape bodyShapeIn, final double maxCheck,
                            final double threshold, final Action ascending, final Action descending,
     ...
     *        action performed
     * @param remove
     *        true if detector should be removed at altitude detection
     * @since 3.1
     */
    public AltitudeDetector(final double altitudeIn, final EllipsoidBodyShape bodyShapeIn, final int slopeSelection,
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        final EllipsoidPoint point = this.bodyShape.buildPoint(pvBody.getPosition(), bodyFrame, state.getDate(),
            BodyPointName.DEFAULT);
        return point.getLLHCoordinates(LLHCoordinatesSystem.ELLIPSODETIC).getHeight() - this.altitude;
    }
