...
* VERSION:4.9:FA:FA-3148:10/05/2022:[PATRIUS] Erreur dans la methode getSignalReceptionDate de la classe AbstractDetector 
...
public abstract class AbstractDetector implements EventDetector {
    ...
    protected AbsoluteDate getSignalEmissionDate(final PVCoordinatesProvider emitter,
                                                 ...
                                                 final AbsoluteDate date, final double epsilon)
        throws PatriusException {
        final AbsoluteDate res;
        if (this.propagationDelayType.equals(PropagationDelayType.INSTANTANEOUS)) {
            // Instantaneous signal
            res = date;
        } else {
            // Take light speed into account
            final SignalPropagationModel model = new SignalPropagationModel(this.inertialFrame, epsilon);
            final SignalPropagation signal = model.computeSignalPropagation(emitter, orbit, date,
                FixedDate.RECEPTION);
    ...
    protected AbsoluteDate getSignalReceptionDate(final PVCoordinatesProvider receiver,
        ...
        if (this.propagationDelayType.equals(PropagationDelayType.INSTANTANEOUS)) {
            ...
            // Take light speed into account
            final SignalPropagationModel model = new SignalPropagationModel(this.inertialFrame, epsilon);
            final SignalPropagation signal = model.computeSignalPropagation(orbit, receiver, date,
                FixedDate.EMISSION);
