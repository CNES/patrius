...
public class JacobiansMapper implements Serializable {
    ...
    /**
     * Get the conversion Jacobian between state parameters and cartesian parameters in propagation
     * frame
     * (or in state frame if propagation frame is undefined).
     *
     * @param state
     *        spacecraft state
    ...
    /**
     * Get the conversion Jacobian between state parameters and cartesian parameters in the provided
     * propagation frame
     * (or in state frame if the provided propagation frame is undefined).
     *
     * @param state
     *        spacecraft state
     * @param orbitTypeIn
     ...
     * @throws PatriusException thrown if jacobian computation failed
     */
    private double[][] getdYdC(final SpacecraftState state, final OrbitType orbitTypeIn,
            final PositionAngle angleTypeIn, final Frame frameIn) throws PatriusException {

        // Use propagation frame if defined
        final Frame frame = frameIn == null ? state.getFrame() : frameIn;
        return state
                .getOrbit()
                .getJacobian(frame, frame, OrbitType.CARTESIAN, orbitTypeIn, PositionAngle.TRUE,
                        angleTypeIn).getData(false);
    }
     ...
     * @see #getParametersJacobian(Parameter, SpacecraftState)
     */
    public double[][] getStateJacobian(final SpacecraftState state, final OrbitType orbitTypeIn,
            final PositionAngle angleTypeIn, final Frame frameIn) throws PatriusException {
        ...
        // get the conversion Jacobian between state parameters and cartesian parameters
        final double[][] dYdC = this.getdYdC(state, orbitTypeIn, angleTypeIn, frameIn);
