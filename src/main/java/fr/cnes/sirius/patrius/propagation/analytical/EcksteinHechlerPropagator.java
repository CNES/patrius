...
public class EcksteinHechlerPropagator extends AbstractPropagator implements
    ...
    public EcksteinHechlerPropagator(final Orbit initialOrbit, final AttitudeProvider attitudeProv,
        ...
        try {
            ...
            super.resetInitialState(new SpacecraftState(this.getAttitudeProvider(), initialOrbit, massProvider));
    ...
    public EcksteinHechlerPropagator(final Orbit initialOrbit,
        ...
        try {
            ...
            // Build initial state
            super.resetInitialState(new SpacecraftState(getAttitudeProviderForces(), getAttitudeProviderEvents(),
                    initialOrbit, massProvider));
    ...
    private PropagationParameters buildInternalParameters(final CircularOrbit orbit)
        ...
        final CircularOrbit orbitBody = convertFrame(orbit, this.frame);
    ...
    private PropagationParameters computeMeanParametersInternal(final CircularOrbit osculating)
        ...
        // Initialization
        final CircularOrbit osculatingOrbit = convertFrame(osculating, this.frame);
    ...
    private CircularOrbit
        ...
        // bodyFrame
        return convertFrame(orbit, propagationFrame);
    }
    ...
    private CircularOrbit propagateOrbitInternal(final AbsoluteDate date, final PropagationParameters params,
        ...
        // semi major axis
        double f = (2.0 - 3.5 * sinI2) * exm * cl1 + (2.0 - 2.5 * sinI2) * eym * sl1 + sinI2 * cl2
        ...
        q = 3.75 * g5 * sinI1;
        f = (2.625 * sinI4 - 3.5 * sinI2 + 1.0) * sl1 + 7.0 / 6.0 * sinI2 * (1.0 - 1.125 * sinI2)
                * sl3 + 21.0 / 80.0 * sinI4 * sl5;
        rda += q * f;

        q = 105.0 / 16.0 * g6 * sinI2;
        f = (3.0 * sinI2 - 1.0 - 33.0 / 16.0 * sinI4) * cl2 + 0.75 * (1.1 * sinI4 - sinI2) * cl4
                - 11.0 / 80.0 * sinI4 * cl6;
        rda += q * f;

        // eccentricity
        f = (1.0 - 1.25 * sinI2) * cl1 + 0.5 * (3.0 - 5.0 * sinI2) * exm * cl2
                + (2.0 - 1.5 * sinI2) * eym * sl2 + 7.0 / 12.0 * sinI2 * cl3 + 17.0 / 8.0 * sinI2
                * (exm * cl4 + eym * sl4);
        final double rdex = qq * f;

        f = (1.0 - 1.75 * sinI2) * sl1 + (1.0 - 3.0 * sinI2) * exm * sl2 + (2.0 * sinI2 - 1.5)
                * eym * cl2 + 7.0 / 12.0 * sinI2 * sl3 + 17.0 / 8.0 * sinI2
                * (exm * sl4 - eym * cl4);
        ...
        // latitude argument
        f = (7.0 - 77.0 / 8.0 * sinI2) * exm * sl1 + (55.0 / 8.0 * sinI2 - 7.50) * eym * cl1
                + (1.25 * sinI2 - 0.5) * sl2 + (77.0 / 24.0 * sinI2 - 7.0 / 6.0)
                * (exm * sl3 - eym * cl3);
        double rdxl = qq * f;
        ...
        return new CircularOrbit(mean.getA() * (1.0 + rda), exm + rdex, eym + rdey, xim + rdxi,
                MathUtils.normalizeAngle(omm + rdom, FastMath.PI), MathUtils.normalizeAngle(xlm
                        + rdxl, FastMath.PI), PositionAngle.MEAN, mean.getFrame(), date,
                mean.getMu());
    }
     ...
     */
    private static CircularOrbit convertFrame(final CircularOrbit orbit, final Frame outputFrame)
                                                                                          throws PatriusException {
