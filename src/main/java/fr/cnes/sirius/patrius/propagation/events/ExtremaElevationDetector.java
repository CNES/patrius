...
public class ExtremaElevationDetector extends AbstractDetector {
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        return getElevationRate(state.getOrbit(), state.getFrame(), state.getDate());
    }
     ...
     * 
     * @param extPVProv
     *        PVCoordinatesProvider of the point for which elevation rate shall be computed
     * @param frame
     ...
     */
    private double getElevationRate(final PVCoordinatesProvider extPVProv, final Frame frame,
        ...
        if (getPropagationDelayType().equals(PropagationDelayType.LIGHT_SPEED)) {
            ...
            if (linkType.equals(LinkType.DOWNLINK)) {
                ...
                final AbsoluteDate stationReceptionDate = getSignalReceptionDate(topo, extPVProv, date);
                ...
                final Transform t = frame.getTransformTo(topo, stationReceptionDate);
                ...
                final AbsoluteDate stationEmissionDate = getSignalEmissionDate(topo, extPVProv, date);

                // Compute satellite PV coordinates in topocentric frame
                // The transformation date is emission date by the station
                final PVCoordinates extPV = extPVProv.getPVCoordinates(date, frame);
                final Transform t = frame.getTransformTo(topo, stationEmissionDate);
                extPVTopo = t.transformPVCoordinates(extPV);
            }
        } else {
            // Instantaneous case
            extPVTopo = extPVProv.getPVCoordinates(date, topo);
        }
