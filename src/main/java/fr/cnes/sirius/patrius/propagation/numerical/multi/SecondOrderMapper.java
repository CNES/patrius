...
class SecondOrderMapper implements SecondOrderStateMapper {
    ...
    public double[] buildFullState(final double[] y,
            final double[] yDot) {
        final double[] res = new double[this.dimension1];
        // Loop on each spacecraft
        int pos = 0;
        int pos2 = 0;
        for (final String id : this.satIdList) {
            // Perform mapping on one satellite
            final SecondOrderMapper1Sat mapper1Sat = new SecondOrderMapper1Sat(this.addEquationsAndTolerances.get(id));
    ...
    public double[] extractY(final double[] fullState) {
        final double[] y = new double[this.dimension2];
        // Loop on each spacecraft
        int pos = 0;
        int pos1 = 0;
        for (final String id : this.satIdList) {
    ...
    public double[] extractYDot(final double[] fullState) {
        final double[] yDot = new double[this.dimension2];
        // Loop on each spacecraft
        int pos = 0;
        int pos1 = 0;
        for (final String id : this.satIdList) {
    ...
    private int get1stOrderDimension() {
        ...
        for (final String id : this.satIdList) {
            ...
            final List<AdditionalEquationsAndTolerances> addEq = this.addEquationsAndTolerances.get(id);
            if (addEq != null) {
    ...
    public void writeExternal(final ObjectOutput oo) throws IOException {
        // Dimensions
        oo.writeInt(this.dimension1);
        oo.writeInt(this.dimension2);
        oo.writeInt(this.satIdList.size());
        // Satellite ID list
        for (final String id : this.satIdList) {
            oo.writeObject(id);
        }
        // Additional equations and tolerances
        oo.writeInt(this.addEquationsAndTolerances.size());
        for (final Entry<String, List<AdditionalEquationsAndTolerances>> entry : this.addEquationsAndTolerances
                .entrySet()) {
            // ID
    ...
    public void readExternal(final ObjectInput oi) throws IOException, ClassNotFoundException {
        // Dimensions
        this.dimension1 = oi.readInt();
        this.dimension2 = oi.readInt();
        // Satellite ID list
        this.satIdList = new ArrayList<>();
        final int m = oi.readInt();
        for (int i = 0; i < m; i++) {
            this.satIdList.add((String) oi.readObject());
        ...
        for (int i = 0; i < n; i++) {
            ...
            final List<AdditionalEquationsAndTolerances> listi = new ArrayList<>();
            // Additional equations and tolerances
            for (int j = 0; j < listSize; j++) {
                listi.add((AdditionalEquationsAndTolerances) oi.readObject());
            }
            this.addEquationsAndTolerances.put(satID, listi);
        }
    }
}
