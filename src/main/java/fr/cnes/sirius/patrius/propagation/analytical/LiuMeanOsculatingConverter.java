...
public class LiuMeanOsculatingConverter implements MeanOsculatingElementsProvider {
    ...
    public Orbit osc2mean(final Orbit orbit) throws PatriusException {
        ...
        // Initialization
        final Orbit osculating = this.convertFrame(orbit, this.frame);

        // Relative tolerance: threshold for each equinoctial parameter
        final double[] tol = { threshold * (1. + MathLib.abs(osculating.getA())),
            threshold * (1. + MathLib.abs(osculating.getEquinoctialEx())),
            threshold * (1. + MathLib.abs(osculating.getEquinoctialEy())),
            threshold * (1. + MathLib.abs(osculating.getHx())), threshold * (1. + MathLib.abs(osculating.getHy())),
    ...
    public Orbit mean2osc(final Orbit orbit) throws PatriusException {
      //CHECKSTYLE: resume MethodLength check
        ...
        final Orbit orbitComputationFrame = convertFrame(orbit, frame);
        ...
        final double ep = MathLib.pow(referenceRadius / p, 2);
        ...
        final double eSp = 0.5 * j2 * ep * (1. - 1.5 * sini2)
        ...
        final double iSp = 3. / 8. * j2 * ep * sin2i * (+e * cos2paf + cos2pa2f + e / 3. * cos2pa3f);

        // Computing the short period variation paSp of the argument of periapsis pa ( also known as omega ).
        final double paSp = 0.75 * j2 * ep * (4 - 5 * sini2) * (f - m + e * sinf)
                + 1.5 * j2 * ep * (1 - 1.5 * sini2)
     ...
     */
    protected Orbit convertFrame(final Orbit orbit,
