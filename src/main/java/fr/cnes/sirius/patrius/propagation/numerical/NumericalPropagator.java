 ...
 * {@link fr.cnes.sirius.patrius.propagation.analytical.EcksteinHechlerPropagator Eckstein-Hechler}, but requires a few
 ...
 * <li>whether {@link AdditionalEquations additional equations} (for example {@link PartialDerivativesEquations
 * Jacobians}) should be propagated along with orbital state ( {@link #addAdditionalEquations(AdditionalEquations)}),
 * <li>the discrete events that should be triggered during propagation ( {@link #addEventDetector(EventDetector)},
 * {@link #clearEventsDetectors()})</li>
 * <li>the binding logic with the rest of the application ({@link #setSlaveMode()},
 * {@link #setMasterMode(double, PatriusFixedStepHandler)}, {@link #setMasterMode(PatriusStepHandler)},
 ...
 * However, specifying only the initial state and a Newtonian gravity model would mean the propagator would use only
 * keplerian forces. In this case, the simpler {@link fr.cnes.sirius.patrius.propagation.analytical.KeplerianPropagator
 * KeplerianPropagator} class would perhaps be more effective.
 * </p>
 * <p>
 ...
 * e<sub>y</sub>, i, &Omega;, &alpha;<sub>M</sub> or &alpha;<sub>E</sub> or &alpha;<sub>v</sub>) in meters and radians,
 * </li>
 ...
 * </pre>
 * <p>
 * The same propagator can be reused for several state extrapolations, by resetting the initial state without modifying
 * the other configuration parameters. However, the same instance cannot be used simultaneously by different threads,
 * the class is <em>not</em> thread-safe.
 * </p>
 * <b> Warning </b> : when using a fixed step handler (method {@link #setMasterMode(double, PatriusFixedStepHandler)},
 ...
 * is synchronized only once per integration step. In any other case (using an {@link PatriusStepHandler} ) for
...
public class NumericalPropagator implements Propagator, Observer {
     ...
     * the instance is empty, i.e. there are no forces at all, not even the Newtonian gravitational one. The
     * defaults are {@link OrbitType#EQUINOCTIAL} for {@link #orbitType propagation orbit type},
     * {@link PositionAngle#TRUE} for {@link #angleType position angle type} and {@link FramesFactory#getGCRF()} for
     * {@link #propagationFrame propagation frame}.
     *
     * <p>
     ...
     * @throws PatriusException problem with the propagation frame creation.
     */
    public NumericalPropagator(final FirstOrderIntegrator integratorIn) throws PatriusException {
        this(integratorIn, FramesFactory.getGCRF(), OrbitType.EQUINOCTIAL, PositionAngle.TRUE);
    }

    /**
     * Create a new instance of NumericalPropagator, based on orbit definition mu. After creation,
     * the instance is empty, i.e. there are no forces at all, not even the Newtonian gravitational one. The
     * defaults are {@link OrbitType#EQUINOCTIAL} for {@link #orbitType propagation orbit type} and
     * {@link PositionAngle#TRUE} for {@link #angleType position angle type}.
     *
     * <p>
     * The new instance of NumericalPropagator is declared as an {@link Observer observer} of the contained integrator.
     * As observer, it gets notified if a detector is deleted in the integrator sub-layer, to update its own detectors
     ...
     * @param propagationFrameIn
     *        The frame used for propagation
     * @throws PatriusException thrown if the chosen propagation frame is not pseudo-inertial.
     */
    public NumericalPropagator(final FirstOrderIntegrator integratorIn, final Frame propagationFrameIn)
        throws PatriusException {
        this(integratorIn, propagationFrameIn, OrbitType.EQUINOCTIAL, PositionAngle.TRUE);
    }

    /**
     * Create a new instance of NumericalPropagator, based on orbit definition mu. After creation,
     * the instance is empty, i.e. there are no forces at all, not even the Newtonian gravitational one.
     * 
     * <p>
     * The new instance of NumericalPropagator is declared as an {@link Observer observer} of the contained integrator.
     * As observer, it gets notified if a detector is deleted in the integrator sub-layer, to update its own detectors
     ...
     *        Numerical integrator to use for propagation
     * @param orbitTypeIn
     *        The orbital parameters type
     * @param propagationFrameIn
     *        The frame used for propagation (must be pseudo-inertial)
     * @param angleTypeIn
     *        The position angle type
     * @throws PatriusException thrown if the chosen propagation frame is not pseudo-inertial.
     */
    public NumericalPropagator(final FirstOrderIntegrator integratorIn, final Frame propagationFrameIn,
                               final OrbitType orbitTypeIn, final PositionAngle angleTypeIn) throws PatriusException {
        // Check that the propagation frame is pseudo-inertial
        if (propagationFrameIn.isPseudoInertial()) {
            this.propagationFrame = propagationFrameIn;
        } else {
        ...
        if (integratorIn != null) {
            ...
            throw new PropagationException(PatriusMessages.ODE_INTEGRATOR_NOT_SET_FOR_ORBIT_PROPAGATION);
        }
     ...
     */
    public double getMu() {
        return this.initialState.getMu();
    }
    ...
    public void setAttitudeProvider(final AttitudeProvider attitudeProvider) {
        ...
        for (int i = 0; i < this.addEquationsAndTolerances.size(); i++) {
            if (this.addEquationsAndTolerances.get(i).getEquations().getName()
                ...
                throw PatriusException.createIllegalStateException(PatriusMessages.ATTITUDE_ADD_EQ_ALREADY_DEFINED);
            }
        }
     ...
     * {@inheritDoc}
     * <p>
     * Note that this method has the side effect of replacing the step handlers of the underlying
     * integrator set up in the {@link #NumericalPropagator(FirstOrderIntegrator) constructor} or
     * the {@link #setIntegrator(FirstOrderIntegrator) setIntegrator} method. So if a specific step
     * handler is needed, it should be added after this method has been callled.
     * </p>
     */
    @Override
     ...
     * integrator set up in the {@link #NumericalPropagator(FirstOrderIntegrator) constructor} or
     * the {@link #setIntegrator(FirstOrderIntegrator) setIntegrator} method. So if a specific step
     * handler is needed, it should be added after this method has been called.
     * </p>
     */
    @Override
     ...
     */
    private AdditionalEquationsAndTolerances selectEquationsAndTolerances(final String name) throws PatriusException {
     ...
     * {@link #setMasterMode(PatriusStepHandler)} or {@link #setEphemerisMode()} since this method
     * reset the integrator step handlers list.
     * </p>
     *
     * <p>
     * <b>WARNING</b>: This method should be called only once and provided mass provider should be
     * the same used for force models.
     * </p>
     *
    ...
    public void setMassProviderEquation(final MassProvider massProvider) {
        ...
        // of event occurring at the same time)
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    try {
                        if (NumericalPropagator.this.nullMassDetector.isTriggered()) {
                            ...
                            // issues part mass may be a little below 0 (-1E-15 for example)
                            ...
                            final double additionalState = interpolator.getInterpolatedState().getAdditionalState(
     ...
     *         name is unknown
     */
    public void setAdditionalStateTolerance(final String name, final double[] absTol, final double[] relTol)
            throws PatriusException {
        // Check that tolerances' sizes are the same as the additional state is done in
        // buildAdditionalTolerances method.
        ...
        try {
            if (this.startDate == null) {
                if (this.initialState == null) {
                    ...
                    throw new PropagationException(PatriusMessages.INITIAL_STATE_NOT_SPECIFIED_FOR_ORBIT_PROPAGATION);
                }
    ...
    /** {@inheritDoc} */
    @Override
    public SpacecraftState propagate(final AbsoluteDate tStart, final AbsoluteDate tEnd) throws PropagationException {
        try {
    ...
    private SpacecraftState propagate(final AbsoluteDate tEnd, final boolean activateHandlers)
        ...
        try {
            ...
            for (int i = 0; i < this.addEquationsAndTolerances.size(); i++) {
                final String name = this.addEquationsAndTolerances.get(i).getEquations().getName();
                if (name.equals(AttitudeType.ATTITUDE.toString())) {
                    this.initialState = this.initialState.addAttitudeToAdditionalStates(AttitudeType.ATTITUDE);
                } else {
                    if (name.equals(AttitudeType.ATTITUDE_FORCES.toString())) {
            ...
            /*
             * Loop on every force models. If the force model implement the interface StepHandler
             * (for now, only ContinuousThrustManeuver does it), then it's added as a step handler to the integrator.
             */
            for (final ForceModel forceModel : this.forceModels) {
                if (forceModel instanceof PatriusStepHandler) {
                    ...
                    stepHandler.initialize(this.orbitType, this.angleType, this.attitudeProviderByDefault, null,
                            this.addStateInfos, activateHandlers, this.referenceDate, this.initialState.getFrame(),
                        this.initialState.getMu());
            ...
            if (this.modeMasterHandler != null) {
                ...
                if (this.attitudeProviderByDefault == null) {
                    this.modeMasterHandler.initialize(this.orbitType, this.angleType, this.attitudeProviderForces,
                        ...
                        this.initialState.getFrame(), this.initialState.getMu());
                } else {
            ...
            if (this.modeEphemerisHandler != null) {
                ...
                if (this.attitudeProviderByDefault == null) {
                    ...
                    this.modeEphemerisHandler.initialize(this.orbitType, this.angleType,
                        ...
                        this.referenceDate, this.initialState.getFrame(), this.initialState.getMu());
                    this.modeEphemerisHandler.setForward(duration >= 0);
                }
            }
            if (this.massModelHandler != null) {
            ...
            // If single attitude treatment expected
            if (this.attitudeProviderByDefault == null) {
                this.initialState = new SpacecraftState(this.stateVector, this.orbitType, this.angleType, date,
                        this.getMu(), this.initialState.getFrame(), this.addStateInfos, this.attitudeProviderForces,
                        this.attitudeProviderEvents);
            } else {
                this.initialState = new SpacecraftState(this.stateVector, this.orbitType, this.angleType, date,
                        this.getMu(), this.initialState.getFrame(), this.addStateInfos, this.attitudeProviderByDefault,
                        null);
            }
    ...
    private void manageStateFrame() throws PatriusException {
        ...
        if (this.getInitialState().getFrame() != this.propagationFrame) {
            ...
            // Build initial state
            this.initialState = new SpacecraftState(attProvForces, attProvEvents, propagationOrbit, this
                .getInitialState().getAdditionalStates());
        }
    ...
    private void checkStatesEquations() throws PatriusException {
        ...
        if (!additionalStates.isEmpty() && !this.addEquationsAndTolerances.isEmpty()) {
            ...
            // Either additional states from SpacecraftStare are empty or additional equations are empty (XOR)
            throw new PatriusException(PatriusMessages.WRONG_CORRESPONDENCE_STATES_EQUATIONS);
        }
    }
    ...
    private void buildAdditionalTolerances() throws PatriusException {
        if (this.integrator instanceof AdaptiveStepsizeIntegrator) {
            ...
            for (final AdditionalEquationsAndTolerances stateAndTol : this.addEquationsAndTolerances) {
                ...
                final int sLength = this.initialState.getAdditionalStates().get(stateAndTol.getEquations()
                    .getName()).length;
                ...
                if (absTol == null || relTol == null) {
                    ...
                    throw new PatriusException(PatriusMessages.ADDITIONAL_STATE_WRONG_TOLERANCES_SIZE);
                }
        ...
        // Additional step: store in AdditionalEquationsAndTolerances objects pos in first/second
        // order state vector
        int index = 3;
    ...
    private void expandToleranceArray() {
        if (this.integrator instanceof AdaptiveStepsizeIntegrator) {
            ...
            resizeArray((AdaptiveStepsizeIntegrator) this.integrator, n, this.addStatesAbsoluteTolerances,
                    this.addStatesRelativeTolerances);
        }
    }
    ...
    protected void setUpEventDetector(final EventDetector osf) {
        ...
        if (this.attitudeProviderByDefault == null) {
            handler = new AdaptedEventDetector(osf, this.addStateInfos, this.orbitType, this.angleType,
                    this.attitudeProviderForces, this.attitudeProviderEvents, this.referenceDate,
                this.initialState.getMu(), this.initialState.getFrame());
        } else {
            handler = new AdaptedEventDetector(osf, this.addStateInfos, this.orbitType, this.angleType,
                this.attitudeProviderByDefault, null, this.referenceDate, this.initialState.getMu(),
    ...
    public static double[][] tolerances(final double dP, final Orbit orbit, final OrbitType type) {
        ...
        if (type == OrbitType.CARTESIAN) {
            ...
            for (int i = 0; i < 6; ++i) {
                ...
                absTol[i] = MathLib.abs(row[0]) * dP + MathLib.abs(row[1]) * dP + MathLib.abs(row[2]) * dP
                        + MathLib.abs(row[3]) * dV + MathLib.abs(row[4]) * dV + MathLib.abs(row[5]) * dV;
            }
    ...
    private class DifferentialEquations implements FirstOrderDifferentialEquations, SecondOrderDifferentialEquations,
        ...
        public void computeDerivatives(final double t, final double[] y, final double[] yDot) {
            ...
            try {
                ...
                if (NumericalPropagator.this.attitudeProviderByDefault == null) {
                    NumericalPropagator.this.currentState = new SpacecraftState(y, NumericalPropagator.this.orbitType,
                            NumericalPropagator.this.angleType, currentDate,
        ...
        public void initDerivatives(final double[] yDot, final Orbit currentOrbit) throws PropagationException {
            ...
            // Store velocity
            this.velocity = currentOrbit.getPVCoordinates().getVelocity();
            for (int i = 0; i < 6; ++i) {
                final double[] jRow = this.jacobian[i];
                this.storedYDot[i] = jRow[0] * this.velocity.getX() + jRow[1] * this.velocity.getY() + jRow[2]
                        * this.velocity.getZ();
            }
        ...
        public void addAcceleration(final Vector3D gamma, final Frame frame) throws PatriusException {
            final Transform t = frame.getTransformTo(NumericalPropagator.this.currentState.getFrame(),
        ...
        @Override
        public void
                computeSecondDerivatives(final double t, final double[] y, final double[] yDot, final double[] yDDot) {
            // Second order derivatives for integrators such as Cowell

            try {
                // At this point: y contains position, yDot contains velocity (cartesian coordinates
                // only)

                // update space dynamics view
                final AbsoluteDate currentDate = NumericalPropagator.this.referenceDate.shiftedBy(t);
                // Build state
                final double[] x = ((CowellIntegrator) NumericalPropagator.this.integrator).getMapper().buildFullState(
                        y, yDot);
                if (NumericalPropagator.this.attitudeProviderByDefault == null) {
                    NumericalPropagator.this.currentState = new SpacecraftState(x, OrbitType.CARTESIAN,
                            NumericalPropagator.this.angleType, currentDate,
                            NumericalPropagator.this.currentState.getMu(),
                            NumericalPropagator.this.currentState.getFrame(), NumericalPropagator.this.addStateInfos,
                            NumericalPropagator.this.attitudeProviderForces,
