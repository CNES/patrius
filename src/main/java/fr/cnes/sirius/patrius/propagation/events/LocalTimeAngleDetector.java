...
public class LocalTimeAngleDetector extends AbstractDetector {
    ...
    public double g(final SpacecraftState s) throws PatriusException {
        ...
        final Vector3D sPV = s.getPVCoordinates().getPosition();

        // Compute local time angle
        Vector3D posTIRF = sPV;
        if (!s.getFrame().equals(FramesFactory.getTIRF())) {
            posTIRF = s.getFrame().getTransformTo(FramesFactory.getTIRF(), s.getDate()).transformVector(sPV);
        }
