...
public abstract class AbstractPartialDerivativesEquations implements AdditionalEquations {
    ...
    public AbstractPartialDerivativesEquations(final String nameIn) {
        this.name = nameIn;
        this.jacobiansProviders = new ArrayList<IJacobiansParameterizable>();
        this.dirty = true;
    ...
    public List<ParameterConfiguration> getSelectedParameters(){
        return new ArrayList<ParameterConfiguration>(this.selectedParameters);
    }
    ...
    public void selectParameters(final Parameter... parameters) {
        for (final Parameter param : parameters) {
            selectParamAndStep(param, Double.NaN);
        }
        this.dirty = true;
    }
    ...
    public List<Parameter> getAvailableParameters() {
        final List<Parameter> available = new ArrayList<Parameter>();
        available.addAll(getNewtonianAttraction().getParameters());
        for (final ForceModel model : getForceModels()) {
    ...
    public JacobiansMapper getMapper() throws PatriusException {
        ...
        return new JacobiansMapper(this.getName(), list, getOrbitType(), getPositionAngle(), getFrame());
    }
     ...
     * {@link #setInitialJacobians(SpacecraftState, int)} to initialize Jacobians with default values or
    ...
    public int getFirstOrderDimension() {
        return ORBIT_DIMENSION * (ORBIT_DIMENSION + paramDim);
    }

    /** {@inheritDoc} */
    @Override
    public int getSecondOrderDimension() {
        return 3 * (ORBIT_DIMENSION + paramDim);
    }

    /** {@inheritDoc} */
    @Override
    public void writeExternal(final ObjectOutput oo) throws IOException {
        oo.writeInt(paramDim);
    ...
    public void computeDerivatives(final SpacecraftState s,
        ...
        for (int k = 0; k < this.getParamDim(); ++k) {
            ...
            final ParameterConfiguration param = this.getSelectedParameters().get(k);
    ...
    public double[] computeSecondDerivatives(final SpacecraftState s) throws PatriusException {
        ...
        final double[][] dXdX0 = getMapper().getStateJacobian(s);
        final double[][] pDotDotMat = new double[3][6];
        for (int i = 0; i < dim; i++) {
            for (int j = 0; j < dim; j++) {
                for (int k = 0; k < dim; k++) {
                    pDotDotMat[i][j] += dAccdPos[i][k] * dXdX0[k][j];
                    pDotDotMat[i][j + 3] += dAccdPos[i][k] * dXdX0[k][j + 3] + dAccdVel[i][k] * dXdX0[k + 3][j + 3];
                }
            }
        }
        ...
        for (int l = 0; l < this.getParamDim(); ++l) {
            // Compute the acceleration gradient with respect to current parameter
            final ParameterConfiguration param = this.getSelectedParameters().get(l);
            final IJacobiansParameterizable provider = param.getProvider();
            Arrays.fill(this.dAccdParam, 0.0);
            provider.addDAccDParam(s, param.getParameter(), this.dAccdParam);
            ...
            final double[] dXdP = getMapper().getParametersJacobian(param.getParameter(), s);
            ...
            // Convert into 1D-vector
            for (int i = 0; i < dim; i++) {
                pDotDot[l + i * getParamDim() + ORBIT_DIMENSION * dim] = pDotDotMat2[i];
            }
        }
    ...
    private void computeDAccDState(final SpacecraftState s) throws PatriusException {
        ...
        if (this.isDirty()) {
            ...
            if (Double.isNaN(this.gethPos())) {
                setHPos(MathLib.sqrt(Precision.EPSILON) * s.getPVCoordinates().getPosition().getNorm());
            }

            // set up Jacobians providers
            this.getJacobiansProviders().clear();
            for (final ForceModel model : getForceModels()) {
                if (model instanceof IJacobiansParameterizable) {
                    ...
                    this.getJacobiansProviders().add(
                            new Jacobianizer(model, this.getSelectedParameters(), this.gethPos()));
                }
            }
            this.getJacobiansProviders().add(getNewtonianAttraction());

            // check all parameters are handled by at least one Jacobian provider
            for (final ParameterConfiguration param : this.getSelectedParameters()) {
            ...
            if (this.getSelectedParameters().size() != this.getParamDim()) {
                throw new PatriusException(PatriusMessages.INITIAL_MATRIX_AND_PARAMETERS_NUMBER_MISMATCH,
                    this.getParamDim(), this.getSelectedParameters().size());
            }
    ...
    public double[] buildAdditionalState(final double[] y,
            final double[] yDot) {
        final double[] res = new double[ORBIT_DIMENSION * (ORBIT_DIMENSION + getParamDim())];
    ...
    public double[] extractY(final double[] additionalState) {
        final double[] y = new double[3 * (ORBIT_DIMENSION + getParamDim())];
        // State transition matrix
    ...
    public double[] extractYDot(final double[] additionalState) {
        final double[] yDot = new double[3 * (ORBIT_DIMENSION + getParamDim())];
        // State transition matrix
        System.arraycopy(additionalState, 3 * ORBIT_DIMENSION, yDot, 0, 3 * ORBIT_DIMENSION);
        // Sensitivity matrix
        for (int i = 0; i < getParamDim(); i++) {
            System.arraycopy(additionalState, ORBIT_DIMENSION * ORBIT_DIMENSION + (i + getParamDim()) * 3, yDot,
    ...
    /**
     * Returns the propagator newtonian attraction.
     * @return the propagator newtonian attraction
     */
