...
public abstract class AbstractPartialDerivativesEquations implements AdditionalEquations {
    ...
    public List<Parameter> getAvailableParameters() {
        ...
        available.addAll(this.getNewtonianAttraction().getParameters());
    ...
    public SpacecraftState setInitialJacobians(final SpacecraftState s,
        ...
        if (mapper.getPropagationFrame() == null) {
            // Special temporary case: if the frame isn't initialized in the mapper (= in the propagator), use the state
            // frame instead
            mapper.setPropagationFrame(s.getFrame());
        }
    ...
    private void computeDAccDState(final SpacecraftState s) throws PatriusException {
        ...
        if (this.isDirty()) {
            ...
            for (final ForceModel model : this.getForceModels()) {
                if (model instanceof IJacobiansParameterizable && !(model instanceof DrozinerAttractionModel)) {
            ...
            this.getJacobiansProviders().add(this.getNewtonianAttraction());
     ...
     */
    protected abstract NewtonianAttractionModel getNewtonianAttraction();
