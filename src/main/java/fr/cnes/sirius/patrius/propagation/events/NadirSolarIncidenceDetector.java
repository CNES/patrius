...
public class NadirSolarIncidenceDetector extends AbstractDetector {
    ...
    /** The body shape. */
    private final BodyShape inBodyShape;
     ...
     * @param bodyShape the body shape
    ...
    public NadirSolarIncidenceDetector(final double incidence, final BodyShape bodyShape,
                                       final double maxCheck, final double threshold, final Action action,
                                       final boolean remove, final CelestialBody sun) throws PatriusException {
        this(INCREASING_DECREASING, incidence, bodyShape, maxCheck, threshold, action, action, remove, remove, sun);
    }

    /**
     * Constructor for the nadir point solar incidence detector with Sun choice.
     * 
     * @param slopeSelection the g-function slope selection (0, 1 or 2)
     * @param incidence the incidence to detect (set 0. to detect the terminator)
     * @param bodyShape the body shape
     * @param maxCheck maximum check (see {@link AbstractDetector})
     * @param threshold threshold (see {@link AbstractDetector})
     * @param actionAtIncreasing action performed at nadir point solar incidence detection at increasing g event
     * @param actionAtDecreasing action performed at nadir point solar incidence detection at decreasing g event
     * @param removeAtIncreasing true if detector should be removed at nadir point solar incidence detection at
     *        increasing g event
     * @param removeAtDecreasing true if detector should be removed at nadir point solar incidence detection at
     *        decreasing g event
     * @param sun Sun
     * @since 4.11.1
     * @throws PatriusException error when loading the ephemeris files
     */
    public NadirSolarIncidenceDetector(final int slopeSelection, final double incidence, final BodyShape bodyShape,
                                       ...
                                       final boolean removeAtDecreasing, final CelestialBody sun)
        throws PatriusException {
        super(slopeSelection, maxCheck, threshold, actionAtIncreasing, actionAtDecreasing, removeAtIncreasing,
                removeAtDecreasing);
        this.inBodyShape = bodyShape;
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        // spacecraft position
        final Vector3D satPos = state.getPVCoordinates(this.inBodyShape.getBodyFrame()).getPosition();

        // nadir point
        final GeodeticPoint satGeo = this.inBodyShape.transform(satPos, this.inBodyShape.getBodyFrame(),
                state.getDate());
        final GeodeticPoint nadirGeo = new GeodeticPoint(satGeo.getLatitude(),
            satGeo.getLongitude(), 0.0);
        final Vector3D nadirPos = this.inBodyShape.transform(nadirGeo);

        // zenith vector from this nadir point
        final Vector3D zenithVect = satPos.subtract(nadirPos);

        // vector : nadir point to sun
        final AbsoluteDate sunDate = getSignalEmissionDate(this.inSun, state.getOrbit(), state.getDate());
        final Vector3D sunPos = this.inSun.getPVCoordinates(sunDate, this.inBodyShape.getBodyFrame())
            .getPosition();
    ...
    public EventDetector copy() {
        try {
            final NadirSolarIncidenceDetector res =
                new NadirSolarIncidenceDetector(this.getSlopeSelection(), this.incidenceRef, this.inBodyShape,
                    this.getMaxCheckInterval(), this.getThreshold(), this.actionAtEntry, this.actionAtExit,
                    this.removeAtEntry, this.removeAtExit, null);
