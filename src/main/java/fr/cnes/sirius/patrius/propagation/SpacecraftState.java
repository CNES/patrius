 ...
 * VERSION:4.8:FA:FA-2998:15/11/2021:[PATRIUS] Discontinuite methode computeBearing de ProjectionEllipsoidUtils
 ...
 *
 * <p>
 * It contains an {@link Orbit} , plus two attitudes (for forces and events computation) and a map
 * of additional states at a current {@link AbsoluteDate}. Orbit and attitudes are guaranteed to be
 * consistent in terms of date and reference frame. The attitudes objects can be copied in the map
 * of additional states with the additional state name "ATTITUDE_FORCES", "ATTITUDE_EVENTS" or
 * "ATTITUDE" (in numerical propagation only). The stored Attitude and the additional state
 * representing the attitude are guaranteed to be equal.
 * </p>
 * <p>
 * The user can declare a SpacecraftState with an input {@link MassProvider}. The masses from each
 * part of the mass model are automatically added to the map of additional states with a name in the
 * form "MASS_<part name>". It is not possible to get back the MassProvider.
 * </p>
 * <p>
 * The state can be slightly shifted to close dates. This shift is based on a simple keplerian model
 * for orbit, a linear extrapolation for both attitudes taking the spin rate into account, no
 * additional states changes. It is <em>not</em> intended as a replacement for proper orbit and
 * attitude propagation but should be sufficient for either small time shifts or coarse accuracy.
 * </p>
 * <p>
...
public class SpacecraftState implements TimeStamped, TimeShiftable<SpacecraftState>, TimeInterpolable<SpacecraftState>,
     ...
     * <p>
     * Attitude (for forces and events computation) are set to null values. No mass informations are
     * added.
     * </p>
     *
     * @param orbitIn
     ...
     * <p>
     * Attitude for forces computation is set to the input attitude. Attitude for events computation
     * is set to null value. No mass informations are added.
     * </p>
     *
     * @param orbitIn
     *        the orbit
     ...
     * </p>
     * <p>
     * The mass states informations contained in the mass provider are added to the additional
     * states map with the name in the form "MASS_<part name>".
     * </p>
     *
     * @param orbitIn
     *        the orbit
     ...
     * <p>
     * The mass states informations contained in the mass provider are added to the additional
     * states list with the name in the form "MASS_<part name>".
     * </p>
     *
     * @param orbitIn
     *        the orbit
     ...
     * <p>
     * The mass states informations contained in the mass provider are added to the additional
     * states list with the name in the form "MASS_<part name>". If an additional state is already
     * defined with the same name as the mass provider state, the additional state is replaced with
     * the informations from the mass provider.
     * </p>
     *
     * @param orbitIn
     *        the orbit
     ...
     * <p>
     * Attitude for forces computation is set to the input attitude provider. Attitude provider for
     * events computation is set to null value. No mass informations are added.
     * </p>
     *
     * @param orbitIn
     *        the orbit
     * @param attitudeProviderIn
     *        the default attitude provider
     */
    public SpacecraftState(final AttitudeProvider attitudeProviderIn, final Orbit orbitIn) {
        this.orbit = orbitIn;
        this.attitude = null;
        this.attitudeEvents = null;
     ...
     */
    public SpacecraftState(final AttitudeProvider attProviderForces, final AttitudeProvider attProviderEvents,
            final Orbit orbitIn) {
        this.orbit = orbitIn;
        this.attitude = null;
        this.attitudeEvents = null;
     ...
     */
    public SpacecraftState(final AttitudeProvider attProviderForces, final AttitudeProvider attProviderEvents,
            final Orbit orbitIn, final Map<String, double[]> addStates) {
        this.orbit = orbitIn;
        this.attitude = null;
        this.attitudeEvents = null;
        this.attitudeProvider = attProviderForces;
     ...
     */
    public SpacecraftState(final AttitudeProvider attProviderForces, final AttitudeProvider attProviderEvents,
            final Orbit orbitIn, final MassProvider massProvider) {
        this.orbit = orbitIn;
        this.attitude = null;
        this.attitudeEvents = null;
        this.attitudeProvider = attProviderForces;
     ...
     */
    public SpacecraftState(final AttitudeProvider attProviderForces, final AttitudeProvider attProviderEvents,
            final Orbit orbitIn, final MassProvider massProvider, final Map<String, double[]> additionalStatesIn) {
        this.orbit = orbitIn;
        this.attitude = null;
        this.attitudeEvents = null;
        this.attitudeProvider = attProviderForces;
     ...
     * <p>
     * If the additional state informations list contains a state with the prefix "ATTITUDE_FORCES"
     * (or "ATTITUDE_EVENTS" or "ATTITUDE"), it means that the attitude for forces computation (or
     * events computation) was added to the additional states map. Then the corresponding Attitude
     * object is initialized with the additional state value. Otherwise, the {@link Attitude} is
     * initialized using the corresponding {@link AttitudeProvider}.
     * </p>
     *
     * @param y
     *        the state vector
    ...
    public SpacecraftState(final double[] y, final OrbitType orbitType, final PositionAngle angleType,
            final AbsoluteDate date, final double mu, final Frame frame,
            final Map<String, AdditionalStateInfo> addStatesInfo, final Attitude attForces, final Attitude attEvents) {
     ...
     * <p>
     * In the SpacecraftState constructors, this method should be called after adding the additional
     * states map given as constructor parameter. If the additional states map previously contained
     ...
     * <p>
     * The additional states representing the attitude are composed of a quaternion and a spin
     * vector. This method should be called after updating additionalStates list.
     * </p>
     *
     * @param date
     ...
     */
    private static void
            checkConsistency(final Orbit orbit, final Attitude attitudeForces, final Attitude attitudeEvents) {
        if (attitudeForces != null) {
            if (!orbit.getDate().equals(attitudeForces.getDate())) {
                throw PatriusException.createIllegalArgumentException(
                        PatriusMessages.ORBIT_AND_ATTITUDE_DATES_MISMATCH, orbit.getDate(), attitudeForces.getDate());
            }
            if (orbit.getFrame() != attitudeForces.getReferenceFrame()) {
                throw PatriusException.createIllegalArgumentException(PatriusMessages.FRAMES_MISMATCH, orbit.getFrame()
                        .getName(), attitudeForces.getReferenceFrame().getName());
            }
        }
        if (attitudeEvents != null) {
            if (!orbit.getDate().equals(attitudeEvents.getDate())) {
                throw PatriusException.createIllegalArgumentException(
                        PatriusMessages.ORBIT_AND_ATTITUDE_DATES_MISMATCH, orbit.getDate(), attitudeEvents.getDate());
            }
            if (orbit.getFrame() != attitudeEvents.getReferenceFrame()) {
                throw PatriusException.createIllegalArgumentException(PatriusMessages.FRAMES_MISMATCH, orbit.getFrame()
                        .getName(), attitudeEvents.getReferenceFrame().getName());
            }
        }
    }
     ...
     * @exception IllegalArgumentException
     *            if attitude object and attitude in the additional states list map are not equal
     *            if additional states map contains (ATTITUDE_FORCES or ATTITUDE_EVENTS) state and
     *            ATTITUDE state
     */
    ...
    private static void checkConsistency(final Attitude attitudeForces, final Attitude attitudeEvents,
        ...
        if (((addStates.containsKey(AttitudeType.ATTITUDE_FORCES.toString())) || (addStates
                .containsKey(AttitudeType.ATTITUDE_EVENTS.toString())))
        ...
        if (addStates.containsKey(AttitudeType.ATTITUDE.toString())) {
            // The attitude contained in the additional state map is compared with the attitude for
            // forces
            // computation.
            if (attitudeForces == null) {
     ...
     * <p>
     * If the additionalStates map already contained a mapping for the name entered, the old state
     * is replaced by the specified state. It is not possible to add an additional state with
     * "ATTITUDE_FORCES" or "ATTITUDE_EVENTS" or "ATTITTUDE" name : it is a reserved name.
     * </p>
     *
     * <p>
     ...
     *         if the name of the additional state to be added is "ATTITUDE_FORCES" or
     *         "ATTITUDE_EVENTS" or
     *         "ATTITUDE".
     ...
     * <p>
     * The additional state name could be "ATTITUDE_FORCES" if {@link AttitudeType#ATTITUDE_FORCES},
     * "ATTITUDE_EVENTS" if {@link AttitudeType#ATTITUDE_EVENTS} or "ATTITUDE" if
     * {@link AttitudeType#ATTITUDE}. If AttitudeType#ATTITUDE, the only one attitude available is
     * added to the additional states map.
     * </p>
     * <p>
     * If an additional state with the name "ATTITUDE_FORCES", "ATTITUDE_EVENTS" or "ATTITUDE" is
     * already in the additional state map, its value will be updated with the new one. The attitude
     * additional state contains the rotation and the spin.
     * </p>
     *
     * <p>
     * <b>{@link SpacecraftState} object state being immutable, a new {@link SpacecraftState} object
     ...
     * @return a new SpacecraftState with the added additional state corresponding to an
     *         SpacecraftState attitude.
     * @throws PatriusException
    ...
    public SpacecraftState addAttitudeToAdditionalStates(final AttitudeType attitudeType) throws PatriusException {
        ...
        switch (attitudeType) {
            ...
            case ATTITUDE_EVENTS:
                ...
                addStates.put(attitudeType.toString(), this.getAttitudeEvents().mapAttitudeToArray());
                break;
            case ATTITUDE:
                if ((addStates.containsKey(AttitudeType.ATTITUDE_FORCES.toString()))
                ...
                addStates.put(AttitudeType.ATTITUDE.toString(), this.getAttitude().mapAttitudeToArray());
                break;
            default:
                // Cannot happen
     ...
     * Add the values of mass parts from MassProvider to additional states map.
     * <p>
     * If the additional state map already contained an additional state from the input mass
     * provider, its value will be updated with the new one.
     *
     * <p>
     * <b>{@link SpacecraftState} object state being immutable, a new {@link SpacecraftState} object
     * is returned.</b>
    ...
    /**
     * <p>
     * Add attitude to the additional states map. It is not possible to add an attitude for events
     * computation if no attitude for forces computation is defined.
     * </p>
     * <p>
     ...
     * <p>
     * The state can be slightly shifted to close dates. This shift is based on a simple keplerian
     * model for orbit, a linear extrapolation for attitude taking the spin rate into account. It is
     * <em>not</em> intended as a replacement for proper orbit and attitude propagation but should
     * be sufficient for small time shifts or coarse accuracy.
     * </p>
     * <p>
     * WARNING : Additional states map is not changed (except for attitude objects if present) !
     * </p>
     * <p>
     * As a rough order of magnitude, the following table shows the interpolation errors obtained
     * between this simple shift method and an
     * {@link fr.cnes.sirius.patrius.propagation.analytical.EcksteinHechlerPropagator
     * Eckstein-Heschler
     * propagator} for an 800km altitude nearly circular polar Earth orbit with
     * {@link fr.cnes.sirius.patrius.attitudes.BodyCenterPointing body center pointing}. Beware that
     * these results may be different for other orbits.
     * </p>
    ...
    public SpacecraftState shiftedBy(final double dt) throws PatriusException {
        ...
        SpacecraftState temp = new SpacecraftState(this.orbit.shiftedBy(dt), attForces, attEvents,
     ...
     * <p>
     * The input sample SpacecraftState should have the same additional states size and name. If no
     * attitudes are defined, an error is handled.
     * </p>
     * <p>
     * Note that the state of the current instance may not be used in the interpolation process,
     * only its type and non interpolable fields are used (for example central attraction
     * coefficient or frame when interpolating orbits). The interpolable fields taken into account
     * are taken only from the states of the sample points. So if the state of the instance must be
     * used, the instance should be included in the sample points.
     * </p>
     *
     * @param date
     *        interpolation date
    ...
    public SpacecraftState interpolate(final AbsoluteDate date, final Collection<SpacecraftState> sample)
            throws PatriusException {
        final List<Orbit> orbits = new ArrayList<>(sample.size());
        final List<Attitude> attitudesForces = new ArrayList<>(sample.size());
        final List<Attitude> attitudesEvents = new ArrayList<>(sample.size());
        // One interpolator for each additional states
        final Map<String, HermiteInterpolator> interpolators = new ConcurrentHashMap<>();
        ...
        for (final SpacecraftState state : sample) {
            ...
            if (!equalsAddStates(addStatesRef, addStates)) {
                // check if the list of additional states contains always the same names, otherwise
                // throw an exception
        ...
        final Map<String, double[]> interpolatedAddStates = new ConcurrentHashMap<>();
     ...
     * <p>
     * The spacecraft frame origin is at the point defined by the orbit, and its orientation is
     * defined by the attitude.
     * </p>
     *
     * @return transform from inertial frame (where orbit/attitude are defined) to current
     *         spacecraft frame (attitude
     ...
     * @param lofType
     *        the LOF type
     * @return transform from inertial frame (where orbit/attitude are defined) to local orbital
     *         frame.
     * @throws PatriusException
     */
    ...
    public Transform toTransform(final Frame frame, final LOFType lofType) throws PatriusException {
        ...
        final Transform transform2 = lofType.transformFromInertial(date, sPV);
        return new Transform(date, transform1, transform2);
    }
     ...
     * <p>
     * The keplerian period is computed directly from semi major axis and central acceleration
     * constant.
     * </p>
     *
     ...
     * <p>
     * The keplerian mean motion is computed directly from semi major axis and central acceleration
     * constant.
     * </p>
     *
     ...
     * for mu. The result is provided as a reference to the internally cached {@link PVCoordinates},
     * so the caller is
     * responsible to copy it in a separate {@link PVCoordinates} if it needs to keep the value for
     * a while.
     *
    ...
    public Attitude getAttitude() throws PatriusException {
        ...
        if (this.attitude == null) {
            if (this.attitudeProvider != null) {
                synchronized (this) {
                    // The attitude is checked a second time in case it's been initialized by an
                    // other thread while the
     ...
     * orbital frame.
     * <p>
     * Warning: this method creates a new local orbital frame at each call. For multiple calls,
     * prefer using {@link #getAttitude(Frame)} with your own {@link LocalOrbitalFrame}.
     * </p>
     *
     * @param lofType
     *        the LOF type
    ...
    public Attitude getAttitude(final LOFType lofType) throws PatriusException {
        final LocalOrbitalFrame lof = new LocalOrbitalFrame(this.getAttitude().getReferenceFrame(), lofType,
                this.orbit, "LOF");
    ...
    public Attitude getAttitudeEvents() throws PatriusException {
        ...
        if (this.attitudeEvents == null) {
            synchronized (this) {
                // The attitude is checked a second time in case it's been initialized by an other
                // thread while the
                // current one was waiting
                if (this.attitudeEvents == null) {
    ...
    public Attitude getAttitudeEvents(final LOFType lofType) throws PatriusException {
        ...
        final LocalOrbitalFrame lofAtttitudeEvents = new LocalOrbitalFrame(
                this.getAttitudeEvents().getReferenceFrame(), lofType, this.orbit, "LOF attitude events");
    ...
    public double[] getAdditionalState(final String name) throws PatriusException {
        final double[] additionalState = this.additionalStates.get(name);
        if (additionalState == null) {
            throw new PatriusException(PatriusMessages.UNKNOWN_ADDITIONAL_STATE, name);
        }
        return additionalState.clone();
    }
    ...
    public Map<String, AdditionalStateInfo> getAdditionalStatesInfos() {
        ...
        final Map<String, AdditionalStateInfo> result = new TreeMap<>();
        for (final Map.Entry<String, double[]> entry : this.additionalStates.entrySet()) {
    ...
    /**
     * Compares additional states. This method checks additional state vector has the same keys (not
     * same values).
     *
     * @param addStates1
    ...
    public static boolean
            equalsAddStates(final Map<String, double[]> addStates1, final Map<String, double[]> addStates2) {
    ...
    public static SpacecraftState getSpacecraftStateLight(final AbsoluteDate date) {
         ...
         * Orbit constructor
         */
        return new SpacecraftState(new CartesianOrbit(new PVCoordinates(Vector3D.ZERO, Vector3D.ZERO, Vector3D.PLUS_I),
                null, date, 0.));
    }
}
