...
public class AzimuthElevationField implements IFieldOfView {
    ...
    public double getAngularDistance(final Vector3D point) {
        ...
        final double pointAzimuth = AzimuthElevationCalculator.getAzimuth(point, this.frameOrientation);
        final double pointElevation = AzimuthElevationCalculator.getElevation(point);
    ...
    public double getElevation(final double azimuth) {
        ...
        if (azimuth == this.azelmask[0][0]) {
            ...
            if (i == 0 && this.azelmask[0][0] != this.azelmask[1][0]) {

                final double azd = this.azelmask[0][0];
                final double azf = this.azelmask[1][0];
                final double eld = this.azelmask[0][1];
                final double elf = this.azelmask[1][1];

                res = linear(azimuth, azd, azf, eld, elf);
            } else {
                // Normal case
                final double azp = this.azelmask[i - 1][0];
                final double elp = this.azelmask[i - 1][1];
                final double azd = this.azelmask[i][0];
                final double azf = this.azelmask[i + 1][0];
                final double eld = this.azelmask[i][1];
                final double elf = this.azelmask[i + 1][1];
