...
public final class FieldAngularFace implements Serializable {
    ...
    public double computeMinAngle(final Vector3D direction) {
        ...
        if (projection.getNorm() <= UtilsPatrius.GEOMETRY_EPSILON) {
            // Specific case
            return MathLib.PI / 2.;
        }
        ...
        if (MathLib.abs(this.angle - angleToV1 - angleToV2) < UtilsPatrius.GEOMETRY_EPSILON) {
            ...
            // Specific rare case: distance to projection might be larger than distance to field defining vectors
            // In this case, keep shortest distance
            final double resv1 = Vector3D.angle(direction, this.vector1);
            final double resv2 = Vector3D.angle(direction, this.vector1);
            if (resv1 < result) {
                result = resv1;
            }
            if (resv2 < result) {
