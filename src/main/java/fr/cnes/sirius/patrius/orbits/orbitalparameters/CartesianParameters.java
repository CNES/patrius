...
public class CartesianParameters extends AbstractOrbitalParameters {
    ...
    public CircularParameters getCircularParameters(final double mu) {
        ...
        final double ex = a2OnR2 * (f * x2 + g * y2);
        final double ey = a2OnR2 * (f * y2 - g * x2);

        // compute latitude argument
        final double beta = 1 / (1 + MathLib.sqrt(MathLib.max(0.0, 1 - ex * ex - ey * ey)));
        final double alphaE = MathLib.atan2(y2 + ey + eSE * beta * ex, x2 + ex - eSE * beta * ey);

        // Return result
        return new CircularParameters(a, ex, ey, i, raan, alphaE, PositionAngle.ECCENTRIC, mu);
    }
    ...
    public ReentryParameters getReentryParameters(final double ae, final double f, final double mu) {
        ...
        try {
            ...
            final GeodeticPoint geodeticPoint = ellipsoid.transform(this.getPosition(), frame, null);
            final double altitude = geodeticPoint.getAltitude();
            final double latitude = geodeticPoint.getLatitude();
            final double longitude = geodeticPoint.getLongitude();
            final Vector3D east = geodeticPoint.getEast();
