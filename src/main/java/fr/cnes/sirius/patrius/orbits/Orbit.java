...
public abstract class Orbit
     ...
     * 
     * @param type
     *        type of the position angle in the state
     * @param gm
     *        attraction coefficient to use
     * @param pDot
     ...
     */
    protected abstract void orbitAddKeplerContribution(final PositionAngle type, final double gm, final double[] pDot);

    /**
     * Call the method {@link #orbitAddKeplerContribution(PositionAngle, double, double[])} implemented in inherited
     * classes of Orbit.
     * 
     * @param type
     *        type of the position angle in the state
    ...
    public void addKeplerContribution(final PositionAngle type, final double gm, final double[] pDot) {
        ensurePseudoInertialFrame(this.frame);
        this.orbitAddKeplerContribution(type, gm, pDot);
    }
    ...
    public RealMatrix getJacobian(final OrbitType numerator, final OrbitType denominator,
        ...
        try {
            return getJacobian(frame, frame, denominator, numerator, positionAngle, positionAngle);
    ...
    public RealMatrix getKeplerianTransitionMatrix(final double dt) {
        try {
            return getJacobian(dt, frame, frame, getType(), getType(), PositionAngle.MEAN, PositionAngle.MEAN);
        } catch (final PatriusException e) {
            // Cannot happen since no conversion is performed
            throw new PatriusRuntimeException(PatriusMessages.INTERNAL_ERROR, e);
        }
