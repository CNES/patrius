...
public class ReentryParameters extends AbstractOrbitalParameters {
     ...
     */
    public ReentryParameters(final double altitudeIn, final double latitudeIn, final double longitudeIn,
                             final double velocityIn, final double slopeIn, final double azimuthIn, final double aeIn,
                             final double fIn, final double mu) {
        super(mu);
    ...
    public CartesianParameters getCartesianParameters() {
        ...
        // Build ellipsoid
        // Ellipsoid frame unused
        final OneAxisEllipsoid ellipsoid = new OneAxisEllipsoid(this.ae, this.f, null);

        // Compute position
        final EllipsoidPoint point = new EllipsoidPoint(ellipsoid, ellipsoid.getLLHCoordinatesSystem(), this.latitude,
            this.longitude, this.altitude, "");
        final Vector3D pos = point.getPosition();

        // Compute local horizontal plane (located in position and normal = zenith)
        final Vector3D zenith = point.getNormal();
        ...
        // Compute local north
        final Vector3D northProj = new Vector3D(Vector3D.dotProduct(u, Vector3D.PLUS_K), u, Vector3D.dotProduct(v,
            Vector3D.PLUS_K), v);

        // Compute velocity (ROL = North, West, Zenith)
        // Azimuth is positive toward East
        final double[] sincosSlope = MathLib.sinAndCos(this.slope);
        ...
        final Vector3D velInlocalFrame = new Vector3D(this.velocity * cosSlope * cosAzimut, this.velocity * cosSlope
        ...
        if (northProj.getNorm() < EPS) {
            ...
            final Rotation rotation = new Rotation(Vector3D.PLUS_K, Vector3D.PLUS_I, zenith, northProj);
            vel = rotation.applyTo(velInlocalFrame);
        }

        // acceleration
        final double r2 = pos.getNormSq();
        final Vector3D acceleration = new Vector3D(-getMu() / (r2 * MathLib.sqrt(r2)), pos);

        // Return result
        return new CartesianParameters(pos, vel, acceleration, getMu());
    }
    ...
    public ApsisAltitudeParameters getApsisAltitudeParameters(final double req) {
        return getKeplerianParameters().getApsisAltitudeParameters(req);
    }
    ...
    public String toString() {
        return new StringBuffer().append("reentry parameters: ").append('{').append("altitude(m): ")
            .append(this.altitude).append("; latitude(deg): ").append(MathLib.toDegrees(this.latitude))
            .append("; longitude(deg): ").append(MathLib.toDegrees(this.longitude)).append("; velocity(m/s): ")
            .append(this.velocity).append("; slope(deg): ").append(MathLib.toDegrees(this.slope))
            .append("; azimuth(deg): ").append(MathLib.toDegrees(this.azimuth)).append("; ae(m): ").append(this.ae)
            .append("; f: ").append(this.f).append(";}").toString();
    }
    ...
    public boolean equals(final Object object) {
        ...
        if (object == this) {
            ...
            isEqual &= getMu() == other.getMu();

            // Reentry parameters
            isEqual &= getAltitude() == other.getAltitude();
            isEqual &= getLatitude() == other.getLatitude();
            isEqual &= getLongitude() == other.getLongitude();
            isEqual &= getVelocity() == other.getVelocity();
            isEqual &= getSlope() == other.getSlope();
            isEqual &= getAzimuth() == other.getAzimuth();
            isEqual &= getAe() == other.getAe();
            isEqual &= getF() == other.getF();
    ...
    public int hashCode() {
        ...
        result = effMult * result + MathUtils.hash(getMu());
        result = effMult * result + MathUtils.hash(getAltitude());
        result = effMult * result + MathUtils.hash(getLatitude());
        result = effMult * result + MathUtils.hash(getLongitude());
        result = effMult * result + MathUtils.hash(getVelocity());
        result = effMult * result + MathUtils.hash(getSlope());
        result = effMult * result + MathUtils.hash(getAzimuth());
        result = effMult * result + MathUtils.hash(getAe());
        result = effMult * result + MathUtils.hash(getF());
