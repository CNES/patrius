 ...
 * orbits.
 * <p>
 * The parameters used internally are the equinoctial elements (see {@link EquinoctialParameters} for more information.
 * </p>
 * <p>
...
public final class EquinoctialOrbit extends Orbit {
    ...
    /** Orbital parameters. */
    private final EquinoctialParameters parameters;
     ...
     */
    public EquinoctialOrbit(final IOrbitalParameters parametersIn, final Frame frame,
        final AbsoluteDate date) {
        super(frame, date, parametersIn.getMu());
        this.parameters = parametersIn.getEquinoctialParameters();
    }
     ...
     */
    public EquinoctialOrbit(final double a, final double ex, final double ey, final double hx,
        final double hy, final double l, final PositionAngle type, final Frame frame,
        final AbsoluteDate date, final double mu) {
        super(frame, date, mu);
        this.parameters = new EquinoctialParameters(a, ex, ey, hx, hy, l, type, mu);
    }
    ...
    public EquinoctialOrbit(final PVCoordinates pvCoordinates, final Frame frame,
        ...
        this.parameters = new CartesianParameters(pvCoordinates, mu).getEquinoctialParameters();
    }
    ...
    public EquinoctialOrbit(final Orbit op) {
        ...
        this.parameters = op.getParameters().getEquinoctialParameters();
    }
    ...
    /** {@inheritDoc} */
    @Override
    protected EquinoctialOrbit orbitShiftedBy(final double dt) {
        return new EquinoctialOrbit(this.parameters.getA(), this.parameters.getEquinoctialEx(),
    ...
    public EquinoctialOrbit interpolate(final AbsoluteDate date, final Collection<Orbit> sample) {
        ...
        for (final Orbit orbit : sample) {
            final EquinoctialOrbit equi = (EquinoctialOrbit) OrbitType.EQUINOCTIAL
                .convertType(orbit);
            ...
            interpolator.addSamplePoint(equi.getDate().durationFrom(date),
                new double[] { equi.getA(), equi.getEquinoctialEx(), equi.getEquinoctialEy(),
    ...
    protected double[][] computeJacobianTrueWrtCartesian() {
        ...
        // tan((lV - lE)/2) = [ex sin lE - ey cos lE] / [sqrt(1-ex^2-ey^2) + 1 - ex cos lE - ey sin
        // lE]
        // leads to
    ...
    protected double[][] computeJacobianCartesianWrtTrue() {
        ...
        // first column d./da
        jacobian[0][0] = position.getX() / a;
        jacobian[1][0] = position.getY() / a;
        jacobian[2][0] = position.getZ() / a;
        jacobian[3][0] = -velocity.getX() / a / 2.;
        jacobian[4][0] = -velocity.getY() / a / 2.;
        jacobian[5][0] = -velocity.getZ() / a / 2.;
        ...
        jacobian[0][5] = coefDHl
            * (exSinLMEyCosL * p1 + onePExCoslPEySinl * (-onePHx2MHy2 * sinL + twoHxHy
                * cosL));
        jacobian[1][5] = coefDHl
            * (exSinLMEyCosL * p2 + onePExCoslPEySinl * (oneMHx2PHy2 * cosL - twoHxHy
                * sinL));
	...
	public boolean equals(final Object object) {
        ...
        if (object == this) { 
            // first fast check
            isEqual = true; 
        } else if (object instanceof EquinoctialOrbit) {
            // cast object to compare parameters
            final EquinoctialOrbit other = (EquinoctialOrbit) object;
