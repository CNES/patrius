 ...
 * <ul>
 * <li>one OREKIT independent and compliant with initial FORTRAN routine entry values: {@link #getDensity(double,
 * double, double, double, double, double, double, double, double, double, double, double, double)}.</li>
...
public class JB2006 implements Atmosphere {
     ...
     * @param f10 10.7-cm Solar flux (1e<sup>-22</sup>*Watt/(m<sup>2</sup>*Hertz)). Tabular time 1.0
     *        day earlier
     * @param f10B 10.7-cm Solar Flux, averaged 81-day centered on the input time
     * @param ap Geomagnetic planetary 3-hour index A<sub>p</sub> for a tabular time 6.7 hours
     *        earlier
    ...
    public double getDensity(final double dateMJD, final double sunRA, final double sunDecli,
                             final double satLon, final double satLat, final double satAlt, final double f10,
                             final double f10B, final double ap, final double s10, final double s10B,
        ...
        // Equation (14)
        final double tc =
            379 + 3.353 * f10B + 0.358 * (f10 - f10B) + 2.094 * (s10 - s10B) + 0.343
                    * (xm10 - xm10B);
        ...
        if (scaledSatAlt <= 105.0) {
            ...
            aln[6] =
                AL10 * (alnh5 + 6.) + hsign
                        * (MathLib.log(tloc4 / tloc3) + fact1 * sum3 * AMW[6]);
        ...
        final double dlrsl =
            0.02 * (scaledSatAlt - 90.) * MathLib.exp(-0.045 * (scaledSatAlt - 90.)) * signum
    ...
    // CHECKSTYLE: stop MethodLength check
    // Reason: Orekit code kept as such
    private static double dTc(final double f10, final double solTimeHour, final double satLat,
                              final double satAlt) {
        // CHECKSTYLE: resume MethodLength check
        // CHECKSTYLE: resume CyclomaticComplexity check
    ...
    private static double xLocal(final double z, final double[] tc) {
        ...
        if (dz <= 0) {
            ...
            return tc[0] + tc[2] * MathLib.atan(tc[3] * dz * (1 + 4.5e-6 * MathLib.pow(dz, 2.5)));
        }
    }
     ...
     * {@link #getDensity(double, double, double, double, double, double, double, double, double,
     * double, double, double, double)} <b> must </b> must be called before calling this function.
     * 
     ...
     * @exception PatriusException if date is out of range of solar activity
     */
    @Override
    public double
     ...
     * @exception PatriusException if date is out of range of solar activity
     */
    private void computeTempDensity(final AbsoluteDate date, final Vector3D position,
        ...
        if (this.cachedDate.compareTo(date) != 0 || position.distance(this.cachedPosition) != 0
            ...
            if (date.compareTo(this.inputParams.getMaxDate()) > 0
                ...
                throw new PatriusException(PatriusMessages.NO_SOLAR_ACTIVITY_AT_DATE, date,
                    this.inputParams.getMinDate(), this.inputParams.getMaxDate());
            }

            // compute modified julian days date
            final double dateMJD =
                date.durationFrom(AbsoluteDate.MODIFIED_JULIAN_EPOCH) / Constants.JULIAN_DAY;
            ...
            final GeodeticPoint sunInBody =
                this.earth.transform(this.sun.getPVCoordinates(date, frame).getPosition(), frame, date);
            this.cachedDensity =
                this.getDensity(dateMJD, sunInBody.getLongitude(), sunInBody.getLatitude(),
                    inBody.getLongitude(), inBody.getLatitude(), inBody.getAltitude(),
                    this.inputParams.getF10(date), this.inputParams.getF10B(date),
                    this.inputParams.getAp(date), this.inputParams.getS10(date),
                    this.inputParams.getS10B(date), this.inputParams.getXM10(date),
