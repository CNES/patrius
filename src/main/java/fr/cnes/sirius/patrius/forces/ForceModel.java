...
public interface ForceModel extends IParameterizable {
    ...
    /**
     * This method enriches the description of this force model parameters with the class of the
     * force model.
     */
    default void enrichParameterDescriptors() {
        final List<Parameter> parameters = getParameters();
        final Class<? extends ForceModel> forceModelClass = this.getClass();
        for (final Parameter param : parameters) {
            if (param != null) {
                param.getDescriptor().addField(StandardFieldDescriptors.FORCE_MODEL,
                        forceModelClass);
            }
        }
    }
