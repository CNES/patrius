...
public abstract class SolarActivityDataReader implements DataLoader, SolarActivityDataProvider {
    ...
    private final TreeMap<AbsoluteDate, Double> f107s = new TreeMap<AbsoluteDate, Double>();

    /** Map for ap and kp values. */
    @SuppressWarnings("PMD.LooseCoupling")
    private final TreeMap<AbsoluteDate, Double[][]> aps = new TreeMap<AbsoluteDate, Double[][]>();
    ...
    public SortedMap<AbsoluteDate, Double> getInstantFluxValues(final AbsoluteDate date1,
        ...
        if (!d1Valid || !d2Valid) {
            ...
            this.cachedFluxs = this.f107s.subMap(date1, date2);
        }
