...
public class OceanTides extends AbstractTides {
     ...
     * Heights map
     */
    private final Map<Double, Double> heightsP = new TreeMap<>();
    /**
     * Heights map
     */
    private final Map<Double, Double> heightsS = new TreeMap<>();
    /**
     * Names map
     */
    private final Map<String, Double> names = new TreeMap<>();
    /**
     * Ocean tides data
     */
     ...
     */
    private final Set<Double> mainWaves = new TreeSet<>();
    /**
     * Doodson numbers of waves that have height data
     */
    private final Set<Double> fileWaves = new TreeSet<>();
    /**
     * Doodson numbers of secondary waves and their pivots
     */
    private final Map<Double, double[]> secWaves = new TreeMap<>();
    /**
     ...
     */
    private final Map<Double, Double> phases = new TreeMap<>();
    /**
    ...
    private void initializeFramework() {
        ...
        final Set<Double> doodsonFileNumbersSet = new TreeSet<>();
        ...
        for (final double d : this.fileWaves) {
            ...
            if (this.heightsP.containsKey(d)) {
                this.phases.put(d, getPhaseConvention(d, this.heightsP.get(d)));
            } else if (this.heightsS.containsKey(d)) {
                this.phases.put(d, getPhaseConvention(d, this.heightsS.get(d)));
    ...
    private TreeMap<Double, double[]> initNewAdmittanceTable() {
        ...
        final TreeMap<Double, double[]> newTable = new TreeMap<>();
     ...
     */
    private static double getPhaseConvention(final double onde, final double h) {
     ...
     */
    private static double deNormalize(final double normalized, final int n, final int mm) {
    ...
    private double[][] deNormalize(final double[][] tab) {
        ...
        for (int i = 0; i < this.l; i++) {
            for (int j = 0; j < MathLib.min(this.m, i + 1); j++) {
                newTab[i][j] = deNormalize(tab[i][j], i, j);
            }
        }
