...
public class SimpleExponentialAtmosphere implements Atmosphere {
     ...
     * 
     * @param shapeIn body shape model
     * @param rho0In Density at the altitude h0
     * @param h0In Altitude of reference (m)
     * @param hscaleIn Scale factor (m)
     */
    public SimpleExponentialAtmosphere(final BodyShape shapeIn, final double rho0In,
                                       final double h0In, final double hscaleIn) {
        this.shape = shapeIn;
        this.rho0 = rho0In;
        this.h0 = h0In;
    ...
    public double
        ...
        final GeodeticPoint gp = this.shape.transform(position, frame, date);
        return this.rho0 * MathLib.exp((this.h0 - gp.getAltitude()) / this.hscale);
    }
