...
public class ThirdBodyAttraction extends JacobiansParameterizable implements ForceModel, GradientModel,
    ...
    /** Additional terms (Cunningham model) */
     ...
     * Simple constructor.
     * Partial derivative computation is set to true by default.
     * Central terms and harmonics are considered.
     *
     * @param bodyIn
     *            the third body to consider (ex: {@link fr.cnes.sirius.patrius.bodies.CelestialBodyFactory#getSun()} or
     *            {@link fr.cnes.sirius.patrius.bodies.CelestialBodyFactory#getMoon()})
     ...
     * @param c
     *            Cosine un-normalized coefficients of the potential field of the third body
     * @param s
     *            Sine un-normalized coefficients of the potential field of the third body
     * @throws PatriusException thrown if body oriented frame could not be retrieved
     */
    public ThirdBodyAttraction(final CelestialBody bodyIn,
            final double eqRad, final double[][] c, final double[][] s) throws PatriusException {
        this(bodyIn, eqRad, c, s, true);
    }
     ...
     */
    public ThirdBodyAttraction(final CelestialBody bodyIn,
            final double eqRad, final double[][] c, final double[][] s, final boolean computePD)
        ...
        this.body = bodyIn;
        this.computePartialDerivativesWrtPosition = computePD;
        this.cunninghamAttractionModel = new CunninghamAttractionModel(
                bodyIn.getBodyOrientedFrame(), eqRad, bodyIn.getGM(), c, s);
        this.k = 1.;
    }
    ...
    public Vector3D computeAcceleration(final SpacecraftState s) throws PatriusException {
        // Central term
        Vector3D gamma = this.computeAcceleration(s.getPVCoordinates(), s.getFrame(), s.getDate());
        ...
        if (this.cunninghamAttractionModel != null) {
            ...
            // Direct effect
            final Vector3D gamma1 = this.cunninghamAttractionModel.computeAcceleration(s);
            // Effect on frame center (= position of central body) that needs to be subtracted from the acceleration
            final Vector3D gamma2 = this.cunninghamAttractionModel.computeAcceleration(frameCenter);
    ...
    public final void addDAccDState(final SpacecraftState s, final double[][] dAccdPos,
        ...
        if (this.computeGradientPosition()) {
            ...
            final double[][] dAdP = this.computeDAccDPos(s.getPVCoordinates(), s.getFrame(), s.getDate());
            ...
            if (this.cunninghamAttractionModel != null) {
                this.cunninghamAttractionModel.addDAccDState(s, dAccdPos, dAccdVel);
            }
        }
    }
     ...
     *         </p>
     */
    private double[][] computeDAccDPos(final PVCoordinates pv, final Frame frame,
                                       final AbsoluteDate date) throws PatriusException {
    ...
    public void checkData(final AbsoluteDate start, final AbsoluteDate end) throws PatriusException {
        // Harmonics if defined
        if (this.cunninghamAttractionModel != null) {
            this.cunninghamAttractionModel.checkData(start, end);
        }
    }
