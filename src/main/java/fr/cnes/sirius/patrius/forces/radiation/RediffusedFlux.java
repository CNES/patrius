...
public class RediffusedFlux {
    ...
    public RediffusedFlux(final int nCorona, final int nMeridian, final Frame bodyFrame,
        ...
        this.checkInput(nCorona, nMeridian);
        ...
        if (!this.ir) {
            ...
            if (a >= b) {
                ...
                for (int i = 0; i < this.flux.length; i++) {
                    this.flux[i] = new ElementaryFlux(Vector3D.ZERO, 0., 0.);
                }
            }
        }
        if (flagComputing) {
     ...
     * @param nMeridian number of meridians
     */
    private void checkInput(final int nCorona, final int nMeridian) {
        // Checking if the number of coronas or meridians is positive or the
        // the number of coronas or meridians are (0,0)
     ...
     * @since 1.2
     */
    private void computingRediffusedFlux(final int nSurfaces, final int nCorona,
            final int nMeridian,
            final IEmissivityModel model, final Frame bodyFrame, final AbsoluteDate dDate, final PVCoordinates satPV,
        ...
        final GeodeticPoint geoNadir = new GeodeticPoint(geoSat.getLatitude(),
            geoSat.getLongitude(), 0.);
        // Geodesic coordinates of nadir point
        final double latitudeNadir = geoNadir.getLatitude();
        final double longitudeNadir = geoNadir.getLongitude();
