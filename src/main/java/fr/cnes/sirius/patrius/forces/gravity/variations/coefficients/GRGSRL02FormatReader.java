...
public class GRGSRL02FormatReader extends VariablePotentialCoefficientsReader {
    ...
    /** Static coef buffer */
    private final Map<DegOrdKey, double[]> staticBuf = new ConcurrentHashMap<>();
    /** DOT coef buffer */
    private final Map<DegOrdKey, double[]> dotBuf = new ConcurrentHashMap<>();
    /** C1A coef buffer */
    private final Map<DegOrdKey, double[]> c1aBuf = new ConcurrentHashMap<>();
    /** S1A coef buffer */
    private final Map<DegOrdKey, double[]> s1aBuf = new ConcurrentHashMap<>();
    /** C2A coef buffer */
    private final Map<DegOrdKey, double[]> c2aBuf = new ConcurrentHashMap<>();
    /** S2A coef buffer */
    private final Map<DegOrdKey, double[]> s2aBuf = new ConcurrentHashMap<>();
    /** DegOrdKey list */
    private final Set<DegOrdKey> entries = new HashSet<>();
