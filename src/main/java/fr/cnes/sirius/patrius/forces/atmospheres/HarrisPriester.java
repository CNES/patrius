...
public class HarrisPriester implements Atmosphere {
     ...
     */
    public HarrisPriester(final PVCoordinatesProvider sunIn, final BodyShape earthIn) {
        this.sun = sunIn;
        this.earth = earthIn;
     ...
     *        the cosine exponent
     */
    public HarrisPriester(final PVCoordinatesProvider sunIn, final BodyShape earthIn, final double nIn) {
        this.sun = sunIn;
        this.earth = earthIn;
        this.tabAltRho = ALT_RHO;
        this.setN(nIn);
    ...
    public HarrisPriester(final PVCoordinatesProvider sunIn, final BodyShape earthIn,
        ...
        this.earth = earthIn;
        this.setN(4);
        this.setTabDensity(tabAltRhoIn);
    }
     ...
     */
    public double getDensity(final double sunRAsc, final double sunDecl, final Vector3D satPos,
                             final double satAlt) throws PatriusException {

        // Check for height boundaries
        if (satAlt < this.getMinAlt()) {
            throw new PatriusException(PatriusMessages.ALTITUDE_BELOW_ALLOWED_THRESHOLD, satAlt, this.getMinAlt());
        }
        if (satAlt > this.getMaxAlt()) {
            return 0.;
        }
    ...
    public double getDensity(final AbsoluteDate date, final Vector3D position,
        ...
        final GeodeticPoint satInBody = this.earth.transform(position, frame, date);
        final double satAlt = satInBody.getAltitude();
        final Vector3D posInBody = this.earth.transform(satInBody);
