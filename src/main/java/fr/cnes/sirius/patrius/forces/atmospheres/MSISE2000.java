...
public class MSISE2000 implements ExtendedAtmosphere {
     ...
     * @param earthBody earth body
     * @param sunBody the sun
     */
    public MSISE2000(final MSISE2000InputParameters data, final BodyShape earthBody,
    ...
    private void computeTempPresDensity(final AbsoluteDate date, final Vector3D position,
        ...
        if (this.cachedDate.compareTo(date) != 0 || !position.equals(this.cachedPosition)
            ...
            final double altitude = point.getAltitude();

            // compute local solar time
            final double resTLoc = this.computeTLoc(date, cmodPosition);
            ...
            this.cachedOutputData = new AtmosphereData(density, output.getT(1), output.getT(0),
                output.getD(0), output.getD(1), output.getD(2), output.getD(3), output.getD(4),
                output.getD(6), output.getD(OUTPUT7), output.getD(OUTPUT8));
     ...
     * 
     * @param date date
     * @param position position in Mean of Date Frame
     * @return local solar time
     * @throws PatriusException it UTC-TAI fails
     */
    private double computeTLoc(final AbsoluteDate date, final Vector3D position)
        throws PatriusException {
