...
public class EmpiricalForce extends JacobiansParameterizable implements ForceModel, GradientModel {
     ...
     */
    private final Frame coeffFrame;

    /** The local orbital frame type of definition of vectors A, B and C. */
    private final LOFType coeffFrameLOFType;
    ...
    /** Key to check if cache values should be recomputed */
    private SpacecraftState spacecraftStateCacheKey;

    /** cosSin cache value */
    private double[] cosSinCache;

    /** Rotation from the acceleration frame (LOF for example) to the spacecraft state frame */
    private Rotation accRotationCache;
    ...
    public EmpiricalForce(final int harmonicFactor, final Vector3D vectorS,
        ...
        this(harmonicFactor, vectorS,
                new ConstantFunction(ax), new ConstantFunction(ay), new ConstantFunction(az),
                new ConstantFunction(bx), new ConstantFunction(by), new ConstantFunction(bz),
                new ConstantFunction(cx), new ConstantFunction(cy), new ConstantFunction(cz),
                null, coeffsFrameLOFType);
    }
    ...
    public EmpiricalForce(final int harmonicFactor, final Vector3D vectorS,
            ...
            final LOFType coeffsFrameLOFType) {
        this(harmonicFactor, vectorS, ax, ay, az, bx, by, bz, cx, cy, cz, null, coeffsFrameLOFType);
    }
    ...
    public EmpiricalForce(final int harmonicFactor, final Vector3D vectorS,
            ...
            final IParamDiffFunction cx, final IParamDiffFunction cy, final IParamDiffFunction cz,
            final Frame coeffsFrame) {
        this(harmonicFactor, vectorS, ax, ay, az, bx, by, bz, cx, cy, cz, coeffsFrame, null);
    }

    /**
     * Private constructor to set all the attributes
     * 
     * @param harmonicFactor
     *        the harmonic factor n.
     * @param vectorS
     ...
     * @param coeffsFrame
     *        the frame of vectors A, B and C.
     * @param coeffFrameLOFType
     *        Local orbital frame type of definition of vectors A, B and C.
     */
    private EmpiricalForce(final int harmonicFactor, final Vector3D vectorS,
            final IParamDiffFunction ax, final IParamDiffFunction ay, final IParamDiffFunction az,
            final IParamDiffFunction bx, final IParamDiffFunction by, final IParamDiffFunction bz,
            final IParamDiffFunction cx, final IParamDiffFunction cy, final IParamDiffFunction cz,
            final Frame coeffsFrame, final LOFType coeffFrameLOFType) {
        super();
        // Parameters handling
        this.coeffAx = ax;
        this.coeffAy = ay;
        ...
        if (vectorS.isZero()) {
            ...
            this.s = vectorS.normalize();
        }

        this.coeffFrame = coeffsFrame;
        this.coeffFrameLOFType = coeffFrameLOFType;
    }
    ...
    public Vector3D computeAcceleration(final SpacecraftState state) throws PatriusException {
        ...
        if (!state.equals(this.spacecraftStateCacheKey)) {
            ...
            this.cosSinCache = this.computeCosSin(state.getPVCoordinates(), this.s);

            // computes the rotation from the local frame to the inertial frame:
            this.accRotationCache = computeRotation(state);

            // Update the cache key
            this.spacecraftStateCacheKey = state;
        }

        final double cosnwt = this.cosSinCache[0];
        final double sinnwt = this.cosSinCache[1];
        ...
        return this.accRotationCache.applyTo(accLocal);
    }

    /**
     * Internal method to compute the rotation from the acceleration frame (LOF for example) to the spacecraft state
     * frame.
     * 
     * @param state
     *        Rotation to the state frame
     * @return the computed rotation
     * @throws PatriusException
     *         if no attitude is defined in the state or if some frame specific error occurs
     */
    private Rotation computeRotation(final SpacecraftState state) throws PatriusException {
        final Rotation rot;
        // computes the rotation from the local frame to the inertial frame:
        if (this.coeffFrame == null) {
            if (this.coeffFrameLOFType == null) {
                ...
                final LocalOrbitalFrame lof = new LocalOrbitalFrame(currentOrbit.getFrame(), this.coeffFrameLOFType,
                    currentOrbit, "lof");
                rot = state.getFrame().getTransformTo(lof, state.getDate()).getRotation();
            }
        } else {
            // the A, B, C definition frame is not a local orbital frame:
            rot = state.getFrame().getTransformTo(this.coeffFrame, state.getDate()).getRotation();
        }
        return rot;
    }
     ...
     * 
     * @param pv
     *        the pv coordinates. It must be in an inertial frame.
     * @param vectorS
     *        the vector S.
    ...
    public double[] computeCosSin(final PVCoordinates pv, final Vector3D vectorS) {
        ...
        final Vector3D posNormalized = pos.normalize();
        // momentum (p x v) norm
        ...
        // initialize cos(nwt)
        out[0] = Vector3D.dotProduct(posNormalized, u);
        // initialize sin(nwt)
        out[1] = Vector3D.dotProduct(posNormalized, v);
    ...
    public void addDAccDParam(final SpacecraftState state, final Parameter param,
        ...
        final double[][] t = this.accRotationCache.getMatrix();

        // Computes the contribution of the partial derivatives of acceleration
        // with respect to final parameter on the x-component
        this.addDACcDparamOnX(param, state, t, this.cosSinCache, dAccdParam);

        // Computes the contribution of the partial derivatives of acceleration
        // with respect to final parameter on the y-component
        this.addDAccDparamOnY(param, state, t, this.cosSinCache, dAccdParam);

        // Computes the contribution of the partial derivatives of acceleration
        // with respect to final parameter on the z-component
        this.addDAccDparamOnZ(param, state, t, this.cosSinCache, dAccdParam);
