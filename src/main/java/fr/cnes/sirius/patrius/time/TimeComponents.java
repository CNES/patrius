...
public class TimeComponents implements Serializable, Comparable<TimeComponents> {
     ...
     * @return second number from 0.0 to Constants.JULIAN_DAY
     */
    public BigDecimal getPreciseSecondsInDay() {
        return new BigDecimal(this.fracSeconds).add(new BigDecimal(this.second))
            .add(new BigDecimal(SEC_IN_MIN).multiply(new BigDecimal(this.minute)))
            .add(new BigDecimal(SEC_IN_HOUR).multiply(new BigDecimal(this.hour)));
    }
     ...
     * Get a string representation of the time.
     * 
     * @param precision digit number of the seconds fractional part
     * @return string representation of the time
     */
    public String toString(final int precision) {
        return this.toString(precision, false);
    }
     ...
     */
    public String toString(final int precision, final boolean isTimeNearLeapSecond) {
        // precision check
        if (precision < 0) {
