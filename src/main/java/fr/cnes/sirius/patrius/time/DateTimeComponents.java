...
public class DateTimeComponents implements Serializable, Comparable<DateTimeComponents> {
    ...
    protected String toString(final int precision, final boolean isTimeNearLeapSecond) {
        ...
        if (isTimeNearLeapSecond) {
            ...
            limit = new BigDecimal(Constants.JULIAN_DAY).add(BigDecimal.ONE).subtract(
                new BigDecimal(FIVE).multiply(new BigDecimal(MathLib
                .pow(TEN, -(precision + 1)))));
        } else {
            // compute the day's upper boundary (23:59:59.9...5) with respect to the precision:
            limit = new BigDecimal(Constants.JULIAN_DAY).subtract(new BigDecimal(FIVE).multiply(new BigDecimal(MathLib
                .pow(TEN, -(precision + 1)))));
        }
        DateComponents shiftedDate = this.date;
        if (this.time.getPreciseSecondsInDay().compareTo(limit) >= 0) {
