 ...
 * <ul>Data are automatically loaded based on provided {@link UTCTAILoader}.</ul>
 * <ul>User can provide its own implementation of these loaders and provide them using 
 * {@link #addUTCTAILoader(UTCTAILoader)}.</ul>
 ...
 * <ul>List of loaders can be cleared and reordered using {@link #clearUTCTAILoaders()}.</ul>
 * </p>
 * <p>
...
public final class TimeScalesFactory implements Serializable {
    ...
    private static volatile UTCScale utc = null;
    ...
    public static UTCScale getUTC() throws PatriusException {
        ...
        if (utc == null) {
            synchronized (TimeScalesFactory.class) {
                if (utc == null) {
                    SortedMap<DateComponents, Integer> entries = new TreeMap<>();
