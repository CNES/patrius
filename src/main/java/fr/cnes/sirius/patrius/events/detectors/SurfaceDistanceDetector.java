...
public class SurfaceDistanceDetector extends DistanceDetector {
    ...
    public enum BodyDistanceType {
        ...
        CLOSEST {
            /** {@inheritDoc} */
            @Override
            public double getDistance(final BodyPoint point) {
                return point.getLLHCoordinates(LLHCoordinatesSystem.BODYCENTRIC_NORMAL).getHeight();
            }
        },
        ...
        RADIAL {
            ...
            @Override
            public double getDistance(final BodyPoint point) {
                return point.getLLHCoordinates(LLHCoordinatesSystem.BODYCENTRIC_RADIAL).getHeight();
            }
        };
         ...
         */
        public abstract double getDistance(BodyPoint point);
    }
     ...
     * {@inheritDoc}
     * 
     * @see fr.cnes.sirius.patrius.events.detectors.DistanceDetector#g
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        // Compute the distance to the point according to the body distance type
        final double closestDistance = this.bodyDistanceType.getDistance(satPosBodyPoint);

        // Compute the result as the difference between the closest distance and the threshold distance
        return closestDistance - this.distance;
    }
