...
public class EarthZoneDetector extends AbstractDetector {
    ...
    public EarthZoneDetector(final BodyShape centralBody, final List<double[][]> zonesPoints,
        ...
        this.fields = new ArrayList<IFieldOfView>();
        for (final double[][] zonePoints : zonesPoints) {
            ...
            this.checkVectors(directions.clone());
     ...
     */
    private void checkVectors(final Vector3D[] directions) {
        ...
        if (length > 3) {
            for (int i = -1; i < length - 3; i++) {
                ...
                for (int j = i + 2; j < length - 2; j++) {
                    this.crossTest(arcRef1, arcRef2, directions[j], directions[j + 1], i1, i + 1, j,
                        j + 1);
                }
            }
        }
     ...
     */
    private void crossTest(final Vector3D arcRef1, final Vector3D arcRef2, final Vector3D arc1,
