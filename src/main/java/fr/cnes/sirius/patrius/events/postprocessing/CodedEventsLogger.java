...
public final class CodedEventsLogger {
    ...
    public CodedEventsLogger() {
        this.loggedEventsSet = new TreeSet<>();
    ...
    public Map<CodingEventDetector, CodedEventsList> buildCodedEventListMap() {
        ...
        final Map<CodingEventDetector, CodedEventsList> map = new ConcurrentHashMap<>();
    ...
    public Map<CodingEventDetector, PhenomenaList>
        ...
        // processed.
        final Set<CodingEventDetector> validDetectors = new HashSet<>();
        ...
        final Map<CodingEventDetector, PhenomenaList> phenomMap = new ConcurrentHashMap<>();
        ...
        for (final CodingEventDetector event : validDetectors) {
            ...
            final PhenomenaList phenomList =
                buildPhenomenaListFromEvents(duringState, valBegin, valEnd, event, ceList);
     ...
     * 
     */
    private static PhenomenaList buildPhenomenaListFromEvents(final SpacecraftState state,
                                                              final AbsoluteDate dateStart,
        ...
        final List<CodedEvent> list = selectStandardEvent(ceList);
        final int listSize = list.size();
        for (int i = 0; i < listSize; i++) {
            final CodedEvent curEvent = list.get(i);
            final Phenomenon phenomenon =
                buildPhenomenonFromEvents(dateStart, dateEnd, event, formerEvent, curEvent, listSize, i);
            if (phenomenon != null) {
     ...
     * 
     */
    private static List<CodedEvent> selectStandardEvent(final List<CodedEvent> list) {
        final List<CodedEvent> newList = new ArrayList<>();
     ...
     * @since 1.1
     */
    private static Phenomenon buildPhenomenonFromEvents(final AbsoluteDate dateStart, final AbsoluteDate dateEnd,
    ...
    private final class CodingWrapper implements EventDetector {
        ...
        public boolean filterEvent(final SpacecraftState state,
                final boolean increasing,
                final boolean forward) throws PatriusException {
            return this.codingDetector.filterEvent(state, increasing, forward);
        }
    }
    ...
    public static final class LoggedCodedEvent implements TimeStamped, Comparable<LoggedCodedEvent> {
         ...
         * 
         * @param o
         *        LoggedCodedEvent to be compared with the instance
         * @return a negative integer or a positive integer as the internal date is less than or greater than the
         *         internal date of the specified LoggedCodedEvent. When the dates are the same, returns 1.
         */
        @Override
        public int compareTo(final LoggedCodedEvent o) {
            if (this.getDate().compareTo(o.getDate()) == 0) {
                // when the dates are the same, returns 1
                return 1;
            }
        ...
        public boolean equals(final Object object) {
            ...
            if (object == this) {
                ...
                // Same object type
                final LoggedCodedEvent other = (LoggedCodedEvent) object;
