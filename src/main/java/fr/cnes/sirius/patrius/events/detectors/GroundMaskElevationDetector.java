 ...
 * </p>
 * <p>
 * Azimuth is defined according to {@link TopocentricFrame#getAzimuth(Vector3D, Frame, AbsoluteDate) getAzimuth}.<br>
 * Elevation is defined according to {@link TopocentricFrame#getElevation(Vector3D, Frame, AbsoluteDate) getElevation}.
 * </p>
 * <p>
...
public class GroundMaskElevationDetector extends AbstractSignalPropagationDetector {
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        final AbsoluteDate recDate = getSignalReceptionDate(state);
        final Vector3D pos = state.getPVCoordinates().getPosition();
        final double azimuth = this.topo.getAzimuth(pos, state.getFrame(), recDate);
        return this.topo.getElevation(pos, state.getFrame(), recDate) - this.getElevation(azimuth);
    }
