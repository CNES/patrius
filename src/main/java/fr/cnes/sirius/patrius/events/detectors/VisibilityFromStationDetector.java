 ...
 * continue} propagation at raising and to {@link fr.cnes.sirius.patrius.events.EventDetector.Action#STOP stop}
...
public class VisibilityFromStationDetector extends AbstractStationToSatDetector {
     ...
     * 
     * @param topoFrame
     *        the station's topocentric frame
     * @param azimElevMask
     *        the azimuth - elevation mask (rad)
     * @param correctionModel
     *        the elevation correction model for the station (set null to ignore tropospheric correction)
     * @param maxCheck
     *        maximal checking interval (s)
     * @param threshold
     *        convergence threshold (s)
     * @param linkTypeIn
     *        the type of link (it can be uplink or downlink). Downlink means satellite to station.
     */
    public VisibilityFromStationDetector(final TopocentricFrame topoFrame, final double[][] azimElevMask,
                                         ...
                                         final double threshold, final LinkType linkTypeIn) {
     ...
     *        target of this sensor model is the above station!
     * @param correctionModel
     *        the elevation correction model for the station (set null to ignore tropospheric correction)
     * @param withMasking
     *        set true to compute maskings of spacecraft's sensor by occulting bodies: in this case, there is no
     *        visibility if one of the masking bodies of the sensor is between the spacecraft and the station
     * @param maxCheck
     *        maximal checking interval (s)
     * @param threshold
     *        convergence threshold (s)
    ...
    public VisibilityFromStationDetector(final TopocentricFrame topoFrame, final double[][] azimElevMask,
        ...
        super(new GeometricStationAntenna(topoFrame, azimElevMask), correctionModel, sensorModel, withMasking,
                maxCheck, threshold, raising, setting, removeRaising, removeSetting, RAISING_SETTING, linkTypeIn);
    }
     ...
     * 
     * @param stationModel
     *        the station sensor geometric model
     * @param correctionModel
     *        the elevation correction model for the station (set null to ignore tropospheric correction)
     * @param maxCheck
     *        maximal checking interval (s)
    ...
    public VisibilityFromStationDetector(final GeometricStationAntenna stationModel, final SensorModel sensorModel,
        ...
        super(stationModel, correctionModel, sensorModel, withMasking, maxCheck, threshold, raising, setting,
                removeRaising, removeSetting, RAISING_SETTING, linkTypeIn);
    }
     ...
     *        convergence threshold (s)
     * @param action
     *        action performed when propagation at raising/setting depending on slope selection
     * @param remove
     *        true if detector should be removed at raising/setting depending on slope selection
     * @param linkTypeIn
     *        the type of link (it can be uplink or downlink). Downlink means satellite to station.
     */
    public VisibilityFromStationDetector(final GeometricStationAntenna stationModel,
        ...
        super(stationModel, correctionModel, null, false, maxCheck, threshold, action, action, remove, remove,
                slopeSelection, linkTypeIn);
     ...
     * 
     * @return the station body point
     */
    public BodyPoint getStationBodyPoint() {
        return this.getStation().getTopoFrame().getFrameOrigin();
    }
    ...
    public BodyShape getBodyShape() {
        return getStationBodyPoint().getBodyShape();
    }
}
