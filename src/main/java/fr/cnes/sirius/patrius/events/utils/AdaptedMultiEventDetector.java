...
/**
 * <p>
 * This interface is copied from {@link fr.cnes.sirius.patrius.events.utils.AdaptedEventDetector} and adapted to
 * multi propagation.
 * </p>
 * <p>
...
public class AdaptedMultiEventDetector implements EventHandler {
    ...
    public Action eventOccurred(final double t, final double[] y, final boolean increasing,
        ...
        try {
            ...
            final EventDetector.Action whatNext = this.multiDetector.eventOccurred(states, increasing,
                forward);
            // Convert action into Commons-Math action
            return AdaptedMonoEventDetector.convertIntoCMAction(whatNext);
        } catch (final PatriusException oe) {
            throw new PatriusExceptionWrapper(oe);
        }
    }
    ...
    public boolean filterEvent(final double t,
        ...
        try {
            // Get current state
            final AbsoluteDate currentDate = this.refDate.shiftedBy(t);
            final Map<String, SpacecraftState> states = this.stateInfo.mapArrayToStates(y, currentDate, this.orbitT,
                    this.angleT, this.attitudeProvidersForces, this.attitudeProvidersEvents, this.mus,
                    this.integrationFrames);
            return multiDetector.filterEvent(states, increasing, forward);
        } catch (final PatriusException oe) {
            // Exception
            throw new PatriusExceptionWrapper(oe);
        }
