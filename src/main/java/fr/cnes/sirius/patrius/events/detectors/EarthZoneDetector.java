 ...
 * {@link fr.cnes.sirius.patrius.events.EventDetector.Action#STOP stop} propagation when the spacecraft
 * enters the zone and to {@link fr.cnes.sirius.patrius.events.EventDetector.Action#CONTINUE continue} when
...
public class EarthZoneDetector extends AbstractDetector {
    ...
    /** The central body. */
    private final EllipsoidBodyShape centralBodyShape;
     ...
     *        longitude}
     */
    public EarthZoneDetector(final EllipsoidBodyShape centralBody, final List<double[][]> zonesPoints) {
     ...
     * The default implementation behaviour is to
     * {@link fr.cnes.sirius.patrius.events.EventDetector.Action#STOP stop} propagation when the spacecraft
     * point enters the zone and to {@link fr.cnes.sirius.patrius.events.EventDetector.Action#CONTINUE
     ...
     * @param exit
     *        when the spacecraft point leave the zone.
     */
    public EarthZoneDetector(final EllipsoidBodyShape centralBody, final List<double[][]> zonesPoints,
    ...
    public EarthZoneDetector(final EllipsoidBodyShape centralBody, final List<double[][]> zonesPoints,
        ...
        for (final double[][] zonePoints : zonesPoints) {
            ...
            for (int i = 0; i < zonePoints.length; i++) {
                directions[i] = new EllipsoidPoint(this.centralBodyShape,
                    this.centralBodyShape.getLLHCoordinatesSystem(), zonePoints[i][0], zonePoints[i][1], 0., "")
                    .getPosition();
            }
     ...
     * @since 3.1
     */
    public EarthZoneDetector(final List<IFieldOfView> fieldsIn, final EllipsoidBodyShape centralBody,
                             final double maxCheck, final double threshold, final Action entry, final Action exit,
                             final boolean removeEntry, final boolean removeExit) {
     ...
     *        The zones : for each of the list, the directions from the earth center define the zone
     * @param bodyFrame
     *        the frame attached to the earth
     */
     ...
     *        convergence threshold (see {@link AbstractDetector})
     */
    public EarthZoneDetector(final List<Vector3D[]> directionsList, final Frame bodyFrame, final double maxCheck,
     ...
     */
    public EarthZoneDetector(final List<Vector3D[]> directionsList, final Frame bodyFrame, final double maxCheck,
                             final double threshold, final Action entry, final Action exit, final boolean removeEntry,
    ...
    /**
     * Validity check of the vectors defining the zone. Two consecutive points must not be too close (e-10 on the norm
     * of their difference)
     * and the arc between two consecutive points must not cross the arc between two consecutive others.
     *
     * @param directions
    ...
    private static void checkVectors(final Vector3D[] directions) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (diff.getNorm() < UtilsPatrius.GEOMETRY_EPSILON) {
                throw PatriusException.createIllegalArgumentException(PatriusMessages.PDB_CLOSE_CONSECUTIVE_DIRECTIONS);
            }
        }
    ...
    /**
     * Tests that a given arc defined by two vectors does not cross a reference arc defined by other vectors. All
     * vectors shall be normalized.
     *
     * @param arcRef1
     *        first vector of the reference arc
     * @param arcRef2
     *        second vector of the reference arc
     * @param arc1
     *        first vector of the arc to test
     * @param arc2
     *        second vector of the arc to test
     * @param i1
     *        number of the arcRef1 point
     * @param i2
     *        number of the arcRef2 point
     * @param j1
     *        number of the arc1 point
     * @param j2
     *        number of the arc2 point
     */
    private static void crossTest(final Vector3D arcRef1, final Vector3D arcRef2, final Vector3D arc1,
        ...
        if (c1 && c2 && c3) {
            throw PatriusException.createIllegalArgumentException(PatriusMessages.PDB_CROSSING_ARCS, i1, i2, j1, j2);
        }
    }
    ...
    public Action eventOccurred(final SpacecraftState s, final boolean increasing, final boolean forward)
        ...
        return increasing ? getActionAtEntry() : getActionAtExit();
    }

    /**
     * Compute the value of the switching function. For each zone : if the zone has been defined with latitudes ans
     * longitudes, this function measures the angular distance from the spacecraft's nadir point to the border of the
     * conic field of space that defines the inside of the zone. If the zone has been defined with direction vectors, it
     * measures the angular distance from the spacecraft itself to the border of that zone. This value is positive when
    ...
    public double g(final SpacecraftState s) throws PatriusException {
        ...
        if (this.centralBodyShape == null) {
            ...
            final EllipsoidPoint nadirPoint = this.centralBodyShape.buildPoint(position, s.getFrame(), s.getDate(),
                BodyPointName.DEFAULT);
            point = nadirPoint.getClosestPointOnShape().getPosition();
        }
    ...
    public EventDetector copy() {
        return new EarthZoneDetector(this.fields, this.centralBodyShape, getMaxCheckInterval(), getThreshold(),
            getActionAtEntry(),
