...
public class VisibilityFromStationDetector extends AbstractDetector {
     ...
     * @param linkTypeIn the type of link (it can be uplink or downlink)
     */
    public VisibilityFromStationDetector(final TopocentricFrame topoFrame, final double[][] azimElevMask,
                                         final SensorModel sensorModel, final AngularCorrection correctionModel,
     ...
     * @param linkTypeIn the type of link (it can be uplink or downlink)
     */
    public VisibilityFromStationDetector(final GeometricStationAntenna stationModel, final SensorModel sensorModel,
                                         final AngularCorrection correctionModel, final boolean withMasking,
                                         final double maxCheck, final double threshold,
                                         final Action raising, final Action setting, final boolean removeRaising,
                                         final boolean removeSetting, final LinkType linkTypeIn) {
        super(maxCheck, threshold, raising, setting, removeRaising, removeSetting);
    ...
    public VisibilityFromStationDetector(final GeometricStationAntenna stationModel,
        ...
        // set the type of link
        this.linkType = linkTypeIn;
        this.inSensorSpacecraft = null;
        this.inSpacecraft = null;
        this.maskingCheck = false;
     ...
     * </p>
     * <p>
     * More calculations are performed in case masking check is asked for: the distance between spacecraft's sensor
     * line of sight and the occulting object is computed. This distance is positive if no masking occurs, negative
     * otherwise.
     * </p>
     * 
     * @param state state
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        if (this.maskingCheck) {
            ...
            this.inSpacecraft.updateMainPartFrame(state);
            ...
            AbsoluteDate stationDate = state.getDate();
            ...
            if (linkType.equals(LinkType.DOWNLINK)) {
                ...
                stationDate = getSignalReceptionDate(this.station, state.getOrbit(), state.getDate());
            } else {
                // It is uplink
                // Emitter is the station, satellite is the receiver, so compute the emission date
                stationDate = getSignalEmissionDate(this.station, state.getOrbit(), state.getDate());
            }

            // Check potential body masking (occulting body between station and spacecraft)
            final double bodyMaskingDistance = this.inSensorSpacecraft.celestialBodiesMaskingDistance(state.getDate(),
                    stationDate, getPropagationDelayType(), linkType);

            // Check potential masking by another spacecraft
            final double spacecraftsMaskingDistance = this.inSensorSpacecraft.spacecraftsMaskingDistance(
                    state.getDate(), stationDate, getPropagationDelayType(), linkType);
        ...
        // Angular distance: positive if the vector is within station's field of view
        final double angleVisi = this.getStation().getFOV().getAngularDistance(correctedVector);
        
        // Both must be positive for visibility to happen
        return MathLib.min(angleVisi, minMaskingDist);
    }
    ...
    private Vector3D getCorrectedVector(final PVCoordinatesProvider pvProv, final AbsoluteDate date,
        ...
        if (linkTypeIn.equals(LinkType.DOWNLINK)) {
            ...
            relevantDate = getSignalReceptionDate(this.station, pvProv, date);
        } else {
            // It is uplink
            // Emitter is the station, satellite is the receiver, so compute the emission date
            relevantDate = getSignalEmissionDate(this.station, pvProv, date);
        }
    ...
    public EventDetector copy() {
        ...
        if (this.getSlopeSelection() == RAISING) {
            ...
            detector = new VisibilityFromStationDetector(this.getStation(), this.getSensor(), this.getCorrection(),
                this.maskingCheck, this.getMaxCheckInterval(), this.getThreshold(), this.getActionAtEntry(),
                this.getActionAtExit(), this.isRemoveAtEntry(), this.isRemoveAtExit(), this.linkType);
        }
