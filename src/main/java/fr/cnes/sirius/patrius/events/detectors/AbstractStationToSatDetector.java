...
public abstract class AbstractStationToSatDetector extends AbstractSignalPropagationDetector {
    ...
    public double g(final SpacecraftState state) throws PatriusException {
        ...
        return MathLib.min(angularDistanceInStationSensor, minMaskingDistance);
    }
    ...
    protected Vector3D getCorrectedVector(final SpacecraftState s) throws PatriusException {
        ...
        if (this.correction != null) {
            ...
            final double trueElevation = this.station.getTopoFrame().getElevation(notCorrectedVector,
                this.station.getTopoFrame(), date);

            // computation of the correction to be applied
            final double elevationCorrection;
            if (trueElevation > this.correction.getMinimalToleratedApparentElevation()) {
                elevationCorrection = this.correction.computeElevationCorrectionFromGeometricElevation(date,
                    trueElevation);
            } else {
                // The angular correction is not able to compute the correction (set to 0 since it is likely that there
                // is no visibility anyway)
