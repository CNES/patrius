...
public class Mercator extends AbstractProjection {
    ...
    /**
     * Azimuth (for rotated Mercator projection), can be 0.
     ...
     */
    public Mercator(final GeodeticPoint pivotIn, final EllipsoidBodyShape shape,
        final double azimuthIn, final boolean centeredIn, final boolean seriesIn) {
        super(pivotIn, shape);

        // longitude of pivot point
        this.lon0 = MathUtils.normalizeAngle(pivotIn.getLongitude(), 0.);
        ...
        final double lat0 = pivotIn.getLatitude();

        // Re-scale CelestialBody radius
        final double pivotScaleFactor = this.getScaleFactor(lat0);
        this.pivotCosLat = MathLib.cos(lat0);
        this.scaledRadius = this.getReference().getEquatorialRadius() * pivotScaleFactor;
        ...
        final EllipsoidBodyShape ell = this.getReference();
    ...
    public Mercator(final double centralMeridian, final EllipsoidBodyShape ref) {
        this(new GeodeticPoint(0., centralMeridian, 0.), ref, 0., false, true);
    }

    /** {@inheritDoc}. */
    @Override
    public final boolean canMap(final GeodeticPoint coordinates) {
        return MathLib.abs(coordinates.getLatitude()) <= MAX_LATITUDE;
    }
    ...
    public Vector2D applyTo(final double lat, final double lon) throws PatriusException {
        ...
        final double latMerc = ProjectionEllipsoidUtils.computeMercatorLatitude(latUsed, this.getReference());

        // compute longitude (relative to the pivot)
        if (this.lon0 == 0 && this.getPivotPoint() != null) {
            this.lon0 = MathUtils.normalizeAngle(this.getPivotPoint().getLongitude(), 0.);
        }
    ...
    public GeodeticPoint applyInverseTo(final double x, final double y, final double alt) throws PatriusException {
        final GeodeticPoint coord = this.applyInverseTo(x, y);
        return new GeodeticPoint(coord.getLatitude(), coord.getLongitude(), alt);
    }

    /**
     * {@inheritDoc} This is the inverse transformation process to convert two dimensional map coordinates into
     * geodetics one. The
     * iterative method used is a Newton-Raphson iterative process. We're searching the solution of F(phi) = 0. At each
     * iterations, we compute a new value of phi : phi' = phi - F(phi)/F'(phi) where phi is the trial value, F(phi) =
     * y(phi) - phi, with y(phi) the direct Mercator transformation, and F'(phi) the derive function of y(phi). The stop
     * condition is satisfied when F(phi) < epsilon (here equal at 1E-14). This method returns geodetic coordinates. The
     * iterative process stop if the iteration's number is bigger than 100.
     */
    @Override
    public GeodeticPoint applyInverseTo(final double x, final double y) throws PatriusException {
        ...
        final double phi = this.computePhi(phi0, yMer);

        // return geodetic point
        return new GeodeticPoint(phi, this.lon0 + xMer, 0.);
    }
    ...
    private double computePhi(final double phi0, final double yMer) {
        ...
        // eccentricity
        final double e = ProjectionEllipsoidUtils.getEccentricity(this.getReference());
        ...
        if (this.series) {
            ...
            // Real phi
            phi = xphi + coefaB * sin2xphi + coefbB * sin4xphi
                + coefcB * 2 * (4 * cosxphi3 - 3 * cosxphi) * (3 * sinxphi - 4 * sinxphi3)
                + coefdB * 2 * cos4xphi * sin4xphi;
        } else {
            ...
            while (MathLib.abs(fPhi) > THRESHOLD) {
                ...
                // If the latitude is out of allowed value, it's set to the maximum
                // latitude value.
                if (phi > MAX_LATITUDE) {
                    phi = MAX_LATITUDE;
    ...
    public final double getDistortionFactor(final double lat) {
        final GeodeticPoint pivot = this.getPivotPoint();
        final double pivotRadius = ProjectionEllipsoidUtils.computeRadiusEastWest(
            pivot.getLatitude(), this.getReference());
        final double latRadius = ProjectionEllipsoidUtils
            .computeRadiusEastWest(lat, this.getReference());
