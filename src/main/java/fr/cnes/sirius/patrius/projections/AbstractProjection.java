...
public abstract class AbstractProjection implements IProjection {
    ...
    /**
     * Pivot point used for projections define by its longitude and latitude.<br>
     * It also contains reference ellipsoid.
     */
    private final EllipsoidPoint pivot;
     ...
     */
    public AbstractProjection(final EllipsoidPoint pivotIn) {
        this.pivot = pivotIn;
    }
    ...
    public final EllipsoidBodyShape getReference() {
        return this.pivot.getBodyShape();
    }

    /**
     * Getter for the projection pivot point. It also contains reference ellipsoid.
     * 
     * @return the projection pivot point
     */
    public final EllipsoidPoint getPivotPoint() {
        return this.pivot;
    }

    /**
     * Getter for the scale factor at a specific latitude.<br>
     ...
     *        Projected point 2
     * @param maxLenght
     *        Distance consistent with the projection scale.<br>
     ...
     */
    public static final List<Vector2D> discretize(final Vector2D p1, final Vector2D p2, final double maxLenght,
                                                  final boolean p2Included) {
     ...
     *        set the parameter to a value <=0, no discretization will be done.<br>
     * @return list of projected points
     * @throws PatriusException
     *         if complex polygon discretization fails
     */
    public final List<Vector2D> discretizeAndApplyTo(final List<EllipsoidPoint> list, final EnumLineProperty ltype,
     ...
     */
    public final List<Vector2D> discretizeCircleAndApplyTo(final List<EllipsoidPoint> list, final double maxLength)
        throws PatriusException {

        // Intermediate variables :
        EllipsoidPoint from = list.get(0);
        EllipsoidPoint to = null;
        // Results :
        final List<EllipsoidPoint> result;
        ...
        if (maxLength > 0) {
            ...
            for (int i = 1; i < list.size(); i++) {
                ...
                final List<EllipsoidPoint> xyIntermediateP = ProjectionEllipsoidUtils.discretizeGreatCircle(from, to,
                    maxLength);
     ...
     *        represent a <b>distance</b> expressed in meters. This will be the maximal distance between two points of
     *        the projected polygon. If you set the parameter to a value <=0 , no dicretization will be done.<br>
     * @return list of Vector2D as projected polygon (lat/lon).
     * @throws PatriusException
     *         if complex polygon discretization fails
     */
    public final List<Vector2D> discretizeRhumbAndApplyTo(final List<EllipsoidPoint> list, final double maxLength)
        throws PatriusException {
     ...
     * @throws PatriusException
     *         if projection of start or end point could not be computed
     */
    public final List<Vector2D>
        applyToAndDiscretize(final EllipsoidPoint from, final EllipsoidPoint to, final double maxLength,
                             final boolean lastIncluded) throws PatriusException {
        ...
        // Compute intermediate points between 'from' and 'to' :
        return discretize(p1, p2, maxLength, lastIncluded);
    }
     ...
     *         if one projection could not be computed
     */
    public final List<Vector2D> applyTo(final List<EllipsoidPoint> list) throws PatriusException {
        final List<Vector2D> result = new ArrayList<>();
     ...
     */
    public final List<EllipsoidPoint> applyInverseTo(final List<Vector2D> list) throws PatriusException {
        final List<EllipsoidPoint> result = new ArrayList<>();
     ...
     */
    public final List<EllipsoidPoint> applyInverseTo(final double[] x, final double[] y) throws PatriusException {
        // Check data length :
     ...
     *         if one projection could not be computed
     */
    private List<Vector2D> applyToAndDiscretize(final List<EllipsoidPoint> list, final double maxLength)
        throws PatriusException {
        // Result
        ...
        for (int i = 1; i < list.size(); i++) {
            ...
            if (maxLength > 0.) {
                ...
                final List<Vector2D> resSegment = discretize(from, to, maxLength, false);
     ...
     *         if one projection could not be computed
     */
    private List<Vector2D> discretizeRhumbFirstAndApplyTo(final List<EllipsoidPoint> list, final double maxLength)
        throws PatriusException {
        // Result
        final List<Vector2D> res = new ArrayList<>();
        // discretization first, then projection
