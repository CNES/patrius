...
public final class ProjectionEllipsoidUtils {
     ...
     * {@link ProjectionEllipsoidUtils#computePointAlongOrthodrome(GeodeticPoint, double, double)}.
     */
    ...
    /**
     * Compute series following Snyder method => "Map Projection, A working manual" written by John
     * P. Snyder, from page 17.
     * 
     ...
     * 
     * @param gv1 geodetic point 1
     * @param gv2 geodetic point 2
     * @param shape Body shape.
     * @return the azimuth angle as a double. Convention used : azimuth is angle from the north
     *         direction to the current direction in CLOCKWISE sense.
     * @throws PatriusException if points are too close from each other
     */
    public static final double computeBearing(final GeodeticPoint gv1, final GeodeticPoint gv2,
            final EllipsoidBodyShape shape) throws PatriusException {
        ...
        final double lon2 = MathUtils.normalizeAngle(gv2Long, lon1);
        ...
        if (dY > Precision.EPSILON) {
            ...
            final double mercatorLat2 = computeMercatorLatitude(gv2Lat, shape);
            final double mercatorLat1 = computeMercatorLatitude(gv1Lat, shape);
     ...
     */
    public static double computeSphericalAzimuth(final GeodeticPoint p1, final GeodeticPoint p2) {
     ...
     * See following link for demonstration of formula :
     * 
    ...
    /**
     * Compute radius of curvature section East/West (also called M or Re). Distance to the
     * Intersection of normal to ellipsoid at the given latitude with pole axis.
     * 
     * @param geodeticLat geodetic latitude must between : - PI/2 and PI/2
     * @param shape Body shape.
     ...
     * line in Mercator). This method comes from libSpace.
     * 
     * @param p1 Point 1
     * @param p2 Point 2
     * @param shape Body shape.
     * @return distance in meters along loxodromic
     * @throws PatriusException if latitude of one point is not between -/+ 89.999 deg
     */
    public static double computeLoxodromicDistance(final GeodeticPoint p1, final GeodeticPoint p2,
            final EllipsoidBodyShape shape) throws PatriusException {
        ...
        if (MathLib.abs(deltaLon) < Precision.EPSILON) {
            return MathLib.abs(computeMeridionalDistance(geodeticLat2, shape)
                    - computeMeridionalDistance(geodeticLat1, shape));
        }

        if (MathLib.abs(geodeticLat1) > Mercator.MAX_LATITUDE
                || MathLib.abs(geodeticLat2) > Mercator.MAX_LATITUDE) {
        ...
        if (dY > LOXODROMIC_PRECISION) {
            // Compute mercator Y for each latitude
            final double y1 = computeMercatorLatitude(geodeticLat1, shape);
            final double y2 = computeMercatorLatitude(geodeticLat2, shape);

            final double k = MathLib.atan2(deltaLon, y2 - y1);
            final double dL = computeMeridionalDistance(geodeticLat2, shape)
                    - computeMeridionalDistance(geodeticLat1, shape);
     ...
     */
    public static final double computeMeridionalDistance(final double geodeticLat,
        ...
        return shape.getEquatorialRadius()
                * (coefAP * geodeticLat +
                evalSinEven(geodeticLat, coefBP, coefCP, coefDP, coefEP));
    }

    /**
     * Compute the geodetic latitude, from a distance from the equator.
     * 
     * @param distance distance from the equator
     * @param shape Body shape.
     * @return geodetic latitude
     */
    public static final double computeInverseMeridionalDistance(final double distance,
            final EllipsoidBodyShape shape) {
        final double[] seriesComp = getSeries(shape);
        final double coefAP = seriesComp[0];
        return computeInverseRectifyingLatitude(
            MathLib.divide(distance, shape.getEquatorialRadius() * coefAP), shape);
    }

    /**
     * Compute the point coordinates from an origin point, an azimuth and a distance along the rhumb
     * line (Loxodrome). The computation is done on the geoid associated to the geodetic point. The
     * method is issue from Snyder, Map Projection, A working manuel, p46/47. The method comes from
     * libSpace The precision is
     * <ul>
     ...
     * @param distance distance along the rhumb line
     * @param azimuth Convention used : azimuth is angle from the north direction to the current
     *        direction in CLOCKWISE sense.
     * @param shape Body shape.
     * @return the resulting geodetic point
     * @throws PatriusException if computed latitudes are out of range
     */
    public static final GeodeticPoint computePointAlongLoxodrome(final GeodeticPoint p1,
            final double distance,
            final double azimuth, final EllipsoidBodyShape shape)
        ...
        if (MathLib.abs(sinAz) < AZIMUTH_PRECISION) {
            // along a meridian
            lat2 = computeInverseMeridionalDistance(computeMeridionalDistance(lat1, shape) + cosAz
                    * distance, shape);
            ...
            lat2 = computeInverseMeridionalDistance(dLat * shape.getEquatorialRadius()
                    + computeMeridionalDistance(lat1, shape), shape);
     ...
     * 
     * @param rectifyingLat rectifying latitude
     * @param shape Body shape.
     * @return geodetic latitude (rad)
     */
    public static double computeInverseRectifyingLatitude(final double rectifyingLat,
            final EllipsoidBodyShape shape) {
        final double[] seriesComp = getSeries(shape);
    ...
    /**
     * Compute the orthodromic distance between two points. This code is issue from
     * <b>Vincenty's</b> works. If points are equal it returns 0, else it calls the the
     * <b>Vincenty</b> method, with more elementary parameters.
     * 
     ...
     */
    public static final double computeOrthodromicDistance(final GeodeticPoint p1, final GeodeticPoint p2,
            final EllipsoidBodyShape shape) {
     ...
     * @return Orthodromic distance on ellipsoid.
     */
    public static final double computeOrthodromicDistance(final double lat1, final double lon1,
    ...
    /**
     * Compute a geodetic point along orthodrome, from a point <code>p<sub>1</sub></code>, at a
     * distance <code>d</code>, in a direction defined from an azimuth. This is the direct geodetic
     * problem. This code is issue from <b>Vincenty</b>'s works.
     * 
     * @see <a href="http://www.ngs.noaa.gov/PUBS_LIB/inverse.pdf"> http://www.ngs.noaa.gov/PUBS_LIB/inverse.pdf</a>
     * @param p1 Origin point.
     * @param distance Distance to compute resulting point.
     * @param azimuthDirection azimuth direction, convention used : azimuth is angle from the north
     *        direction to the current direction in CLOCKWISE sense.
     * @param shape Body shape.
     * @return the geodetic point
     */
    public static final GeodeticPoint computePointAlongOrthodrome(final GeodeticPoint p1,
            final double distance, final double azimuthDirection,
            final EllipsoidBodyShape shape) {
        // Ellipsoid variables
        final double a = shape.getEquatorialRadius();
        final double f = shape.getFlattening();
    ...
    /**
     * Discretize a rhumb line into <i>N</i> segments, between two points. The result returned
     * contains the first and the last point.
     * 
     * @param from first geodetic point
     * @param to ending geodetic point
     * @param maxLength This distance is used when a discretization occurs : i.e : when the line
     *        property is not coherent with the projection ( {@link EnumLineProperty}). This
     *        parameter represent a <b>distance</b> expressed in meters. This will be the maximal
     *        distance between two points of the projected polygon. If you set the parameter to a
     *        value <=0 no discretization will be done.<br>
     * @param shape Body shape.
     * @return a list of geodetic points
     * @throws PatriusException if points are too close from each other
     */
    public static final List<GeodeticPoint> discretizeRhumbLine(final GeodeticPoint from,
            final GeodeticPoint to, final double maxLength,
            final EllipsoidBodyShape shape)
            throws PatriusException {

        if (maxLength <= 0.) {
            // no discretization. the two points are returned
            final ArrayList<GeodeticPoint> result = new ArrayList<>(1);
            result.add(from);
            return result;
        }

        // Compute full length :
        final double fullLength = computeLoxodromicDistance(from, to, shape);
        ...
        // Get the azimuth :
        final double az = computeBearing(from, to, shape);
        // Resulting point list :
        final List<GeodeticPoint> list = new ArrayList<>();
        // Add first point :
        list.add(from);
        ...
        if (nbPoint == 1) {
            ...
            for (int i = 1; i < nbPoint; i++) {
                ...
                pointToAdd = computePointAlongLoxodrome(tmpPoint, delta, az, shape);
                // Add it :
                list.add(pointToAdd);
                tmpPoint = pointToAdd;
    ...
    /**
     * Discretize a great circle into <i>N</i> segments, between two points. The result returned
     * contains the first and the last point.
     * 
     * @param from first geodetic point
     * @param to ending geodetic point
     ...
     * @return a list of geodetic points
     */
    public static final List<GeodeticPoint> discretizeGreatCircle(final GeodeticPoint from,
            final GeodeticPoint to, final double maxLength,
            final EllipsoidBodyShape shape) {

        if (maxLength <= 0.) {
            // no discretization. the two points are returned
            final ArrayList<GeodeticPoint> result = new ArrayList<>(2);
            result.add(from);
            result.add(to);
            return result;
        }
        ...
        // Compute full length :
        final double fullLength = computeOrthodromicDistance(from, to, shape);
        // compute number of intermediate points
        final double maximalDistance = MathLib.abs(maxLength);
        final int nbPoint = (int) MathLib.divide(fullLength, maximalDistance);
        ...
        if (nbPoint == 1) {
            ...
            for (int i = 1; i < nbPoint; i++) {
                // Compute new point position :
                pointToAdd = computePointAlongOrthodrome(tmpPoint, delta, az, shape);
                // Add it :
                list.add(pointToAdd);
                tmpPoint = pointToAdd;
                // Compute new azimuth :
     ...
     */
    private static double evalSinEven(final double xCoef, final double aCoef, final double bCoef,
     ...
     */
    public static GeodeticPoint computeCenterPointAlongLoxodrome(final GeodeticPoint p1,
            final GeodeticPoint p2,
            final EllipsoidBodyShape body) throws PatriusException {

        if (p1.getLatitude() == p2.getLatitude() && p1.getLongitude() == p2.getLongitude()) {
            // Quick escape - Only the latitude & longitude values are used in each GeodeticPoint.
            return p1;
        }
        // Loxodromic distance between P1 and P2
        final double distance = ProjectionEllipsoidUtils.computeLoxodromicDistance(p1, p2, body);

        // Bearing cap from P1 to P2
        final double cap = ProjectionEllipsoidUtils.computeBearing(p1, p2, body);

        // Compute the center
        return ProjectionEllipsoidUtils.computePointAlongLoxodrome(p1, distance / 2., cap, body);
    }
}
