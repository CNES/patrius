...
/**
 * Orbital covariance associated with a multiple orbits.
 * <p>
 * This class associates a {@linkplain Covariance} instance with multiple orbits, their common date
 * being the date of definition of the orbital covariance. The frame, orbit type (Cartesian,
 * Keplerian, etc) and position angle type (mean, true, eccentric) in which it is expressed are
 * specified at construction.
 * </p>
 * <p>
 * The covariance matrix must be at least N by N, where N is the number of orbits multiplied by the
 * number of orbital parameters. Each orbit is associated with K consecutive rows/columns, where the
 * first six represent the uncertainty on the orbital parameters and the remaining ones represent
 * the uncertainty on the additional parameters. The parameter descriptors of these first six
 * rows/columns must be associated to an {@linkplain StandardFieldDescriptors#ORBITAL_COORDINATE
 * orbital coordinate descriptor}, and these descriptors must be mapped to a valid
 * {@linkplain OrbitalCoordinate} (one with the expected orbit type and state vector index).
 * </p>
 *
...
public class MultiOrbitalCovariance extends AbstractOrbitalCovariance<MultiOrbitalCovariance> {
     ...
     * <p>
     * The covariance matrix must be at least N by N, where N is the number of orbits multiplied by
     * the number of orbital parameters. Each orbit is associated with K consecutive rows/columns,
     * where the first six represent the uncertainty on the orbital parameters and the remaining
     * ones represent the uncertainty on the additional parameters. The parameter descriptors of
     * these first six rows/columns are automatically associated with an
     * {@linkplain StandardFieldDescriptors#ORBITAL_COORDINATE orbital coordinate descriptor},
     * mapped to a valid {@linkplain OrbitalCoordinate}.
     * </p>
     *
     ...
     * where the first six represent the uncertainty on the orbital parameters and the remaining
     * ones represent the uncertainty on the additional parameters. The parameter descriptors of
     * these first six rows/columns must be associated to an
     * {@linkplain StandardFieldDescriptors#ORBITAL_COORDINATE orbital coordinate descriptor}, and
     * these descriptors must be mapped to a valid {@linkplain OrbitalCoordinate} (one with the
     * expected orbit type and state vector index).
     * </p>
     *
     * @param covariance
     *        the covariance matrix
     ...
     * <p>
     * The tested array is considered invalid if it is {@code null}, if its length does not match
     * the specified number of orbits, if it contains a negative value, or if the reconstructed
     * covariance size (that is, the total number of additional parameters plus six times the number
     * of orbits) does not match the actual covariance size.
     * </p>
     *
     ...
     *         the reconstructed covariance size does not match the actual covariance size
     */
    ...
    public OrbitalCovariance getOrbitalCovariance(final int index) {
        ...
        // Start index and number of elements to be extracted
        final int startIndex = this.startIndices[index];
        final int nbElements = ORBIT_DIMENSION + this.nbAdditionalParameters[index];

        // Extract the covariance associated with the selected orbit
        final int[] indices = IntStream.range(startIndex, startIndex + nbElements).toArray();
