 ...
 * <p>
 * This class associates a {@linkplain Covariance} instance with a given orbit, its date being the date of definition of
 * the orbital covariance. The frame, orbit type (Cartesian, Keplerian, etc) and position angle type (mean, true,
 * eccentric) in which it is expressed can also be specified at construction if they are not the frame, orbit type and
 * position angle type of the associated orbit.
 * </p>
 * <p>
 ...
 * {@linkplain fr.cnes.sirius.patrius.math.parameter.StandardFieldDescriptors#ORBITAL_COORDINATE orbital coordinate
 * descriptor}, and this descriptor must be mapped to a valid {@linkplain OrbitalCoordinate} (one with the expected
...
public class OrbitalCovariance extends AbstractOrbitalCovariance<OrbitalCovariance> {
     ...
     * angle type.
     * <p>
     * The covariance matrix must be at least six by six, where the first six rows/columns represent the uncertainty on
     * the orbital parameters and the remaining ones represent the uncertainty on the additional parameters. The
     * parameter descriptors of these first six rows/columns are automatically associated with an
     * {@linkplain fr.cnes.sirius.patrius.math.parameter.StandardFieldDescriptors#ORBITAL_COORDINATE
     ...
     * The covariance matrix must be at least six by six, where the first six rows/columns represent the uncertainty on
     * the orbital parameters and the remaining ones represent the uncertainty on the additional parameters. The
     * parameter descriptors of these first six rows/columns must be associated to an
     * {@linkplain fr.cnes.sirius.patrius.math.parameter.StandardFieldDescriptors#ORBITAL_COORDINATE orbital coordinate
     * descriptor}, and this descriptor must be mapped to a valid {@linkplain OrbitalCoordinate} (one with the expected
     * orbit type and state vector index).
     * </p>
