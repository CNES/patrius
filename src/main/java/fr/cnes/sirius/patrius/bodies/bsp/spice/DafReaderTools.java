...
public final class DafReaderTools {
     ...
     * 
     * @param file
     *        RandomAccessFile already opened from where the string is read
     * @param length
     ...
     */
    public static String readString(final RandomAccessFile file, final int length) throws IOException {
     ...
     * 
     * @param nRecord
     *        File record we want to access
     * @return file byte where the record starts
     * @throws MathIllegalArgumentException
     *         if the nRecord isn't valid
     */
    ...
    /**
     * Transform an address into a record number and a word inside the record.<br>
     * There are 128 words in each record, being each 8 bytes (a double precision).
     * 
     * @param address
     *        byte address we want to transform
     * @param record
     *        (output) file record where the address points
     * @param word
     *        (output) word inside the record where the address points
     * @throws MathIllegalArgumentException
     *         if the address isn't valid
     */
    public static void address2RecordWord(final int address, final int[] record, final int[] word) {
        if (address < 0) {
