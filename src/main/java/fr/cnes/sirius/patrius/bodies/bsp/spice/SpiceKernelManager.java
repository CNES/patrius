...
public final class SpiceKernelManager {
     ...
     * 
     * @param file
     *        SPICE kernel file (for now, only binary DAF files are allowed)
     * @throws PatriusException
     *         if there is any file loading or pool problem
     * @throws IOException
     *         if there is any file loading problem
     */
    public static void loadSpiceKernel(final String file) throws PatriusException, IOException {
        ...
        if (first) {
            first = false;
            loadedKernel = new ArrayList<>(MAXFILES);

            PoolSpice.setWatch(CLASS_NAME, NKNOWN, known);
            update = PoolSpice.checkUpdates(CLASS_NAME);
        }

        // We don't want external interactions with the kernel pool to have any affect on loadSpiceKernel's watch so we
        // check the watcher here prior to the call to ZZLDKER.
        update = PoolSpice.checkUpdates(CLASS_NAME);
     ...
     * 
     * @param type
     *        a kernel type (actually only SPK is allowed. add other types)
     * @return the number of kernels of type TYPE
     */
     ...
     * 
     * @throws PatriusException
     *         if there is a problem unloading a file
     */
    public static void clearAllKernels() throws PatriusException {
        if (loadedKernel == null) {
            return;
        }
        // Unloading all kernels is actually much less work than unloading just a few of them. We unload all of the
        // binary kernels via the "unload" routines for their respective subsystems, then clear the kernel pool.
        final Iterator<SpiceKernelInfo> it = loadedKernel.iterator();
        while (it.hasNext()) {
        ...
        // Although it's not strictly necessary, we initialize SpiceKernelManager's database arrays. This step may
        // occasionally be helpful for debugging.
        loadedKernel.clear();

        // Calling CLPOOL doesn't remove watches, but it does send a message to each agent indicating that its variables
        // have been touched. Clear this indication by calling PoolSpice.checkUpdates. (This is done for safety; the
        // current implementation of loadSpiceKernel doesn't require it.)
     ...
     * <p>
     * The usual usage of loadSpiceKernel is to load each file needed by your program exactly one time. However, it is
     * possible to load a kernel more than one time. The effect of unloading a kernel that has been loaded more than
     * once is to "undo" the last loading of the kernel.
     * </p>
     * <p>
     ...
     * 
     * @param file
     *        path of the file to be unload from the system
     * @throws IOException
     *         if there is any problem unloading or loading a file
     * @throws PatriusException
     *         if there is any problem unloading or loading a file
     */
    ...
    public static void unloadKernel(final String file) throws IOException, PatriusException {
        ...
        // First locate the file we need to unload, we search backward through the list of loaded files so that we
        ...
        if (SpiceCommon.SPK.equals(ski.getType())) {
            // Count the occurrences of the file in the database. Stop if we reach two occurrences.
            int nmult = 0;
            ...
            while (it2.hasNext() && nmult < 2) {
                ...
                if (ski.getHandle() == ski2.getHandle()) {
                    // To be safe, make sure we're not looking at a text kernel with a random, matching handle value
        ...
        // Update any source pointers affected by the deletion of the Ith database entry
        while (it.hasNext()) {
            ski = it.next();
            if (ski.getSource() > idx) {
                // This pointer is affected by the deletion of the Ith database entry
                throw new PatriusException(PatriusMessages.PDB_IMPOSSIBLE_TO_GET_HERE_SPICE);
            }
        }

        // If any SPK files were unloaded, we need to reload everything to establish the right priority sequence for
        // segments
     ...
     * 
     * @param file
     *        The name of a file to be examined
     * @throws PatriusException
     *         if there is a problem managing the file or the type is not SPK extend to other types
     * @throws IOException
     *         if there is a problem managing the file (open/read/close)
     */
    public static void getArchAndType(final File file) throws PatriusException, IOException {
        ...
        if (found) {
            ...
            // If the file is not already open (probably the case that happens most frequently) we try opening it for
            // direct access and see if we can locate the idword.
        }
        ...
        // If the architecture is DAF and the type is unknown, '?', then we have either an SPK file, a CK file, or
        // something we don't understand. Let's check it out.
        if (type.trim().equals(SpiceCommon.UNKNOWN)) {
            // We have a DAF file and we do not know what the type is. This situation can occur for older SPK and CK
            // files, before the ID word was used to store type information.
            //
            // We use Bill's (WLT'S) magic heuristics to determine the type of the file.
            //
            // Open the file and pass the handle to the private routine that deals with the dirty work.
    ...
    /**
     * Determine the architecture and type of a file and load the file into the appropriate SPICE subsystem (for the
     * moment, only DAF SPK files are allowed).
     * <p>
     ...
     * @throws PatriusException
     *         if there is a problem finding the file, determining its arch/type or loading it
     * @throws IOException
     *         if there is a problem determining its arch/type or loading it
     */
    ...
    /**
     * This routine determines whether or not a DAF file attached to the supplied handle is an SPK, CK or unknown file.
     * 
     * @param handle
     *        the handle of a DAF file open for read access
     ...
     */
    private static String ckOrSpk(final int handle) throws PatriusException {

        // These parameters give the number of integer and double precision components of the descriptor for SPK and CK
        // files
        final int nd = 2;
        final int ni = 6;
        ...
        // We've got the correct values for ND and NI, examine the descriptor for the first array
        ...
        // The following parameters point to the various slots in the integer portion of the DAF descriptor where the
        // values are located
        final int bodid = 0;
        final int cenid = bodid + 1;
        final int spkfrm = cenid + 1;
        final int spktyp = spkfrm + 1;
        // Look at the slot where the angular velocity flag would be located if this is a CK file
        final String cktyp = "CK";
        final int angvel = ic[spktyp];

        // Test 1. The value of ANGVEL may do the trick right at the start
        if (angvel == 0) {
            return cktyp;
        } else if (angvel > 1) {
        ...
        // Test 2. If this is an SPK file, it has a type 01 segment. See if this is something orbiting the solar system
        // barycenter.
        throw new PatriusException(PatriusMessages.PDB_IMPOSSIBLE_TO_GET_HERE_SPICE);
