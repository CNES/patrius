...
/**
 * This class allows the translation of SPICE body identifiers
 * to the body names and vice-versa. It will look first in the built-in database,
 * and then in the kernel manager if it has read bodies information.
 * 
...
public final class SpiceBody {
    ...
    /**
     * Private routine for all initialization needed the first time 
     * a method from the class is called.
     * @throws PatriusException if there is a problem with the counter array or the pool initialization
     */
    private static void init() throws PatriusException{
        ...
        // Loop through the input arrays to populate lists. We do it
        // backwards to pick and register only the highest priority (latest)
        // values for each normalized name.
        nameList = new ArrayList<String>(MAXBOD);
        norList = new ArrayList<String>(MAXBOD);
        idList = new ArrayList<Integer>(MAXBOD);
     ...
     * for that body. 
     * This routine is a translation of the BODC2N routine from the Spice
     * library
     * 
     ...
     * @throws PatriusException if there is a problem in the Pool
     */
    public static String bodyCode2Name(final int code) throws PatriusException {
        ...
        // update. This ensures that any initial assignments are properly
        // processed.
        final boolean update = PoolSpice.checkUpdatesIfCounterUpdate(pulctr, CLASSNAME);
        
        if ( update || noData ) {
            // Conservatively increment the SpiceBody state counter
        ...
        // codes list. Check the built-in codes list.
        ...
        if (index >= 0) {
            if (extker) {
                ...
                if (j >= 0){
                    ...
                    // return " " as not found, as the contents of the kernel pool
     ...
     * @param found (out) returns if the body was found
     * @return SPICE integer ID code for the named body.
     * @throws PatriusException if there is a Pool or initialization problem
     */
    public static int bodyName2Code(final String name, final boolean[] found) throws PatriusException {       
        ...
        // name up in the built-in and kernel pool names lists.
        ...
        // names list. Check the built-in names list.
     ...
     * This routine is a translation of the BODC2S routine from the SPICE
     * library
     * 
     * @param code : Integer ID code to be translated into a name.
     * @return A common name for the body identified by code. 
     *         If the code is not found, an empty string is returned
     * @throws PatriusException if there is a problem translating the code to the name
     */
    ...
    /**
     * Translate a string representing a body name into its code. If the name is a string
     * representation of a integer, the integer represented is returned.
     * Based on 
     * @param name String representing a body name.
     * @param found (out) return if the body was found
     * @return Code corresponding to the string. If 0, it was not found.
     * @throws PatriusException if there is a problem translating the name into the code.
     */
     ...
     * integer code, but bypass calling {@link SpiceBody#bodyString2Code(String)} and return saved values
     * provided by the caller if the name is the same as the saved name and the SpiceBody state did not change.
     * Based on the routine ZZBODS2C routine of the SPICE library
     * @param usrctr (in/out) SpiceBody state counter saved by the caller.
     * @param savName (in/out) Body name saved by the caller.
     * @param savCode (in/out) Body ID code saved by the caller. 
     * @param savFound (in/out) Translation success flag saved in the caller.
     * @param name Body name.
     * @param found (out) Translation success flag.
     * @return Body ID code.
     * @throws PatriusException if there is a problem checking the state or translating.
     */
    public static int bodyString2CodeBypass(final CounterArray usrctr, 
                                            final String[] savName, 
                                            final int[] savCode, 
                                            final boolean[] savFound, 
                                            final String name, 
     ...
     * @param usrctr SpiceBody state counter tracked by the caller
     ...
     * @throws PatriusException if there is a problem in the pool or with the information retrieved
     */
    private static boolean zzbodker() throws PatriusException {
        ...
        // Populate the lists
        kernamList = new ArrayList<String>(NROOM);
        kernorList = new ArrayList<String>(NROOM);
        keridList = new ArrayList<Integer>(NROOM);
    ...
    /**
     * Normalize a String by trimming any spaces at the beginning or the end and by 
     * letting no more than 1 space in a row between words.
