...
/**
 * This class is intended to provide low-level services
 * for the creation, updating, and reading of direct access
...
public final class DafHandleManager {
    ...
    /**
     * Boolean indicating whether it is the first time calling a 
     * method from the class for initialization purposes.
     */
    private static boolean first = true;
    /**
     * NEXT stores the next handle to be used for file access. It is
    ...
    /**
     * List containing the binary file formats considered in the 
     * SPICE library
     */
    private static List<String> binaryFileFormat = new ArrayList<String>();
     ...
     */
    private static List<Integer> fileTableHandle = new ArrayList<Integer>();
    /**
     * List containing the binary format of each file opened
     */
    private static List<String> fileTableBFF = new ArrayList<String>();
    /**
     * List containing the architecture of each file opened
     */
    private static List<String> fileTableArch = new ArrayList<String>();
    /**
    ...
    /**
     * Initialize DafHandleManager data.
     ...
     * This method is based on the ZZDDHOPN routine
     * in the SPICE library
     * @param fname Name of file to be loaded.
     * @param accessMethod Access method used to load the file.
     * @param archi Expected architecture of the file to load.
     * @return Handle assigned to file.
     * @throws IOException if there is a problem with the file
     * @throws PatriusException if there is a problem with the SPICE algorithm (including access method)
     */
    static int openFile(final String fname,
            final String accessMethod,
            final String archi) throws IOException, PatriusException {
     ...
     * @param handle File handle associated with the file to close.
     * @param arch Expected architecture of the handle to close.
     * @throws PatriusException if the architecture does not correspond to the one saved
     */
    static void closeFile(final int handle,
            final String arch) throws PatriusException {
        if (first) {
            init();
        }
     ...
     * 
     * @param handle File handle assigned to file of interest.
     * @return Name of the file associated with HANDLE.
     */
    static String getFilename(final int handle) {
        final int findex = fileTableHandle.indexOf(Integer.valueOf(handle));
     ...
     */
    static boolean getFound(final int handle) {
        return fileTableHandle.contains(Integer.valueOf(handle));
    }

    /** 
     * Indicate the binary file format of the file associated to
     *  handle.
     * Inspired by ZZDDHNFO routine in the SPICE library.
     * 
     * @param handle File handle assigned to file of interest.
     ...
     */
    static String getBinaryFileFormatFromHandle(final int handle) {
        return fileTableBFF.get(fileTableHandle.indexOf(handle));
    }

    /** 
     * Indicate the architecture of the file associated to
     *  handle.
     * Inspired by ZZDDHNFO routine in the SPICE library.
     * 
     * @param handle File handle assigned to file of interest.
     ...
     */
    static String getArchitectureFromHandle(final int handle) {
        return fileTableArch.get(fileTableHandle.indexOf(handle));
    }

    /**
     * Retrieve handle associated with filename.
     * @param fname Name of a file previously loaded.
     * @return Corresponding file handle. If 0, it was not found.
     * @throws PatriusException if the file is not found in the list
     */
    static int getHandle(final String fname) throws PatriusException {
        if (first) {
            init();
        }
     ...
     * 
     * @param handle Handle associated with the file of interest.
     * @return Corresponding File object
     */
    static File getFile(final int handle) {
     ...
     * @param pathToFile Name of the file to convert to a handle.
     * @return Boolean indicating if FNAME's HANDLE was found.
     * @throws PatriusException if the file does not exist or it is unreadable
     */
    ...
    /**
     * Look for the binary file format indicated in the first
     * file record.
     * Based on the ZZDDHPPF routine in the SPICE library
     * @param file File object attached to the binary file
     * @param arch File architecture
     * @return binary file format
     * @throws IOException if there is a problem reading the file
     * @throws PatriusException if the ftp check fails
     */
    private static String getBinaryFileFormat(final File file,
            final String arch) throws IOException, PatriusException {
