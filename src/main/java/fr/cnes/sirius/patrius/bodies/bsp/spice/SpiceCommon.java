...
/**
 * Class containing constants and auxiliary methods for the rest
 * of the SPICE related classes.
 * @author T0281925
 *
 */
public final class SpiceCommon {
     ...
     * 
     * @param idword is the ID word from a SPICE binary kernel file
     * @return a 2 component array containing the architecture and the file type
     *         identified from the idword
     * @throws PatriusException if the architecture is not DAF TODO change if more architectures are allowed
     */
     ...
     * 
     * @param ftp String that may contain the FTP validation string.
    ...
    public static boolean ftpCheck(final String ftp) {
        ...
        if (!isThere) {
            ...
            // In the event that FSMIDX is non-negative, we know that substr
            // is a substring of MEMSTR, and thus we have validated all the
            // test clusters from the file.
            if (fsmidx >= 0) {
                err = false;

                // If FSMIDX is negative, then we do not yet know whether or not the
                ...
                // is a substring of what's in substr.
            } else {
     ...
     * @param s Any character string.
     * @param ch a character
     * @return The function returns the index of the first character of STR
     *         that is not the character in input. If no such character is found,
     *         the function returns -1
     */ 
    public static int indexOfNoChar(final String s,
                                    final char ch) {
        int idx = 0;
     ...
     * 
     * @param sum Array summary
     * @param nd Number of double precision components
     * @param ni Number of integer components
     * @param dc (out) Double precision components
     * @param ic (out) Integer components
     */
    public static void unpackSummary(final double[] sum,
            final int nd,
            final int ni,
            final double[] dc,
            final int[] ic) {
