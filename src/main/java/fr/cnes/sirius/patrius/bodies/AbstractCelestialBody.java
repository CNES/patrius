 ...
 * <p>
 * This abstract implementation provides basic services that can be shared by most implementations
 * of the {@link CelestialBody} interface. It holds the attraction coefficient and build the
 * body-centered frames automatically using the definitions of pole and prime meridianspecified by
 * the IAU/IAG Working Group on Cartographic Coordinates and Rotational Elements of the Planets and
 * Satellites (WGCCRE).
 * </p>
 *
 * @see IAUPole
...
public abstract class AbstractCelestialBody implements CelestialBody {
    ...
    private final CelestialBodyFrame icrfFrame;
     ...
     */
    protected AbstractCelestialBody(final String nameIn, final AbstractAttractionModel attractionModelIn,
            final IAUPole iauPoleIn, final Frame parentFrame) {
        name = nameIn;
        iauPole = iauPoleIn;
        icrfFrame = new CelestialBodyFrame(parentFrame, new ICRFOriented(parentFrame), nameIn
    ...
    protected AbstractCelestialBody(final String nameIn, final double gmIn,
        ...
        icrfFrame = new CelestialBodyFrame(parentFrame, new ICRFOriented(parentFrame), nameIn
                + SPACE + ICRF_FRAME_NAME, true, this);
        attractionModel = new NewtonianAttractionModel(icrfFrame, gmIn);
    ...
    protected AbstractCelestialBody(final CelestialBodyFrame icrf, final String nameIn,
            final AbstractAttractionModel attractionModelIn, final IAUPole iauPoleIn) {
        name = nameIn;
        iauPole = iauPoleIn;
        icrfFrame = icrf;
        attractionModel = attractionModelIn;
    ...
    private final void setFrameTree() {
        ...
        eme2000Frame = new CelestialBodyFrame(icrfFrame, new EME2000Provider(), name
                + SPACE + EME2000_FRAME_NAME, true, this);
        constantInertialFrame = new CelestialBodyFrame(icrfFrame, new InertiallyOriented(
                GlobalIAUPoleType.CONSTANT), name + SPACE + INERTIAL_FRAME_CONSTANT_MODEL,
                true, this);
        meanInertialFrame = new CelestialBodyFrame(icrfFrame, new InertiallyOriented(
                GlobalIAUPoleType.CONSTANT_SECULAR), name + SPACE + INERTIAL_FRAME_MEAN_MODEL,
                true, this);
        trueInertialFrame = new CelestialBodyFrame(icrfFrame, new InertiallyOriented(
                GlobalIAUPoleType.CONSTANT_SECULAR_HARMONICS), name + SPACE
                + INERTIAL_FRAME_TRUE_MODEL, true, this);
        constantRotatingFrame = new CelestialBodyFrame(constantInertialFrame,
                new BodyOriented(GlobalIAUPoleType.CONSTANT), name + SPACE
                        + ROTATING_FRAME_CONSTANT_MODEL, false, this);
        meanRotatingFrame = new CelestialBodyFrame(meanInertialFrame, new BodyOriented(
                GlobalIAUPoleType.CONSTANT_SECULAR), name + SPACE + ROTATING_FRAME_MEAN_MODEL,
                false, this);
        trueRotatingFrame = new CelestialBodyFrame(trueInertialFrame, new BodyOriented(
                GlobalIAUPoleType.CONSTANT_SECULAR_HARMONICS), name + SPACE
                + ROTATING_FRAME_TRUE_MODEL, false, this);
    }
    ...
    public CelestialBodyFrame getInertialFrameConstantModel() {
        return constantInertialFrame;
    }

    /** {@inheritDoc} */
    @Override
    public CelestialBodyFrame getInertialFrameMeanModel() {
        return meanInertialFrame;
    }

    /** {@inheritDoc} */
    @Override
    public CelestialBodyFrame getInertialFrameTrueModel() {
        return trueInertialFrame;
    }

    /** {@inheritDoc} */
    @Override
    public CelestialBodyFrame getRotatingFrameConstantModel() {
        return constantRotatingFrame;
    }

    /** {@inheritDoc} */
    @Override
    public CelestialBodyFrame getRotatingFrameMeanModel() {
        return meanRotatingFrame;
    }

    /** {@inheritDoc} */
    @Override
    public CelestialBodyFrame getRotatingFrameTrueModel() {
        return trueRotatingFrame;
    }
    ...
    public PVCoordinates getPVCoordinates(final AbsoluteDate date, final Frame frame) throws PatriusException {
        return getEphemeris().getPVCoordinates(date, frame);
    }
    ...
    public Frame getNativeFrame(final AbsoluteDate date,
        ...
        return getEphemeris().getNativeFrame(date, frame);
    }
    ...
    public String toString() {
        ...
        builder.append("- Inertial frame: " + getInertialFrameConstantModel().toString() + end);
        builder.append("- Mean equator frame: " + getInertialFrameMeanModel().toString() + end);
        builder.append("- True equator frame: " + getInertialFrameTrueModel().toString() + end);
        builder.append("- Constant rotating frame: " + getRotatingFrameConstantModel().toString()
                + end);
        builder.append("- Mean rotating frame: " + getRotatingFrameMeanModel().toString() + end);
        builder.append("- True rotating frame: " + getRotatingFrameTrueModel().toString() + end);
        builder.append("- IAU pole origin: " + iauPole.toString() + " ("
                + iauPole.getClass() + ")" + end);
    ...
    private class ICRFOriented implements TransformProvider {
        ...
        public Transform
                getTransform(final AbsoluteDate date, final boolean computeSpinDerivatives)
                        throws PatriusException {
            return this
                    .getTransform(date, FramesFactory.getConfiguration(), computeSpinDerivatives);
        }
        ...
        public Transform getTransform(final AbsoluteDate date, final FramesConfiguration config,
            ...
            final PVCoordinates pv = getPVCoordinates(date,
                    parentFrame);
    ...
    private class InertiallyOriented implements TransformProvider {
        ...
        public Transform getTransform(final AbsoluteDate date, final FramesConfiguration config,
            ...
            // methods
            final Vector3D pole = iauPole
                    .getPole(date, iauPoleType);
            ...
            if (!pole.equals(Vector3D.PLUS_K)) {
                final Vector3D poleDerivative = iauPole
                        .getPoleDerivative(date, iauPoleType);
                try {
                    final AngularCoordinates coord = new AngularCoordinates(new PVCoordinates(
                            Vector3D.PLUS_K, Vector3D.ZERO), new PVCoordinates(Vector3D.PLUS_I,
                            Vector3D.ZERO), new PVCoordinates(pole, poleDerivative),
                            new PVCoordinates(qNode, Vector3D.crossProduct(Vector3D.PLUS_K,
    ...
    private class BodyOriented implements TransformProvider {
        ...
        public Transform getTransform(final AbsoluteDate date, final FramesConfiguration config,
                final boolean computeSpinDerivatives) throws PatriusException {
            final double w = iauPole.getPrimeMeridianAngle(date,
                    iauPoleType);
            final double wdot = iauPole.getPrimeMeridianAngleDerivative(
                    date, iauPoleType);
            final Vector3D acc = computeSpinDerivatives ? Vector3D.ZERO : null;
            return new Transform(date, new Rotation(Vector3D.PLUS_K, w), new Vector3D(wdot,
                    Vector3D.PLUS_K), acc);
        }
    }
}
