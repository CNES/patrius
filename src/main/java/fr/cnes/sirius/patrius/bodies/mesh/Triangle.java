...
public class Triangle implements Serializable {
    ...
    /**
     * Boolean representing triangle status for fast algorithms.<br>
     * This boolean can be set alternatively to true/false.
     */
    ...
    public Vector3D[] closestPointTo(final Line line) {
        ...
        if (intersects) {
            ...
            if (line.getMinAbscissa() != Double.NEGATIVE_INFINITY && pointInTriangle(minAbsPoint)) {
                // Triangle's plane
                final Plane plane = new Plane(this.vertices[0].getPosition(), this.vertices[1].getPosition(),
            ...
            final Vector3D[] candidatePoints =
                new Vector3D[] { closestSegmentPoint(line,
                    new Segment(this.vertices[0].getPosition(), this.vertices[1].getPosition(), null)),
                    closestSegmentPoint(line,
                        new Segment(this.vertices[1].getPosition(), this.vertices[2].getPosition(), null)),
                    closestSegmentPoint(line,
                        new Segment(this.vertices[2].getPosition(), this.vertices[0].getPosition(), null)), };
     ...
     * @param point
     *        a point
     * @return closest point of triangle to provided point
     */
    public Vector3D closestPointTo(final Vector3D point) {
        ...
        final Plane plane = new Plane(this.vertices[0].getPosition(), this.vertices[1].getPosition(),
            this.vertices[2].getPosition());
        final Vector3D projection = plane.toSpace(plane.toSubSpace(point));
        // If projection belongs to triangle, returns it, otherwise, find closest point on edges
        Vector3D result;
        if (pointInTriangle(projection)) {
            result = projection;
        } else {
            // Build all lines containing triangle edges
            final Line[] lines = { new Line(this.vertices[0].getPosition(), this.vertices[1].getPosition()),
                new Line(this.vertices[1].getPosition(), this.vertices[2].getPosition()),
                new Line(this.vertices[2].getPosition(), this.vertices[0].getPosition()) };
            // Get abscissa of point for all edge lines
            final double[] abscissas = { lines[0].getAbscissa(point), lines[1].getAbscissa(point),
                lines[2].getAbscissa(point) };

            // Find closest point for all edges
            // Unnecessary initialization (required by Java compiler)
            result = this.center;
            double minDist2 = Double.POSITIVE_INFINITY;
            for (int i = 0; i < abscissas.length; i++) {
                final Vector3D start = this.vertices[i].getPosition();
                final Vector3D end = this.vertices[(i + 1) % 3].getPosition();
                final Vector3D closestPoint;
                if (abscissas[i] < lines[i].getAbscissa(start)) {
                    // Projection on line is outside segment
                    // Closest point is segment start
                    closestPoint = start;
                } else if (abscissas[i] > lines[i].getAbscissa(end)) {
                    // Projection on line is outside segment
                    // Closest point is segment end
                    closestPoint = end;
                } else {
                    // Standard case: closest point is projection on segment
                    closestPoint = lines[i].pointAt(abscissas[i]);
                }
                // Store distance squared between edge closest point and point
                // (Use distance squared for computation time optimization)
                final double distance2 = Vector3D.distanceSq(closestPoint, point);
    ...
    public final boolean pointInTriangle(final Vector3D point) {
        ...
        // Vertices
        final Vector3D vertex0 = this.vertices[0].getPosition();
        final Vector3D vertex1 = this.vertices[1].getPosition();
        final Vector3D vertex2 = this.vertices[2].getPosition();
     ...
     */
    public static final double dotProduct(final Vector3D v1, final Vector3D v2) {
    ...
    public String toString() {
        return "TriangleID: " + this.id + " ; Vertices[v0ID: " + this.vertices[0].getID() + "; v1ID: "
                + this.vertices[1].getID() + "; v2ID: " + this.vertices[2].getID() + "]";
    }
}
