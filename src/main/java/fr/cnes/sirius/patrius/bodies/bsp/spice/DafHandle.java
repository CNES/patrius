 ...
 * produce a single result.) As each DAF is opened, it is assigned a file handle, which is used to keep track of the
 * file internally, and which is used by the calling program to refer to the file in all subsequent calls to DAF
 * methods.
 * </p>
 * <p>
...
public final class DafHandle {
    ...
    private static List<Integer> fileHandleList = new ArrayList<>(SpiceCommon.FILE_TABLE_SIZE);

    /** List containing the handles in the file table. */
    private static List<Integer> fileTableHandle = new ArrayList<>(SpiceCommon.FILE_TABLE_SIZE);

    /** List containing the number of doubles in a summary in the file table. */
    private static List<Integer> fileTableNDouble = new ArrayList<>(SpiceCommon.FILE_TABLE_SIZE);

    /** List containing the number of integers in a summary in the file table. */
    private static List<Integer> fileTableNInteger = new ArrayList<>(SpiceCommon.FILE_TABLE_SIZE);

    /**
     * List containing the number of links created for a file (number of times it has been opened and not closed) in the
     * file table.
     */
    private static List<Integer> fileTableLink = new ArrayList<>(SpiceCommon.FILE_TABLE_SIZE);
    ...
    /**
     * Open a DAF for subsequent read requests.<br>
     ...
     * @throws IOException
     *         if there is a problem opening, reading or closing the file
     * @throws PatriusException
     *         if there is a problem related with the SPICE algorithm
     */
    public static int openReadDAF(final String filename) throws IOException, PatriusException {
        ...
        // See if this file is already present in the file table.
        // If it is simply increment its link count by one, check out and return.
        ...
        // At this point, we know that we have a valid DAF file, and we're set up to read from it, so ...
        // Update the file table to include information about our newly opened DAF
    ...
    /**
     * Close the DAF associated with a given handle.<br>
     * Based on DAFCLS from the SPICE library.
     * 
     * @param handle
     *        Handle of DAF to be closed.
     * @throws PatriusException
     *         if there is a problem in the HandleManager closing the file
     */
    public static void closeDAF(final int handle) throws PatriusException {
        // Is this file even open? If so, decrement the number of links to the file.
        // If the number of links drops to zero, physically close the file and remove it from the file buffer.
    ...
    /**
     * Return the summary format associated with a handle.<br>
     * Based on DAFHSF from the SPICE library.
     * 
     * @param handle
     *        Handle of a DAF file.
     * @return int array containing the number of double precision (first element) and integer (second element)
     *         components in summaries
     * @throws PatriusException
     *         if the file wasn't opened beforehand
     */
    ...
    /**
     * Return the name of the file associated with a handle.<br>
     ...
     * @throws PatriusException
     *         if the filename in the HandleManager is empty
     */
     ...
     * 
     * @param file
     *        Path to the file we are interested in
     * @param handle
     *        (out) contains the handle associated to the file if it was loaded
    ...
    /**
     * Return a SPICE set containing the handles of all currently open DAFS.<br>
    ...
    /**
     * Signal an error if a DAF file handle does not designate a DAF that is open for a specified type of access.<br>
     * Based on DAFSIH from the SPICE library.
     * 
     * @param handle
     *        HANDLE to be validated
     * @param access
     *        String indicating access type
     * @throws PatriusException
     *         if the access is not correct or the handle not found in the list
     */
    public static void checkHandleAccess(final int handle, final String access) throws PatriusException {
