 ...
 * loaded by loadSpkFile, which among other things, loads the file into
 * the DAF system.
 * 
 * Up to MAX_FILES files may be loaded for use simultaneously, and a
 ...
 * searching by unloading it via a call to unloadSpkFile.
 * 
...
public final class SpkFile {
    ...
    /**
     * is the maximum number of bodies whose segments can be
     * buffered by SPKSFS.
     */
    private static final int MAX_BODIES = 10000;
    /**
     * is the maximum number of ephemeris files that can be
     * loaded by loadSpkFile at any given time for use by the
     * readers.
     */
    private static final int MAX_FILES = 5000;
    /**
     * NEXT is incremented whenever a new file is loaded to give the file
     * number of the file.
     */
    private static int next;
    /**
     * Doubly linked list containing for each body, the segment list that
     * corresponds to the body among other informations
     */
    private static List<SpkBody> segmentTablePool = new LinkedList<SpkBody>();
    /**
     * Contains the handle of each file that has been loaded for use with the SPK readers.
     */
    private static List<Integer> fileTableHandle = new ArrayList<Integer>(MAX_FILES);
    /**
     * Contains the file number of each file
     ...
     */
    private static List<Integer> fileTableNumFile = new ArrayList<Integer>(MAX_FILES);
     ...
     * @throws IOException if there is problem opening the file
     * @throws PatriusException if there is a problem opening or closing the file
     */
     ...
     * @throws PatriusException if there is  problem closing the file
     */
     ...
     * @param handle Handle of file containing the applicable segment.
     * @param descr Descriptor of the applicable segment.
     * @param ident Identifier of the applicable segment.
     * @return a boolean indicating whether or not a segment was found.
     * @throws PatriusException if there is a problem while searching in the file
     * @throws IOException if there is a problem while beginning a segment search.
     */
    public static boolean searchSegment(final int body, 
                                        final double et, 
                                        final int[] handle, 
                                        final double[] descr, 
        ...
        while ( !hopeless.equals(status) ) {
            ...
            if ( unknown.equals(status) ) {
                ...
                currentBody = new SpkBody(body,
                                          0,
                                          fileTableNumFile.get(fileTableNumFile.size() - 1).intValue(), 
                                          fileTableNumFile.get(fileTableNumFile.size() - 1).intValue() + 1,
                                          Double.MIN_VALUE,
                                          Double.MAX_VALUE,
                                          new double[5],
                                          "",
