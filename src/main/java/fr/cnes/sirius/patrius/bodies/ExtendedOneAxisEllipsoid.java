...
public class ExtendedOneAxisEllipsoid implements EllipsoidBodyShape {
    ...
    public GeodeticPoint getIntersectionPoint(final Line line, final Vector3D close, final Frame frame,
        ...
        if (points.length == 0) {
            ...
            for (int i = 0; i < points.length; i++) {
                ...
                // minimum abscissa
                if (newLine.getAbscissa(points[i]) > newLine.getMinAbscissa()) {
                    if (firstValidPoint) {
                        minDist = points[i].distance(close);
    ...
    public Vector3D[] getIntersectionPoints(final Line line, final Frame frame,
        ...
        final List<Vector3D> pointsList = new ArrayList<Vector3D>();
    ...
    public double getLocalRadius(final Vector3D posObserver, final Frame frame, final AbsoluteDate date,
        ...
        if (MathLib.abs(1 - this.g2) > Precision.DOUBLE_COMPARISON_EPSILON) {
            ...
            final Vector3D psat = frame.getTransformTo(this.inFrame, date).transformPosition(posObserver);
            ...
            return MathLib.min(MathLib.max(psat.getNorm() * MathLib.sin(Vector3D.angle(h.subtract(psat).normalize(),
                psat.negate().normalize())), this.getConjugateRadius()), this.getEquatorialRadius());
        } else {
            return this.aeIn;
        }
    }
    ...
    /** {@inheritDoc} */
    @Override
    public ExtendedOneAxisEllipsoid resize(final MarginType marginType, final double marginValue)
        throws PatriusException {
        ...
        if (marginType.equals(MarginType.DISTANCE)) {
            ...
            if (marginValue > -this.getEquatorialRadius() * (1 - this.getFlattening())) {
                ...
                aAlt = aeIn + marginValue;
                // Modify flattening
                fAlt = 1. - (aeIn * (1. - fIn) + marginValue) / aAlt;
            } else {
        ...
        // Return new ExtendedOneAxisEllipsoid with modified equatorial radius and flattening
        return new ExtendedOneAxisEllipsoid(aAlt, fAlt, inFrame, inName);
    }
}
