 ...
 * It is build as an adaptation of a variety of routines from the 
 * SPICE library.
 * <p>
 *
 * @author
...
public final class SpkReader {
     ...
     * 
     * @param file is the name of an SPK file.
     * @param ids is an initialized SPICE set data structure. IDS
     *            optionally may contain a set of ID codes on input; on
     *            output, the data already present in IDS will be combined
     *            with ID code set found for the file SPKFNM. The elements 
     *            of sets are unique; hence each ID code in IDS appears only
     *            once, even if the SPK file contains multiple segments for
     *            that ID code.             
     * @throws IOException if there is a problem managing the file
     * @throws PatriusException if there is problem managing or reading the file
     */
    public static void spkObjects(final String file, 
                                  final Set<Integer> ids) throws PatriusException, IOException {
        ...
        // See whether SpiceKernelManager.getArchAndType 
        // thinks we've got an SPK file.
     ...
     * relative to an observing body. Never corrected for light
     * time (planetary aberration) or stellar aberration.
     * This is based on the SPKEZR routine of the SPICE library.
     * 
     * @param target is the name of a target body. Optionally, you may
     ...
     *               target.
     * @param epoch is the ephemeris time, expressed as seconds past J2000
     *               TDB, at which the state of the target body relative to
     *               the observer is to be computed. EPOCH refers to time at
     *               the observer's location.
     * @param ref is the name of the reference frame relative to which
     *               the output state vector should be expressed. This may
     *               be any frame supported by the SPICE system
     * @param obs is the name of an observing body. Optionally, you
     *               may supply the ID code of the object as an integer
     *               string. For example, both 'EARTH' and '399' are
     *               legitimate strings to supply to indicate the
     *               observer is Earth.
     * @param lightTime is the one-way light time between the observer and
     *               target in seconds.
     * @return a Cartesian state vector representing the position
     *               and velocity of the target body relative to the
     *               specified observer. It is expressed with respect
     ...
     *               observer's location at EPOCH to the
     ...
     * @throws PatriusException if there is a counter, frame retrieving or state calculation problem 
     * @throws IOException if there is a problem while calculating the state.
     */
    public static double[] getStateRelativeToBody(final String target, 
                                                  final double epoch, 
                                                  final String ref, 
                                                  final String obs, 
    ...
    /**
     * Computes the geometric state, T(t), of the target
     ...
     * 
     * @param target is the standard NAIF ID code for a target body.
     * @param epoch is the epoch (ephemeris time) at which the state
     *              of the target body is to be computed.
     * @param ref is the name of the reference frame to which the
     *            vectors returned by the routine should be rotated.
     * @param obs is the standard NAIF ID code for an observing body.
     * @param lightTime (out) is the one-way light time in seconds from the
     *                        observing body to the geometric position of the
     *                        target body at the specified epoch.
     * @return 6-dimensional vector that contains the geometric
     ...
     * @throws PatriusException if there is a problem getting the frames, getting the state or rotating vectors
     * @throws IOException if there is a problem while reading the file.
     */
    private static double[] getGeometricStateRelativeToBodyFromID(final int target, 
                                                                  final double epoch, 
                                                                  final String ref, 
                                                                  final int obs, 
        ...
        // Since the upgrade to use counter bypass SpiceFrame.frameName2IdBypass
        // became more efficient in looking up frame IDs than SpiceChangeFrame.intertialRefFrameNumber. So the
        // original order of calls "SpiceChangeFrame.intertialRefFrameNumber first, 
        // SpiceFrame.frameName2IdBypass second" was switched to
        ...
        // SpiceChangeFrame.intertialRefFrameNumber for
        // 'DEFAULT' frame is *not* copied to the saved code SVREFI (which
        // would be set to 0 by SpiceFrame.frameName2IdBypass) to make sure that on subsequent
        // calls SpiceFrame.frameName2IdBypass does not do a bypass (as SVREFI always forced look
        // up) and calls SpiceChangeFrame.intertialRefFrameNumber again to reset the 'DEFAULT's frame ID
        ...
        // Note the check for FAILED in the loop.  If SpiceFrame.frameName2IdBypass
        // or getStateRelativeToCenterOfMotion happens to fail during execution, and the
        ...
        // as we go.  We use SpkFile.searchSegment to search through loaded
        // files to find the first segment applicable to CTARG(1)
        // and time EPOCH.  Then we use getStateRelativeToCenterOfMotion to compute the state
        // of the body CTARG(1) at EPOCH in the segment that was found
        ...
        // first elements:  TARGET and the state of TARGET relative
        // to itself.
        int i = 0;
        ctarg[i] = target;
        boolean found = true;
        ...
        if (i == CHLEN - 1) {
            while (found &&
                ...
                if(found) {
                    // Get the state of CTARG(CHLEN) relative to some center of motion.  The new center
                    // overwrites the old. The state is called STEMP.
                    ...
                    // Add STEMP to the state of TARG relative to the old center to get the state of TARG
                    // relative to the new center. Overwrite the last element of STARG.
        ...
        while (found && cobs != SOLAR_SYSTEM_BARYCENTER && ctpos == -1) {
            ...
            if(found) {
                ...
                // Add STEMP to the state of OBS relative to the old COBS to get the state of OBS
        ...
        // If CTPOS is -1 at this point, it means we have not found a common node though we have
        ...
        if (ctpos == -1) {
            ...
            throw new PatriusException(PatriusMessages.PDB_INSUFFICIENT_DATA_FOR_STATE, 
                                       tname, oname, String.valueOf(epoch));
        }
        ...
        if (tframe[ctpos] == cframe) {
            ...
            // If both frames are inertial we use SpiceChangeFrame.SpiceChangeFrame
            // instead of SpiceChangeFrame.stateTransformationMatrix to get things into a common frame.
     ...
     * @param ref (out) Target reference frame.
     * @param center (out) Center of state.
     * @return a 6-array containing position and velocity.
     * @throws PatriusException if there is a problem reading or evaluating the segment.
     */
    public static double[] getStateRelativeToCenterOfMotion(final int handle, 
                                                             final double[] descr, 
                                                             final double epoch, 
                                                             final int[] ref, 
                                                             final int[] center) throws PatriusException {
        final double[] record;
