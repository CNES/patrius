...
public class OneAxisEllipsoid implements BodyShape {
    ...
    /**
     * Compute the geodetic point correesponding to the given intersection point.
     * 
     * @param intersection the intersection point
     * @return the geodetic point corresponding to the given intersection point
     */
    private GeodeticPoint computeGeodeticPoint(final Vector3D intersection) {
        // Retrieve every coordinate of the intersection point
        ...
        // Build the geodetic point from the computed latitude and longitude
    ...
    public GeodeticPoint getIntersectionPoint(final Line line, final Vector3D close,
        ...
        final double closeAbscissa = lineInBodyFrame.toSubSpace(close).getX();
        final double k =
            (MathLib.abs(k1 - closeAbscissa) < MathLib.abs(k2 - closeAbscissa)) ? k1 : k2;
        Vector3D intersection = lineInBodyFrame.toSpace(new Vector1D(k));
        
        // initialize to null the geodetic point to return
        GeodeticPoint res = null;
        
        // check if the chosen intersection point is on the right side of the line by comparing its abscissa with the
        // minimum abscissa
        if (lineInBodyFrame.getAbscissa(intersection) > lineInBodyFrame.getMinAbscissa()) {
            // Build result
            res = computeGeodeticPoint(intersection);
        } else {
            // Define tolerance
            final double tolerance = 1e-15;
            // check if k is k1
            if (Math.abs(k - k1) < tolerance) {
                // k is k1
                // select the other intersection point, i.e. k2
                intersection = lineInBodyFrame.toSpace(new Vector1D(k2));
            } else {
                // k is k2
                // select the other intersection point, i.e. k1
                intersection = lineInBodyFrame.toSpace(new Vector1D(k1));
            }
            // check if the other intersection point is on the right side of the line by comparing its abscissa with the
            // minimum abscissa
            if (lineInBodyFrame.getAbscissa(intersection) > lineInBodyFrame.getMinAbscissa()) {
                // Build result
                res = computeGeodeticPoint(intersection);
