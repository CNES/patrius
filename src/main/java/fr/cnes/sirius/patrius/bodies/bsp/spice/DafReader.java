...
public final class DafReader {
    ...
    /**
     * Read the contents of the file record of a DAF.<br>
     ...
     * 
     * @param handle
     *        Handle of an open DAF file
     * @param nd
     *        (output) Number of double precision components in summaries
     * @param ni
     *        (output) Number of integer components in summaries
     * @param ifname
     *        (output) Internal file name
     * @param fward
     *        (output) Forward list pointer
     * @param bward
     *        (output) Backward list pointer
     * @param free
     *        (output) Free address pointer
     * @throws PatriusException
     *         if the file is not open or there is a problem while reading the first record
     */
    public static void readFileRecord(final int handle, final int[] nd, final int[] ni, final String[] ifname,
                                      final int[] fward, final int[] bward, final int[] free) throws PatriusException {

        // Check to be sure that HANDLE is attached to a file that is open with read access. If the call fails, check
        // out and return.
        DafHandle.checkHandleAccess(handle, "READ");

        // Retrieve all but the internal file name directly from the file record. Read the internal file name into a
        // temporary string, to be sure of the length. Check FOUND.
     ...
     * @throws IOException
     *         retrieves the errors from CommentSectionDAF.readComments
     * @throws PatriusException
     *         retrieves the errors from CommentSectionDAF.readComments
     */
    ...
    /**
     * Read the double precision data bounded by two addresses within a DAF.<br>
     * Based on the DAFGDA routine from the SPICE library.
     * 
     * @param handle
     *        Handle of a DAF
     * @param begin
     *        Initial address within file
     * @param end
     *        Final address within file
     * @return data contained between begin and end
     * @throws PatriusException
     *         retrieves the errors from ReadDoublePrecisionDAF.readContentDPRecord
     */
    public static double[] readDataDaf(final int handle, final int begin, final int end) throws PatriusException {
        ...
        // Get as many records as needed. Return the last part of the first record, the first part of the last record,
        // and all of every record in between. Any record not found is assumed to be filled with zeros.
        int next = 0;
        ...
        for (int record = beginRecord[0]; record <= endRecord[0]; record++) {
            ...
            if (beginRecord[0] == endRecord[0]) {
                ...
                // If this is the first record, we read from the indicated first word to read until the end
                first = beginWord[0];
                last = lastWord;
            } else if (record == endRecord[0]) {
                // If this is the last record, we read from the beginning of the record until the indicated as last word
                // to be read
                first = firstWord;
                last = endWord[0];
            } else {
