...
/**
 * Class dedicated to the reading of the comment section of
 * binary DAF files. 
 * 
...
public final class CommentSectionDAF {
    ...
    /**
     * Boolean indicating if it is the first time calling any method
     * in the class for initialization purposes
     */
    ...
    /**
     * Characters indicating the end of line and the end of the comment
     * section
     */
    ...
    private static void init() {
        ...
        filchr = new ArrayList<Integer>(SpiceCommon.FILE_TABLE_SIZE);
        filcnt = new ArrayList<Integer>(SpiceCommon.FILE_TABLE_SIZE);
        filhan = new ArrayList<Integer>(SpiceCommon.FILE_TABLE_SIZE);
        lstpos = new ArrayList<Integer>(SpiceCommon.FILE_TABLE_SIZE);
        lstrec = new ArrayList<Integer>(SpiceCommon.FILE_TABLE_SIZE);
    }

    /**
     * This method will read the comments from the comment area of
     ...
     * 
     * This method can be used to ``simultaneously'' extract comments
     ...
     * @throws IOException in case there is a problem opening or reading the file
     * @throws PatriusException in case there is a problem regarding the SPICE algorithm
     */
     ...
     * @param handle handle associated to the file
     * @param readFile the current file
     * @return integer array containing the total number of chars and records that contains the comments section.
     * @throws IOException if there is a problem reading the file
     * @throws PatriusException if the character that determines the end of the section is not found
     */
    private static int getNumberOfCharsInCommentSection(final int handle, 
     ...
     * @param nchars number of characters read
     * @param ncomc number of characters in the comment section 
     * @param handle handle associated with the current file
     * @param recno record number 
     * @param curpos current position
     * @param index Index in the list where the information is stored
     * @throws PatriusException if the number of files is already the maximum.
     */
    private static void saveInfo(final int nchars, 
                                 final int ncomc, 
                                 final int handle, 
                                 final int recno,
                                 final int curpos,
                                 final int index) throws PatriusException {
     ...
     * @param index the index concerned in the lists. If it is -1, nothing to remove.
     */
    private static void cleanLists(final int index) {
        // -1 <= INDEX < NFILES, and we only want to remove things
