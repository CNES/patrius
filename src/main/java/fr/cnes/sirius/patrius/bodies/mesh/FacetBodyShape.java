 ...
 * <p>
 * This class offers an optimal storage under a Binary Space Partition Tree (BSP Tree). Each of the
 * body facet (class {@link Triangle}) is linked to its neighbors. Each of the body vertex (class
 * {@link Vertex}) is also linked to its neighboring triangles {@link Triangle}. Hence this class
 * provides very efficient methods (O(log n)) for intersection computation, neighbors computation,
 * etc.
 * </p>
 * <p>
 * This class implements the interface {@link BodyShape}:
 * <ul>
 * <li>As a {@link BodyShape}, this class can be used in conjunction with {@link EclipseDetector}
 * and {@link SensorModel}.</li>
 * </ul>
 * </p>
 *
...
public class FacetBodyShape implements BodyShape {
        ...
        for (final Triangle triangle : triangles) {
            for (final Vertex vertex : triangle.getVertices()) {
                for (final Triangle triangle2 : vertex.getNeighbors()) {
                    ...
                    if (triangle.isNeighborByVertexID(triangle2)
                            && !triangle.getNeighbors().contains(triangle2)) {
     ...
     */
    private OneAxisEllipsoid buildReferenceEllipsoid(final Map<Integer, Vertex> vertices) {
        ...
        final MultivariateFunction func = point -> {
            ...
            for (final Vertex v : vertices.values()) {
                ...
                final Vector3D vTh = new Vector3D(a * cosLat[i1] * cosLon[i1], a * cosLat[i1]
                        * sinLon[i1], a * (1.0 - f) * sinLat[i1]);
        ...
        final PointValuePair res = optimizer.optimize(new MaxEval(MAX_EVAL), new ObjectiveFunction(func),
            GoalType.MINIMIZE, new InitialGuess(new double[] { (minNorm + maxNorm) / 2., 0 }),
    ...
    private OneAxisEllipsoid buildInnerEllipsoid(final Map<Integer, Vertex> vertices) {
        // Return the inner ellipsoid (whose equatorial radius is equal to the minimum distance)
        // strictly contained in
        // the mesh
        return new OneAxisEllipsoid(getMinNorm(), getFittedEllipsoid().getFlattening(),
    ...
    private OneAxisEllipsoid buildOuterEllipsoid(final Map<Integer, Vertex> vertices) {
        // Return the outer ellipsoid (whose polar radius is equal to the maximum distance)
        // englobing the shape
        return new OneAxisEllipsoid(getMaxNorm()
                / (1 - getFittedEllipsoid().getFlattening()),
                fittedEllipsoid.getFlattening(), getBodyFrame(), getName());
    }
    ...
    private OneAxisEllipsoid getTransformEllipsoid() {
        OneAxisEllipsoid ellipsoid = null;
        switch (ellipsoidType) {
            case INNER_SPHERE:
                // Inner sphere
                ellipsoid = getInnerSphere();
                break;
            case OUTER_SPHERE:
                // Outer sphere
                ellipsoid = getOuterSphere();
                break;
            case INNER_ELLIPSOID:
                // Inner ellipsoid
                ellipsoid = getInnerEllipsoid();
                break;
            case OUTER_ELLIPSOID:
                // Outer ellipsoid
                ellipsoid = getOuterEllipsoid();
                break;
            case FITTED_ELLIPSOID:
                // Fitted ellipsoid
                ellipsoid = getFittedEllipsoid();
                break;
            default:
                // Cannot happen
    ...
    public Vector3D[] getIntersectionPoints(final Line line, final Frame frame,
        ...
        if (intersections != null) {
            ...
            for (int i = 0; i < intersections.length; i++) {
                ...
                if (!isDuplicate
                        && lineInBodyFrame.getAbscissa(currentInterPoint) > lineInBodyFrame
                                .getMinAbscissa()) {
     ...
     * <p>
     * A line may have several intersection points with a closed surface (we consider the one point
     * case as a degenerated two points case). The close parameter is used to select which of these
     * points should be returned. The selected point is the one that is closest to the close point.
     * </p>
     *
     * @param line
    ...
    public Intersection getIntersection(final Line line, final Vector3D close, final Frame frame,
        ...
        if (intersections != null) {
            ...
            // distance of the
            // first valid point will be smaller than this minimum distance
            ...
            for (int i = 0; i < intersections.length; i++) {
                ...
                // abscissa with the
                // minimum abscissa
                if (lineInBodyFrame.getAbscissa(currentInterPoint) > lineInBodyFrame
                        .getMinAbscissa()) {
                    if (firstValidPoint) {
     ...
     * {@link #getIntersectionPoint(Line, Vector3D, Frame, AbsoluteDate)} if altitude is close to 0.
     * An exception is thrown otherwise.
     * </p>
     */
    @Override
    public GeodeticPoint getIntersectionPoint(final Line line, final Vector3D close,
            final Frame frame, final AbsoluteDate date, final double altitude)
            throws PatriusException {
        if (MathLib.abs(altitude) >= EPS_ALTITUDE) {
            throw new PatriusException(
                    PatriusMessages.UNAVAILABLE_FACETBODYSHAPE_INTERSECTION_POINT_METHOD);
        }
     ...
     * {@inheritDoc}
     * <p>
     * Warning: this method considers the body either as the inner sphere, the outer ellipsoid or
     * the fitted ellipsoid depending on the {@link EllipsoidType} attribute.
     * </p>
     */
    @Override
    public GeodeticPoint
            transform(final Vector3D point, final Frame frame, final AbsoluteDate date)
                    throws PatriusException {
     ...
     * <p>
     * Altitude is relative to the inner sphere to the body (which is the largest sphere strictly
     * inside the mesh).
     * </p>
     *
     ...
     */
    public double getLocalAltitude(final double latitude, final double longitude)
            throws PatriusException {
     ...
     * {@inheritDoc}
     * <p>
     * To compute the apparent radius (in meters), the algorithm iterates, until convergence,
     * between the minimum and the maximum angles (and so radii too), given by the apparent angles
     * (and so radii too) of the inner and outer ellipsoid of this occulting body. Given the plane
     * containing the observer, the occulted body and this occulting body, if the line lying on this
     * plane and connecting the observer to the hypothetical tangential point of this occulting body
     * actually intersects this occulting body, the search for the correct angle value by which this
     * line shall be rotated (within the given plane) to be tangential to this occulting body will
     * continue towards a larger angle, otherwise it will continue towards a smaller angle. Each
     * time, this line is rotated by the current value of the angle and the value of the apparent
     * radius is computed thanks to the current angle and the distance between the observer and this
     * occulting body. Convergence is reached when the absolute value of the difference between the
     * current value of the apparent radius and the previous one is smaller than a specified
     * threshold (in meters).
     * </p>
     * */
    @Override
    public double getApparentRadius(final PVCoordinatesProvider pvObserver,
        ...
        if (Vector3D.angle(obsOcculting, posOcculted.subtract(posObserver)) != 0) {
            ...
            while (i < maxApparentRadiusSteps && Math.abs(error) > getThreshold()) {
                ...
                final Vector3D[] geodeticInterPoints = getIntersectionPoints(new Line(posObserver,
     ...
     * <li>Order 2 returns provided triangle, the immediate neighbors of provided triangle and also
     ...
     */
    public List<Triangle> getNeighbors(final GeodeticPoint point, final double maxDistance)
            throws PatriusException {
     ...
     * @throws PatriusException thrown if computation failed (should not happen)
     */
    public List<Triangle> getNeighbors(final GeodeticPoint point, final int order)
            throws PatriusException {
        final Vector3D pos = transform(point);
    ...
    public List<Triangle> getNeighbors(final Vector3D pos, final double maxDistance)
            throws PatriusException {
        // Get closest triangle, which is considered to be triangle at same latitude/longitude of
        // provided point
    ...
    public FieldData getFieldData(final SpacecraftState state, final IFieldOfView fieldOfView,
        ...
        final Transform t = new Transform(state.getDate(), state.getAttitude(getBodyFrame())
                .getRotation());
        final Transform tInv = t.getInverse();
    ...
    private List<Triangle> getFastVisibleTriangles(final Intersection intersection,
            final IFieldOfView fieldOfView, final Vector3D pos, final Transform t)
            throws PatriusException {
        // List of remaining triangles to treat in recursive algorithm
        final List<Triangle> remainingTriangles = new ArrayList<>();
     ...
     */
    public boolean isVisible(final Triangle triangle, final Vector3D pos,
            final IFieldOfView fieldOfView, final Transform t) throws PatriusException {
        // Check orientation
     ...
     * <p>
     * Computed eclipse status is exact and takes into account full body shape. This method cannot
     * however be used in conjunction with {@link EclipseDetector} since they use a different
     * framework.
     * </p>
     * <p>
     * This method uses the two following hypotheses: the propagation of the Sun light is
     * instantaneous and the Sun is a point source. Please note that the apparent angle of the Sun
     * as seen from the Earth is about 0.5 degrees, while the one seen from Mars is, to the nearest,
     * roughly 0.38 degrees.
     * </p>
     *
     * @param date
     *            date
    ...
    public List<Triangle> getVisibleAndEnlightenedTriangles(final List<SpacecraftState> states,
            final PVCoordinatesProvider sun, final IFieldOfView fieldOfView)
            throws PatriusException {
        // Initialization
        final List<Triangle> res = new ArrayList<>();
        ...
        for (final SpacecraftState state : states) {
            ...
            final Vector3D sunPos = sun.getPVCoordinates(state.getDate(), getBodyFrame())
                    .getPosition();

            for (final Triangle triangle : triangles) {
                if (!res.contains(triangle)) {
                    if (isVisible(triangle, pos, fieldOfView, t)
                            && isVisible(triangle, sunPos, null, null)) {
                        res.add(triangle);
                    }
                }
            }
    ...
    /**
     * Returns the fitted ellipsoid which is the ellipsoid (a, f) which minimizes the distance to
     * all vertices.
     *
     * @return the fitted ellipsoid
     */
    public OneAxisEllipsoid getFittedEllipsoid() {
        // If the fitted ellipsoid is null, build it
        if (fittedEllipsoid == null) {
            fittedEllipsoid = buildReferenceEllipsoid(meshProvider.getVertices());
        }
    ...
    /**
     * Returns the inner ellipsoid which is the largest ellipsoid strictly contained in the mesh and
     * centered around (0,
     * 0, 0).
     *
     * @return the inner ellipsoid
     */
    public OneAxisEllipsoid getInnerEllipsoid() {
        // If the inner ellipsoid is null, build it
        if (innerEllipsoid == null) {
            innerEllipsoid = buildInnerEllipsoid(meshProvider.getVertices());
        }
        return innerEllipsoid;
    }

    /**
     * Returns the outer ellipsoid which is the smallest ellipsoid englobing the shape and centered
     * around (0, 0, 0).
     *
     * @return the outer ellipsoid
     */
    public OneAxisEllipsoid getOuterEllipsoid() {
        // If the outer ellipsoid is null, build it
        if (outerEllipsoid == null) {
            outerEllipsoid = buildOuterEllipsoid(meshProvider.getVertices());
        }
        return outerEllipsoid;
    }

    /**
     * Returns the inner sphere which is the largest sphere strictly contained in the mesh and
     * centered around (0, 0,
     * 0).
     *
     * @return the inner sphere
     */
    public OneAxisEllipsoid getInnerSphere() {
    ...
    /**
     * Returns the outer sphere which is the smallest sphere englobing the shape and centered around
     * (0, 0, 0).
     *
     * @return the outer sphere
     */
    public OneAxisEllipsoid getOuterSphere() {
    ...
    public FacetBodyShape resize(final MarginType marginType, final double marginValue) throws PatriusException {
        ...
        if (marginType.equals(MarginType.DISTANCE)) {
            ...
            if (marginValue > -getMinNorm()) {
                ...
                for (int i = 0; i < trianglesArray.length; i++) {
                    ...
                    for (int j = 0; j < currentVertices.length; j++) {
                        ...
                        if (currentVertices[j].getPosition().getNorm() != 0) {
                            newVertices[j] = new Vertex(currentVertices[j].getID(), currentVertices[j].getPosition()
                                .normalize().scalarMultiply(currentVertices[j].getPosition().getNorm() + marginValue));
                    ...
                    // Modify vertices of the current triangle
                    newTriangles[i] = new Triangle(newVertices[0], newVertices[1], newVertices[2]);
                }
            } else {
        ...
        // Copy and return the new FacetCelestialBody with modified vertices of triangles FacetCelestialBody
     ...
     */
    @Override
    public Vector3D[] closestPointTo(final Line line, final Frame frame, final AbsoluteDate date)
        throws PatriusException {
        // Convert line to body frame
        ...
        for (final Triangle triangle : triangles) {
            final Vector3D[] closestPointsTriangle = triangle.closestPointTo(lineInBodyFrame);
