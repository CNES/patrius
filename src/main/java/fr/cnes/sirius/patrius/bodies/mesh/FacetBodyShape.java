 ...
 * </p>
 * <p>
 * This class implements the interface {@link GeometricBodyShape}:
 * <ul>
 ...
 * @since 4.6
 */
public class FacetBodyShape implements GeometricBodyShape {
     ...
     */
    public FacetBodyShape(final String name, final Frame bodyFrame, final PVCoordinatesProvider pvProv,
                          final EllipsoidType ellipsoidTypeIn,
    ...
    /**
     * Build fitted ellipsoid which is the ellipsoid (a, f) which minimizes the distance to all vertices.
    ...
    private ExtendedOneAxisEllipsoid buildReferenceEllipsoid(final Map<Integer, Vertex> vertices) {
        ...
        final PointValuePair res = optimizer.optimize(
            ...
            GoalType.MINIMIZE,
            new InitialGuess(new double[] { (this.minNorm + this.maxNorm) / 2., 0 }),
            new SimpleBounds(new double[] { this.minNorm, 0. },
     ...
     */
    private ExtendedOneAxisEllipsoid buildInnerEllipsoid(final Map<Integer, Vertex> vertices) {
        // Return the inner ellipsoid (whose equatorial radius is equal to the minimum distance) strictly contained in
        // the mesh
        return new ExtendedOneAxisEllipsoid(this.getMinNorm(), this.getFittedEllipsoid().getFlattening(),
     ...
     */
    private ExtendedOneAxisEllipsoid buildOuterEllipsoid(final Map<Integer, Vertex> vertices) {
        // Return the outer ellipsoid (whose polar radius is equal to the maximum distance) englobing the shape
        return new ExtendedOneAxisEllipsoid(this.getMaxNorm() / (1 - this.getFittedEllipsoid().getFlattening()),
            this.fittedEllipsoid.getFlattening(), getBodyFrame(), getName());
    }
    ...
    private ExtendedOneAxisEllipsoid buildInnerSphere(final Map<Integer, Vertex> vertices) {
        // Return the inner sphere (whose radius is equal to the minimum distance) strictly contained in the mesh
        return new ExtendedOneAxisEllipsoid(this.getMinNorm(), 0., getBodyFrame(), getName());
    }
     ...
     * @return outer sphere
     */
    private ExtendedOneAxisEllipsoid buildOuterSphere(final Map<Integer, Vertex> vertices) {
        // Return the outer sphere (whose radius is equal to the maximum distance) englobing the shape
        return new ExtendedOneAxisEllipsoid(this.getMaxNorm(), 0., getBodyFrame(), getName());
    }
     ...
     */
    private ExtendedOneAxisEllipsoid getTransformEllipsoid() {
        ExtendedOneAxisEllipsoid ellipsoid = null;
        switch (this.ellipsoidType) {
            case INNER_SPHERE:
                // Inner sphere
                ellipsoid = this.getInnerSphere();
                break;
            case FITTED_ELLIPSOID:
                // Fitted ellipsoid
                ellipsoid = this.getFittedEllipsoid();
                break;
            case OUTER_ELLIPSOID:
                // Outer ellipsoid
                ellipsoid = this.getOuterEllipsoid();
                break;
            default:
                // Cannot happen
    ...
    public Vector3D[] getIntersectionPoints(final Line line,
        ...
        if (intersections != null) {
            ...
            for (int i = 0; i < intersections.length; i++) {
                ...
                // Check if the current intersection point is duplicate and whether it is on the right side of the line
    ...
    public Intersection getIntersection(final Line line,
        ...
        if (intersections != null) {
            // Define the minimum squared distance to positive infinity to be sure that the squared distance of the
     ...
     * */
    @Override
    public double getLocalRadius(final Vector3D posObserver, final Frame frame, final AbsoluteDate date,
        ...
        final Vector3D posOcculted = occultedBody.getPVCoordinates(date, frame).getPosition();
        // Retrieve the position of this occulting body
        final Vector3D posOcculting = this.getPVCoordinates(date, frame).getPosition();
        ...
        if (Vector3D.angle(obsOcculting, posOcculted.subtract(posObserver)) != 0) {
            ...
            while (Math.abs(error) > this.getThreshold()) {
                ...
                // body
                final Vector3D[] geodeticInterPoints = this.getIntersectionPoints(new Line(posObserver,
                    posObserver.add(obsTangentialPoint), posObserver), frame, date);
    ...
    /**
     * Returns the neighbors of provided triangle whose center is closer than provided distance of provided fitted
     ...
     * @param maxDistance max distance
     * @param order order of "neighborhood"
     * @return the neighbors of provided triangle whose center is closer than provided distance of provided fitted
     *         point AND closer in terms of "neighborhood" order
     */
    ...
    /**
     * Returns the body surface pointed data as {@link SurfacePointedData} for each state in provided list. The light
     * propagation is considered as instantaneous.
     * 
    ...
    public boolean isInEclipse(final AbsoluteDate date,
        ...
        final Transform toBodyFrame = frame.getTransformTo(this.getBodyFrame(), date);
    ...
    public FacetBodyShape resize(final MarginType marginType, final double marginValue) throws PatriusException {
        ...
        final Map<Integer, Vertex> newVerticesMap = new ConcurrentHashMap<Integer, Vertex>();
        // Retrieve triangles
        final Triangle[] trianglesArray = this.getTriangles();
        ...
        if (marginType.equals(MarginType.DISTANCE)) {
            ...
            // meaning
            if (marginValue > -this.getMinNorm()) {
                // Loop on triangles
        ...
        return new FacetBodyShape(this.getName(), this.bodyFrame, this.pvProvider, this.ellipsoidType,
            newMeshProvider);
    }
    ...
    public PVCoordinates getPVCoordinates(final AbsoluteDate date, final Frame frame) throws PatriusException {
        return this.pvProvider.getPVCoordinates(date, frame);
    }
    ...
    public Frame getNativeFrame(final AbsoluteDate date,
        ...
        return this.pvProvider.getNativeFrame(date, frame);
    }
