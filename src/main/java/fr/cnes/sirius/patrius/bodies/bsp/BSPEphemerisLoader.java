...
public class BSPEphemerisLoader implements JPLEphemerisLoader, DataLoader {
    ...
    public BSPEphemerisLoader(final String supportedNames) {
        ...
        this.ephemeris = new HashMap<String, BSPEphemerisLoader.BSPCelestialBodyEphemeris>();
    }

    /**
     * Read all BSP segments.
     * @param bspFile BSP file
     * @return a map of the segments contained in the file
     * @throws SpiceErrorException thrown if reading failed
     * @throws PatriusException thrown if reading failed
     * @throws IOException 
     */
     ...
     */
    private int[] readRecord(final int handle) throws PatriusException, IOException {
    ...
    /**
     * Read all segments.
     * @param nd kernel record size
     * @param ni kernel record size
     * @return segments
     * @throws SpiceErrorException thrown if reading failed
     * @throws PatriusException thrown if reading failed
     */
    private Map<String, BSPCelestialBodyEphemeris> readSegments(final int nd,
            final int ni) throws PatriusException {

        // Segment list to return
        final Map<String, BSPCelestialBodyEphemeris> ephemerisList = 
                new ConcurrentHashMap<String, BSPCelestialBodyEphemeris>();
        ...
        while (found) {
            // While found is true, we are scrolling through the
            // CK kernel array by array and getting the data of each
            // segment descriptor.
    ...
    public CelestialBodyEphemeris loadCelestialBodyEphemeris(final String name) throws PatriusException {
        if (ephemeris.get(name.toUpperCase(Locale.US)) == null) {
        ...
        // Get body ephemeris
        final CelestialBodyEphemeris res = ephemeris.get(name.toUpperCase(Locale.US));
        if (res == null) {
            // Body not in file
    ...
    public void loadData(final InputStream input,
        ...
        ephemeris.putAll(readSegments(bspFile));
    }
     ...
     */
    private double[] scalarArrayMultiplication(final double[] array,
            final double scalar) {
    ...
    /**
     * Set Spice J2000 convention.
     * @param newConvention Spice J2000 convention
     */
    ...
    /**
     * Returns the Spice J2000 convention.
     ...
     * </p>
     * @param patriusFrame an existing PATRIUS frame
     * @param bspBodyName a body from BSP file, its IAU frame will become the provided PATRIUS frame
     * @throws PatriusException thrown if body not in file
     */
    public void linkFramesTrees(final Frame patriusFrame,
            final String bspBodyName) throws PatriusException {
        // Try load body
        // Checks if body exists, if not an exceptiin is thrown
    ...
    private static class BSPCelestialBodyEphemeris implements CelestialBodyEphemeris {
        ...
        public PVCoordinates getPVCoordinates(final AbsoluteDate date,
            ...
            final PVCoordinates pvParentFrame = segment.getRawPVCoordinates(date);
            // Get PV in output frame
            if (frame.equals(segment.getObserverFrame())) {
                // General case, if ephemeris automatically accessed from Celestial Body
                return pvParentFrame;
            } else {
                final Transform t = segment.getObserverFrame().getTransformTo(frame, date);
        ...
        public Frame getNativeFrame(final AbsoluteDate date,
                final Frame frame) throws PatriusException {
            return segment.getObserverFrame();
        }
    }
    
    /**
     * DAF Segment of BSP file.
    ...
    private class DAFSegment implements Serializable {
        ...
        private List<DAFSegment> children;

        /**
         * Constructor.
         * @param targetID target ID
         * @param centerID center ID
         * @param frameID center frame ID
         * @throws PatriusException thrown if conversion failed
         */
        public DAFSegment(final int targetID,
                final int centerID,
                final int frameID) throws PatriusException {
            ...
            this.children = new ArrayList<DAFSegment>();
        }

        /**
         * Returns the PV coordinates of target of segment in center frame.
         * @param date a date
        ...
        private PVCoordinates getRawPVCoordinates(final AbsoluteDate date) throws PatriusException {
            ...
            // Call Spice routine
            final double[] state = SpkReader.getStateRelativeToBody(targetName, nbrSecAfterJ2000InTDB, 
                                                                    observerFrameName, observerName, lt);
        ...
        /**
         * Returns the segment observer frame. Built recursively on the fly (lazy initialization).
         * @return the segment observer frame
         * @throws PatriusException thrown if failed to retrieve body name from ID
         */
        ...
        /**
         * Set observer frame. Also set frame for segments having the same parent. This avoid different references
         * @param frame frame to set
         */
        private void setObserverFrame(final Frame frame) {
            observerFrame = frame;
            if (parent != null) {
                for(final DAFSegment dafSegment : parent.children) {
        ...
        private String buildFullObserverFrameName() {
            return "IAU_" + convention + "_" + observerName;
        }
        ...
        private void buildFrame() throws PatriusException {
            ...
            if (bspBodyLink == null) {
                ...
                final DAFSegment bspBodyLinkSegment = ephemeris.get(bspBodyLink).segment;
                if (bspBodyLinkSegment.observerName == observerName) {
                    // Reach user root frame, inverted transform
                    setObserverFrame(new CelestialBodyFrame(rootPatriusFrame,
                    ...
                    String currentTargetName = bspBodyLink;
                    int index = childOf(currentTargetName);
                    while (index == -1) {
                        final DAFSegment currentSegment = ephemeris.get(currentTargetName).segment;
                    ...
                    if (index == -1) {
                        ...
                        setObserverFrame(new CelestialBodyFrame(children.get(index).getObserverFrame(),
        ...
        private void buildFrameUpward() throws PatriusException {
            if (observerID == 0) {
                ...
                final DAFSegment parentSegment = ephemeris.get(observerName).segment;
         ...
         * @param isInverted true if transform should be inverted
         * @return transform from parent to self
         */
        private TransformProvider getTransformProvider(final DAFSegment segment,
