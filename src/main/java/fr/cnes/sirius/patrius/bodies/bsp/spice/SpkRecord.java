...
/**
 * Class for the reading and evaluation of SPK records.<br>
...
public final class SpkRecord {
     ...
     *        Evaluation epoch
     * @return RECORD.<br>
     *         It is an array of data from the specified segment which, when evaluated at epoch ET, will give the state
     *         (position and velocity) of the target body identified by the input segment descriptor. The descriptor
     *         specifies the center of motion and reference frame of the state.
     *         <p>
     ...
     *         In the above record:
     *         <ul>
     *         <li>Times are expressed as seconds past J2000 TDB</li>
     *         <li>Position components have units of km</li>
     *         </ul>
     *         RECORD must be declared by the caller with size large enough to accommodate the largest record that can
     *         be returned by this routine.
     * @throws PatriusException
     *         if there is a problem reading the records.
     */
    public static double[] readType2(final int handle, final double[] descr, final double epoch)
        throws PatriusException {
        ...
        // The segment is made up of a number of logical records, each having the same size, and covering the same
        // length of time.
        //
        // We can determine which record to return using the input epoch, the initial time of the first record's
        // coverage interval, and the length of the interval covered by each record. These constants are located at the
        // end of the segment, along with the size of each logical record and the total number of records.
     ...
     * @return a 6-vector, in order: the components of XYZDOT are X, Y, Z, X', Y', and Z'. Units for state evaluations
     *         will be km and km/sec. Units for angles will be radians and radians/sec.
     * @throws PatriusException
     *         if there is a problem with the number of coefs or the domain radius
     */
    public static double[] evaluateType2(final double epoch, final double[] record) throws PatriusException {
        // The first number in the record is the record size. Following it are two numbers that will be used later, then
        // the three sets of coefficients. The number of coefficients for each variable can be determined from the
        // record size, since there are the same number of coefficients for each variable.
        ...
        for (int i = 0; i <= 2; i++) {
            // The coefficients for each variable are located contiguously, following the first three words in the
            // record
            final int cofloc = ncof * i + 3;

            // interpolateChebyshevExpansion needs as input the coefficients, the degree of the polynomial, the epoch,
            // and also two variable transformation parameters, which are located, in our case, in the second and third
            // slots of the record.
            final double[] comp = interpolateChebyshevExpansion(Arrays.copyOfRange(record, cofloc, cofloc + ncof),
                degp, Arrays.copyOfRange(record, 1, 3), epoch);
            xyzdot[i] = comp[0];
            xyzdot[i + 3] = comp[1];
        }
     ...
     *         if there is a problem reading the records
     */
    public static double[] readType3(final int handle, final double[] descr, final double epoch)
        throws PatriusException {
        final double[] dc = new double[SpkFile.ND];
        final int[] ic = new int[SpkFile.NI];
     ...
     *         if there is a problem with the number of coefs or the domain radius
     */
    public static double[] evaluateType3(final double epoch, final double[] record) throws PatriusException {
        // The first number in the record is the record size. Following it are two numbers that will be used later, then
        // the three sets of coefficients. The number of coefficients for each variable can be determined from the
        // record size, since there are the same number of coefficients for each variable.
        ...
        for (int i = 0; i <= 5; i++) {
            ...
            // and also two variable transformation parameters, which are located, in our case, in the second and third
            // slots of the record.
            state[i] = evaluateChebyshevExpansion(Arrays.copyOfRange(record, cofloc, cofloc + ncof), degp,
                Arrays.copyOfRange(record, 1, 3), epoch);
        }
    ...
    /**
     * Return the value of a polynomial and its derivative, evaluated at the input X, using the coefficients of the
     * Chebyshev expansion of the polynomial.
     * <p>
     * Based on the CHBINT routine from the SPICE routine.
     * </p>
     * 
     * @param chparam
     *        DEGP+1 Chebyshev polynomial coefficients
     * @param degp
     *        Degree of polynomial
     * @param x2s
     *        Transformation parameters of polynomial
     * @param x
     *        Value for which the polynomial is to be evaluated
     * @return double array containing the value of the polynomial at X and the value of the derivative of the
     *         polynomial at X
     */
    private static double[] interpolateChebyshevExpansion(final double[] chparam, final int degp, final double[] x2s,
                                                          final double x) {
    ...
    /**
     * Return the value of a polynomial evaluated at the input X using the coefficients for the Chebyshev expansion of
     * the polynomial.
     * <p>
     ...
     */
    private static double evaluateChebyshevExpansion(final double[] chparam, final int degp, final double[] x2s,
                                                     final double x) {
        // Transform X to S and initialize temporary variables
        final double s = (x - x2s[0]) / x2s[1];
        final double s2 = 2 * s;
