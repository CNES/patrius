...
public class StarConvexFacetBodyShape extends FacetBodyShape implements StarConvexBodyShape {
    ...
    /**
     * Constructor for a star convex FacetBodyShape. The {@link FacetBodyShape#getMaxSlope() maxSlope} value of the mesh
     * must be smaller than PI/2 so it is considered star convex. Otherwise, use the more generic {@link FacetBodyShape}
     * constructor.
     *
     * @param name
     *        body name
     ...
     * @param meshLoader
     *        mesh loader
     * @throws PatriusException thrown if loading failed or if the given mesh is not star convex
     */
    public StarConvexFacetBodyShape(final String name, final Frame bodyFrame, final EllipsoidType ellipsoidTypeIn,
        ...
        if (this.getMaxSlope() >= MathLib.PI / 2) {
            throw new PatriusException(PatriusMessages.INVALID_MESH_STARCONVEX);
        }
    }
