...
/**
 * Class for reading double precision records from DAFs
 * readContentDPRecord, getContentSummaryRecord are the only approved means for reading
 * double precision records to and from DAFs.
 * They keep track of which records have been read
...
public final class ReadDoublePrecisionDAF {
     ...
     * 
     * Checks the DAF record buffer to see if the requested
     ...
     * 
     * Bad values of begin and end are not signaled as errors. The
     * for loop will manage.
     * 
     * Based on the DAFGSR routine from the SPICE library
     * 
     * @param handle is the handle associated with a DAF.
     * @param record is the record number of a particular double precision
     *               record within the DAF, whose contents are to be read.
     * @param begin is the first word in the specified record to be returned.
     * @param end is the final word in the specified record to be returned.
     * @param found (out) true if the specified record was found
     * @return contains the specified portion of the specified record from the specified file
     * @throws PatriusException if there is a problem in the reading of the file
     */
    public static double[] getContentSummaryRecord(final int handle,
                                                   final int record,
                                                   final int begin,
                                                   final int end,
     ...
     * @throws PatriusException if there is a problem in the reading of the file
     */
    public static double[] readContentDPRecord(final int handle,
                                               final int record,
                                               final int begin,
                                               final int end,
                                               final boolean[] found) throws PatriusException {
        ...
        if (found[0]) {
            ...
            // We have to substract 1 to begin and end
     ...
     * 
     * @param handle is the handle associated with the DAF.
     * @param record is the record number of a particular summary record
     *        within the DAF, whose contents are to be read.
     * @param nd is the number of double precision components in each array summary in the specified file.
     * @param ni is the number of integer components in each array summary in the specified file.
     * @param fnd (out) is TRUE when the specified record is found
     ...
     */
    private static double[] readSumDescRecord(final int handle,
                                              final int record,
                                              final int nd,
                                              final int ni,
     ...
     * @throws PatriusException if there is a problem reading the file
     */
    private static double[] readDataRecord(final int handle,
                                           final int record,
                                           final boolean[] fnd) throws IOException, PatriusException {
        // Assume the data record will not be found, until it has been read
        // from the file, and if necessary, successfully translated.
        fnd[0] = false;
    ...
    /**
     * Get the data concerned from RBDAT and increment the request counter
     * @param data (out) Data that we wanted to read from the file
     * @param begin first component of the buffer that we want
     * @param end last component of the buffer that we want
     * @param bufloc position in the buffer desired
     */
    private static void getResultAndIncrementRequest(final double[] data, 
                                                     final int begin, 
                                                     final int end, 
                                                     final int bufloc) {
