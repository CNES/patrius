...
public final class SpiceFrame {
    ...
    /** Boolean indicating if it's the first call to any method of the class. */
    ...
    private static List<Integer> builtIdList = new ArrayList<>(NCOUNT);

    /** Built-in name-based frames list. */
    private static List<String> builtNameList = new ArrayList<>(NCOUNT);

    /** Array of the official SPICE names for the recognized frames (both inertial and non-inertial). */
    private static String[] name = new String[NCOUNT];

    /**
     * Array of the official SPICE names for the recognized frames (both inertial and non-inertial) (read in kernel).
     */
    private static String[] kname = new String[MAXKFR];

    /** Array parallel to NAME of SPICE ID codes for the various frames. */
    private static int[] idcode = new int[NCOUNT];

    /** Array parallel to NAME of SPICE ID codes for the various frames (read in kernel). */
    private static int[] kidcode = new int[MAXKFR];

    /** Array parallel to NAME of body ID codes for the centers of frames. */
    private static int[] center = new int[NCOUNT];

    /** Array parallel to NAME of body ID codes for the centers of frames (read in kernel). */
    private static int[] kcenter = new int[MAXKFR];

    /** Array parallel to NAME of inertial frame types for the various frames. These include INERTL, PCK, CK, etc. */
    private static int[] type = new int[NCOUNT];

    /**
     * Array parallel to NAME of inertial frame types for the various frames. These include INERTL, PCK, CK, etc. (read
     * in kernel).
     */
    private static int[] ktype = new int[MAXKFR];

    /**
     * Array parallel to NAME of the ID code for the frame within the TYPE of the frame. Once the class of the frame has
     * been identified by TYPE, TYPID is used to access the information specific about this frame.
     */
    private static int[] typid = new int[NCOUNT];

    /**
     * Array parallel to NAME of the ID code for the frame within the TYPE of the frame. Once the class of the frame has
     * been identified by TYPE, TYPID is used to access the information specific about this frame (read in kernel).
     */
    ...
    private static void init() {
        ...
        kernelIdList = new ArrayList<>(MAXKFR);
        kernelNameList = new ArrayList<>(MAXKFR);
     ...
     * 
     * @param code
     *        an integer code for a reference frame
     * @return the name associated with the reference frame
     * @throws PatriusException
     *         if there is a problem with the counter array or with the pool
     */
    public static String frameId2Name(final int code) throws PatriusException {
        ...
        if (index >= 0) {
            ...
            if (kindex >= 0) {
                ...
                // The ID wasn't in the list, see if we can find this frame in the kernel pool
                ...
                if (line.size() == 1) {
                    ...
                    // Note that since we did not collect all needed information about this frame, we will not try to
                    // add it to the list
                }
            }
        }
    ...
    public static int frameName2Id(final String frame) throws PatriusException {
        ...
        // Determine the location of the requested item in the array of names
        String pname = frame.trim().toUpperCase(Locale.US);

        final int item = builtNameList.indexOf(pname);

        // If the name is in our list, we can just look up its ID code in the parallel array
        int id = 0;
        if (item >= 0) {
            id = idcode[item];
        } else {
            ...
            if (kindex >= 0) {
                ...
                // The name wasn't in the list, see if we can find this frame in the kernel pool
                ...
                if (values.size() == 1) {
                    ...
                    // If we made it to this point, we successfully mapped the kernel frame name to its ID. Add this
                    // pair to the name-based list.
     ...
     * 
     * @param frCode
     *        the ID code for some frame
     * @param cent
     *        (out) the center of the frame
     * @param frClass
     *        (out) the class (type) of the frame
     * @param classId
     *        (out) the ID code for the frame within its class
     * @return {@code true} if the requested information is available
     * @throws PatriusException
     *         if there is an initialization problem
     */
    public static boolean frameInfo(final int frCode, final int[] cent, final int[] frClass, final int[] classId)
        throws PatriusException {
        // For efficiency, J2000 deserves special treatment
        if (frCode == 1) {
            cent[0] = 0;
     ...
     *        Class ID of frame
     * @param frCode
     *        (out) ID code of the frame identified by FRCLSS, CLSSID
     * @param frName
     *        (out) Name of the frame identified by FRCLSS, CLSSID
     * @param cent
     *        (out) Center of the frame identified by FRCLSS, CLSSI
     * @return {@code true} if the requested information is available
     */
    public static boolean getFrameFromClass(final int frClass, final int classId, final int[] frCode,
                                            final String[] frName, final int[] cent) {
        // Perform any needed first pass initializations
        if (first) {
            init();
        }
        ...
        // Unfortunately we did not find a frame associated with the input class and class ID in the built-in list. We
        // need to look for this frame in the kernel POOL. Since neither of these input values appears in a kernel
        // variable name, we may have to look at all of the frame specifications in the kernel pool. Start out by
        // looking the frame class assignments from any loaded frame specifications.
     ...
     * 
     * @param usrctr
     *        POOL state counter saved by the caller
     * @param savnam
     *        Frame name saved by the caller
     * @param savcde
     *        Frame ID code saved by the caller
     * @param nameF
     *        Frame name
     * @return the frame ID code
     * @throws PatriusException
     *         if there is a problem checking a counter update or translating the name to Id
     */
    public static int frameName2IdBypass(final CounterArray usrctr, final String[] savnam, final int[] savcde,
        ...
        if (!update && savcde[0] != 0 && savnam[0].equals(nameF)) {
            // No change in the POOL state, the saved name was successfully resolved earlier, and input and saved names
            // are the same. Return saved ID.
            code = savcde[0];
        } else {
            // POOL state changed, or the saved name was never successfully resolved earlier, or input and saved names
            // are different. Call NAMFRM to look up ID and reset saved values.
