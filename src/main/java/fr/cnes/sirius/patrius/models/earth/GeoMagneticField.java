...
public class GeoMagneticField {
     ...
     */
    public GeoMagneticField(final String modelNameIn, final double epochIn,
                            final int maxNIn, final int maxNSecIn,
        ...
        for (int n = 1; n <= maxNIn; n++) {
            ...
            this.schmidtQuasiNorm[index] =
                this.schmidtQuasiNorm[index1] * (2 * n - 1) / n;
    ...
    public GeoMagneticElements calculateField(final double latitude,
        ...
        final GeodeticPoint gp = new GeodeticPoint(MathLib.toRadians(latitude),
            MathLib.toRadians(longitude),
            height * 1000d);

        // return the magnetic elements
        return this.calculateField(gp);
    }
     ...
     * @return the {@link GeoMagneticElements} at the given geodetic point
     */
    public GeoMagneticElements calculateField(final GeodeticPoint gp) {
        ...
        final Vector3D magFieldSph = this.summation(sph, vars, legendre);
     ...
     */
    public GeoMagneticElements calculateField(final Vector3D point,
        ...
        final GeodeticPoint gp = body.transform(point, frame, date);
        // return the magnetic elements
        return transformedField.calculateField(gp);
    }
    ...
    public GeoMagneticField transformModel(final double year) throws PatriusException {
        ...
        final GeoMagneticField transformed = new GeoMagneticField(this.modelName, year, this.maxN, this.maxNSec,
            this.validityStart, this.validityEnd);
     ...
     *         (i.e. no secular variations available)
     */
    public GeoMagneticField transformModel(final GeoMagneticField otherModel,
                                           final double year) throws PatriusException {

        // the model can only be transformed within its validity period
        if (year < this.validityStart || year > this.validityEnd) {
            // Exception
            throw new PatriusException(PatriusMessages.OUT_OF_RANGE_TIME_TRANSFORM,
                this.modelName, String.valueOf(this.epoch), year, this.validityStart, this.validityEnd);
        }
        ...
        // Initialize model
        final GeoMagneticField transformed = new GeoMagneticField(this.modelName, year, newMaxN, 0,
            this.validityStart, this.validityEnd);
     ...
     */
    private static SphericalCoordinates transformToSpherical(final GeodeticPoint gp) {
        ...
        final double heightAboveEllipsoid = gp.getAltitude() / KM_TO_M;
        ...
        return new SphericalCoordinates(r, gp.getLongitude(), MathLib.asin(MathLib.min(1.0,
            MathLib.max(-1.0, zp / r))));
    }
     ...
     */
    private static Vector3D rotateMagneticVector(final SphericalCoordinates sph, final GeodeticPoint gp,
    ...
    private Vector3D summation(final SphericalCoordinates sph, final SphericalHarmonicVars vars,
        ...
        for (int n = 1; n <= this.maxN; n++) {
            for (int m = 0; m <= n; m++) {
                ...
                by += vars.relativeRadiusPower[n] *
                        (this.g[index] * vars.smLambda[m] - this.h[index] * vars.cmLambda[m]) * m * legendre.mP[index];
                /**
                 * <pre>
                 *        nMax     (n+2)   n    m            m            m
                ...
                bx -= vars.relativeRadiusPower[n] *
                        (this.g[index] * vars.cmLambda[m] + this.h[index] * vars.smLambda[m]) * legendre.mPDeriv[index];
            }
        }
        ...
        if (MathLib.abs(cosPhi) > THRESHOLD) {
            ...
            // exactly +/-90.
            by = this.summationSpecial(sph, vars);
        }
    ...
    private double summationSpecial(final SphericalCoordinates sph, final SphericalHarmonicVars vars) {
        ...
        for (int n = 1; n <= this.maxN; n++) {
            ...
            by +=
                vars.relativeRadiusPower[n] *
                        (this.g[index] * vars.smLambda[1] - this.h[index] * vars.cmLambda[1]) * mPcupS[n]
                        * this.schmidtQuasiNorm[index];
        }
                ...
                for (int m = 0; m <= n; m++) {
                    ...
                    if (n == m) {
                        ...
                        if (m > n - 2) {
                            ...
                            this.mPDeriv[index] =
                                x * this.mPDeriv[index2] - z * this.mP[index2] - k * this.mPDeriv[index1];
                        }
                    }
